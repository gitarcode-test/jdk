...
public class EnumContentsTest extends JavadocTester {
  ...
  public void testEnumContents() throws Exception {
    javadoc(
        ...
        "-doclet",
        "EnumContentsTest$ThisDoclet",
        "pkg1");
    checkExit(Exit.OK);

    String expect =
        Files.readAllLines(Path.of(testSrc).resolve("expected.out")).stream()
  ...
  public static class ThisDoclet extends TestDoclet {
    ...
    void printClass(TypeElement te) {
      ...
      out.format("%s %s%n", te.getKind().toString().toLowerCase(), te.getQualifiedName());
      out.format("  name: %s / %s / %s%n", te.getSimpleName(), te.asType(), te.getQualifiedName());
      out.format("  superclass:%n    %s%n", te.getSuperclass());
