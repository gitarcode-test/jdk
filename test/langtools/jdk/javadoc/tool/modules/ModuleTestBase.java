...
public class ModuleTestBase extends TestRunner {
  ...
  /**
   * Execute methods annotated with @Test, and throw an exception if any errors are reported..
   *
  ...
  private Task.Result execTask0(boolean isNegative, String... args) {
    ...
    currentTask = isNegative ? et.options(args).run(Expect.FAIL) : et.options(args).run();
    return currentTask;
  }
  ...
  public static class ModulesTesterDoclet implements Doclet {
    ...
    String hasDocComments(Element e) {
      ...
      return comment != null && !comment.isEmpty() ? "hasDocComments" : "noDocComments";
    }
      ...
      for (Element e : set) {
        ...
        new SimpleElementVisitor14<Void, Void>() {
          ...
          protected Void defaultAction(Element e, Void p) {
            ...
            CharSequence fqn =
                new SimpleElementVisitor14<CharSequence, Void>() {
                  @Override
    ...
    Set<Element> getAllSelectedElements(DocletEnvironment docenv) {
      ...
      for (PackageElement pe : ElementFilter.packagesIn(elements)) {
        ...
        if (mdle != null) addEnclosedElements(docenv, result, mdle);
    ...
    public Set<Doclet.Option> getSupportedOptions() {
      Option[] options = {
        new Option() {
          private final List<String> someOption = Arrays.asList("-hasDocComments");
