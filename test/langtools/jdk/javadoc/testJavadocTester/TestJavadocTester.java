 ...
 *
 * <p>It is not a direct test of the javadoc tool or the output generated by the Standard Doclet,
 * although both are indirectly used as part of this test.
 *
 * <p>The test works by exercising the {@code JavadocTester} API with a series of positive and
 * negative tests. The {@code passed} and {@code failed} methods are overridden to record the
 * messages reported by the underlying instance, so that the messages can subsequently be verified.
 * Also, {@code printSummary} is overridden to suppress the default action to throw {@code Error}
...
public class TestJavadocTester extends JavadocTester {
  ...
  /**
   * Overrides the default implementation of {@code passed} to record the argument. {@inheritDoc}
   *
  ...
  /**
   * Overrides the default implementation of {@code failed} to record the argument. {@inheritDoc}
   *
   * @param message a short description of the outcome
   */
  @Override
  ...
  /**
   * Overrides the default implementation of {@code printSummary} to suppress the error thrown as a
   * result of errors reported by {@code JavadocTester}. Instead, an error is thrown if any errors
   * are found by the tests in this class.
   */
  @Override
  ...
  /**
   * Checks the content of messages reported by the {@code passed} and {@code failed} methods in
   * {@code JavadocTester}. The messages are saved by the local overloads of those methods in this
   * class.
   *
   * <p>Because some of the messages are <em>very</em> long, it is enough to pass in initial
   * substrings of the expected messages.
   *
   * <p>Note that messages reported by {@code JavadocTester} use filenames as given to the various
   * {@code check...} calls. By convention, these always use {@code /} as the file separator, and
   * not the platform file separator.
   *
   ...
   *
   * <p>It is helpful/important to distinguish the messages written as a side-effect of the
   * underlying tests from the messages used to report the outcome of the tests that verify those
   * messages. Instead of interposing to mark the messages written as a side effect of the
   * underlying tests, we leave those messages unchanged, and instead, mark the messages reporting
   * whether those messages are as expected or not.
   *
  ...
  TestJavadocTester setup() throws IOException {
    ...
    javadoc("-d", "out", "-sourcepath", src.toString(), "-noindex", "-nohelp", "p");
    return this;
  }
  ...
  public void testSimpleStringCheck() {
    messages.clear();
    new OutputChecker("p/C.html").check("Second sentence", "abc123", "def456");
  ...
  public void testSimpleNegativeStringCheck_expected() {
    messages.clear();
    new OutputChecker("p/C.html").setExpectFound(false).check("Third sentence.");
    checkMessages(
        """
  ...
  public void testSimpleNegativeStringCheck_unexpected() {
    messages.clear();
    new OutputChecker("p/C.html").check("Third sentence.");
    checkMessages(
        """
  ...
  public void testSimpleRegexCheck() {
    ...
    new OutputChecker("p/C.html")
        .check(
            Pattern.compile("S.cond s.nt.nc."),
  ...
  public void testUnordered_expected() {
    ...
    new OutputChecker("p/C.html")
        ...
        .check("Second sentence", "First sentence");
    checkMessages(
        """
        Passed: out/p/C.html: the following text was found:
        Second sentence""",
  ...
  public void testUnordered_unexpected() {
    messages.clear();
    new OutputChecker("p/C.html").check("Second sentence", "First sentence");
    checkMessages(
        """
        Passed: out/p/C.html: the following text was found:
        Second sentence""",
  ...
  public void testEmpty() {
    messages.clear();
    new OutputChecker(Output.STDERR).checkEmpty();
  ...
  public void testBadFile() {
    ...
    new OutputChecker("does-not-exist.html")
        .check("abcdef", "very long string ".repeat(10))
        .check(Pattern.quote("abcdef"), Pattern.quote("very long string".repeat(10)));
  ...
  public void testUnique() {
    ...
    checkMessages(
        "Passed: out/p/C.html: id=\"m1()\" is unique",
  ...
  private String[] fix(String... items) {
    return Stream.of(items).map(this::fix).toArray(String[]::new);
  }
}
