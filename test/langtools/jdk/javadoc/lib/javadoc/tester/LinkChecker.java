...
public class LinkChecker extends HtmlChecker {
  ...
  public void report() {
    ...
    if (!missingFiles.isEmpty()) {
      ...
      missingFiles.stream().sorted().forEach(this::reportMissingFile);
    ...
    for (IDTable t : allFiles.values()) {
      anchors += t.map.values().stream().filter(e -> !e.getReferences().isEmpty()).count();
    }
    for (IDTable t : allURIs.values()) {
      anchors += t.map.values().stream().filter(e -> !e.references.isEmpty()).count();
    }
  ...
  static class Position implements Comparable<Position> {
    ...
    public boolean equals(Object obj) {
      if (this == obj) {
        ...
        return Objects.equals(this.path, other.path) && this.line == other.line;
      }
    }
