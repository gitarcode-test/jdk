...
public class MyTaglet implements Taglet {
  ...
  /**
   * Refines an existing tag ({@code @since} that provides a {@code List<DocTree>}, so that we can
   * better test positions within the tree node. The alternative, defining a new tag, would use
   * {@code UnknownBlockTagTree} which just provides a single {@code Doc Tree}.
   *
  ...
  public String toString(List<? extends DocTree> tags, Element element) {
    ...
    DocTreePathScanner<Void, Diagnostic.Kind> s =
        new DocTreePathScanner<>() {
          ...
          public Void visitEntity(EntityTree node, Diagnostic.Kind k) {
            if (node.getName().contentEquals("#x1f955")) {
              String message =
                  "This is a " + k.toString().toLowerCase(Locale.ROOT) + ": this is not a caret";
