...
public class ToolSimpleTest extends ReplToolTesting {
  ...
  public void testRemaining() {
    test(
        ...
        (a) -> assertCommand(a, "int v; int c", "v ==> 0\n" + "c ==> 0"));
  }
  ...
  public void testChainedThrow() {
    test(
        ...
        (a) ->
            assertCommand(
                a,
                "void n() throws Exception { try { p(); } catch (Exception ex) { throw new"
                    + " IOException(\"bar\", ex); }}",
                "|  created method n()"),
        (a) ->
            assertCommand(
                a,
                "void m() { try { n(); } catch (Exception ex) { throw new RuntimeException(\"foo\","
                    + " ex); }}",
                "|  created method m()"),
        (a) ->
            assertCommand(
                ...
                "|  Exception java.lang.RuntimeException: foo\n"
                    ...
                    + " \"String.toString()\" because \"null\" is null\n"
                    + "|        at p (#1:1)\n"
                    + "|        ..."),
        ...
        (a) ->
            assertCommand(
                a,
                "m()",
                "|  attempted to call method n() which cannot be invoked until method p() is"
                    + " declared"));
  }
    ...
    test(
        (a) ->
            assertCommandCheckOutput(
                a,
                "URI u = new URI(\"http\", null, \"h\", -1, \"a\" + (char)0x04, null, null);",
                (s) ->
                    assertTrue(
                        s.contains("URISyntaxException") && !s.contains("JShellTool"),
                        "Output: '" + s + "'")),
        (a) ->
            assertCommandCheckOutput(
                a,
                ...
                (s) ->
                    assertTrue(
                        s.contains("java.lang.Exception")
                            && !s.contains("UnknownFormatConversionException"),
                        "Output: '" + s + "'")));
  }
    ...
    test(
        (a) ->
            assertCommand(
                a,
                "undefined x",
                "|  created variable x, however, it cannot be referenced until class undefined is"
                    + " declared"),
  ...
  public void testUnresolved() {
    test(
        (a) ->
            assertCommand(
                a,
                "int f() { return g() + x + new A().a; }",
                "|  created method f(), however, it cannot be invoked until method g(), variable x,"
                    + " and class A are declared"),
        (a) ->
            assertCommand(
                a,
                "f()",
                "|  attempted to call method f() which cannot be invoked until method g(), variable"
                    + " x, and class A are declared"),
        (a) ->
            assertCommandOutputStartsWith(
                a,
                "int g() { return x; }",
                "|  created method g(), however, it cannot be invoked until variable x is"
                    + " declared"),
        (a) ->
            assertCommand(
                a,
                "g()",
                "|  attempted to call method g() which cannot be invoked until variable x is"
                    + " declared"));
  }
    ...
    test(
        (a) ->
            assertCommand(
                ...
                "|  created method f(int), however, it cannot be invoked until method f(int) is"
                    + " declared"),
        (a) ->
            assertCommand(
                a,
                "f(13)",
                "|  attempted to call method f(int) which cannot be invoked until method f(int) is"
                    + " declared"),
        (a) ->
            assertCommand(
                a,
                " abstract void m(Blah b);",
                "|  created method m(Blah), however, it cannot be referenced until class Blah, and"
                    + " method m(Blah) are declared"));
  }
  ...
  public void testUnknownCommand() {
    test(
        (a) ->
            assertCommand(
                a, "/unknown", "|  Invalid command: /unknown\n" + "|  Type /help for help."));
  }
  ...
  public void testInvalidClassPath() {
    test(
        a ->
            assertCommand(
                a,
                "/env --class-path snurgefusal",
                "|  File 'snurgefusal' for '--class-path' is not found."),
        a ->
            assertCommand(
                a, "/env --class-path ?", "|  File '?' for '--class-path' is not found."));
  }
    ...
    test(
        ...
        (a) ->
            assertCommandOutputStartsWith(
                a,
                "/drop",
                "|  In the /drop argument, please specify an import, variable, method, or class to"
                    + " drop."));
  }
    ...
    test(
        ...
        a ->
            assertCommandCheckOutput(
                ...
                "/drop",
                assertStartsWith(
                    "|  In the /drop argument, please specify an import, variable, method, or class"
                        + " to drop.")),
        a -> assertVariable(a, "int", "a"),
            ...
            assertCommandCheckOutput(
                ...
                s -> {
                  ...
                  for (int i = 0; i < lines.length; ++i) {
                    String l = lines[i];
                    assertTrue(l.startsWith("| "), "Expected /help line to start with | :\n" + l);
                    assertTrue(l.length() <= 80, "/help line too long: " + l.length() + "\n" + l);
                  }
                }));
  }
  ...
  private void assertHelp(boolean a, String command, String... find) {
    assertCommandCheckOutput(
        ...
        s -> {
          for (String f : find) {
            assertTrue(
                s.contains(f), "Expected output of " + command + " to contain: " + f + "\n" + s);
          }
        });
  }
  ...
  private void checkLineToList(String in, List<String> match) {
    ...
    String[] res = trimmed.isEmpty() ? new String[0] : trimmed.split("\n");
  ...
  public void testListArgs() {
    ...
    test(
        a -> assertCommandCheckOutput(a, "/list -all", s -> checkLineToList(s, START_UP)),
        ...
        a -> assertCommandCheckOutput(a, "/list -start", s -> checkLineToList(s, START_UP)),
        a -> assertCommandCheckOutput(a, "/list -all", s -> checkLineToList(s, startVarList)),
        ...
        a ->
            assertCommandCheckOutput(
                ...
                s -> {
                  assertTrue(
                      Pattern.matches(".*aardvark.*\\R.*weevil.*\\R.*s3.*import.*", s.trim()),
                      "No match: " + s);
                }),
        a -> assertCommandOutputStartsWith(a, "/list " + arg, "|  No such snippet: " + arg));
  }
  ...
  public void testVarsArgs() {
    ...
    test(
        a -> assertCommandCheckOutput(a, "/vars -all", s -> checkLineToList(s, startVarList)),
        ...
        a -> assertCommandCheckOutput(a, "/vars -start", s -> checkLineToList(s, startVarList)),
        a -> assertCommandOutputStartsWith(a, "/vars -all", "|    int aardvark = 0\n|    int a = "),
        a -> assertCommandOutputStartsWith(a, "/vars 1-4", "|    int aardvark = 0\n|    int a = "),
        a ->
            assertCommandOutputStartsWith(
                a, "/vars f", "|  This command does not accept the snippet 'f'"),
  ...
  public void testMethodsArgs() {
    ...
    test(
        new String[] {"--startup", "PRINTING"},
        a ->
            assertCommandCheckOutput(
                a, "/methods -all", s -> checkLineToList(s, printingMethodList)),
        a ->
            assertCommandCheckOutput(
                a, "/methods -start", s -> checkLineToList(s, printingMethodList)),
        a ->
            assertCommandCheckOutput(
                a, "/methods print println printf", s -> checkLineToList(s, printingMethodList)),
        a ->
            assertCommandCheckOutput(
                a, "/methods println", s -> assertEquals(s.trim().split("\n").length, 10)),
        a -> assertCommandCheckOutput(a, "/methods", s -> checkLineToList(s, printingMethodList)),
    ...
    test(
        ...
        a ->
            assertCommand(
                ...
                "|  created method junk(), however, it cannot be referenced until class GARBAGE,"
                    + " and variable TRASH are declared"),
        ...
        a ->
            assertCommand(
                ...
                "|    double m(int)\n"
                    + "|    GARBAGE junk()\n"
                    + "|       which cannot be referenced until class GARBAGE, and variable TRASH"
                    + " are declared\n"
                    + "|    int tyer()\n"
    ...
    test(
        ...
        a ->
            assertCommand(
                ...
                "|  created class C, however, it cannot be referenced until class NONE is"
                    + " declared"),
        a ->
            assertCommand(
                a,
                "class D { void m() { System.out.println(nada); } }",
                "|  created class D, however, it cannot be instantiated or its methods invoked"
                    + " until variable nada is declared"),
        a ->
            assertCommand(
                a,
                "/types",
                "|    class C\n"
                    ...
                    + "|    class D\n"
                    + "|       which cannot be instantiated or its methods invoked until variable"
                    + " nada is declared\n"));
  }
  ...
  public void testTypesArgs() {
    ...
    test(
        a -> assertCommandCheckOutput(a, "/types -all", s -> checkLineToList(s, startTypeList)),
        a -> assertCommandCheckOutput(a, "/types -start", s -> checkLineToList(s, startTypeList)),
  ...
  public void testCompoundStart() {
    test(
        ...
        (a) -> assertCommand(a, "printf(\"%4.2f\", Math.PI)", "", "", null, "3.14", ""),
        (a) ->
            assertCommand(
                a,
                "jar(\"--version\")",
                ...
                "",
                null,
                "jar " + System.getProperty("java.version") + "\n",
                ""));
  }
  ...
  public void testJavaSeStart() {
    test(
        new String[] {"--startup", "JAVASE"},
        (a) -> assertCommand(a, "ZoneOffsetTransitionRule.TimeDefinition.WALL", "$1 ==> WALL"));
  }
    ...
    test(
        ...
        (a) ->
            assertCommandCheckOutput(
                ...
                s ->
                    assertTrue(
                        s.split("import ").length > 160, "not enough imports for JAVASE:\n" + s)));
  }
  ...
  public void testCommandPrefix() {
    test(
        a ->
            assertCommandCheckOutput(
                a, "/s", assertStartsWith("|  Command: '/s' is ambiguous: /save, /set")),
        a -> assertCommand(a, "int var", "var ==> 0"),
        a -> assertCommandCheckOutput(a, "/va", assertStartsWith("|    int var = 0")),
        a ->
            assertCommandCheckOutput(
                a, "/save", assertStartsWith("|  '/save' requires a filename argument.")));
  }
  ...
  public void testOptionFeedback() {
    test(
        Locale.ROOT,
        false,
        new String[] {"--feedback", "concise", "--no-startup"},
        "",
        (a) -> assertCommand(a, "1+1", "$1 ==> 2"),
        (a) -> assertCommand(a, "int x = 5", ""));
  }
  ...
  public void testCompoundOptions() {
    Consumer<String> confirmNoStartup =
        s -> {
  ...
  public void testOptionR() {
    test(
        new String[] {"-R-Dthe.sound=blorp", "--no-startup"},
        (a) -> assertCommand(a, "System.getProperty(\"the.sound\")", "$1 ==> \"blorp\""));
  }

  @Test
  public void testWrapSourceHandlerDiagCrash() {
    test(
        new String[] {"--add-exports", "jdk.javadoc/ALL-UNNAMED"},
        (a) -> assertCommand(a, "1+1", "$1 ==> 2"));
  }
    ...
    test(
        (a) ->
            assertCommandOutputContains(
                a, "<Z extends Runnable&CharSequence> Z get1() { return null; }", "get1()"),
        (a) -> assertCommandOutputContains(a, "var g1 = get1()", "g1"),
        (a) -> assertCommand(a, "/vars g1", "|    CharSequence&Runnable g1 = null"),
        (a) ->
            assertCommandOutputContains(
                a, "<Z extends Number&CharSequence> Z get2() { return null; }", "get2()"),
  ...
  public void testAnonymous() {
    test(
        ...
        (a) ->
            assertCommandOutputContains(
                a, "/vars r1", "|    <anonymous class extending Object> r1 = "),
        (a) ->
            assertCommandOutputContains(
                a, "var r2 = new Runnable() { public void run() { } }", "r2"),
        (a) ->
            assertCommandOutputContains(
                a, "/vars r2", "|    <anonymous class implementing Runnable> r2 = "),
        (a) -> assertCommandOutputContains(a, "import java.util.stream.*;", ""),
        (a) ->
            assertCommandOutputContains(
                a,
                "var list = Stream.of(1, 2, 3).map(j -> new Object() { int i = j;"
                    + " }).collect(Collectors.toList());",
                "list"),
        (a) ->
            assertCommandOutputContains(
                a, "/vars list", "|    List<<anonymous class extending Object>> list = "));
  }
  ...
  public void testRecords() {
    test(
        ...
        (a) ->
            assertCommandOutputContains(
                a,
                "record R(int i) { public int g() { return j; } }",
                "|  created record R, however, it cannot be instantiated or its methods invoked"
                    + " until variable j is declared"),
        (a) ->
            assertCommandOutputContains(
                a,
                "new R(0)",
                "|  attempted to use record R which cannot be instantiated or its methods invoked"
                    + " until variable j is declared"));
  }
  ...
  public void testImportChange() {
    for (String feedback : new String[] {"verbose", "normal"}) {
      test(
          (a) ->
              assertCommandOutputContains(
                  a, "/set feedback " + feedback, "|  Feedback mode: " + feedback),
          ...
          (a) ->
              assertCommandOutputContains(
                  ...
                  "|    update replaced variable v1 which cannot be referenced until this error is"
                      + " corrected:"),
          (a) -> assertCommandOutputContains(a, "var b = java.util.List.of(\"bb\")", "b ==> [bb]"),
  ...
  public void testSwitchStatementExpressionDisambiguation() {
    test(
        ...
        (a) ->
            assertCommandOutputStartsWith(
                a,
                "switch (0) { default -> throw new IllegalStateException(); }",
                "|  Exception java.lang.IllegalStateException"));
    test(
        false,
        new String[] {"--no-startup", "-C-source", "-C8"},
        (a) -> assertCommand(a, "int i;", "i ==> 0"),
    ...
    test(
        a ->
            assertCommandOutputContains(
                ...
                "created method m(I,int), however, it cannot be referenced until class I is"
                    + " declared"));
  }
  ...
  public void testSelfReference() {
    test(
        (a) ->
            assertCommandOutputContains(
                a, "var a = a;", "cannot use 'var' on self-referencing variable"));
  }
}
