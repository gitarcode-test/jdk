...
/**
 * Base class for template-driven JUnit javac tests that support on-the-fly source file generation,
 * compilation, classloading, execution, and separate compilation.
 *
 * <p>Manages a set of templates (which have embedded tags of the form {@code #\{NAME\}}), source
 * files (which are also templates), and compile options. Test cases can register templates and
 * source files, cause them to be compiled, validate whether the set of diagnostic messages output
 * by the compiler is correct, and optionally load and run the compiled classes.
 *
 * @author Brian Goetz
 */
...
public abstract class JavacTemplateTestBase {
  ...
  /**
   * Add a File to the class path to be used when loading classes; File values will generally be the
   * result of a previous call to {@link #compile()}. This enables testing of separate compilation
   * scenarios if the class path is set up properly.
   */
  ...
  protected void resetCompileOptions() {
  ...
  protected void resetSourceFiles() {
    sourceFiles.clear();
  }
  ...
  protected void assertCompileSucceeded() {
    if (diags.errorsFound()) fail("Expected successful compilation");
  }
  ...
  /**
   * If the provided boolean is true, assert all previous compiles succeeded, otherwise assert that
   * a compile failed.
   */
  protected void assertCompileSucceededIff(boolean b) {
    if (b) assertCompileSucceeded();
    else assertCompileFailed();
  }

  /** Assert that a previous call to compile() failed */
  protected void assertCompileFailed() {
    if (!diags.errorsFound()) fail("Expected failed compilation");
  }

  /** Assert that a previous call to compile() failed with a specific error key */
  protected void assertCompileFailed(String key) {
    if (!diags.errorsFound()) fail("Expected failed compilation: " + key);
  ...
  protected void assertCompileFailedOneOf(String... keys) {
    ...
    boolean found = false;
    for (String k : keys) if (diags.containsErrorKey(k)) found = true;
    fail(
        String.format(
            "Expected compilation error with one of %s, found %s",
            Arrays.asList(keys), diags.keys()));
  }
  ...
  protected void assertCompileErrors(String... keys) {
    if (!diags.errorsFound()) fail("Expected failed compilation");
    for (String k : keys) if (!diags.containsErrorKey(k)) fail("Expected compilation error " + k);
  }
  ...
  /**
   * Compile all registered source files, optionally generating class files and returning a File
   * describing the directory to which they were written
   */
  ...
  /**
   * Compile all registered source files, using the provided list of class paths for finding
   * required classfiles, optionally generating class files and returning a File describing the
   * directory to which they were written
   */
  ...
  private File compile(
      List<File> classpaths, List<? extends JavaFileObject> files, boolean generate)
      throws IOException {
    JavaCompiler systemJavaCompiler = ToolProvider.getSystemJavaCompiler();
    try (StandardJavaFileManager fm = systemJavaCompiler.getStandardFileManager(null, null, null)) {
      if (classpaths.size() > 0) fm.setLocation(StandardLocation.CLASS_PATH, classpaths);
      JavacTask ct =
          (JavacTask) systemJavaCompiler.getTask(null, fm, diags, compileOptions, null, files);
      if (generate) {
  ...
  protected Class<?> loadClass(String className, File... destDirs) {
    try {
      ...
      for (File f : destDirs) list.add(new URL("file:" + f.toString().replace("\\", "/") + "/"));
