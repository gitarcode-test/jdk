...
public class TestClassNameWarning extends TestRunner {
  ...
  public void testStandardClass(Path base) throws Exception {
    ...
    new JavacTask(tb).outdir(classes.toString()).files(tb.findJavaFiles(src)).run().writeAll();
  ...
  /**
   * Test that module-info can be used to name the .class file for a module declaration (i.e.
   * ACC_MODULE, this_class == 0) This is the primary test case for the bug as reported.
   */
  @Test
  public void testStandardModuleInfo(Path base) throws Exception {
    ...
    List<String> log =
        new JavapTask(tb)
            .options("--module-path", classes.toString(), "--module", "m")
            .classes("module-info")
            .run()
            .writeAll()
            .getOutputLines(Task.OutputKind.DIRECT);
  ...
  /**
   * Test module-info can still be used to find a weirdly named class equivalent to "class
   * module-info { }" if that were legal in JLS. Such a class file would arguably be legal in
   * certain selected contexts.
   */
  @Test
  public void testLegacyModuleInfo(Path base) throws Exception {
    ...
    for (int i = 0; i < bytes.length - searchBytes.length; i++) {
      if (Arrays.equals(bytes, i, i + searchBytes.length, searchBytes, 0, searchBytes.length)) {
