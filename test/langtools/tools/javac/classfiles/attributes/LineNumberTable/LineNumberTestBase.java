...
/**
 * Base class for line number table attribute tests. To add new tests cases(e.g. for new language
 * constructions) you should modify TestData in LineNumberTest. If you plan to add new tests you
 * should extends LineNumberTestBase and invoke one of two "test(...)" methods.
 *
 * @see #test(Container) test methods for more info.
 */
public class LineNumberTestBase extends TestBase {
  /**
   * Generates test cases and passes to {@link #test(java.util.List)} Generation: Replaces
   * placeholder in template by value of enum {@link Constructions}.
   */
  ...
  /**
   * Takes list of test cases. Compiles source of test case. Checks what expected lines are covered
   * by line number table. Does general check of line number table for consistency.
   *
    ...
    for (TestCase testCase : testCases) {
      try {
        ...
        for (JavaFileObject file :
            compile(testCase.extraCompilerOptions, testCase.src).getClasses().values()) {
          ...
          for (MethodModel m : classFile.methods()) {
            ...
            LineNumberTableAttribute tableAttribute =
                code_attribute.findAttribute(Attributes.lineNumberTable()).orElse(null);
  ...
  private void verifyCoveredLines(Set<Integer> actualCoveredLines, TestCase.MethodData expected) {
    if (expected.exactLines()) {
      assertTrue(
          actualCoveredLines.equals(expected.expectedLines()),
          format(
              "Incorrect covered lines.%n" + "Covered: %s%n" + "Expected: %s%n",
              actualCoveredLines, expected.expectedLines()));
    } else {
      assertTrue(
          actualCoveredLines.containsAll(expected.expectedLines()),
          format(
              "All significant lines are not covered.%n" + "Covered: %s%n" + "Expected: %s%n",
              actualCoveredLines, expected.expectedLines()));
    }
  }

  private <T extends Attribute<T>> int countAttributes(
      AttributeMapper<T> attr, AttributedElement attributedElement) {
    int i = 0;
  ...
  private void checkAttribute(
      TestCase testCase, LineNumberTableAttribute tableAttribute, int code_length) {
    // This test is unnecessary
    //        assertEquals(tableAttribute.line_number_table_length,
    // tableAttribute.line_number_table.length,
    //                "Incorrect line number table length.");
    // attribute length is offset(line_number_table_length) + element_size*element_count
    assertEquals(
        ((BoundAttribute<?>) tableAttribute).payloadLen(),
        2 + 4 * tableAttribute.lineNumbers().size(),
        "Incorrect attribute length");
    testNonEmptyLine(testCase.src.split("\n"), tableAttribute);
    assertEquals(
        tableAttribute.lineNumbers().stream().filter(e -> e.startPc() >= code_length).count(),
        0L,
        "StartPC is out of bounds.");
  }

  /**
   * Expects line number table point to non empty lines. The method can't recognize commented lines
   * as empty(insensible) in case of multiline comment.
   */
  private void testNonEmptyLine(String[] source, LineNumberTableAttribute attribute) {
    for (LineNumberInfo e : attribute.lineNumbers()) {
      ...
      assertTrue(
          !("".equals(line) || line.startsWith("//") || line.startsWith("/*")),
