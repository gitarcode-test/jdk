...
/**
 * Base class for tests of inner classes attribute. The scenario of tests: 1. set possible values of
 * class modifiers. 2. according to set class modifiers, a test generates sources and golden data
 * with {@code generateTestCases}. 3. a test loops through all test cases and checks InnerClasses
 * attribute with {@code test}.
 *
 * <p>Example, possible flags for outer class are {@code Modifier.PRIVATE and Modifier.PUBLIC},
 * possible flags for inner class are {@code Modifier.EMPTY}. At the second step the test generates
 * two test cases: 1. public class A { public class B { class C {} } } 2. public class A { private
 * class B { class C {} } }
 */
public abstract class InnerClassesTestBase extends TestResult {

  private Modifier[] outerAccessModifiers = {
    Modifier.EMPTY, Modifier.PRIVATE, Modifier.PROTECTED, Modifier.PUBLIC
  };
  private Modifier[] outerOtherModifiers = {
    Modifier.EMPTY, Modifier.STATIC, Modifier.FINAL, Modifier.ABSTRACT
  };
   ...
   * Sets properties.
   *
   * <p>Returns generated list of test cases. Method is called in {@code test()}.
   */
  ...
  private void test(String classToTest, TestCase test, String... skipClasses) {
    ...
    try {
      ...
      ClassModel cm =
          readClassFile(
              compile(getCompileOptions(), test.getSource()).getClasses().get(classToTest));
      ...
      checkEquals(
          innerClasses.attributeName(), "InnerClasses", "innerClasses.attribute_name_index");
      // Inner Classes attribute consists of length (2 bytes)
      // and 8 bytes for each inner class's entry.
      checkEquals(
          ((BoundAttribute<?>) innerClasses).payloadLen(),
          2 + 8 * class2Flags.size(),
          "innerClasses.attribute_length");
      checkEquals(
          innerClasses.classes().size(), class2Flags.size(), "innerClasses.number_of_classes");
      ...
      for (InnerClassInfo e : innerClasses.classes()) {
        ...
        if (cm.majorVersion() >= 51 && e.innerClass().index() == 0) {
          ...
          // The outer_class_info_index of sun.tools.classfile will return 0 if it is not a member
          // of a class or interface.
          checkEquals(
              out == null ? 0 : out.index(),
              0,
              "outer_class_info_index " + "in case of inner_name_index is zero : " + baseName);
        }
        String className = baseName.replaceFirst(".*\\$", "");
        checkTrue(class2Flags.containsKey(className), className);
        checkTrue(
            visitedClasses.add(className), "there are no duplicates in attribute : " + className);
        // Convert the Set<string> to Set<AccessFlag>
        Set<AccessFlag> accFlags =
            class2Flags.get(className).stream()
                .map(str -> AccessFlag.valueOf(str.substring(str.indexOf("_") + 1)))
                .collect(Collectors.toSet());
        checkEquals(e.flags(), accFlags, "inner_class_access_flags " + className);
  ...
  /**
   * Methods generates list of test cases. Method generates all possible combinations of acceptable
   * flags for nested inner classes.
   *
  ...
  protected List<TestCase> generateTestCases() {
    ...
    for (List<Modifier> outerMod : outerMods) {
      ...
      sb.append("public class InnerClassesSrc {")
          .append(toString(outerMod))
          .append(' ')
          .append(outerClassType)
          .append(' ')
          .append(prefix)
          .append(' ')
          .append('\n');
      ...
      getAdditionalFlags(
          class2Flags, outerClassType, outerMod.toArray(new Modifier[outerMod.size()]));
  ...
  private List<List<Modifier>> getAllCombinations(
      Modifier[] accessModifiers, Modifier[] otherModifiers) {
  ...
  /**
   * Method is called in generateTestCases(). If you need to add additional access flags, you should
  ...
  public void getAdditionalFlags(
      Map<String, Set<String>> class2Flags, ClassType type, Modifier... mods) {
