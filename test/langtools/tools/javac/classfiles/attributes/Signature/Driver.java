...
/**
 * The main class of Signature tests. Driver reads golden data of each class member that must have a
 * Signature attribute, after that the class compares expected data with actual one.
 *
 * <p>Example of usage Driver: java Driver Test
 *
 * <p>Each member of the class Test should have @ExpectedSignature annotations if it must have the
 * Signature attribute. Anonymous class cannot be annotated. So its enclosing class should be
 * annotated and method isAnonymous of ExpectedSignature must return true.
 */
public class Driver extends TestResult {
  ...
  private Map<String, ExpectedSignature> getExpectedExecutableSignatures(
      Executable[] executables, Predicate<Executable> filterBridge) {
  ...
  private Map<String, ExpectedSignature> getExpectedMethodSignatures(Class<?> clazz) {
    Map<String, ExpectedSignature> methodSignatures =
        getExpectedExecutableSignatures(
            clazz.getDeclaredMethods(), m -> !((java.lang.reflect.Method) m).isBridge());
    methodSignatures.putAll(
        getExpectedExecutableSignatures(clazz.getDeclaredConstructors(), m -> true));
  ...
  private void checkAllMembersFound(
      Set<String> found, Map<String, ExpectedSignature> signatures, String message) {
    if (signatures != null) {
      checkContains(
          found,
  ...
  private void testMethods(
      Map<String, ExpectedSignature> expectedSignatures, ClassModel classFile) {
  ...
  private String getMethodName(MethodModel method) {
    ...
    for (int i = 0; i < method.methodTypeSymbol().parameterCount(); ++i) {
      ...
      String prefix =
          para.componentType() == null ? para.packageName() : para.componentType().packageName();
      methodName
          .append(prefix)
          .append(Objects.equals(prefix, "") ? "" : ".")
          .append(para.displayName());
    }
    methodName.append(")");
  ...
  private void testAttribute(
      String memberName, Supplier<SignatureAttribute> sup, ExpectedSignature expectedSignature) {

    SignatureAttribute attribute = sup.get();
    if (expectedSignature != null && checkNotNull(attribute, memberName + " must have attribute")) {
      checkEquals(attribute.attributeName(), "Signature", "Attribute's name : " + memberName);
      checkEquals(
          ((BoundAttribute<?>) attribute).payloadLen(), 2, "Attribute's length : " + memberName);
      checkEquals(
          attribute.signature().stringValue(),
