...
/**
 * Base class for LocalVariableTable and LocalVariableTypeTable attributes tests. To add tests cases
 * you should extend this class. Then implement {@link #getVariableTables} to get LocalVariableTable
 * or LocalVariableTypeTable attribute. Then add method with local variables. Finally, annotate
 * method with information about expected variables and their types by several {@link
 * LocalVariableTestBase.ExpectedLocals} annotations. To run test invoke {@link #test()} method. If
 * there are variables with the same name, set different scopes for them.
 *
 * @see #test()
 */
public abstract class LocalVariableTestBase extends TestBase {
  ...
  /**
   * Finds expected variables with their type in VariableTable. Also does consistency checks, like
   * variables from the same scope must point to different indexes.
   */
  public void test() throws IOException {
    ...
    if (failed > 0)
      throw new RuntimeException(
          format("Failed %d out of %d. See logs.", failed, testMethods.size()));
  }

  public void test(
      String methodName, Map<String, String> expectedLocals2Types, Map<String, Integer> sig2scope)
      throws IOException {
    ...
    for (MethodModel m : classFile.methods()) {
      ...
      if (methodName.equals(mName)) {
        ...
        List<VariableTable.Entry> entries =
            variableTables.stream().flatMap(table -> table.entries().stream()).collect(toList());
  ...
  private void generalLocalVariableTableCheck(List<? extends VariableTable> variableTables) {
    for (VariableTable localTable : variableTables) {
      ...
      assertEquals(
          localTable.localVariableTableLength(),
          localTable.entries().size(),
      ...
      assertEquals(
          ...
          2 + (5 * 2) * localTable.localVariableTableLength(),
          "Incorrect attribute length");
    }
  }

  private void generalEntriesCheck(
      List<VariableTable.Entry> entries, CodeAttribute code_attribute) {
    for (VariableTable.Entry e : entries) {
      assertTrue(
          e.index() >= 0 && e.index() < code_attribute.maxLocals(),
          "Index "
              + e.index()
              + " out of variable array. Size of array is "
              + code_attribute.maxLocals());
      ...
      assertTrue(
          e.startPC() + e.length() <= code_attribute.codeLength(),
          format(
              "StartPC+Length > code length.%n" + "%s%n" + "code_length = %s",
              e, code_attribute.codeLength()));
    }
  }

  private void checkNamesAndTypes(
      List<VariableTable.Entry> entries, Map<String, String> expectedLocals2Types) {
    Map<String, List<String>> actualNames2Types =
        entries.stream()
            .collect(
                groupingBy(
                    VariableTable.Entry::name, mapping(VariableTable.Entry::type, toList())));
    for (Map.Entry<String, String> name2type : expectedLocals2Types.entrySet()) {
      ...
      assertTrue(
          actualNames2Types.containsKey(name),
          format("There is no record for local variable %s%nEntries: %s", name, entries));

      assertTrue(
          actualNames2Types.get(name).contains(type),
          format(
              "Types are different for local variable %s%nExpected type: %s%nActual type: %s",
              name, type, actualNames2Types.get(name)));
    }
  }

  private void assertIndexesAreUnique(
      ...
      mapEntry.getValue().stream()
          ...
          .forEach(
              e ->
                  assertTrue(
                      e.getValue().size() == 1,
                      "Multiple variables point to the same index in common scope. "
                          + e.getValue()));
    }
  }

  private void checkDoubleAndLongIndexes(
      Collection<VariableTable.Entry> entries, Map<String, Integer> scopes, int maxLocals) {
    // check every scope separately
    Map<Object, List<VariableTable.Entry>> entriesByScope = groupByScope(entries, scopes);
    for (List<VariableTable.Entry> entryList : entriesByScope.values()) {
      Map<Integer, VariableTable.Entry> index2Entry =
          entryList.stream().collect(toMap(VariableTable.Entry::index, e -> e));
          ...
          .forEach(
              e -> {
                assertTrue(
                    e.index() + 1 < maxLocals,
                    format(
                        "Index %s is out of variable array. Long and double occupy 2 cells."
                            + " Size of array is %d",
                        e.index() + 1, maxLocals));
                assertTrue(
                    !index2Entry.containsKey(e.index() + 1),
                    format(
                        "An entry points to the second cell of long/double entry.%n%s%n%s",
                        e, index2Entry.get(e.index() + 1)));
              });
    }
  }
  ...
  private Map<Object, List<VariableTable.Entry>> groupByScope(
    ...
    return entries.stream()
        .collect(groupingBy(e -> scopes.getOrDefault(e.name() + "&" + e.type(), DEFAULT_SCOPE)));
  }
  ...
  /**
   * LocalVariableTable and LocalVariableTypeTable are similar. VariableTable interface is
   * introduced to test this attributes in the same way without code duplication.
   */
      ...
      default String dump() {
        return format(
            "Entry{"
                ...
                + "%n    length  = %d"
                + "%n}",
            name(), type(), index(), startPC(), length());
      }
    }
  }
