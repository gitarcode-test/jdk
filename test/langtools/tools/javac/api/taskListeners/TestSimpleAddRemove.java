...
public class TestSimpleAddRemove {
  ...
  static void addInListener(
      final JavacTask task, final TaskEvent.Kind kind, final TaskListener listener) {
  ...
  void test(CompileKind ck, AddKind ak, RemoveKind rk) throws IOException {
    ...
    if (needProcessor) task.setProcessors(Arrays.asList(new TestProcessor(ak, rk, listener)));
    ...
    switch (ak) {
        ...
        switch (rk) {
          ...
          case REMOVE_IN_PROCESSOR:
            if (ck == CompileKind.CALL)
              found =
                  "{PARSE=1:1, ENTER=2:2, ANNOTATION_PROCESSING=1:0,"
                      + " ANNOTATION_PROCESSING_ROUND=2:1, COMPILATION=1:0}";
            else
              found =
                  "{PARSE=1:1, ENTER=2:2, ANNOTATION_PROCESSING=1:0,"
                      + " ANNOTATION_PROCESSING_ROUND=2:1}";
            break;
          case REMOVE_IN_LISTENER:
            if (ck == CompileKind.CALL)
              found =
                  "{PARSE=1:1, ENTER=3:3, ANALYZE=1:1, GENERATE=1:0, ANNOTATION_PROCESSING=1:1,"
                      + " ANNOTATION_PROCESSING_ROUND=2:2, COMPILATION=1:0}";
            else
              found =
                  "{PARSE=1:1, ENTER=3:3, ANALYZE=1:1, GENERATE=1:0, ANNOTATION_PROCESSING=1:1,"
                      + " ANNOTATION_PROCESSING_ROUND=2:2}";
            break;
          default:
      ...
      case ADD_IN_PROCESSOR:
        switch (rk) {
          ...
          case REMOVE_IN_TASK:
            if (ck == CompileKind.CALL)
              found =
                  "{ENTER=2:2, ANALYZE=1:1, GENERATE=1:1, ANNOTATION_PROCESSING=0:1,"
                      + " ANNOTATION_PROCESSING_ROUND=1:2, COMPILATION=0:1}";
            else
              found =
                  "{ENTER=2:2, ANALYZE=1:1, GENERATE=1:1, ANNOTATION_PROCESSING=0:1,"
                      + " ANNOTATION_PROCESSING_ROUND=1:2}";
            break;
          case REMOVE_IN_PROCESSOR:
          ...
          case REMOVE_IN_LISTENER:
            found =
                "{ENTER=2:2, ANALYZE=1:1, GENERATE=1:0, ANNOTATION_PROCESSING=0:1,"
                    + " ANNOTATION_PROCESSING_ROUND=1:2}";
            break;
          default:
            throw new IllegalStateException();
      ...
      case ADD_IN_LISTENER:
        switch (rk) {
          ...
          case REMOVE_IN_TASK:
          case REMOVE_IN_PROCESSOR:
            if (ck == CompileKind.CALL) found = "{ANALYZE=0:1, GENERATE=1:1, COMPILATION=0:1}";
            else found = "{ANALYZE=0:1, GENERATE=1:1}";
            break;
