...
class T8016177f {
  ...
  interface Stream<T> extends BaseStream<T, Stream<T>> {
    ...
    <R> R collect(
        Supplier<R> resultFactory, BiConsumer<R, ? super T> accumulator, BiConsumer<R, R> combiner);
  ...
  <T, U, R, S_IN extends BaseStream<T, S_IN>, S_OUT extends BaseStream<U, S_OUT>>
      R exerciseTerminalOps(
          TestData<T, S_IN> data, Function<S_IN, S_OUT> streamF, Function<S_OUT, R> terminalF) {
    return null;
  }

  <O> TestData.OfRef<O> ofCollection(Collection<O> collection) {
    return null;
  }
  ...
  void test2(Function<Double, Integer> fdi, TestData.OfRef<Double> td, Stream<Integer> si) {
    exerciseTerminalOps(
        ofCollection((List<Double>) null), s -> s.map(fdi), s -> s.toArray(Integer[]::new));
  }
}
