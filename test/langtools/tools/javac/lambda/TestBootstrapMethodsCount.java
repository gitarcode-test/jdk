...
public class TestBootstrapMethodsCount {
  ...
  public void run(JavaCompiler comp) {
    ...
    JavacTaskImpl ct =
        (JavacTaskImpl)
            comp.getTask(null, null, dc, Arrays.asList("-g"), null, Arrays.asList(source));
    ...
    if (dc.diagFound) {
      throw new AssertionError(
          String.format(
              "Diags found when compiling following code\n%s\n\n%s",
  ...
  void verifyBytecode() {
    ...
    try {
      ...
      if (length != 1) {
        throw new Error(
            "Bad number of method specifiers " + "in BootstrapMethods attribute: " + length);
      }
    } catch (Exception e) {
      e.printStackTrace();
  ...
  class Indifier extends TreeScanner<Void, Void> implements TaskListener {
    ...
    public Void visitMethodInvocation(MethodInvocationTree node, Void p) {
      ...
      if (!oldSym.isConstructor()) {
        ident.sym =
            new Symbol.DynamicMethodSymbol(
                oldSym.name, oldSym.owner, bsm.asHandle(), oldSym.type, new LoadableConstant[0]);
      }
      return null;
    }
