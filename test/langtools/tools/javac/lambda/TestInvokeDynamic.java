...
public class TestInvokeDynamic extends ComboInstance<TestInvokeDynamic> {
    ...
    STRING("Hello!", "String", "Ljava/lang/String;") {
      ...
      boolean check(PoolEntry poolEntry) throws Exception {
        return (poolEntry instanceof StringEntry)
            && ((StringEntry) poolEntry).stringValue().equals(value);
      }
    },
    CLASS(null, "Class<?>", "Ljava/lang/Class;") {
      ...
      boolean check(PoolEntry poolEntry) throws Exception {
        return (poolEntry instanceof ClassEntry)
            && ((ClassEntry) poolEntry).name().equalsString("java/lang/String");
      }
    },
    INTEGER(1, "int", "I") {
      @Override
      boolean check(PoolEntry poolEntry) throws Exception {
        return (poolEntry instanceof IntegerEntry)
            && ((IntegerEntry) poolEntry).intValue() == (Integer) value;
      }
    },
    LONG(1L, "long", "J") {
      @Override
      boolean check(PoolEntry poolEntry) throws Exception {
        return (poolEntry instanceof LongEntry)
            && ((LongEntry) poolEntry).longValue() == (Long) value;
      }
    },
    FLOAT(1.0f, "float", "F") {
      @Override
      boolean check(PoolEntry poolEntry) throws Exception {
        return (poolEntry instanceof FloatEntry)
            && ((FloatEntry) poolEntry).floatValue() == (Float) value;
      }
    },
    DOUBLE(1.0, "double", "D") {
      @Override
      boolean check(PoolEntry poolEntry) throws Exception {
        return (poolEntry instanceof DoubleEntry)
            && ((DoubleEntry) poolEntry).doubleValue() == (Double) value;
      }
    },
    METHOD_HANDLE(null, "MethodHandle", "Ljava/lang/invoke/MethodHandle;") {
      @Override
      boolean check(PoolEntry poolEntry) throws Exception {
        if (!(poolEntry instanceof MethodHandleEntry handleInfo)) return false;
  ...
  public static void main(String... args) throws Exception {
    new ComboTestHelper<TestInvokeDynamic>()
        ...
        .withArrayDimension(
            "SARG", (x, arg, idx) -> x.saks[idx] = arg, 3, StaticArgumentKind.values())
  ...
  void verifyBytecode(Result<Iterable<? extends JavaFileObject>> res) {
    ...
    try (InputStream is = res.get().iterator().next().openInputStream()) {
      ...
      BootstrapMethodsAttribute bsm_attr =
          cm.findAttribute(Attributes.bootstrapMethods()).orElseThrow();
      ...
      if (bsm_spec.arguments().size() != arity.arity) {
        fail("Bad number of static invokedynamic args " + "in BootstrapMethod attribute");
        return;
      }
  ...
  class Indifier extends TreeScanner<Void, Void> implements TaskListener {
    ...
    public Void visitMethodInvocation(MethodInvocationTree node, Void p) {
      ...
      if (!oldSym.isConstructor()) {
        ...
        ident.sym =
            new Symbol.DynamicMethodSymbol(oldSym.name, oldSym.owner, bsm, oldSym.type, staticArgs);
      }
      return null;
    }
