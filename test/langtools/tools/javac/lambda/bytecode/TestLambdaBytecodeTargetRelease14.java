...
public class TestLambdaBytecodeTargetRelease14
    extends ComboInstance<TestLambdaBytecodeTargetRelease14> {
  ...
  static class MethodKind {
    ...
    boolean isOK() {
      if (isDefault() && (!inInterface() || isStatic())) {
        return false;
      } else if (inInterface() && ((!isStatic() && !isDefault()) || isPrivate())) {
        return false;
      } else {
        return true;
      }
  ...
  public static void main(String... args) throws Exception {
    new ComboTestHelper<TestLambdaBytecodeTargetRelease14>()
        ...
        .withArrayDimension(
            "ACCESS", (x, acc, idx) -> x.accessKinds[idx] = acc, 2, AccessKind.values())
        .withArrayDimension(
            "STATIC", (x, sk, idx) -> x.staticKinds[idx] = sk, 2, StaticKind.values())
        .withArrayDimension(
            "DEFAULT", (x, dk, idx) -> x.defaultKinds[idx] = dk, 2, DefaultKind.values())
      ...
      for (CodeElement ce : ea.elementList()) {
        if (ce instanceof InvokeDynamicInstruction indy) {
          ...
          if (!indyInfo.type().equalsString(makeIndyType())) {
            fail(
                "type mismatch for CONSTANT_InvokeDynamic_info "
                    + res.compilationInfo()
                    + "\n"
