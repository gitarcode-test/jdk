...
public class ClassPathWithDoubleQuotesTest extends TestRunner {
  ...
  private static final String[] jarArgs = {
    "cf", "test/jarOut/J.jar", "-C", "test/jarSrc", "J.java"
  };
  public static final String NEW_LINE = System.getProperty("line.separator");
  private static final List<String> expectedFailureOutput1 =
      List.of(
          "A.java:1:18: compiler.err.cant.resolve.location: kindname.class, J, , ,"
              + " (compiler.misc.location: kindname.class, A, null)",
          "A.java:1:23: compiler.err.cant.resolve.location: kindname.class, B, , ,"
              + " (compiler.misc.location: kindname.class, A, null)",
          "2 errors");
  private static final List<String> expectedFailureOutput2A =
      List.of(
          ...
          "2 errors",
          "2 warnings");
  private static final List<String> expectedFailureOutput2B =
      List.of(
  ...
  public void test(Path base) throws Exception {
    ...
    /**
     * In any system there can be three possible scenarios: 1 - The system swallows the problem
     * character (the quote in this case) and the test case compiles 2 - The problem character gets
     * into javac, but it's not bad enough to trigger InvalidPathException, but it does mean you
     * can't find the file you're looking for 3 - The problem character gets into javac and is bad
     * enough to trigger InvalidPathException, in which case javac needs to handle the exception in
     * a reasonable way.
     */

    // testing scenario 1
    System.err.println(
        "invoking javac EXEC mode without double quotes in the CLASSPATH env variable");
    ...
    // testing scenario 2
    System.err.println(
        "Simulate a system in which double quotes are preserved in the environment variable,"
    ...
    List<String> log2 =
        new JavacTask(tb, Task.Mode.EXEC)
            .envVar("CLASSPATH", "\"test/jarOut/J.jar" + File.pathSeparator + "test/src\"")
            .options(
                "-Xlint:path", "-XDrawDiagnostics", "-J-Duser.language=en", "-J-Duser.country=US")
            .files("test/src/A.java")
            .run(Task.Expect.FAIL)
            .writeAll()
            .getOutputLines(Task.OutputKind.STDERR);
    log2 =
        log2.stream()
            .filter(s -> !s.matches("^Picked up .*JAVA.*OPTIONS:.*"))
            .collect(Collectors.toList());
    Assert.check(
        log2.equals(expectedFailureOutput2A) || log2.equals(expectedFailureOutput2B),
        "unexpected output");
  }
}
