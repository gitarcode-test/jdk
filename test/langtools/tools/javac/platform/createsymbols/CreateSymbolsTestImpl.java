...
public class CreateSymbolsTestImpl {
  ...
  void testMethodRemoved() throws Exception {
    ...
    doTest(
        "package t; public class T { public void m() throws IllegalStateException { } public void"
            + " a() { } }",
        "package t; public class T {                                                  public void"
            + " a() { } }",
  ...
  void testAnnotationAttributeDefaultvalue() throws Exception {
    // TODO: this only verifies that there is *some* value, but we should also verify there is a
    // specific value:
    doTest(
        "package t; public @interface T { }",
        "package t;\n"
            + "public @interface T {\n"
            ...
            + "    public java.lang.annotation.RetentionPolicy enumValue() default"
  ...
  void testConstantTest() throws Exception {
    ...
    doPrintElementTest(
        "package t; public class T { public static final int A = 1; }",
        ...
        "package t;\n\n"
            ...
            + "}\n");
    doPrintElementTest(
        "package t; public class T { public static final boolean A = false; }",
  ...
  void testParseAnnotation() throws Exception {
    CreateSymbols.parseAnnotations(
        "@Lsun/Proprietary+Annotation;@Ljdk/Profile+Annotation;(value=I1)", new int[1]);
    CreateSymbols.parseAnnotations("@Ltest;(value={\"\"})", new int[1]);
    CreateSymbols.parseAnnotations(
        "@Ljava/beans/ConstructorProperties;(value={\"path\"})", new int[1]);
  ...
  void testStringCharLiterals() throws Exception {
    doPrintElementTest(
        "package t;"
            ...
            + "}",
        "package t;" + "    public class T {" + "    public static final char c = '\\uffff';" + "}",
        "t.T",
        "package t;\n\n"
            + "public class T {\n"
  ...
  void testGenerification() throws Exception {
    doTest(
        "package t; public class T { public class TT { public Object t() { return null; } } }",
  ...
  void doTest(String code7, String code8, String testCode, Expect result7, Expect result8)
      throws Exception {
    ...
    new JavacTask(tb)
        .sources(testCode)
        .options(
            "-d", scratch.toAbsolutePath().toString(), "-classpath", computeClassPath(classes, "7"))
        .run(result7)
        .writeAll();
    new JavacTask(tb)
        .sources(testCode)
        .options(
            "-d", scratch.toAbsolutePath().toString(), "-classpath", computeClassPath(classes, "8"))
        .run(result8)
        .writeAll();
  }
  ...
  private static String computeClassPath(Path classes, String version) throws IOException {
    try (Stream<Path> elements = Files.list(classes)) {
      return elements
          .filter(el -> el.getFileName().toString().contains(version))
  ...
  void doPrintElementTest(
      ...
      throws Exception {
    doPrintElementTest(
        new String[] {code7}, new String[] {code8}, className7, printed7, className8, printed8);
  }
  ...
  void doPrintElementTest(
    ...
    out =
        new JavacTask(tb, Task.Mode.CMDLINE)
            .options(
                "-d",
                scratch.toAbsolutePath().toString(),
                "-classpath",
                computeClassPath(classes, "7"),
                "-Xprint",
                className7)
            .run(Expect.SUCCESS)
  ...
  void doTestEquivalence(String code7, String code8, String testClass) throws Exception {
    Path classes = prepareVersionedCTSym(new String[] {code7}, new String[] {code8});
    Path classfile =
        classes.resolve("78").resolve("java.base").resolve(testClass.replace('.', '/') + ".class");
...
""");
    doPrintElementTest(
        "package t;" + "public record R() {" + "}",
        "package t;" + "public record R(int i) {" + "}",
        "t.R",
        """
        package t;
        \n\
  ...
  void doTestComplex(
      String printClass, String expected, String depSuccess, String depFailure, String... code)
      throws Exception {
    ToolBox tb = new ToolBox();
    ...
    compileAndPack(output, ver9Jar, code);
    ...
    ExcludeIncludeList acceptAll =
        new ExcludeIncludeList(null, null) {
          @Override
    ...
    new CreateSymbols()
        .createSymbols(
            null,
            symbolsDesc.toAbsolutePath().toString(),
            classesZip.toAbsolutePath().toString(),
            0,
            "9",
            "",
            modules.toString(),
    ...
    try (Stream<Path> elements = Files.list(classes)) {
      modulePath =
          elements
              .filter(el -> el.getFileName().toString().contains("9"))
              .map(el -> el.resolve("m"))
              .map(el -> el.toAbsolutePath().toString())
              .collect(Collectors.joining(File.pathSeparator));
    }
...
""",
  ...
  void doTestData(String data, String... code) throws Exception {
    String testClasses = System.getProperty("test.classes");
    Path output = Paths.get(testClasses, "test-data" + i++);
    deleteRecursively(output);
    Files.createDirectories(output);
  ...
  void doTestIncluded(String code, String... includedClasses) throws Exception {
    ...
    try {
      includeAll = false;
      Path classes =
          prepareVersionedCTSym(
              new String[] {code}, new String[] {"package other; public class Other {}"});
  ...
  Path prepareVersionedCTSym(String[] code7, String[] code8, Consumer<Path> adjustClassFiles)
      throws Exception {
    String testClasses = System.getProperty("test.classes");
    Path output = Paths.get(testClasses, "test-data" + i++);
    deleteRecursively(output);
  ...
  void testGenerate(Path jar7, Path jar8, Path descDest, String version, String classDest)
      throws IOException {
    deleteRecursively(descDest);

    List<VersionDescription> versions =
        Arrays.asList(
            new VersionDescription(jar7.toAbsolutePath().toString(), "7", null),
  ...
  void testModuleMainClass() throws Exception {
    ...
    Path moduleInfo = classesDir.resolve("9").resolve("m").resolve("module-info.class");

    cf.parse(moduleInfo).attributes().stream()
        .filter(attr -> attr instanceof ModuleMainClassAttribute)
        .forEach(
            attr -> {
              String expectedMain = "Lmain/Main;";
              String mainClass =
                  ((ModuleMainClassAttribute) attr).mainClass().asSymbol().descriptorString();
              if (!Objects.equals(expectedMain, mainClass)) {
                throw new AssertionError(
                    "Expected " + expectedMain + " as a main class, " + "but got: " + mainClass);
              }
            });
  }
  ...
  void compileAndPack(Path output, Path outputFile, Consumer<Path> adjustClassFiles, String... code)
      throws Exception {
    ToolBox tb = new ToolBox();
    ...
    new JavacTask(tb)
        .sources(code)
        .options("-d", scratch.toAbsolutePath().toString())
        .run(Expect.SUCCESS);
                ...
                if (element instanceof ModuleAttribute) {
                  builder.with(
                      ModulePackagesAttribute.ofNames(
                          packages.stream().map(pack -> PackageDesc.of(pack)).toList()));
                }
              });
      try (OutputStream out = Files.newOutputStream(moduleInfo)) {
        out.write(newData);
