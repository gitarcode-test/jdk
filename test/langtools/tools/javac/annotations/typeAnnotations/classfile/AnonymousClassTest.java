...
public class AnonymousClassTest {
  ...
  static void testAnonymousClassDeclaration() throws Exception {
    ClassModel cm =
        ClassFile.of().parse(Paths.get(ToolBox.testClasses, "AnonymousClassTest$1.class"));
    ...
    assertEquals(
        ...
        rvta.annotations().stream().map(a -> annotationDebugString(cm, null, a)).collect(toSet()));
  }
  ...
  static void testInnerClassMethod() throws Exception {
    ...
    assertEquals(
        Set.of("@LAnonymousClassTest$TA;(2) NEW, offset=0, location=[INNER_TYPE, INNER_TYPE]"),
        annotations.stream().map(a -> annotationDebugString(cm, cAttr, a)).collect(toSet()));
  }
  ...
  private static String annotationDebugString(
      ClassModel cm, CodeAttribute cAttr, TypeAnnotation annotation) {
    TypeAnnotation.TargetInfo info = annotation.targetInfo();
    int offset =
        info instanceof TypeAnnotation.OffsetTarget offsetInfo
            ? cAttr.labelToBci(offsetInfo.target())
            : -1;
    String name;
    try {
    ...
    for (TypeAnnotation.TypePathComponent path : annotation.targetPath()) {
      if (path.typePathKind() == TypeAnnotation.TypePathComponent.Kind.INNER_TYPE)
        location.add(path.typePathKind().name());
      else location.add(path.typePathKind() + "(" + path.typeArgumentIndex() + ")");
    }
    return String.format(
        "@%s(%s) %s, offset=%d, location=%s",
        name, annotationValueDebugString(cm, annotation), info.targetType(), offset, location);
  }
