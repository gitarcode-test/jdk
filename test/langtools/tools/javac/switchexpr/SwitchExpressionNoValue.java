...
public class SwitchExpressionNoValue extends ComboInstance<SwitchExpressionNoValue> {
  ...
  public static void main(String... args) throws Exception {
    new ComboTestHelper<SwitchExpressionNoValue>()
        .withDimension(
            "SWITCH_EXPRESSION",
            (x, method) -> x.switchExpression = method,
            SwitchExpression.values())
        .withDimension(
            "EXPRESSION", (x, expression) -> x.expression = expression, Expression.values())
        .withDimension("CONTEXT", (x, context) -> x.context = context, Context.values())
        .withFilter(
            test ->
                test.context.expressionType == test.expression.expressionType
  ...
  public enum SwitchExpression implements ComboParameter {
    INT(
        "switch (i) { case 0 -> throw new RuntimeException(\"test\"); default -> {if (true) throw"
            + " new RuntimeException(\"test\"); else yield 0; } }",
        ExpressionType.INT),
    BOOLEAN(
        "switch (i) { case 0 -> throw new RuntimeException(\"test\"); default -> {if (true) throw"
            + " new RuntimeException(\"test\"); else yield true; } }",
  ...
  public enum Expression implements ComboParameter {
    ...
    BINARY_LONGER1(
        "3 + #{SWITCH_EXPRESSION} + #{SWITCH_EXPRESSION} + #{SWITCH_EXPRESSION}",
        ExpressionType.INT),
    BINARY_LONGER2(
        "3 + switch (0) { default -> 0; } + #{SWITCH_EXPRESSION} + #{SWITCH_EXPRESSION}",
        ExpressionType.INT),
    BINARY_LONGER3(
        "3 + #{SWITCH_EXPRESSION} + switch (0) { default -> 0; } + #{SWITCH_EXPRESSION}",
        ExpressionType.INT),
