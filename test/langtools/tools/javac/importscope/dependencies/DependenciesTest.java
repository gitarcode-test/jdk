 ...
 *     jdk.compiler/com.sun.tools.javac.file jdk.compiler/com.sun.tools.javac.tree
...
public class DependenciesTest {
  ...
  void run() throws IOException {
    ...
    try (Stream<Path> tests = Files.list(src)) {
      tests.map(p -> Files.isRegularFile(p) ? Stream.of(p) : silentWalk(p)).forEach(this::runTest);
    }
  }
  ...
  void runTest(Stream<Path> inputs) {
    ...
    try (JavacFileManager fm = tool.getStandardFileManager(null, null, null)) {
      ...
      SearchAnnotations scanner = new SearchAnnotations(reconTrees, reconTask.getElements());
      ...
      if (!scanner.topLevel2Expected.equals(deps.topLevel2Completing)) {
        throw new IllegalStateException(
            "expected=" + scanner.topLevel2Expected + "; actual=" + deps.topLevel2Completing);
      }
    } catch (IOException ex) {
      throw new IllegalStateException(ex);
    } finally {
  ...
  static final class SearchAnnotations extends TreePathScanner<Void, Void> {
    ...
    final Map<String, Set<PhaseDescription>> topLevel2Expected = new HashMap<>();

    public SearchAnnotations(Trees trees, Elements elements) {
      this.trees = trees;
      this.elements = elements;
      this.triggersCompleteAnnotation = elements.getTypeElement(TriggersComplete.class.getName());
  ...
  static final class ClearAnnotations extends TreePathScanner<Void, Void> {
    ...
    void removeCurrentNode() {
      ...
      spans2Clear.add(
          new int[] {
            (int) positions.getStartPosition(topLevel, node),
  ...
  static final class PhaseDescription {
    ...
    public String toString() {
      return "@annotations.TriggersComplete(of="
          + flatname
          + ".class,"
          + "at=annotations.Phase."
          + phase
          + ')';
    }
