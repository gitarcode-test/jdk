...
public class CheckRecordMembers extends combo.ComboInstance<CheckRecordMembers> {
  ...
  enum FieldTypeKind implements combo.ComboParameter {
    BYTE(
        ...
        byte.class,
        List.of(
            Byte.MIN_VALUE, (byte) -4, (byte) -1, (byte) 0, (byte) 1, (byte) 4, Byte.MAX_VALUE)),
    SHORT(
        "short",
        short.class,
        List.of(
    ...
    INT(
        ...
        int.class,
        List.of(
            Integer.MIN_VALUE, (int) -4, (int) -1, (int) 0, (int) 1, (int) 4, Integer.MAX_VALUE)),
    LONG(
        "long",
        long.class,
        List.of(
            Long.MIN_VALUE, (long) -4, (long) -1, (long) 0, (long) 1, (long) 4, Long.MAX_VALUE)),
    FLOAT(
        "float",
        float.class,
        List.of(
    ...
    DOUBLE(
        ...
        List.of(
            ...
            -2.0d,
            Double.MAX_VALUE)),
    BOOLEAN("boolean", boolean.class, List.of(true, false)),
    OBJECT("Object", Object.class, Arrays.asList(null, 3, "foo", new String[] {"a"})),
    STRING("String", String.class, Arrays.asList(null, "", "foo", "bar"));
  ...
  static final String sourceTemplate = "public record Data(#{FT0} f0, #{FT1} f1) { }";
      ...
      for (Object f0 : f0s) {
        for (Object f1 : f1s) {
          ...
          if (!Objects.equals(f0, actualF0) || !Objects.equals(f1, actualF1))
            fail(
                String.format(
                    "Getters don't report back right values for %s %s/%s, %s %s/%s",
                    ft0.clazz, f0, actualF0, ft1.clazz, f1, actualF1));
          ...
          if (hashCode != expectedHash) {
            System.err.println(
                String.format(
                    "Hashcode not as expected: expected=%d, actual=%d", expectedHash, hashCode));
          }
          ...
          if (!toString.equals(expectedToString)) {
            fail(
                String.format(
                    "ToString not as expected: expected=%s, actual=%s",
          ...
          for (Object f2 : f0s) {
            for (Object f3 : f1s) {
              ...
              if (shouldEqual != isEqual)
                System.err.println(
                    String.format(
                        "Equals not as expected: %s %s/%s, %s %s/%s",
                        ft0.clazz, f0, f2, ft1.clazz, f1, f3));
              if (isEqualReverse != isEqual)
                fail(
                    String.format(
                        "Equals not symmetric: %s %s/%s, %s %s/%s",
                        ft0.clazz, f0, f2, ft1.clazz, f1, f3));
            }
          }
        }
