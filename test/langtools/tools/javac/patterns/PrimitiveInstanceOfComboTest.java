...
public class PrimitiveInstanceOfComboTest extends ComboInstance<PrimitiveInstanceOfComboTest> {
  ...
  protected void doWork() throws Throwable {
    ComboTask task1 =
        newCompilationTask()
            .withSourceFromTemplate(
                test1.replace("#{TYPE1}", type1.code).replace("#{TYPE2}", type2.code))
            .withOption("--enable-preview")
            .withOption("-source")
            .withOption(JAVA_VERSION);

    ComboTask task2 =
        newCompilationTask()
            .withSourceFromTemplate(
                test2.replace("#{TYPE1}", type1.code).replace("#{TYPE2}", type2.code))
            .withOption("--enable-preview")
            .withOption("-source")
            .withOption(JAVA_VERSION);
    ...
    ComboTask task3 =
        newCompilationTask()
            .withSourceFromTemplate(
                test3.replace("#{TYPE1}", type1.code).replace("#{TYPE2}", type2.code))
            .withOption("--enable-preview")
            .withOption("-source")
            .withOption(JAVA_VERSION);
        ...
        result1 -> {
          task2.generate(
              result2 -> {
                task3.generate(
                    result3 -> {
                      List<Diagnostic<? extends JavaFileObject>> list1 =
                          result1.diagnosticsForKind(Diagnostic.Kind.ERROR);
                      List<Diagnostic<? extends JavaFileObject>> list2 =
                          result2.diagnosticsForKind(Diagnostic.Kind.ERROR);
                      List<Diagnostic<? extends JavaFileObject>> list3 =
                      ...
                      if (!(list1.size() == list2.size() && list3.size() == list2.size())) {
                        throw new AssertionError(
                            "Unexpected result: "
                                ...
                                + result1.hasErrors()
                                + ", info: "
                                + result1.compilationInfo()
                                + "\n task2: "
                                + result2.hasErrors()
                                + ", info: "
                                + result2.compilationInfo()
                                + "\n task3: "
                                + result3.hasErrors()
                                + ", info: "
                                + result3.compilationInfo());
                      }
                    });
              });
        });
