 ...
 * @library /tools/lib
 * @enablePreview
 * @modules jdk.compiler/com.sun.tools.javac.api jdk.compiler/com.sun.tools.javac.jvm
 *     jdk.compiler/com.sun.tools.javac.main jdk.compiler/com.sun.tools.javac.platform
...
public class JavaBaseTest {
  ...
  final List<List<String>> modifiers =
      List.of(List.of("static"), List.of("transitive"), List.of("static", "transitive"));
  ...
  void test(List<String> mods, String target, Mode mode) {
    ...
    try {
      Path base =
          Paths.get(String.join("-", mods)).resolve(target).resolve(mode.name().toLowerCase());
  ...
  void testSource(Path base, List<String> mods, String target) throws Exception {
    ...
    tb.writeJavaFiles(src, "module m { requires " + String.join(" ", mods) + " java.base; }");
    Path modules = Files.createDirectories(base.resolve("modules"));
    boolean expectOK = target.equals("9");

    JavacTask jct = new JavacTask(tb).outdir(modules);

    if (target.equals("current")) jct.options("-XDrawDiagnostics");
    else jct.options("-XDrawDiagnostics", "--release", target);

    String log =
        jct.files(tb.findJavaFiles(src))
  ...
  void testClass(Path base, List<String> mods, String target) throws Exception {
    ...
    Path src = base.resolve("src");
    tb.writeJavaFiles(src, "module mx { requires m; }");
    Path modules = Files.createDirectories(base.resolve("modules"));
    ...
    String log =
        new JavacTask(tb)
            .outdir(modules)
            .options("-XDrawDiagnostics", "--module-path", base.resolve("test-modules").toString())
    ...
    if (!expectOK) {
      ...
      for (String mod : mods) {
        String flag =
            mod.equals("static") ? "ACC_STATIC_PHASE (0x0040)" : "ACC_TRANSITIVE (0x0020)";
        String message =
            "- compiler.err.cant.access: m.module-info,"
  ...
  void createClass(Path base, List<String> mods, String target) throws Exception {
    ...
    tb.writeJavaFiles(src1, "module m { requires java.base; }");
    Path modules1 = Files.createDirectories(base.resolve("interim-modules"));

    JavacTask jct = new JavacTask(tb).outdir(modules1);

    if (!target.equals("current")) {
      jct.options("--release", target);
    }

    jct.files(tb.findJavaFiles(src1)).run(Task.Expect.SUCCESS);
    ...
    List<ModuleRequireInfo> requires =
        Arrays.asList(new ModuleRequireInfo[modAttr1.requires().size()]);
    ...
    byte[] newBytes =
        ClassFile.of()
            .transformClass(
                cm1,
                ClassTransform.dropping(ce -> ce instanceof ModuleAttribute)
                    .andThen(
                        ClassTransform.endHandler(classBuilder -> classBuilder.with(modAttr2))));
