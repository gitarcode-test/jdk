...
public class AnnotationProcessing extends ModuleTestBase {
  ...
  public void testAPSingleModule(Path base) throws Exception {
    ...
    tb.writeJavaFiles(m1, "module m1x { }", "package impl; public class Impl { }");
    ...
    if (!log.isEmpty()) throw new AssertionError("Unexpected output: " + log);
  }
  ...
  public void testAPMultiModule(Path base) throws Exception {
    ...
    tb.writeJavaFiles(m1, "module m1x { }", "package impl1; public class Impl1 { }");

    tb.writeJavaFiles(m2, "module m2x { }", "package impl2; public class Impl2 { }");
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
      if (module2ExpectedEnclosedElements == null) {
        ...
        for (String moduleDef : expectedEnclosedElements.split(",")) {
          ...
          module2ExpectedEnclosedElements.put(
              module2Packages[0], List.of(module2Packages[1].split(":")));
        }
      }
      ...
      for (Element root : roundEnv.getRootElements()) {
        ...
        assertEquals(module2ExpectedEnclosedElements.get(moduleName), actualElements);
  ...
  public static final class VerifyUsesProvidesAP extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
      ...
      assertEquals(
          "api.Api", provides.iterator().next().getService().getQualifiedName().toString());
      assertEquals(
          "impl.Impl",
          provides.iterator().next().getImplementations().get(0).getQualifiedName().toString());
  ...
  public void testPackageNoModule(Path base) throws Exception {
    ...
    tb.writeJavaFiles(src, "package api; public class Api { }");

    String log =
        new JavacTask(tb)
            .options(
                "-processor",
                VerifyPackageNoModule.class.getName(),
                "-source",
                "8",
                "-Xlint:-options")
            .outdir(classes)
  ...
  public void testQualifiedClassForProcessing(Path base) throws Exception {
    ...
    tb.writeJavaFiles(m1, "module m1x { }", "package impl; public class Impl { int m1x; }");

    tb.writeJavaFiles(m2, "module m2x { }", "package impl; public class Impl { int m2x; }");
    ...
    for (Mode mode : new Mode[] {Mode.API, Mode.CMDLINE}) {
      ...
      if (!expected.equals(log)) throw new AssertionError("Unexpected output: " + log);
    }
  }
  ...
  public void testModuleInRootElements(Path base) throws Exception {
    ...
    tb.writeJavaFiles(m1, "module m1x { exports api; }", "package api; public class Api { }");
  ...
  public void testAnnotationsInModuleInfo(Path base) throws Exception {
    ...
    tb.writeJavaFiles(m1, "@Deprecated module m1x { }");

    Path m2 = moduleSrc.resolve("m2x");

    tb.writeJavaFiles(m2, "@SuppressWarnings(\"\") module m2x { }");
    ...
    List<String> expectedLog = List.of("Note: AP Invoked", "Note: AP Invoked");
  ...
  public void testAnnotationsWithoutTargetInModuleInfo(Path base) throws Exception {
    ...
    List<String> expectedLog =
        List.of("Note: m1x/test.A AP Invoked", "Note: m1x/test.A AP Invoked");

    List<String> actualLog =
        new JavacTask(tb)
            .options(
                "-processor",
                AnnotationsWithoutTargetInModuleInfo.class.getName()
                    + ","
  ...
  public void testGenerateInMultiModeAPI(Path base) throws Exception {
    ...
    for (String module : Arrays.asList("", "m1x/")) {
      for (String originating : Arrays.asList("", ", jlObject")) {
        tb.writeJavaFiles(m1, "package test; class Test { api1.Impl i; }");
        ...
        runCompiler(
            ...
            moduleSrc,
            classes,
            "createSource(() -> filer.createSourceFile(\""
                + module
                + "api1.Impl\""
                + originating
                + "), \"api1.Impl\", \"package api1; public class Impl {}\")",
            "--module-source-path",
            moduleSrc.toString());
        ...
        runCompiler(
            ...
            moduleSrc,
            classes,
            "createClass(() -> filer.createClassFile(\""
                + module
                + "api1.Impl\""
                + originating
                + "), \"api1.Impl\", \"package api1; public class Impl {}\")",
    ...
    runCompiler(
        ...
        m1,
        classes,
        "doReadResource(() -> filer.getResource(StandardLocation.MODULE_SOURCE_PATH, \"m1x/api1\","
            + " \"api\"), \"1\")",
        "--module-source-path",
        moduleSrc.toString());
    ...
    runCompiler(
        ...
        m1,
        classes,
        "createSource(() -> filer.createResource(StandardLocation.CLASS_OUTPUT, \"m1x/impl\","
            + " \"impl\"), \"impl\", \"impl\")",
        "--module-source-path",
        moduleSrc.toString());
    ...
    for (String pack : Arrays.asList("clash", "doesnotexist")) {
      tb.writeJavaFiles(m1, "package test; class Test { " + pack + ".Pass i; }");
      runCompiler(
          base,
          moduleSrc,
          classes,
    ...
    String[] failingCases = {
      ...
      "expectFilerException(() -> filer.createResource(StandardLocation.CLASS_OUTPUT, \"clash\","
          + " \"impl\"))",
      "expectFilerException(() -> filer.getResource(StandardLocation.CLASS_OUTPUT, \"clash\","
          + " \"impl\"))",
      ...
      "expectFilerException(() -> filer.createResource(StandardLocation.CLASS_OUTPUT,"
          + " \"doesnotexist\", \"impl\"))",
      "expectFilerException(() -> filer.getResource(StandardLocation.CLASS_OUTPUT,"
          + " \"doesnotexist\", \"impl\"))",
      ...
      // wrong module given:
      "expectException(() -> filer.getResource(StandardLocation.SYSTEM_MODULES,"
          + " \"java.compiler/java.lang\", \"Object.class\"))",
    };

    for (String failingCode : failingCases) {
      System.err.println("failing code: " + failingCode);
      runCompiler(
          base, moduleSrc, classes, failingCode, "--module-source-path", moduleSrc.toString());
    }
  }

  public abstract static class GeneratingAP extends AbstractProcessor {
  ...
  public void testGenerateSingleModule(Path base) throws Exception {
    ...
    tb.writeJavaFiles(m1, "module m1x { }", "package test; class Test { impl.Impl i; }");
    ...
    for (String[] options :
        new String[][] {
          new String[] {"-sourcepath", m1.toString()},
      ...
      for (String module : Arrays.asList("", "m1x/")) {
        for (String originating : Arrays.asList("", ", jlObject")) {
          tb.writeJavaFiles(m1, "package test; class Test { impl.Impl i; }");
          ...
          runCompiler(
              ...
              "createSource(() -> filer.createSourceFile(\""
                  ...
                  + "impl.Impl\""
                  + originating
                  + "), \"impl.Impl\", \"package impl; public class Impl {}\")",
              options);
          ...
          runCompiler(
              ...
              classes,
              """
createSource(() -> filer.createResource(StandardLocation.CLASS_OUTPUT, "impl", "impl\""""
                  + originating
                  + "), \"impl\", \"impl\")",
              options);
    ...
    runCompiler(
        ...
        m1,
        classes,
        "doReadResource(() -> filer.getResource(StandardLocation.SOURCE_PATH, \"impl\","
            + " \"resource\"), \"1\")",
        "-sourcepath",
        m1.toString());
    ...
    runCompiler(
        ...
        m1,
        classes,
        "expectFilerException(() -> filer.getResource(StandardLocation.SOURCE_PATH, \"m1x/impl\","
            + " \"resource\"))",
        "-sourcepath",
        m1.toString());
    ...
    runCompiler(
        ...
        m1,
        classes,
        "checkResourceExists(() -> filer.getResource(StandardLocation.SYSTEM_MODULES,"
    ...
    runCompiler(
        ...
        m1,
        classes,
        "expectFilerException(() -> filer.getResource(StandardLocation.SYSTEM_MODULES,"
    ...
    runCompiler(
        ...
        m1,
        classes,
        "createSource(() -> filer.createResource(StandardLocation.CLASS_OUTPUT, \"impl\","
            + " \"impl\"), \"impl\", \"impl\")",
        "--module-source-path",
        src.toString());
    assertFileExists(classes, "m1x", "impl", "impl");
    ...
    String[] failingCases = {
      ...
      "expectFilerException(() -> filer.createResource(StandardLocation.CLASS_OUTPUT, \"m1x/fail\","
          + " \"Fail\"))",
    ...
    for (String failingCode : failingCases) {
      System.err.println("failing code: " + failingCode);
      runCompiler(base, m1, classes, failingCode, "-sourcepath", m1.toString());
    }

    tb.deleteFiles(m1.resolve("module-info.java"));
    tb.writeJavaFiles(m1, "package test; class Test { }");
  ...
  private void runCompiler(Path base, Path src, Path classes, String code, String... options)
      throws IOException {
  ...
  private void runCompiler(
      ...
      Path classes,
      String code,
      Consumer<Path> generateToClasses,
      String... options)
      throws IOException {
    ...
    opts.add("-processor");
    opts.add("AP");
    new JavacTask(tb).options(opts).outdir(classes).files(findJavaFiles(src)).run().writeAll();
  }
...
"""
  ...
  public void testGenerateInUnnamedModeAPI(Path base) throws Exception {
    ...
    tb.writeJavaFiles(src, "class T {}");

    new JavacTask(tb)
        .options(
            "-processor", UnnamedModeAPITestAP.class.getName(),
  ...
  public void testGenerateInNoModeAPI(Path base) throws Exception {
    ...
    new JavacTask(tb)
        .options(
            "-processor",
            UnnamedModeAPITestAP.class.getName(),
            "-source",
            "8",
            "-target",
            "8",
  ...
  public void testDisambiguateAnnotations(Path base) throws Exception {
    ...
    tb.writeJavaFiles(
        m3, "module m3x { requires m1x; }", "package impl; import api.*; @A @B public class T {}");

    Path m4 = src.resolve("m4x");

    tb.writeJavaFiles(
        m4, "module m4x { requires m2x; }", "package impl; import api.*; @A @B public class T {}");
    ...
    log =
        new JavacTask(tb)
            .options(
                "-processor",
                SelectAnnotationATestAP.class.getName()
                    + ","
    ...
    expected = List.of("SelectAnnotationBTestAP", "SelectAnnotationBTestAP");
  ...
  public void testDisambiguateAnnotationsUnnamedModule(Path base) throws Exception {
    ...
    List<String> expected = List.of("SelectAnnotationBTestAP", "SelectAnnotationBTestAP");
  ...
  public void testUnboundLookupNew(Path base) throws Exception {
      ...
      List<String> log =
          new JavacTask(tb)
              .options(
                  ...
                  "-processor",
                  UnboundLookupNew.class.getName(),
...

    {
      ...
      tb.writeJavaFiles(m1, "module m1x { exports test; }", "package test; public class Test { }");

      tb.writeJavaFiles(m2, "module m2x { requires m1x; }", "package test; public class Test { }");
...

    {
      ...
      tb.writeJavaFiles(m1, "module m1x { }", "package test; public class Test { }");

      tb.writeJavaFiles(
          m2, "module m2x { requires m1x; requires m3x; }", "package test; public class Test { }");

      tb.writeJavaFiles(m3, "module m3x { }", "package test; public class Test { }");
...

    {
      ...
      tb.writeJavaFiles(m2, "module m2x { requires m1x; requires m3x; }");

      tb.writeJavaFiles(m3, "module m3x { exports test; }", "package test; public class Test { }");

      tb.writeJavaFiles(m4, "module m4x { }", "package test; public class Test { }");

      {
        ...
        List<Set<String>> expected =
            Arrays.asList(
                variants(
                    "module-info.java:1:1: compiler.err.package.clash.from.requires: m2x, test,"
                        + " m1x, m3x"),
...

      {
        ...
        List<String> expected =
            Arrays.asList(
                "module-info.java:1:1: compiler.err.package.clash.from.requires: m2x, test, m1x,"
                    + " m3x",
...

    {
      ...
      tb.writeJavaFiles(m2, "module m2x { requires m1x; }");
  ...
  public void testUnboundLookup(Path base) throws Exception {
    ...
    tb.writeJavaFiles(src, "package impl.conflict.src; public class Impl { }");
    ...
    new JavacTask(tb)
        .options(
            ...
            System.getProperty("test.class.path"),
            "-processor",
            UnboundLookupGenerate.class.getName(),
            "-XDrawDiagnostics")
        .outdir(classes)
        .files(findJavaFiles(moduleSrc))
        .run()
  ...
  public static final class UnboundLookup extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
      ...
      assertTypeElementNotFound(
          "impl.conflict.module.Impl"); // check that the warning/note is produced only once
      assertPackageElementNotFound("impl.conflict.module");
      assertTypeElementExists(
          "impl.conflict.src.Impl",
          processingEnv.getOptions().containsKey("unnamedmodule") ? "" : "m1x");
      assertPackageElementExists(
          "impl.conflict.src",
          processingEnv.getOptions().containsKey("unnamedmodule") ? "" : "m1x");
      assertTypeElementNotFound("impl.conflict.clazz.pkg");
      assertPackageElementNotFound(
          "unique"); // do not return packages without members in module mode
    ...
    private void assertTypeElementExists(String name, String expectedModule) {
      assertElementExists(
          name, "class", processingEnv.getElementUtils()::getTypeElement, expectedModule);
    }

    private void assertPackageElementExists(String name, String expectedModule) {
      assertElementExists(
          name, "package", processingEnv.getElementUtils()::getPackageElement, expectedModule);
    }

    private void assertElementExists(
        String name, String type, Function<String, Element> getter, String expectedModule) {
      ...
      if (!mod.getQualifiedName().contentEquals(expectedModule)) {
        throw new AssertionError(
            name + " found in an unexpected module: " + mod.getQualifiedName());
      }
    }
  ...
  public void testWrongDefaultTargetModule(Path base) throws Exception {
    ...
    List<String> log =
        new JavacTask(tb)
            .options("--default-module-for-created-files=m!", "-XDrawDiagnostics")
            .outdir(classes)
            .files(findJavaFiles(src))
            .run(Task.Expect.FAIL)
            .writeAll()
    ...
    List<String> expected =
        Arrays.asList("- compiler.err.bad.name.for.option: --default-module-for-created-files, m!");
  ...
  public void testClassPhantomPackageClash(Path base) throws Exception {
    ...
    try (StandardJavaFileManager fm =
      ...
      new JavacTask(tb)
          .fileManager(testFM)
          .options(
              "-processor", NoOpTestAP.class.getName(),
              "-sourcepath", src.toString())
          .outdir(classes)
          .files(src.resolve("module-info.java"), src.resolve("api").resolve("Nested.java"))
          .run()
          .writeAll()
          .getOutputLines(OutputKind.STDERR);
    }
