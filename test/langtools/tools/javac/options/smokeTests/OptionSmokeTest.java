 ...
 * @library /tools/lib
 * @modules jdk.compiler/com.sun.tools.javac.api jdk.compiler/com.sun.tools.javac.code
 *     jdk.compiler/com.sun.tools.javac.main jdk.compiler/com.sun.tools.javac.util
...
public class OptionSmokeTest extends TestRunner {
  ...
  public void optionA2(Path base) throws Exception {
    doTest(
        base,
        "error: key in annotation processor option '-A1e=2' is not a dot-separated sequence of"
            + " identifiers",
        "-A1e=2");
  }
  ...
  public void profileAndBSP(Path base) throws Exception {
    doTest(
        base,
        "error: profile and bootclasspath options cannot be used together",
        String.format(
            "-profile compact1 -bootclasspath . -target %s -source %s",
            Source.DEFAULT.name, Source.DEFAULT.name));
  }
  ...
  public void optionNotAvailableWithTarget(Path base) throws Exception {
    doTest(
        base,
        String.format("error: option -profile not allowed with target %s", Source.DEFAULT.name),
  ...
  public void optionTooMany(Path base) throws Exception {
    doTest(
        base,
        "error: option --default-module-for-created-files can only be specified once",
  ...
  public void noSrcFiles(Path base) throws Exception {
    doTestNoSource(
        base, "error: no source files", String.format("-target %s", Source.DEFAULT.name));
  }
  ...
  public void sourceAndTargetMismatch(Path base) throws Exception {
    doTest(
        base,
        String.format(
            "error: specified target release %s is too old for the specified source release %s",
            Source.MIN.name, Source.DEFAULT.name),
  ...
  public void targetConflictsWithDefaultSource(Path base) throws Exception {
    doTest(
        base,
        String.format(
            "error: specified target release %s is too old for the default source release %s",
            Source.MIN.name, Source.DEFAULT.name),
  ...
  //         doTest(base, String.format("warning: profile compact2 is not valid for target release
  // %s", Source.MIN.name),
  //                 String.format("-profile compact2 -target %s -source %s", Source.MIN.name,
  // Source.MIN.name));
  //     }
  ...
  public void fileNotFound(Path base) throws Exception {
    ...
    Assert.check(
        log.startsWith(String.format("error: file not found: notExistent%sT.java", fileSeparator)),
  ...
  public void notADirectory(Path base) throws Exception {
    doTest(
        base,
        String.format(
            "error: not a directory: notADirectory%ssrc%sDummy.java", fileSeparator, fileSeparator),
  ...
  public void notAFile(Path base) throws Exception {
    ...
    String log =
        new JavacTask(tb, Task.Mode.CMDLINE)
            ...
            .getOutput(Task.OutputKind.DIRECT);
    Assert.check(
        log.startsWith(String.format("error: not a file: notAFile%sdir.java", fileSeparator)));
  }
  ...
  public void badValueForOption(Path base) throws Exception {
    doTestNoSource(
        base,
        "error: bad value for --patch-module option: \'notExistent\'",
  ...
  public void patchModuleMoreThanOnce(Path base) throws Exception {
    doTestNoSource(
        base,
        "error: --patch-module specified more than once for m",
    ...
    List<String> log =
        new JavacTask(tb, Task.Mode.EXEC)
            .envVar(
                "JDK_JAVAC_OPTIONS",
                String.format(
                    "--add-exports jdk.compiler%scom.sun.tools.javac.jvm=\"ALL-UNNAMED",
                    fileSeparator))
  ...
  public void releaseVersionNotSupported(Path base) throws Exception {
    doTestNoSource(base, "error: release version 99999999 not supported", "--release 99999999");
  }
  ...
  public void releaseAndBootclasspath(Path base) throws Exception {
    ...
    doTestNoSource(
        base,
        "error: option -Xbootclasspath: cannot be used together with --release",
        String.format("--release %s -Xbootclasspath:any", Source.DEFAULT.name));
    doTestNoSource(
        base,
        "error: option -Xbootclasspath/p: cannot be used together with --release",
    ...
    doTestNoSource(
        base,
        "error: option -extdirs cannot be used together with --release",
  ...
  public void consistentSystemOptionHandlingWithAnEmptyDirectory(Path base) throws Exception {
    ...
    doTestNoSource(
        base,
        "error: illegal argument for --system: %s".formatted(base),
        String.format("--system %s", base));
  }
