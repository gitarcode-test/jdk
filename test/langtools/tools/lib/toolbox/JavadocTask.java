...
public class JavadocTask extends AbstractTask<JavadocTask> {
  ...
  public JavadocTask sourcepath(String sourcepath) {
    this.sourcepath =
        Stream.of(sourcepath.split(ToolBox.pathSeparator))
            .filter(s -> !s.isEmpty())
            .map(s -> Paths.get(s))
  ...
  public JavadocTask files(Path... files) {
    this.files = Stream.of(files).map(Path::toString).collect(Collectors.toList());
    return this;
  }
  ...
  public JavadocTask files(List<Path> files) {
    this.files = files.stream().map(Path::toString).collect(Collectors.toList());
    return this;
  }

  /**
   * Sets the sources to be documented. Each source string is converted into an in-memory object
   * that can be passed directly to the tool.
   *
  ...
  public JavadocTask sources(String... sources) {
    fileObjects =
        Stream.of(sources).map(s -> new ToolBox.JavaSource(s)).collect(Collectors.toList());
    return this;
  }
  ...
  /**
   * Sets the doclet class to be invoked by javadoc. Note: this is applicable only in API mode.
   *
   ...
   *
   * @return a Result object indicating the outcome of the execution and the content of any output
   *     written to stdout, stderr, or the main stream by the tool.
   */
  @Override
  public Task.Result run() {
    if (mode == Task.Mode.EXEC) return runExec();
    ...
    try {
      ...
      if (outdir != null)
        setLocationFromPaths(
            DocumentationTool.Location.DOCUMENTATION_OUTPUT, Collections.singletonList(outdir));
      if (classpath != null) setLocationFromPaths(StandardLocation.CLASS_PATH, classpath);
      if (sourcepath != null) setLocationFromPaths(StandardLocation.SOURCE_PATH, sourcepath);
      List<String> allOpts = new ArrayList<>();
      if (options != null) allOpts.addAll(options);
      ...
      if (internalFileManager != null) internalFileManager.close();
    }
  }
  ...
  private List<String> getAllArgs() {
    ...
    if (classes != null) args.addAll(classes);
    if (files != null) args.addAll(files);

    return args;
  }

  private String toSearchPath(List<Path> files) {
    return files.stream().map(Path::toString).collect(Collectors.joining(ToolBox.pathSeparator));
  }
  ...
  private Iterable<? extends JavaFileObject> joinFiles(
    ...
    if (fileObjects == null) return filesAsFileObjects;
    List<JavaFileObject> combinedList = new ArrayList<>();
    for (JavaFileObject o : filesAsFileObjects) combinedList.add(o);
