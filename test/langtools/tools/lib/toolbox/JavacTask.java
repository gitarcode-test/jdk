...
public class JavacTask extends AbstractTask<JavacTask> {
  ...
  /**
   * Sets the sources to be compiled or analyzed. Each source string is converted into an in-memory
   * object that can be passed directly to the compiler.
   *
  ...
  public Result run(Expect expect) {
    ...
    return run(
        ...
        (exitCode, testName) -> {
          if (exitCode == 4) {
            throw new TaskError(
                "Task " + testName + " failed due to a javac crash " + "(exit code 4)");
          }
        });
  }
   ...
   *
   * @return a Result object indicating the outcome of the compilation and the content of any output
   *     written to stdout, stderr, or the main stream by the compiler.
  ...
  private void setLocationFromPaths(StandardLocation location, List<Path> files)
      throws IOException {
