...
public class CheckModuleTest {
  ...
  public void compileAll() throws Exception {
    ...
    modules.forEach(mn -> assertTrue(CompilerUtils.compileModule(SRC_DIR, MODS_DIR, mn)));
  }
    ...
    return new Object[][] {
      {
        ...
        new ModuleMetaData[] {
          ...
          // suggested version
          new ModuleMetaData("mIV").requires("java.compiler"),
          // reduced version
          new ModuleMetaData("mIV").requires("java.compiler")
        }
      },
  ...
  public void modularTest(String name, ModuleMetaData[] data) throws Exception {
    ...
    try (JdepsUtil.Command jdeps = JdepsUtil.newCommand(cmd)) {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        descriptors[i].requires().stream().forEach(req -> metaData.checkRequires(req));
      }
