 ...
 * @test
 * @bug 8080608
 * @summary Test that jdeps verbose output has a summary line when dependencies are found within the
 *     same archive. For each testcase, compare the result obtained from jdeps with the expected
 *     result.
 * @modules jdk.jdeps/com.sun.tools.jdeps java.base/sun.security.x509
...
public class JdepsDependencyClosure {
  ...
  /**
   * Helper class used to store arguments to pass to {@code JdepsDependencyClosure.test} as well as
   * expected results.
   */
  static class TestCaseData {
    ...
    TestCaseData(
        Map<String, Set<String>> expectedDependencies,
        String expectedText,
        boolean closure,
        String[] args) {
    ...
    public void test() {
      if (expectedDependencies != null) {
        String format =
            closure ? "Running (closure): jdeps %s %s %s %s" : "Running: jdeps %s %s %s %s";
     ...
     *
     * @param pattern The pattern that will passed through to jdeps -e This is expected to match
     *     only one class.
     * @param arcPath The archive to analyze. A jar or a class directory.
     * @param classes For each reported archive dependency couple, the expected list of classes in
     *     the source that will be reported as having a dependency on the class in the target that
     *     matches the given pattern.
     * @param dependencies For each archive dependency couple, a singleton list containing the name
     *     of the class in the target that matches the pattern. It is expected that the pattern will
     *     match only one class in the target. If the pattern matches several classes the expected
     *     text may no longer match the jdeps output.
     * @param archives A list of archive dependency couple in the form {{sourceName1, sourcePath1,
     *     targetDescription1, targetPath1} {sourceName2, sourcePath2, targetDescription2,
     *     targetPath2} ... } For a JDK module - e.g. java.base, the targetDescription is usually
     *     something like "JDK internal API (java.base)" and the targetPath is usually the module
     *     name "java.base".
     * @param closure Whether jdeps should be recursively invoked to build the closure.
     * @return An instance of TestCaseData containing all the information needed to perform the
     *     jdeps invokation and test its output.
     */
    public static TestCaseData make(
        String pattern,
        String arcPath,
        String[][] classes,
        String[][] dependencies,
        String[][] archives,
        boolean closure) {
      final String[] args = new String[] {"-e", pattern, "-v", arcPath};
        ...
        expectedText +=
            Stream.of(classes[i])
                .map(
                    (cn) ->
                        String.format(
                            JDEPS_VERBOSE_TEXT_FORMAT,
                            cn,
                            dependencies[index][0],
                            archives[index][2]))
                .reduce(
                    String.format(JDEPS_SUMMARY_TEXT_FORMAT, archives[i][0], archives[index][3]),
                    (s1, s2) -> s1.concat(s2));
  ...
  static TestCaseData makeTestCaseOne() {
    ...
    return TestCaseData.make(
        "use.internal.UseJdkInternalClass", arcPath, classes, dependencies, archives, false);
  }
  ...
  static TestCaseData makeTestCaseTwo() {
    ...
    String[][] classes =
        new String[][] {{"use.internal.UseJdkInternalClass", "use.internal.UseJdkInternalClass2"}};
    String[][] dependencies = new String[][] {{"sun.security.x509.X509CertInfo"}};
    String[][] archive =
        new String[][] {
          {arcName, arcPath, "JDK internal API (java.base)", "java.base"},
        };
    return TestCaseData.make(
        "sun.security.x509.X509CertInfo", arcPath, classes, dependencies, archive, false);
  }
  ...
  static TestCaseData makeTestCaseThree() {
    ...
    final String[][] dependencies =
        new String[][] {
          {"use.internal.UseJdkInternalClass"}, {"use.internal.UseClassWithJdkInternal"}
        };
    final String[][] archives =
        new String[][] {
          {arcName, arcPath, arcName, arcPath},
    ...
    return TestCaseData.make(
        "use.internal.UseJdkInternalClass", arcPath, classes, dependencies, archives, true);
  }
  ...
  static TestCaseData makeTestCaseFour() {
    ...
    return TestCaseData.make(
        "sun.security.x509.X509CertInfo", arcPath, classes, dependencies, archives, true);
  }

  static final List<Supplier<TestCaseData>> dataSuppliers =
      Arrays.asList(
  ...
  /**
   * The OutputStreamParser is used to parse the format of jdeps. It is thus dependent on that
   * format.
   */
      ...
      if (b == sep[sepi]) {
        if (++sepi == sep.length) {
          ...
          if (lines[0].toString().startsWith("  ")) {
            throw new RuntimeException(
                "Bad formatting: " + "summary line missing for\n" + lines[0]);
          }
   ...
   * The main method.
   *
   * <p>Can be run in two modes:
   *
   * <ul>
   *   <li>From jtreg: expects 1 argument in the form {@code --test:<test-nb>}
   *   <li>From command line: expected syntax is {@code -e <pattern> -v jar [jars..]}
   * </ul>
   *
   * <p>When called from the command line this method will call jdeps recursively to build a closure
   * of the dependencies on {@code <pattern>} and print a summary.
   *
   * <p>When called from jtreg - it will call jdeps either once only or recursively depending on the
   * pattern.
   *
  ...
  public static void test(
      String[] args, Map<String, Set<String>> expected, String expectedText, boolean closure) {
    try {
  ...
  public static void printDiagnostic(
      String[] args, String expectedText, Throwable t, boolean closure) {
    if (expectedText != null || t instanceof TextFormatException) {
      System.err.println("=====   TEST FAILED   =======");
      System.err.println(
          "command: " + Stream.of(args).reduce("jdeps", (s1, s2) -> s1.concat(" ").concat(s2)));
  ...
  public static void doTest(
    ...
    while (!search.isEmpty()) {
      ...
      searched.add(args[1]);
      Map<String, Set<String>> deps = alldeps.computeIfAbsent(args[1], (k) -> new HashMap<>());
      ...
      if (VERBOSE) {
        System.out.println(
            "Found: "
                + deps.values().stream().flatMap(s -> s.stream()).collect(Collectors.toSet()));
      }
    ...
    if (expectedText != null && COMPARE_TEXT) {
      ...
      if (text.toString().equals(expectedText)) {
        ...
        throw new TextFormatException(
            "jdeps output is not as expected", expectedText, text.toString());
      }
    }
    if (expected != null) {
      if (expected.equals(result)) {
        ...
        throw new RuntimeException(
            "Bad dependencies: Expected " + expected + " but found " + result);
      } else {
        throw new TextFormatException(
            "Bad dependencies: Expected " + expected + " but found " + result,
            expectedText,
