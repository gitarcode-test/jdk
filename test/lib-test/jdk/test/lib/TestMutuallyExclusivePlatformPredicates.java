...
/**
 * @test
 * @summary Verify that for each group of mutually exclusive predicates defined in
 *     jdk.test.lib.Platform one and only one predicate evaluates to true.
 * @library /test/lib
...
public class TestMutuallyExclusivePlatformPredicates {
  ...
  private static enum MethodGroup {
    ...
    private MethodGroup(String... methodNames) {
      this.methodNames = Collections.unmodifiableList(Arrays.asList(methodNames));
    }
  }
  ...
  public static void main(String args[]) {
    ...
    notIgnoredMethodGroups.forEach(TestMutuallyExclusivePlatformPredicates::verifyPredicates);

    TestMutuallyExclusivePlatformPredicates.verifyCoverage();
  }

  /**
   * Verifies that one and only one predicate method defined in {@link jdk.test.lib.Platform}, whose
   * name included into methodGroup will return {@code true}.
   *
  ...
  /**
   * Verifies that all predicates defined in {@link jdk.test.lib.Platform} were either tested or
   * explicitly ignored.
   */
