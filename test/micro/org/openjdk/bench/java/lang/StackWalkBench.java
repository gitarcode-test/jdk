...
public class StackWalkBench {
  ...
  /**
   * Build a call stack of a given size, then run trigger code in it. (Does not account for existing
   * frames higher up in the JMH machinery).
   */
  ...
  /** Call stack to test filtering. TestMarker will make a call on the stack. */
  ...
  /** StackWalker.getCallerClass() with generated call stack of the given depth. */
  @Benchmark
  ...
  /**
   * Baseline for generating a call stack of the given depth. (No StackWalk operations are executed)
   */
  @Benchmark
  ...
  /**
   * Use StackWalker.walk() to filter the StackFrames, looking for the TestMarker class, which will
   * be (approximately) 'mark' calls back up the call stack.
   */
  @Benchmark
  ...
  /**
   * Use StackWalker.walk() to filter the StackFrames, looking for the TestMarker class, which will
   * be (approximately) depth/2 calls back up the call stack.
   */
  @Benchmark
            ...
            new Runnable() {
              public void run() {
                ...
                sw.walk(
                    s -> {
                      localBH.consume(
                          s.filter((f) -> TestMarker.class.equals(f.getDeclaringClass()))
                              .findFirst()
                              .get()
                              .getDeclaringClass());
                      return null;
  ...
  //    static PerfCounter streamTime =
  // PerfCounter.newPerfCounter("jdk.stackwalk.testStreamsElapsedTime");
