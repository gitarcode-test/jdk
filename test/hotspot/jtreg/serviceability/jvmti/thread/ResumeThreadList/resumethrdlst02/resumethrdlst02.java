 ...
 * @test
 *
 * @summary converted from VM Testbase nsk/jvmti/ResumeThreadList/resumethrdlst002.
 * VM Testbase keywords: [quick, jpda, jvmti, noras]
 * VM Testbase readme:
 * DESCRIPTION
 *     This JVMTI test exercises JVMTI thread function ResumeThreadList().
 *     This tests checks that each suspended thread resumed by ResumeThreadList()
 *     will run and finish.
 * COMMENTS
 *
 * @library /test/lib
 * @run main/othervm/native -agentlib:resumethrdlst02=-waittime=5,threads=10 resumethrdlst02
 */
...
class resumethrdlst02Thread extends Thread {
    private volatile boolean threadReady = false;
    private volatile boolean shouldFinish = false;

    // make thread with specific name
    public resumethrdlst02Thread(String name) {
        super(name);
    }
