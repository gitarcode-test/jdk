...
public class resumethrdlst001 extends DebugeeClass {

    // load native library if required
    static {
        System.loadLibrary("resumethrdlst001");
    }
    ...
    public static int run(String argv[], PrintStream out) {
        return new resumethrdlst001().runIt(argv, out);
    }
    ...
    // tested thread
    resumethrdlst001Thread threads[] = null;
    int threadsCount = 0;
    ...
    public int runIt(String argv[], PrintStream out) {
        ...
        threads = new resumethrdlst001Thread[threadsCount];
        for (int i = 0; i < threadsCount; i++) {
            threads[i] = new resumethrdlst001Thread("TestedThread #" + i);
        }
...
class resumethrdlst001Thread extends Thread {
    private volatile boolean threadReady = false;
    private volatile boolean shouldFinish = false;

    // make thread with specific name
    public resumethrdlst001Thread(String name) {
        super(name);
    }
