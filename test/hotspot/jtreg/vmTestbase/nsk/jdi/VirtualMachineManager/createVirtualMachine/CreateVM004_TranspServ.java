...
public class CreateVM004_TranspServ extends TransportService {
    ...
    void handshake(Socket s, long timeout) throws IOException {
        ...
        while (received < hello.length) {
            ...
            try {
                ...
                throw new IOException("##> CreateVM004_TranspServ: Handshake timeout");
            }
            if (n < 0) {
                s.close();
                throw new IOException
                    ("##> CreateVM004_TranspServ: Handshake FAILED - connection prematurally closed!");
            }
            received += n;
        }
        for (int i=0; i<hello.length; i++) {
            if (b[i] != hello[i]) {
                throw new IOException
                    ("##> CreateVM004_TranspServ: Handshake FAILED - unrecognized message from target VM");
            }
        }
    ...
    public String description() {
        return "SocketTransportService for nsk/jdi/VirtualMachineManager/createVirtualMachine/createVM004 test.";
    }
    ...
    public Capabilities capabilities() {
        return new CreateVM004_TranspServCapabilities();
    }
    ...
    public Connection attach(String address, long attachTimeout, long handshakeTimeout)
        ...
        if (address == null) {
            throw new NullPointerException("##> CreateVM004_TranspServ: attach() - address is null");
        }
        if (attachTimeout < 0 || handshakeTimeout < 0) {
            throw new IllegalArgumentException("##> CreateVM004_TranspServ: attach() - timeout is negative");
        }
        ...
        try {
            ...
            throw new IllegalArgumentException(
                "##> CreateVM004_TranspServ: attach() - unable to parse port number in address");
        }
        ...
        try {
            ...
            throw new TransportTimeoutException
                ("##> CreateVM004_TranspServ: attach() - timed out trying to establish connection");
        }
        ...
        return new CreateVM004_Connection(s);
    }
    ...
    public ListenKey startListening(String address) throws IOException {
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "##> CreateVM004_TranspServ: startListening() - unable to parse port number in address");
        }
    ...
    public void stopListening(ListenKey listener) throws IOException {
        if (!(listener instanceof SocketListenKey)) {
            throw new IllegalArgumentException
                ("##> CreateVM004_TranspServ: stopListening() - Invalid listener");
        }
    ...
    public Connection accept(ListenKey listener, long acceptTimeout, long handshakeTimeout) throws IOException {
        if (acceptTimeout < 0 || handshakeTimeout < 0) {
            throw new IllegalArgumentException
                ("##> CreateVM004_TranspServ: accept() - timeout is negative");
        }
        if (!(listener instanceof SocketListenKey)) {
            throw new IllegalArgumentException
                ("##> CreateVM004_TranspServ: accept() - Invalid listener");
        }
        ServerSocket ss;
        ...
        try {
            ...
            throw new TransportTimeoutException
                ("##> CreateVM004_TranspServ: accept() - timeout waiting for connection");
        }
...
} // end of CreateVM004_TranspServ class

class CreateVM004_Connection extends Connection {
    ...
    CreateVM004_Connection(Socket socket) throws IOException {
        this.socket = socket;
    ...
    public byte[] readPacket() throws IOException {
        ...
        synchronized (receiveLock) {
            ...
            if (len < 0) {
                throw new IOException
                    ("##> CreateVM004_Connection: readPacket() - protocol error: invalid Packet's length");
            }
            ...
            while (len > 0) {
                ...
                if (count < 0) {
                    throw new EOFException
                        ("##> CreateVM004_Connection: readPacket() - read() method returns negative value");
                }
                len -= count;
                off += count;
            }
    ...
    public void writePacket(byte b[]) throws IOException {
        ...
        if (b.length < 11) {
            throw new IllegalArgumentException
                ("##> CreateVM004_Connection: writePacket() - packet is insufficient size");
        }
        int b0 = b[0] & 0xff;
        int b1 = b[1] & 0xff;
        int b2 = b[2] & 0xff;
        ...
        if (len > b.length) {
            throw new IllegalArgumentException
                ("##> CreateVM004_Connection: writePacket() - length mis-match");
        }
...
} // end of CreateVM004_Connection class
...
} // end of CreateVM004_TranspServCapabilities class
