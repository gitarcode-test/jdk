...
public class CreateVM005_TranspServ extends TransportService {
    ...
    void handshake(Socket s, long timeout) throws IOException {
        ...
        while (received < hello.length) {
            ...
            if (n < 0) {
                ...
                throw new IOException
                    ("##> CreateVM005_TranspServ: Handshake FAILED - connection prematurally closed!");
            }
            received += n;
        }
        for (int i=0; i<hello.length; i++) {
            if (b[i] != hello[i]) {
                throw new IOException
                    ("##> CreateVM005_TranspServ: Handshake FAILED - unrecognized message from target VM");
            }
        }
    ...
    public Connection attach(String address, long attachTimeout, long handshakeTimeout)
        ...
        try {
            ...
            throw new IllegalArgumentException(
                "##> CreateVM005_TranspServ: attach() - unable to parse port number in address");
        }
        ...
        try {
            ...
            throw new TransportTimeoutException
                ("##> CreateVM005_TranspServ: attach() - timed out trying to establish connection");
        }
    ...
    public ListenKey startListening(String address) throws IOException {
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "##> CreateVM005_TranspServ: startListening() - unable to parse port number in address");
        }
    ...
    public void stopListening(ListenKey listener) throws IOException {
        if (!(listener instanceof SocketListenKey)) {
            throw new IllegalArgumentException
                ("##> CreateVM005_TranspServ: stopListening() - Invalid listener");
        }
    ...
    public Connection accept(ListenKey listener, long acceptTimeout, long handshakeTimeout) throws IOException {
        if (acceptTimeout < 0 || handshakeTimeout < 0) {
            throw new IllegalArgumentException
                ("##> CreateVM005_TranspServ: accept() - timeout is negative");
        }
        if (!(listener instanceof SocketListenKey)) {
            throw new IllegalArgumentException
                ("##> CreateVM005_TranspServ: accept() - Invalid listener");
        }
        ServerSocket ss;
        ...
        try {
            ...
            throw new TransportTimeoutException
                ("##> CreateVM005_TranspServ: accept() - timeout waiting for connection");
        }
...
} // end of CreateVM005_TranspServ class
...
class CreateVM005_Connection extends Connection {
    ...
    public byte[] readPacket() throws IOException {
        if (!isOpen()) {
            throw new ClosedConnectionException
                ("##> CreateVM005_Connection: readPacket() - connection is closed");
        }
        synchronized (receiveLock) {
            ...
            if (len < 0) {
                throw new IOException
                    ("##> CreateVM005_Connection: readPacket() - protocol error: invalid Packet's length");
            }
            ...
            while (len > 0) {
                ...
                if (count < 0) {
                    throw new EOFException
                        ("##> CreateVM005_Connection: readPacket() - read() method returns negative value");
                }
                len -= count;
                off += count;
            }
    ...
    public void writePacket(byte b[]) throws IOException {
        if (!isOpen()) {
            throw new ClosedConnectionException
                ("##> CreateVM005_Connection: writePacket() - connection is closed");
        }
        ...
        if (b.length < 11) {
            throw new IllegalArgumentException
                ("##> CreateVM005_Connection: writePacket() - packet is insufficient size");
        }
        int b0 = b[0] & 0xff;
        int b1 = b[1] & 0xff;
        int b2 = b[2] & 0xff;
        ...
        if (len > b.length) {
            throw new IllegalArgumentException
                ("##> CreateVM005_Connection: writePacket() - length mis-match");
        }
...
} // end of CreateVM005_Connection class
