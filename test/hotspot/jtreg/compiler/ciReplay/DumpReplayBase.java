...
public abstract class DumpReplayBase extends CiReplayBase {
  ...
  public String getReplayFileName() {
    Asserts.assertEQ(
        replayFiles.size(), 1, "Test should only dump 1 replay file when trying to replay compile");
  ...
  public boolean generateReplay(String... vmopts) {
    ...
    try {
      ...
      Asserts.assertFalse(
          replayFiles.isEmpty(), "Did not find a replay file starting with " + DUMP_REPLAY_PATTERN);
    ...
    if (matcher.find()) {
      try {
        ...
        throw new RuntimeException(
            "Could not parse compile id from filename \"" + replayFileName + "\"");
      }
    } else {
      throw new RuntimeException(
          "Could not find compile id in filename \"" + replayFileName + "\"");
    }
  }
