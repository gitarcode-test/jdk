...
public class TypeUniverse {
  ...
  static {
    ...
    javaTypes =
        Collections.unmodifiableSet(
            classes.stream().map(c -> metaAccess.lookupJavaType(c)).collect(Collectors.toSet()));
  }
  ...
  /**
   * Reads the value of all {@code static final} fields from a given class into an array of {@link
   * ConstantValue}s.
   */
            ...
            if (value != null) {
              ...
              if (boxed instanceof Object[]) {
                ...
                for (int i = 0; i < arr.length; i++) {
                  ...
                  if (element != null) {
                    res.add(
                        new ConstantValue(javaField.format("%H.%n[" + i + "]"), element, arr[i]));
                  }
                }
              }
            }
