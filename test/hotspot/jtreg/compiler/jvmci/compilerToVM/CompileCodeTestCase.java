...
public class CompileCodeTestCase {
  ...
  public CompileCodeTestCase(Object receiver, Executable executable, int bci) {
  ...
  public Pair<Object, ? extends Throwable> invoke(Object[] args) {
    ...
    throw new Error(executable + " has unsupported type " + executable.getClass());
  }

  public NMethod compile(int level) {
    String directive =
        "[{ match: \""
            + executable.getDeclaringClass().getName().replace('.', '/')
            + "."
  ...
  public InstalledCode toInstalledCode() {
    ...
    return CTVMUtilities.getInstalledCode(
        resolvedJavaMethod, executable.getName(), address, entryPoint);
  }
  ...
  private abstract static class Dummy implements Interface {
    protected Dummy() {}
  ...
  static {
    ...
    map.put(
        CompileCodeTestCase.Dummy.class,
    ...
    map.put(
        CompileCodeTestCase.Interface.class,
