...
public class PoolHelper extends MethodHolder {
  ...
  /**
   * Filters only those methods who belong to Klass or its internal class Internal and named as
   * "method" or is a constructor
   */
  public static final Predicate<Executable> METHOD_FILTER =
      executable -> {
        String methodName = executable.getName();
        String className = executable.getDeclaringClass().getName();
        return className.matches(".*(Klass)(\\$Internal)?")
            && (methodName.equals("method") || methodName.equals(className)); // if method is <init>
      };
   ...
   */
  public List<Pair<Executable, Callable<?>>> getAllMethods(Predicate<Executable> filter) {
