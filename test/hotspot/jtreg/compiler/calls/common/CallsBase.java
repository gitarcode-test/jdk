...
public abstract class CallsBase {
  ...
  protected CallsBase() {
    try {
      ...
      calleeMethod = getClass().getDeclaredMethod("callee", getCalleeParametersTypes());
  ...
  protected Class[] getCalleeParametersTypes() {
    return new Class[] {int.class, long.class, float.class, double.class, String.class};
  }
  ...
  protected final boolean compilationLevelsSupported() {
    ...
    boolean calleeCompLevelSupported =
        compileCallee <= 0
            || (compileCallee > 0
  ...
  /**
   * Run basic logic of a test by doing compile action(if needed). An arguments can be
   * -compileCallee $calleeCompilationLevel and/or -compileCaller $callerCompilationLevel and/or
   * -nativeCaller and/or -nativeCallee to indicate that native methods for caller/callee should be
   * used
   *
    ...
    if (compilationLevelsSupported()) {
      ...
      synchronized (lock) {
        ...
        if (compileCallee > 0 && !compileMethod(calleeMethod, compileCallee)) {
          System.out.println(
              "WARNING: Blocking compilation failed for calleeMethod (timeout?). Skipping.");
          return;
        }
        if (checkCalleeCompilationLevel) {
          Asserts.assertEQ(
              expectedCalleeCompilationLevel,
        ...
        if (compileCaller > 0 && !compileMethod(callerMethod, compileCaller)) {
          System.out.println(
              "WARNING: Blocking compilation failed for callerMethod (timeout?). Skipping.");
          return;
        }
        if (checkCallerCompilationLevel) {
          Asserts.assertEQ(
              expectedCallerCompilationLevel,
      ...
      System.out.println(
          "WARNING: Requested compilation levels are "
  ...
  /**
   * A method checking values. Should be used to verify if all parameters are passed as expected.
   * Parameter N should have a value indicating number "N" in respective type representation.
   */
