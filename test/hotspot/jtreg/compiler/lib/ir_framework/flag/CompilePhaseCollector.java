...
/**
 * This class collects all unique compile phases associated with all {@link IR @IR} annotations of a
 * method. If {@link CompilePhase#DEFAULT} is found, then we look up the default compile phases of
 * all {@link IRNode} placeholder strings.
 *
 * @see FlagVM
...
class CompilePhaseCollector {
  ...
  /**
   * Returns a map "method name -> compile phases set" that can be used by {@link
   * CompilerDirectivesFlagBuilder}.
   */
  public static Map<String, Set<CompilePhase>> collect(Class<?> testClass) {
    ...
    try {
      for (Method method : irAnnotatedMethods) {
        methodNameToCompilePhasesMap.put(
            testClass.getCanonicalName() + "::" + method.getName(), collectCompilePhases(method));
      }
    } catch (TestFormatException e) {
      // Create default map and let the IR matcher report the format failures later in the driver
      // VM.
