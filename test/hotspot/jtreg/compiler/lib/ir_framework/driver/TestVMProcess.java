...
/**
 * This class prepares, creates, and runs the "test" VM with verification of proper termination. The
 * class also stores information about the test VM which is later queried for IR matching. The
 * communication between this driver VM and the test VM is done over a dedicated socket.
 *
 * @see TestVM
...
public class TestVMProcess {
  ...
  private static final boolean PREFER_COMMAND_LINE_FLAGS =
      Boolean.getBoolean("PreferCommandLineFlags");
  private static final int WARMUP_ITERATIONS = Integer.getInteger("Warmup", -1);
  private static final boolean VERIFY_VM =
      Boolean.getBoolean("VerifyVM") && Platform.isDebugBuild();
  ...
  private void prepareTestVMFlags(
      List<String> additionalFlags,
      TestFrameworkSocket socket,
      Class<?> testClass,
      Set<Class<?>> helperClasses,
      int defaultWarmup) {
    ...
    // Add server property flag that enables test VM to print encoding for IR verification last and
    // debug messages.
    ...
    if (WARMUP_ITERATIONS < 0 && defaultWarmup != -1) {
      // Only use the set warmup for the framework if not overridden by a valid -DWarmup property
      // set by a test.
  ...
  private static String[] getVerifyFlags() {
    return new String[] {
      "-XX:+UnlockDiagnosticVMOptions", "-XX:+VerifyOops", "-XX:+VerifyStack",
          "-XX:+VerifyLastFrame",
  ...
  private void start() {
    ...
    process
        ...
        .add(
            1,
            "-DReproduce=true"); // Add after "/path/to/bin/java" in order to rerun the test VM
                                 // directly
    commandLine =
        "Command Line:"
  ...
  /**
   * Process the socket output: All prefixed lines are dumped to the standard output while the
   * remaining lines represent the IR encoding used for IR matching later.
   */
  ...
  /**
   * Exit code was non-zero of test VM. Check the stderr to determine what kind of exception that
   * should be thrown to react accordingly later.
   */
  private void throwTestVMException() {
    ...
    if (stdErr.contains("TestFormat.throwIfAnyFailures")) {
      ...
      throw new TestFormatException(
          System.lineSeparator() + System.lineSeparator() + matcher.group());
    } else if (stdErr.contains("NoTestsRunException")) {
      throw new NoTestsRunException(
          ">>> No tests run due to empty set specified with -DTest and/or -DExclude. "
  ...
  private String getExceptionInfo() {
    ...
    return "TestFramework test VM exited with code "
        + exitCode
        + System.lineSeparator()
        + stdOut
        + System.lineSeparator()
