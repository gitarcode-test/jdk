...
/**
 * Prints an encoding to the dedicated test framework socket whether @IR rules of @Test methods
 * should be applied or not. This is done during the execution of the test VM by checking the active
 * VM flags. This encoding is eventually parsed and checked by the IRMatcher class in the driver VM
 * after the termination of the test VM. IR rule indices start at 1.
 */
public class IREncodingPrinter {
  ...
  private static final List<String> irTestingPlatforms =
  ...
  private static final List<String> verifiedCPUFeatures =
      new ArrayList<String>(
          Arrays.asList(
              // x86
              "fma",
  ...
  /**
   * Emits "<method>,{ids}" where {ids} is either: - indices of all @IR rules that should be
   * applied, separated by a comma - "-1" if no @IR rule should not be applied
   */
  ...
  private void printDisableReason(
      String method, String reason, String[] apply, int ruleIndex, int ruleMax) {
    TestFrameworkSocket.write(
        "Disabling IR matching for rule "
            + ruleIndex
            + " of "
            + ruleMax
            + " in "
  ...
  private boolean shouldApplyIrRule(IR irAnno, String m, int ruleIndex, int ruleMax) {
    ...
    if (isIRNodeUnsupported(irAnno)) {
      return false;
    } else if (irAnno.applyIfPlatform().length != 0
        && !hasAllRequiredPlatform(irAnno.applyIfPlatform(), "applyIfPlatform")) {
      printDisableReason(
          m, "Constraint not met (applyIfPlatform)", irAnno.applyIfPlatform(), ruleIndex, ruleMax);
      return false;
    } else if (irAnno.applyIfPlatformAnd().length != 0
        && !hasAllRequiredPlatform(irAnno.applyIfPlatformAnd(), "applyIfPlatformAnd")) {
      printDisableReason(
          m,
          "Not all constraints are met (applyIfPlatformAnd)",
          irAnno.applyIfPlatformAnd(),
          ruleIndex,
          ruleMax);
      return false;
    } else if (irAnno.applyIfPlatformOr().length != 0
        && !hasAnyRequiredPlatform(irAnno.applyIfPlatformOr(), "applyIfPlatformOr")) {
      printDisableReason(
          m,
          "None of the constraints are met (applyIfPlatformOr)",
          irAnno.applyIfPlatformOr(),
          ruleIndex,
          ruleMax);
      return false;
    } else if (irAnno.applyIfCPUFeature().length != 0
        && !hasAllRequiredCPUFeature(irAnno.applyIfCPUFeature(), "applyIfCPUFeature")) {
      printDisableReason(
          m,
          "Feature constraint not met (applyIfCPUFeature)",
          irAnno.applyIfCPUFeature(),
          ruleIndex,
          ruleMax);
      return false;
    } else if (irAnno.applyIfCPUFeatureAnd().length != 0
        && !hasAllRequiredCPUFeature(irAnno.applyIfCPUFeatureAnd(), "applyIfCPUFeatureAnd")) {
      printDisableReason(
          m,
          "Not all feature constraints are met (applyIfCPUFeatureAnd)",
          irAnno.applyIfCPUFeatureAnd(),
          ruleIndex,
          ruleMax);
      return false;
    } else if (irAnno.applyIfCPUFeatureOr().length != 0
        && !hasAnyRequiredCPUFeature(irAnno.applyIfCPUFeatureOr(), "applyIfCPUFeatureOr")) {
      printDisableReason(
          m,
          "None of the feature constraints met (applyIfCPUFeatureOr)",
          irAnno.applyIfCPUFeatureOr(),
          ruleIndex,
          ruleMax);
      return false;
    } else if (irAnno.applyIf().length != 0 && !hasAllRequiredFlags(irAnno.applyIf(), "applyIf")) {
      printDisableReason(
          m, "Flag constraint not met (applyIf)", irAnno.applyIf(), ruleIndex, ruleMax);
      return false;
    } else if (irAnno.applyIfNot().length != 0
        && !hasNoRequiredFlags(irAnno.applyIfNot(), "applyIfNot")) {
      printDisableReason(
          m, "Flag constraint not met (applyIfNot)", irAnno.applyIfNot(), ruleIndex, ruleMax);
      return false;
    } else if (irAnno.applyIfAnd().length != 0
        && !hasAllRequiredFlags(irAnno.applyIfAnd(), "applyIfAnd")) {
      printDisableReason(
          m,
          "Not all flag constraints are met (applyIfAnd)",
          irAnno.applyIfAnd(),
          ruleIndex,
          ruleMax);
      return false;
    } else if (irAnno.applyIfOr().length != 0
        && hasNoRequiredFlags(irAnno.applyIfOr(), "applyIfOr")) {
      printDisableReason(
          m,
          "None of the flag constraints met (applyIfOr)",
          irAnno.applyIfOr(),
          ruleIndex,
          ruleMax);
      return false;
  ...
  private void checkIRAnnotations(IR irAnno) {
    TestFormat.checkNoThrow(
        irAnno.counts().length != 0 || irAnno.failOn().length != 0,
  ...
  private boolean isIRNodeUnsupported(IR irAnno) {
    try {
      ...
      TestFrameworkSocket.write(
          "Skip Rule " + ruleIndex + ": " + e.getMessage(),
          TestFrameworkSocket.DEFAULT_REGEX_TAG,
          true);
      return true;
    }
    return false;
  ...
  private boolean hasAllRequiredFlags(String[] andRules, String ruleType) {
    ...
    for (int i = 0; i < andRules.length; i++) {
      ...
      i++;
      TestFormat.check(
          i < andRules.length, "Missing value for flag " + flag + " in " + ruleType + failAt());
      String value = andRules[i].trim();
      if (!check(flag, value) && returnValue) {
        // Rule will not be applied but keep processing the other flags to verify that they are
        // sane.
        returnValue = false;
      }
    }
  ...
  private boolean hasAllRequiredPlatform(String[] andRules, String ruleType) {
    ...
    for (int i = 0; i < andRules.length; i++) {
      ...
      TestFormat.check(
          i < andRules.length,
          "Missing value for platform " + platform + " in " + ruleType + failAt());
      String value = andRules[i].trim();
  ...
  private boolean checkPlatform(String platform, String value) {
    ...
    String endianess =
        ByteOrder.nativeOrder().equals(ByteOrder.BIG_ENDIAN) ? "big-endian" : "little-endian";

    String currentPlatform =
        os + " " + arch + " " + (Platform.is32bit() ? "32-bit" : "64-bit") + " " + endianess;
  ...
  private boolean hasAllRequiredCPUFeature(String[] andRules, String ruleType) {
    ...
    for (int i = 0; i < andRules.length; i++) {
      ...
      TestFormat.check(
          i < andRules.length,
          "Missing value for cpu feature " + feature + " in " + ruleType + failAt());
      String value = andRules[i].trim();
  ...
  private boolean checkCPUFeature(String feature, String value) {
    ...
    return (trueValue && cpuFeatures.contains(feature))
        || (falseValue && !cpuFeatures.contains(feature));
  }
  ...
  private boolean hasNoRequiredFlags(String[] orRules, String ruleType) {
    ...
    for (int i = 0; i < orRules.length; i++) {
      ...
      i++;
      TestFormat.check(
          i < orRules.length, "Missing value for flag " + flag + " in " + ruleType + failAt());
  ...
  private boolean check(String flag, String value) {
    ...
    if (actualFlagValue != null) {
      return checkFlag(
          Double::parseDouble, "floating point", flag, value, (Double) actualFlagValue);
    }
    ...
    // This could be improved if the Whitebox offers a "isVMFlag" function. For now, just check if
    // we can actually set
    // a value for a string flag. If we find this value, it's a string flag. If null is returned,
    // the flag is unknown.
  ...
  private <T extends Comparable<T>> boolean checkFlag(
