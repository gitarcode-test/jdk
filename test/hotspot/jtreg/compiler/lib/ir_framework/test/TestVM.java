...
/**
 * This class' main method is called from {@link TestFramework} and represents the so-called "test
 * VM". The class is the heart of the framework and is responsible for executing all the specified
 * tests in the test class. It uses the Whitebox API and reflection to achieve this task.
 */
public class TestVM {
  ...
  public static final int WARMUP_ITERATIONS =
      Integer.parseInt(System.getProperty("Warmup", "2000"));
  ...
  public static final boolean TEST_C1 =
      (TIERED_COMPILATION && TIERED_COMPILATION_STOP_AT_LEVEL.getValue() < CompLevel.C2.getValue())
          || CLIENT_VM;
  ...
  private static final boolean SHUFFLE_TESTS =
      Boolean.parseBoolean(System.getProperty("ShuffleTests", "true"));
  // Use separate flag as VERIFY_IR could have been set by user but due to other flags it was
  // disabled by flag VM.
  private static final boolean PRINT_VALID_IR_RULES = Boolean.getBoolean("ShouldDoIRVerification");
  protected static final long PER_METHOD_TRAP_LIMIT =
      (Long) WHITE_BOX.getVMFlag("PerMethodTrapLimit");
  protected static final boolean PROFILE_INTERPRETER =
      (Boolean) WHITE_BOX.getVMFlag("ProfileInterpreter");
  private static final boolean FLIP_C1_C2 = Boolean.getBoolean("FlipC1C2");
  private static final boolean IGNORE_COMPILER_CONTROLS =
      Boolean.getBoolean("IgnoreCompilerControls");
  ...
  private Set<Class<?>> helperClasses =
      null; // Helper classes that contain framework annotations to be processed.
  ...
  private void addHelperClasses(String[] args) {
    ...
    if (helperClassesList != null) {
      TestRun.check(
          Arrays.stream(helperClassesList).noneMatch(Objects::isNull),
      ...
      for (Class<?> helperClass : helperClassesList) {
        ...
        TestRun.check(
            !this.helperClasses.contains(helperClass),
  ...
  private void checkAnnotationsInClass(Class<?> c, String clazzType) {
    ...
    for (Method m : methods) {
      TestFormat.checkNoThrow(
          getAnnotation(m, Test.class) == null,
          "Cannot use @Test annotation in " + clazzType + " " + c + " at " + m);
      TestFormat.checkNoThrow(
          getAnnotation(m, Run.class) == null,
          "Cannot use @Run annotation in " + clazzType + " " + c + " at " + m);
      TestFormat.checkNoThrow(
          getAnnotation(m, Check.class) == null,
          "Cannot use @Check annotation in " + clazzType + " " + c + " at " + m);
      TestFormat.checkNoThrow(
          getAnnotation(m, Setup.class) == null,
          "Cannot use @Setup annotation in " + clazzType + " " + c + " at " + m);
    }
  }

  /**
   * Once everything is initialized and set up, start collecting tests and executing them
   * afterwards.
   */
  private void start() {
    setupTests();
  ...
  private void setupTests() {
    ...
    // Make sure to first setup test methods and make them non-inlineable and only then process
    // compile commands.
    setupDeclaredTests();
  ...
  private void addBaseTests() {
    declaredTests.forEach(
        (m, test) -> {
          if (test.getAttachedMethod() == null) {
            try {
              Arguments argumentsAnno = getAnnotation(m, Arguments.class);
              TestFormat.check(
                  argumentsAnno != null || m.getParameterCount() == 0,
                  "Missing @Arguments annotation to define arguments of " + m);
  ...
  private void addReplay() {
    ...
    TestFramework.check(
        WHITE_BOX.addCompilerDirective(directive) == 1, "Failed to add DUMP_REPLAY directive");
  }
  ...
  private void applyClassAnnotations(Class<?> c) {
    ...
    if (level == CompLevel.SKIP || level == CompLevel.WAIT_FOR_COMPILATION) {
      TestFormat.failNoThrow(
          "Cannot define compilation level SKIP or WAIT_FOR_COMPILATION in "
              + "@ForceCompileClassInitializer at "
              + c);
      return;
    }
    ...
    if (level != CompLevel.SKIP) {
      ...
      TestFormat.checkNoThrow(
          ...
          "Failed to enqueue <clinit> of "
              + c
              + " for compilation. Did you specify "
  ...
  private void checkClassAnnotations(Executable ex) {
    TestFormat.checkNoThrow(
        getAnnotation(ex, ForceCompileClassInitializer.class) == null,
        "@ForceCompileClassInitializer only allowed at classes but not at method " + ex);
  }

  /**
   * Exclude a method from compilation with a compiler randomly. Return the compiler for which the
   * method was made not compilable.
   */
  ...
  private void applyIndependentCompilationCommands(Executable ex) {
    ...
    checkCompilationCommandAnnotations(
        ex, forceInlineAnno, dontInlineAnno, forceCompileAnno, dontCompileAnno);
    ...
    if (EXCLUDE_RANDOM
        && getAnnotation(ex, Test.class) == null
        && forceCompileAnno == null
        && dontCompileAnno == null) {
  ...
  private void checkCompilationCommandAnnotations(
      Executable ex,
      ForceInline forceInlineAnno,
      DontInline dontInlineAnno,
      ForceCompile forceCompileAnno,
      DontCompile dontCompileAnno) {
    ...
    TestFormat.check(
        (testAnno == null && runAnno == null && checkAnno == null)
            || Stream.of(forceCompileAnno, dontCompileAnno, forceInlineAnno, dontInlineAnno)
                .noneMatch(Objects::nonNull),
        "Cannot use explicit compile command annotations (@ForceInline, @DontInline, "
            + "@ForceCompile or @DontCompile) together with @Test, @Check or @Run: "
            + ex
            + ". Use compLevel in @Test for fine tuning.");
    if (Stream.of(forceInlineAnno, dontCompileAnno, dontInlineAnno).filter(Objects::nonNull).count()
        > 1) {
      // Failure
      TestFormat.check(
          dontCompileAnno == null || dontInlineAnno == null,
          "@DontInline is implicitely done with @DontCompile annotation at " + ex);
      TestFormat.fail(
          "Cannot mix @ForceInline, @DontInline and @DontCompile at the same time at " + ex);
    }
    TestFormat.check(
        forceInlineAnno == null || dontInlineAnno == null,
        "Cannot have @ForceInline and @DontInline at the same time at " + ex);
    if (forceCompileAnno != null && dontCompileAnno != null) {
      ...
      TestFormat.check(
          dontCompileCompiler != Compiler.ANY,
          "Cannot have @DontCompile(Compiler.ANY) and @ForceCompile at the same time at " + ex);
      TestFormat.check(
          forceCompileLevel != CompLevel.ANY,
          "Cannot have @ForceCompile(CompLevel.ANY) and @DontCompile at the same time at " + ex);
      TestFormat.check(
          forceCompileLevel.isNotCompilationLevelOfCompiler(dontCompileCompiler),
          "Overlapping compilation level and compiler with @ForceCompile and @DontCompile at "
              + ex);
    }
  }
  ...
  private void applyForceCompileCommand(Executable ex) {
    ...
    if (forceCompileAnno != null) {
      ...
      TestFormat.check(
          compLevel != CompLevel.SKIP && compLevel != CompLevel.WAIT_FOR_COMPILATION,
  ...
  static void enqueueForCompilation(Executable ex, CompLevel requestedCompLevel) {
    ...
    if (compLevel != CompLevel.SKIP) {
      ...
      System.out.println(
          "Skipped compilation on level "
              + requestedCompLevel
              + " due to VM flags not allowing it.");
    }
  }

  /**
   * Collect all @Setup annotated methods and add them to setupMethodMap, for convenience to
   * reference later from tests with @Arguments(setup = "setupMethodName").
   */
  ...
  private void addSetupMethod(Method m) {
    TestFormat.checkNoThrow(
        getAnnotation(m, Test.class) == null, "@Setup method cannot have @Test annotation: " + m);
    TestFormat.checkNoThrow(
        getAnnotation(m, Check.class) == null, "@Setup method cannot have @Check annotation: " + m);
    TestFormat.checkNoThrow(
        getAnnotation(m, Arguments.class) == null,
        "@Setup method cannot have @Arguments annotation: " + m);
    TestFormat.checkNoThrow(
        getAnnotation(m, Run.class) == null, "@Setup method cannot have @Run annotation: " + m);
    Method mOverloaded = setupMethodMap.put(m.getName(), m);
    TestFormat.checkNoThrow(
        mOverloaded == null, "@Setup method cannot be overloaded: " + mOverloaded + " with " + m);
    m.setAccessible(true);
  }

  /**
   * Setup @Test annotated method an add them to the declaredTests map to have a convenient way of
   * accessing them once setting up a framework test (base checked, or custom run test).
   */
  private void setupDeclaredTests() {
    for (Method m : testClass.getDeclaredMethods()) {
      ...
      try {
        if (testAnno != null) {
          ...
          TestFormat.checkNoThrow(
              !m.isAnnotationPresent(IR.class) && !m.isAnnotationPresent(IRs.class),
              "Found @IR annotation on non-@Test method " + m);
          TestFormat.checkNoThrow(
              !m.isAnnotationPresent(Warmup.class) || getAnnotation(m, Run.class) != null,
    ...
    TestFormat.checkNoThrow(
        !declaredTests.isEmpty(), "Did not specify any @Test methods in " + testClass);
  }
  ...
  private void addDeclaredTest(Method m) {
    ...
    if (warmup != null) {
      ...
      TestFormat.checkNoThrow(
          warmupIterations >= 0, "Cannot have negative value for @Warmup at " + m);
    }

    if (!IGNORE_COMPILER_CONTROLS) {
      // Don't inline test methods by default. Do not apply this when -DIgnoreCompilerControls=true
      // is set.
      WHITE_BOX.testSetDontInlineMethod(m, true);
    }
  ...
  private void checkTestAnnotations(Method m, Test testAnno) {
    TestFormat.check(
        !testMethodMap.containsKey(m.getName()),
    ...
    TestFormat.check(
        checkAnno == null && runAnno == null,
        m + " has invalid @Check or @Run annotation while @Test annotation is present.");

    TestFormat.checkNoThrow(
        Arrays.stream(m.getParameterTypes()).noneMatch(AbstractInfo.class::isAssignableFrom),
        "Cannot "
            + AbstractInfo.class
            + " or any of its subclasses as parameter type at "
            + "@Test method "
            + m);

    TestFormat.checkNoThrow(
        !AbstractInfo.class.isAssignableFrom(m.getReturnType()),
        "Cannot "
            + AbstractInfo.class
            + " or any of its subclasses as return type at "
            + "@Test method "
            + m);
  }
  ...
  private static CompLevel restrictCompLevel(CompLevel compLevel) {
    ...
    if ((TIERED_COMPILATION || CLIENT_VM)
        && compLevel.getValue() > TIERED_COMPILATION_STOP_AT_LEVEL.getValue()) {
      return CompLevel.SKIP;
    }
    return compLevel;
  }
  ...
  /**
   * Verify that the helper classes do not contain illegal framework annotations and then apply the
   * actions as specified by the different helper class annotations.
   */
  ...
  /**
   * First set up checked (with @Check) and custom run tests (with @Run). All remaining
   * unmatched/unused @Test methods are treated as base tests and set up as such later.
   */
  private void setupCheckedAndCustomRunTests() {
    for (Method m : testClass.getDeclaredMethods()) {
      ...
      try {
        TestFormat.check(
            argumentsAnno == null || (checkAnno == null && runAnno == null),
  ...
  /**
   * Set up a checked test by first verifying the correct format of the @Test and @Check method and
   * then adding it to the allTests list which keeps track of all framework tests that are
   * eventually executed.
   */
  private void addCheckedTest(Method m, Check checkAnno, Run runAnno) {
    ...
    TestFormat.check(
        getAnnotation(testMethod, Arguments.class) != null || testMethod.getParameterCount() == 0,
        "Missing @Arguments annotation to define arguments of "
            + testMethod
            + " required by "
            + "checked test "
            + m);
    ...
    CheckedTest checkedTest =
        new CheckedTest(test, m, checkAnno, parameter, shouldExcludeTest(testMethod.getName()));
  ...
  private void checkCheckedTest(
      Method m, Check checkAnno, Run runAnno, Method testMethod, DeclaredTest test) {
    TestFormat.check(
        runAnno == null, m + " has invalid @Run annotation while @Check annotation is present.");
    TestFormat.check(
        testMethod != null,
        "Did not find associated test method \""
    ...
    TestFormat.check(
        test != null,
        "Missing @Test annotation for associated test method "
            + testMethod
            + " for @Check at "
            + m);
    ...
    TestFormat.check(
        ...
        "Cannot use @Test "
            + testMethod
            + " for more than one @Run or one @Check method. Found: "
            + m
            + ", "
            + attachedMethod);
  }
  ...
  private CheckedTest.Parameter getCheckedTestParameter(Method m, Method testMethod) {
    boolean firstParameterTestInfo =
        m.getParameterCount() > 0 && m.getParameterTypes()[0].equals(TestInfo.class);
    boolean secondParameterTestInfo =
        m.getParameterCount() > 1 && m.getParameterTypes()[1].equals(TestInfo.class);
    ...
    switch (m.getParameterCount()) {
      ...
      case 1 -> {
        TestFormat.checkNoThrow(
            firstParameterTestInfo || m.getParameterTypes()[0] == testReturnType,
            "Single-parameter version of @Check method "
                + m
                + " must match return type of @Test "
                + testMethod);
        parameter =
            firstParameterTestInfo
                ? CheckedTest.Parameter.TEST_INFO_ONLY
                : CheckedTest.Parameter.RETURN_ONLY;
      }
      case 2 -> {
        TestFormat.checkNoThrow(
            m.getParameterTypes()[0] == testReturnType && secondParameterTestInfo,
            "Two-parameter version of @Check method "
                + m
                + " must provide as first parameter the same"
                + " return type as @Test method "
                + testMethod
                + " and as second parameter an object of "
                + TestInfo.class);
      ...
      default ->
          TestFormat.failNoThrow(
              "@Check method "
                  + m
                  + " must provide either a none, single or two-parameter variant.");
    }
    return parameter;
  }

  /**
   * Set up a custom run test by first verifying the correct format of the @Test and @Run method and
   * then adding it to the allTests list which keeps track of all framework tests that are
   * eventually executed.
   */
  private void addCustomRunTest(Method m, Run runAnno) {
    ...
    dontCompileAndDontInlineMethod(m);
    CustomRunTest customRunTest =
        new CustomRunTest(m, getAnnotation(m, Warmup.class), runAnno, tests, shouldExcludeTest);
    allTests.add(customRunTest);
    if (PRINT_VALID_IR_RULES) {
      tests.forEach(
          test ->
              irMatchRulePrinter.emitRuleEncoding(test.getTestMethod(), customRunTest.isSkipped()));
    }
  }

  /** Only allow parameters as specified in {@link Run}. */
  private void checkCustomRunTest(
      Method m, String testName, Method testMethod, DeclaredTest test, RunMode runMode) {
    TestFormat.check(
        testMethod != null,
        "Did not find associated @Test method \""
            + m.getDeclaringClass().getName()
    ...
    TestFormat.check(
        ...
        "Cannot use @Test "
            + testMethod
            + " for more than one @Run/@Check method. Found: "
            + m
            + ", "
            + attachedMethod);
    ...
    TestFormat.check(
        argumentsAnno == null,
        "Cannot use @Arguments at test method "
            + testMethod
            + " in combination with @Run method "
            + m);
    Warmup warmupAnno = getAnnotation(testMethod, Warmup.class);
    TestFormat.checkNoThrow(
        warmupAnno == null,
        "Cannot set @Warmup at @Test method "
            + testMethod
            + " when used with its @Run method "
            + m
            + ". Use @Warmup at @Run method instead.");
    Test testAnno = getAnnotation(testMethod, Test.class);
    TestFormat.checkNoThrow(
        runMode != RunMode.STANDALONE || testAnno.compLevel() == CompLevel.ANY,
        "Setting explicit compilation level for @Test method "
            + testMethod
            + " has no effect "
            + "when used with STANDALONE @Run method "
            + m);
  }

  private void checkRunMethod(Method m, Run runAnno) {
    TestFormat.check(
        runAnno.test().length > 0, "@Run method " + m + " must specify at least one test method");
    TestFormat.checkNoThrow(
        m.getParameterCount() == 0
            || (m.getParameterCount() == 1 && m.getParameterTypes()[0].equals(RunInfo.class)),
        "@Run method "
            + m
            + " must specify either no parameter or exactly one "
            + RunInfo.class
            + " parameter.");
    Warmup warmupAnno = getAnnotation(m, Warmup.class);
    TestFormat.checkNoThrow(
        warmupAnno == null || runAnno.mode() != RunMode.STANDALONE,
        "Cannot set @Warmup at @Run method "
            + m
            + " when used with RunMode.STANDALONE. The @Run method is only invoked once.");
  }
  ...
  /**
   * Ensure that all compilations that were enforced (added to compilation queue) by framework
   * annotations are completed. Wait if necessary for a short amount of time for their completion.
   */
  private void checkForcedCompilationsCompleted() {
    ...
    do {
      forceCompileMap
          ...
          .removeIf(
              entry ->
                  WHITE_BOX.getMethodCompilationLevel(entry.getKey())
    ...
    throw new TestRunException(
        "Could not force compile the following @ForceCompile methods:"
            + System.lineSeparator()
            + builder.toString());
  }
  ...
  private void runTests() {
    ...
    if (testFilterPresent) {
      ...
      if (testList.isEmpty()) {
        // Throw an exception to inform the user about an empty specified test set with -DTest
        // and/or -DExclude
    ...
    if (SHUFFLE_TESTS) {
      // Execute tests in random order (execution sequence affects profiling). This is done by
      // default.
    ...
    // Execute all tests and keep track of each exception that is thrown. These are then reported
    // once all tests
    ...
    for (AbstractTest test : testList) {
      ...
      if (PRINT_TIMES || VERBOSE) {
        ...
        if (VERBOSE) {
          System.out.println(
              "Done " + test.getName() + ": " + duration + " ns = " + (duration / 1000000) + " ms");
        }
      }
      if (GC_AFTER) {
  ...
  public static void assertDeoptimizedByC1(Method m) {
    if (notUnstableDeoptAssertion(m, CompLevel.C1_SIMPLE)) {
      TestRun.check(
          compiledByC1(m) != TriState.Yes || PER_METHOD_TRAP_LIMIT == 0 || !PROFILE_INTERPRETER,
  ...
  public static void assertDeoptimizedByC2(Method m) {
    if (notUnstableDeoptAssertion(m, CompLevel.C2)) {
      TestRun.check(
          compiledByC2(m) != TriState.Yes || PER_METHOD_TRAP_LIMIT == 0 || !PROFILE_INTERPRETER,
  ...
  public static void assertCompiledAtLevel(Method m, CompLevel level) {
    TestRun.check(
        compiledAtLevel(m, level) != TriState.No,
        m + " should have been compiled at level " + level.name());
  }
  ...
  public static void assertCompiled(Method m) {
    TestRun.check(
        compiledByC1(m) != TriState.No || compiledByC2(m) != TriState.No,
  ...
  private static TriState compiledAtLevel(Method m, CompLevel level) {
    if (WHITE_BOX.isMethodCompiled(m, false)) {
      switch (level) {
        ...
        default ->
            throw new TestRunException("compiledAtLevel() should not be called with " + level);
      }
    }
    if (!USE_COMPILER
        || XCOMP
