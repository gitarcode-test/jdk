...
public class TestJcmd {
  ...
  private static final int TIME_TO_RUN_CONTAINER_PROCESS =
      (int) (10 * Utils.TIMEOUT_FACTOR); // seconds
  ...
  private static long testJcmdGetPid(String className) throws Exception {
    ...
    OutputAnalyzer out = new OutputAnalyzer(pb.start()).shouldHaveExitValue(0);
  ...
  private static String generateCustomDockerfile() throws Exception {
    ...
    sb.append(
        String.format(
            "FROM %s:%s\n",
            DockerfileConfig.getBaseImageName(), DockerfileConfig.getBaseImageVersion()));
  ...
  private static Process startObservedContainer() throws Exception {
    ...
    return ProcessTools.startProcess(
