...
public class TestEagerReclaimHumongousRegionsLog {
  ...
  public static void runTest() throws Exception {
    ...
    Asserts.assertTrue(
        lines.length % 6 == 0,
        "There seems to be an unexpected amount of log messages (total: "
            + lines.length
            + ") per GC");
    ...
    for (int i = 0; i < lines.length; i += 6) {
      ...
      Asserts.assertLTE(
          reclaimed,
          candidate,
          "The number of reclaimed objects must be less or equal than the number of candidates");
      ...
      if (reclaimed > 0) {
        ...
        Asserts.assertGT(
            ...
            "Number of regions reclaimed ("
                + (before - after)
                + ") must be larger than number of objects reclaimed ("
                + reclaimed
                + ")");
      }
    }
