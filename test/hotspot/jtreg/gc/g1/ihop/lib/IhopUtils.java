...
/**
 * Utility class to extract IHOP related information from the GC log. The class provides a number of
 * static method to be used from tests.
 */
public class IhopUtils {

  // Examples of GC log for IHOP:
  // [0.402s][debug][gc,ergo,ihop] GC(9) Do not request concurrent cycle initiation (still doing
  // mixed collections) occupancy: 66060288B allocation request: 0B threshold: 59230757B (88.26)
  // source: end of GC
  // [0.466s][debug][gc,ergo,ihop] GC(18) Request concurrent cycle initiation (occupancy higher than
  // threshold) occupancy: 52428800B allocation request: 0B threshold: 0B (0.00) source: end of GC

  /** Patterns are used for extracting occupancy and threshold from GC log. */
  private static final Pattern OCCUPANCY = Pattern.compile("occupancy: (\\d+)B");

  private static final Pattern THRESHOLD = Pattern.compile("threshold: (\\d+)B");

  /** Messages related to concurrent cycle initiation. */
  private static final String CYCLE_INITIATION_MESSAGE =
      "Request concurrent cycle initiation (occupancy higher than threshold)";

  private static final String CYCLE_INITIATION_MESSAGE_FALSE =
      "Do not request concurrent cycle initiation (still doing mixed collections)";
  private static final String ADAPTIVE_IHOP_PREDICTION_ACTIVE_MESSAGE = "prediction active: true";
  ...
  /**
   * Checks that memory occupancy is greater or equal to the threshold. This methods searches for
   * occupancy and threshold in the GC log corresponding Conc Mark Cycle initiation and compare
   * their values.If no CMC initiation happens, does nothing.
   *
            ...
            item -> {
              ...
              if (occupancy < threshold) {
                ...
                throw new RuntimeException(
                    "Concurrent cycle initiation is unexpected. Occupancy ("
                        + occupancy
                        + ") is less then threshold ("
                        + threshold
                        + ")");
              }
              System.out.printf(
                  "Concurrent cycle was initiated with occupancy = %d and threshold = %d%n",
                  occupancy, threshold);
            });
  }
  ...
  private static List<String> getErgoMessages(OutputAnalyzer outputAnalyzer) {
    return IhopUtils.findInLog(
        outputAnalyzer, CYCLE_INITIATION_MESSAGE, CYCLE_INITIATION_MESSAGE_FALSE);
  }
  ...
  public static void checkErgoMessagesExist(OutputAnalyzer outputAnalyzer) {
    ...
    if (!(output.contains(CYCLE_INITIATION_MESSAGE)
        | output.contains(CYCLE_INITIATION_MESSAGE_FALSE))) {
