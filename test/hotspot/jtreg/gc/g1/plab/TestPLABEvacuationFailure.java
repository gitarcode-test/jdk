...
/**
 * The test runs the AppPLABEvacuationFailure application to provoke a number of Evacuation
 * Failures, parses GC log and analyzes PLAB statistics. The test checks that both fields
 * 'failure_waste' and 'failure_used' for Evacuation Failure statistic are non zero, and zero for
 * other statistics.
 */
public class TestPLABEvacuationFailure {
   ...
   */
  private static final List<String> FAILURE_STAT_FIELDS =
      new ArrayList<>(Arrays.asList("failure used", "failure wasted"));
  ...
  private static void runTest(
      int wastePct, int plabSize, int parGCThreads, int heapSize, boolean plabIsFixed)
      throws Throwable {
  ...
  private static void checkResults() {
    ...
    checkValuesIsZero(
        valuesToCheck,
        "Expect that SURVIVOR PLAB failure statistics should be 0 when no evacuation failure");

    valuesToCheck = getNonEvacFailureOldStats();
    checkValuesIsZero(
        valuesToCheck,
        "Expect that OLD PLAB failure statistics should be 0 when no evacuation failure");

    valuesToCheck = getEvacFailureSurvivorStats();
    checkValuesIsZero(
        valuesToCheck,
        "Expect that failure statistics should be 0 in SURVIVOR PLAB statistics at evacuation"
            + " failure");

    valuesToCheck = getEvacFailureOldStats();
    checkValuesIsNotZero(
        valuesToCheck,
        "Expect that failure statistics should not be 0 in OLD PLAB statistics at evacuation"
            + " failure");
  }
  ...
  private static void checkValues(
      Map<Long, PlabInfo> logItems, String errorMessage, boolean expectZero) {
    logItems
        .entrySet()
        .forEach(
            item ->
  ...
  private static Map<Long, PlabInfo> getNonEvacFailureOldStats() {
    return logParser.getExcludedSpecifiedStats(
        evacuationFailureIDs, LogParser.ReportType.OLD_STATS, FAILURE_STAT_FIELDS);
  }

  private static Map<Long, PlabInfo> getEvacFailureSurvivorStats() {
    return logParser.getSpecifiedStats(
        evacuationFailureIDs, LogParser.ReportType.SURVIVOR_STATS, FAILURE_STAT_FIELDS);
  }
