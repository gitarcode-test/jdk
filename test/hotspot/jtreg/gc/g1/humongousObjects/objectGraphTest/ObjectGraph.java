...
public class ObjectGraph {
   ...
   */
  public static void propagateTransitiveProperty(
      Set<Object[]> nodes, Consumer<Object[]> operation) {
  ...
  /**
   * Builds object graph using description from list of parsed nodes. Graph uses Object[] as nodes,
   * first n elements of array are links to connected nodes, others are null. Then runs visitors on
   * generated graph
   *
   ...
   */
  public static Object[] generateObjectNodes(
      List<TestcaseData.FinalParsedNode> parsedNodes,
      Map<
              Predicate<TestcaseData.FinalParsedNode>,
              BiConsumer<TestcaseData.FinalParsedNode, Object[][]>>
          visitors,
      int humongousAllocationSize,
      int simpleAllocationSize) {
    ...
    for (int i = 0; i < parsedNodes.size(); ++i) {
      objectNodes[i] =
          new Object
              [(parsedNodes.get(i).isHumongous ? humongousAllocationSize : simpleAllocationSize)];
    }
  ...
  /**
   * Enumerates graph starting with provided vertexes. All vertexes that are reachable from the
   * provided ones are marked
   *
   * @param markedParents provided vertexes
   * @param markVertex lambda which marks vertexes
   */
  public static void enumerateAndMark(
      Deque<Object[]> markedParents, Consumer<Object[]> markVertex) {
