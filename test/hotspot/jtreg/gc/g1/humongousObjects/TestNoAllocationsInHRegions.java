 ...
 * @run main/othervm -XX:+UseG1GC -XX:+UnlockDiagnosticVMOptions -XX:+WhiteBoxAPI
 *     -Xbootclasspath/a:. -XX:G1HeapRegionSize=1M -Xms200m -Xmx200m -XX:MaxTenuringThreshold=0
...
public class TestNoAllocationsInHRegions {
  ...
  private static final int[] HUMONGOUS_SIZES = {
    G1_REGION_SIZE / 2, G1_REGION_SIZE + 1, G1_REGION_SIZE * 2 + 1
  };
  private static final int ALLOC_THREAD_COUNT = 5;

  // We fill specified part of heap with humongous objects - we need public static to prevent escape
  // analysis to
  // collect this field
          ...
          new Runnable[] {
            ...
            () -> {
              if (maxAllocationMemory - usedMemory != 0) {
                int arraySize =
                    random.nextInt(Math.min(maxAllocationMemory - usedMemory, MAX_ALLOCATION_SIZE));
                ...
                if (arraySize != 0) {
                  ...
                  if (WB.g1BelongsToHumongousRegion(WB.getObjectAddress(allocation))) {
                    String errorMessage =
                        String.format(
                            "Non-humongous allocation of byte array of length %d and size %d with"
                                + " address %d was made in Humongous Region",
                            allocation.length,
                            WB.getObjectSize(allocation),
            ...
            () -> {
              ...
              if (wrongHumongousAllocations.size() > 0) {
                wrongHumongousAllocations.stream()
                    .forEach(
                        a ->
                            System.out.format(
                                "Non-humongous allocation of byte array of length %d and"
  ...
  public static void main(String[] args) {
    if (args.length != 2) {
      throw new Error(
          "Test Bug: Expected duration (in seconds) and percent of allocated regions were not "
    ...
    int regionsToAllocate =
        (int) ((double) initialFreeRegionsCount / 100.0 * percentOfAllocatedHeap);
    long freeRegionLeft = initialFreeRegionsCount - regionsToAllocate;

    System.out.println(
        "Regions to allocate: " + regionsToAllocate + "; regions to left free: " + freeRegionLeft);

    int maxMemoryPerAllocThread =
        (int)
            ((Runtime.getRuntime().freeMemory() / 100.0 * (100 - percentOfAllocatedHeap))
                / ALLOC_THREAD_COUNT
                * 0.5);

    System.out.println(
        "Using "
            + maxMemoryPerAllocThread / 1024
            + "KB for each of "
            + ALLOC_THREAD_COUNT
            + " allocation threads");
    ...
    while (WB.g1NumFreeRegions() > freeRegionLeft) {
      try {
        ...
        // It probably means that heap is fragmented which is strange since the test logic should
        // avoid it
        System.out.println(
            "Warning: OOM while allocating humongous objects - it likely means "
