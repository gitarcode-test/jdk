...
/**
 * @notest THIS IS NOT A TEST. This is a test generator, should be run only when jcstress is changed
 * @library /test/lib /
 * @run main applications.jcstress.TestGenerator
 */

/**
 * Use jcstress test suite to generate jtreg tests in 'test.src' or current directory. Used version
 * is defined in JcstressRunner class.
 *
 * <p>This generator depends on testlibrary, therefore it should be compiled and added to classpath.
 * One can replace @notest by @test in jtreg test description above to run this class with jtreg.
 *
...
public class TestGenerator {
  ...
  public static String DESC_FORMAT =
      "\n"
          ...
          + " * @key external-dep\n"
          + " * @library /test/lib /\n"
          + " * @run driver/timeout=21600 "
          + JcstressRunner.class.getName()
          // verbose output
          + " -v"
          // test name
    ...
    try {
      ...
      ProcessBuilder pb =
          ProcessTools.createLimitedTestJavaProcessBuilder(
              "-jar", path.toAbsolutePath().toString(), "-l");
