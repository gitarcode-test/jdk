...
public class CertUtils {
  ...
  */
   ...
   * @throws NoSuchAlgorithmException if no Provider supports a KeyFactorySpi implementation for the
   *     specified algorithm
   ...
   * @return the X509Certificate
   * @throws CertificateException if the certificate type is not supported or cannot be parsed
   * @throws IOException
   */
  ...
  public static X509Certificate getCertFromFile(String certFilePath)
      throws CertificateException, IOException {
    return getCertFromStream(Files.newInputStream(Paths.get(TEST_SRC, certFilePath)));
  }
  ...
  public static X509CRL getCRLFromFile(String crlFilePath)
    ...
    try (FileInputStream fis = new FileInputStream(crlFile)) {
      return (X509CRL) CertificateFactory.getInstance("X.509").generateCRL(fis);
    }
  }
   ...
   * Read a bunch of certs from files and create a CertPath from them.
   *
   * @param relPath relative path containing certs (must end in file.separator)
   * @param fileNames an array of <code>String</code>s that are file names
   * @throws Exception on error
   */
   ...
   * @throws Exception on error
   */
  public static PKIXCertPathValidatorResult validate(CertPath path, PKIXParameters params)
      throws Exception {
  ...
  /**
   * This class is useful for overriding one or more methods of an X509Certificate for testing
   * purposes.
   */
