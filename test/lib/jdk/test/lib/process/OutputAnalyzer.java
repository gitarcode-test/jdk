...
public final class OutputAnalyzer {
  ...
  /**
   * Create an OutputAnalyzer, a utility class for verifying output and exit value from a Process
   *
   * @param process Process to analyze
   * @param cs The charset used to convert stdout/stderr from bytes to chars or null for the default
   *     charset.
  ...
  /**
   * Delegate waitFor to the OutputBuffer. This ensures that the progress and timestamps are logged
   * correctly.
   */
  public void waitFor() {
    buffer.waitFor();
   ...
   *
   * @throws RuntimeException If stdout was not empty
   */
   ...
   *
   * @throws RuntimeException If stderr was not empty
   */
  ...
  /**
   * Verify that the stderr contents of output buffer is empty, after filtering out the Hotspot
   * warning messages
   *
   * @throws RuntimeException If stderr was not empty
   */
  ...
  /**
   * Verify that the stderr contents of output buffer is empty, after filtering out all messages
   * matching "warning" (case insensitive)
   *
   * @throws RuntimeException If stderr was not empty
   */
   ...
   *
   * @throws RuntimeException If stdout was empty
   */
   ...
   *
   * @throws RuntimeException If stderr was empty
   */
  ...
  /**
   * Verify that the stdout and stderr contents of output buffer matches the pattern
   *
   * @param regexp
  ...
  /**
   * Verify that the stderr contents of output buffer does not match the pattern
   *
   * @param regexp
   * @throws RuntimeException If the pattern was found
   */
  ...
  /**
   * Get the captured group of the first string matching the pattern. stderr is searched before
   * stdout.
   *
  ...
  /**
   * Get the first string matching the pattern. stderr is searched before stdout.
   *
  ...
  /**
   * Report summary that will help to diagnose the problem Currently includes: - standard input
   * produced by the process under test - standard output - exit code Note: the command line is
   * printed by the ProcessTools
   */
  ...
  /**
   * Get the contents of the output buffer (stdout and stderr) as list of strings. Output will be
   * split by newlines.
   *
  ...
  /**
   * Verifies that the stdout and stderr contents of output buffer are empty, after filtering out
   * the HotSpot warning messages.
   *
   * @throws RuntimeException If the stdout and stderr are not empty
   */
  ...
  /**
   * Verify that the stderr contents of output buffer matches the pattern, after filtering out the
   * Hotspot warning messages
   *
   * @param pattern
   * @throws RuntimeException If the pattern was not found
   */
  ...
  /**
   * Verify that the stderr contents of output buffer matches the pattern, after filtering out the
   * Hotspot deprecation warning messages
   *
   * @param pattern
   * @throws RuntimeException If the pattern was not found
   */
  ...
  /**
   * Returns the contents of the output buffer (stdout and stderr), without those JVM warning msgs,
   * as list of strings. Output is split by newlines.
   *
   * @return Contents of the output buffer as list of strings
   */
  ...
  /**
   * Verify that the stdout and stderr contents of output buffer match the {@code pattern} line by
   * line. The whole output could be matched or just a subset of it.
   *
   * @param fromPattern The pattern of line (excluded) from where output will be matched. Set {@code
   *     fromPattern} to null for matching from the first line.
   * @param toPattern The pattern of line (excluded) until where output will be matched. Set {@code
   *     toPattern} to null for matching until the last line.
  ...
  /**
   * Verify that the stdout contents of output buffer matches the {@code pattern} line by line. The
   * whole stdout could be matched or just a subset of it.
   *
   * @param fromPattern The pattern of line (excluded) from where stdout will be matched. Set {@code
   *     fromPattern} to null for matching from the first line.
   * @param toPattern The pattern of line (excluded) until where stdout will be matched. Set {@code
   *     toPattern} to null for matching until the last line.
   * @param pattern Matching pattern
   */
  public OutputAnalyzer stdoutShouldMatchByLine(
      String fromPattern, String toPattern, String pattern) {
    return shouldMatchByLine(getStdout(), fromPattern, toPattern, pattern);
  }

  private OutputAnalyzer shouldMatchByLine(
      String buffer, String fromPattern, String toPattern, String pattern) {
    List<String> lines = asLines(buffer);

    int fromIndex = 0;
    if (fromPattern != null) {
      fromIndex =
          indexOf(lines, fromPattern, 0) + 1; // + 1 -> apply 'pattern' to lines after 'from' match
      Asserts.assertGreaterThan(
          fromIndex,
          0,
          "The line matched with pattern '"
              + fromPattern
              + "' from where the output should match can not be found");
    }
    ...
    if (toPattern != null) {
      ...
      Asserts.assertGreaterThan(
          ...
          "The line matched with pattern '"
              + toPattern
              + "' until where the output should match can not be found");
    }

    List<String> subList = lines.subList(fromIndex, toIndex);
    Asserts.assertFalse(
        subList.isEmpty(),
        "There are no lines to check:"
            + " range "
            + fromIndex
            + ".."
            + toIndex
    ...
    subList.stream()
        ...
        .ifPresent(
            line ->
                Asserts.fail("The line '" + line + "' does not match pattern '" + pattern + "'"));
  ...
  private void searchLinesForMultiLinePattern(
