...
public class JibArtifactManager implements ArtifactManager {
  ...
  public static JibArtifactManager newInstance() throws ClassNotFoundException {
    ...
    try {
      ...
      ClassLoader classLoader =
          new URLClassLoader(jarUrls, JibArtifactManager.class.getClassLoader().getParent());
      ...
      try {
        Object jibArtifactInstaller =
            jibServiceFactory.getMethod("createJibArtifactInstaller").invoke(null);
  ...
  private Path download(String jibVersion, Map<String, Object> artifactDescription)
      throws Exception {
    return invokeInstallerMethod("download", jibVersion, artifactDescription);
  }

  private Path install(String jibVersion, Map<String, Object> artifactDescription)
      throws Exception {
    return invokeInstallerMethod("install", jibVersion, artifactDescription);
  }

  private Path invokeInstallerMethod(
      String methodName, String jibVersion, Map<String, Object> artifactDescription)
      throws Exception {
    // Temporarily replace the context classLoader
    Thread currentThread = Thread.currentThread();
    ClassLoader oldContextLoader = currentThread.getContextClassLoader();
    ...
    try {
      Method m =
          classLoader
              .loadClass("com.oracle.jib.api.JibArtifactInstaller")
