...
public final class FileUtils {
   ...
   *     deleted because the directory is not empty (optional specific exception)
   ...
   * @throws IOException If an I/O error occurs. Any such exceptions are caught internally. If only
   *     one is caught, then it is re-thrown. If more than one exception is caught, then the second
   *     and following exceptions are added as suppressed exceptions of the first one caught, which
   *     is then re-thrown.
   */
  ...
  /**
   * Checks whether all file systems are accessible. This is performed by checking free disk space
   * on all mounted file systems via a separate, spawned process. File systems are considered to be
   * accessible if this process completes successfully before a given fixed duration has elapsed.
   *
   * @implNote On Unix this executes the {@code df} command in a separate process and on Windows
  ...
  /**
   * Checks whether all file systems are accessible and there are no duplicate mount points. This is
   * performed by checking free disk space on all mounted file systems via a separate, spawned
   * process. File systems are considered to be accessible if this process completes successfully
   * before a given fixed duration has elapsed.
   *
   * @implNote On Unix this executes the {@code df} command in a separate process and on Windows
   *     always returns {@code true}.
   ...
   *     inclusive.
   * @param toLine the last line to patch. This is the number you see in an editor, inclusive. Set
   *     {@code toLine} to {@code fromLine - 1} if you only want to insert lines.
   * @param from lines to remove, used to ensure the correct lines are removed. Can be multiple
   *     lines or empty. It's compared to existing lines with all lines trimmed and no new lines at
   *     both ends. Ignored if null.
   * @param to the newly added lines, can be multiple lines or empty. New line at end is optional.
   *     Cannot be null.
   ...
   */
  public static void patch(Path path, int fromLine, int toLine, String from, String to)
      throws IOException {
    ...
    if (from != null) {
      ...
      String actuallyRemoved =
          subList.stream().map(String::trim).collect(Collectors.joining("\n")).trim();
      String wantToRemove = from.lines().map(String::trim).collect(Collectors.joining("\n")).trim();
      if (!actuallyRemoved.equals(wantToRemove)) {
        throw new IOException(
            "Removed not the same: ["
                + String.join("\\n", subList)
                + "] and ["
                + from.replaceAll("\\n", "\\\\n")
                + "]");
