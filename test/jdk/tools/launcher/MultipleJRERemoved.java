...
public class MultipleJRERemoved extends TestHelper {
  ...
  /**
   * Check all combinations of flags: "-version:", "-jre-restrict-search",
   * "-jre-no-restrict-search". Test expects to see errors.
   */
  @Test
    ...
    for (Flag flag1 : Flag.values()) {
      for (Flag flag2 : Flag.values()) {
        for (Flag flag3 : Flag.values()) {
          ...
          List<String> flagValues = flags.stream().map(Flag::value).collect(Collectors.toList());
  ...
  private enum Flag {
    ...
    JRE_RESTRICT_SEARCH(
        ...
        "Error: Specifying an alternate JDK/JRE is no longer supported.\n"
            + "The related flags -jre-restrict-search | -jre-no-restrict-search are also no longer"
            + " valid."),
    JRE_NO_RESTRICT_SEARCH(
