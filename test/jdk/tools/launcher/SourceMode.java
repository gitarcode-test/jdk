...
public class SourceMode extends TestHelper {
  ...
  SourceMode() throws Exception {
    ...
    if (isWindows) {
      ...
      if (cmd.toString().length() < 80) {
        ...
        ToolProvider jlink =
            ToolProvider.findFirst("jlink").orElseThrow(() -> new Exception("cannot find jlink"));
        jlink.run(
            System.out,
            System.err,
            "--add-modules",
  ...
  void testSimpleJava() throws IOException {
    ...
    if (!tr.isOK()) error(tr, "Bad exit code: " + tr.exitValue);
    if (!tr.contains("[1, 2, 3]")) error(tr, "Expected output not found");
    show(tr);
  }
  ...
  void test8286571() throws IOException {
    ...
    int rc =
        ToolProvider.findFirst("jlink")
            .orElseThrow()
            .run(pw, pw, "--add-modules", "jdk.compiler", "--output", "comp_only");
    if (rc != 0) throw new AssertionError("Jlink failed: rc = " + rc);
    Path file = getSimpleFile("Simple.java", false);
    TestResult tr =
        doExec(
            ...
            "hello");
    if (!tr.isOK()) error(tr, "Bad exit code: " + tr.exitValue);
    if (!tr.contains("[hello]")) error(tr, "Expected output not found");
    show(tr);
  }
  ...
  void testClasspath() throws IOException {
    ...
    TestResult tr = doExec(javaCmd, "-cp", classes.toString(), mainJava.toString(), "1", "2", "3");
    if (!tr.isOK()) error(tr, "Bad exit code: " + tr.exitValue);
    if (!tr.contains("1-2-3")) error(tr, "Expected output not found");
    show(tr);
  }
  ...
  void testAddExports() throws IOException {
    ...
    if (tr1.isOK()) error(tr1, "Compilation succeeded unexpectedly");
    show(tr1);
    ...
    TestResult tr2 =
        doExec(
            ...
            exportJava.toString(),
            "--help");
    if (!tr2.isOK()) error(tr2, "Bad exit code: " + tr2.exitValue);
    if (!(tr2.contains("demo") && tr2.contains("Usage"))) error(tr2, "Expected output not found");
    show(tr2);
  }
  ...
  void testClassNamedJava() throws IOException {
    ...
    TestResult tr = doExec(javaCmd, "-cp", classes.toString(), "HelloWorld.java");
    if (!tr.isOK()) error(tr, "Command failed");
    if (!tr.contains("HelloWorld.java")) error(tr, "Expected output not found");
    show(tr);
  }
  ...
  void testSourceClasspath() throws IOException {
    ...
    if (tr.isOK()) error(tr, "Command succeeded unexpectedly");
    if (!tr.contains("file not found: HelloWorld")) error(tr, "Expected output not found");
    show(tr);
  }
  ...
  void testSourceNoArg() throws IOException {
    ...
    if (tr.isOK()) error(tr, "Command succeeded unexpectedly");
    if (!tr.contains("--source requires source version")) error(tr, "Expected output not found");
    show(tr);
  }
  ...
  void testSourceJarConflict() throws IOException {
    ...
    TestResult tr = doExec(javaCmd, "--source", thisVersion, "-jar", simpleJar.toString());
    if (tr.isOK()) error(tr, "Command succeeded unexpectedly");
  ...
  void testTerminalOptionInShebang() throws IOException {
    ...
    Path base = Files.createDirectories(Paths.get("testTerminalOptionInShebang"));
  ...
  void testTerminalOptionInShebangAtFile() throws IOException {
    ...
    TestResult tr = doExec(bad.toString());
    if (!tr.contains(
        "Option -version in @testBadAtFile/bad.at is " + "not allowed in this context"))
      error(tr, "Expected output not found");
    show(tr);
  }
