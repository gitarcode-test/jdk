...
public final class Main {
  ...
  public static void main(String args[]) throws Throwable {
    ...
    List<TestInstance> orderedTests =
        tests.stream()
    ...
    if (listTests) {
      ...
      orderedTests.stream()
          .forEach(
              test ->
                  System.out.println(
                      String.format("%s; workDir=[%s]", test.fullName(), test.workDir())));
      return;
    }
  ...
  private static void runTests(List<TestInstance> tests) {
    ...
    TKit.log(String.format("[  PASSED  ] %d %s", passedCount, passedCount == 1 ? "test" : "tests"));
  ...
  private static long reportDetails(
      List<TestInstance> tests,
      String label,
      Predicate<TestInstance> selector,
      boolean printWorkDir) {

    final Function<TestInstance, String> makeMessage =
        test -> {
          if (printWorkDir) {
            return String.format("%s %s; workDir=[%s]", label, test.fullName(), test.workDir());
          }
    ...
    if (count != 0) {
      TKit.log(
          String.format("%s %d %s, listed below", label, count, count == 1 ? "test" : "tests"));
      tests.stream().filter(selector).map(makeMessage).forEachOrdered(TKit::log);
    }

    return count;
  }

  private static RuntimeException reportSummary(
      List<TestInstance> tests, String label, Predicate<TestInstance> selector) {
    final long count = tests.stream().filter(selector).count();
    if (count != 0) {
      final String message = String.format("%d %s %s", count, label, count == 1 ? "TEST" : "TESTS");
      TKit.log(message);
