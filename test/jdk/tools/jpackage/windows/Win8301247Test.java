...
/**
 * Test that terminating of the parent app launcher process automatically terminates child app
 * launcher process.
 */
...
public class Win8301247Test {
  ...
  private static Optional<Long> findMainAppLauncherPID(JPackageCommand cmd, int expectedCount) {
    ...
    if (expectedCount == 0) {
      TKit.assertEquals(
          ...
          output.getFirst().trim(),
          "Check no app launcher processes found running");
      return Optional.empty();
    }

    String[] headers =
        Stream.of(output.getFirst().split("\\s+", 2))
            .map(String::trim)
            .map(String::toLowerCase)
            .toArray(String[]::new);
    Pattern pattern;
    if (headers[0].equals("parentprocessid") && headers[1].equals("processid")) {
      pattern = Pattern.compile("^(?<ppid>\\d+)\\s+(?<pid>\\d+)\\s+$");
    } else if (headers[1].equals("parentprocessid") && headers[0].equals("processid")) {
      pattern = Pattern.compile("^(?<pid>\\d+)\\s+(?<ppid>\\d+)\\s+$");
    } else {
      throw new RuntimeException("Unrecognizable output of \'wmic process\' command");
    }
    ...
    TKit.assertEquals(
        expectedCount,
        processes.size(),
        String.format("Check [%d] app launcher processes found running", expectedCount));
