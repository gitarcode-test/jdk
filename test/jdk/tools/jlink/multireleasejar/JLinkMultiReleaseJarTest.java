...
public class JLinkMultiReleaseJarTest {
  ...
  public void initialize() throws IOException {
    ...
    String[] args = {
      ...
      "m1.jar",
      "-C",
      basemods.resolve("m1").toString(),
      ".",
      "--release ",
      String.valueOf(JarFile.runtimeVersion().major()),
      "-C",
      rtmods.resolve("m1").toString(),
      "."
    };
    ...
    Files.move(
        rtmods.resolve("m1").resolve("module-info.class"), userdir.resolve("module-info.class"));
  ...
  private void javac(Path source, Path destination, String srcpath) throws IOException {
    ...
    try (Stream<Path> pathStream = Files.walk(source)) {
      args = Stream.concat(args, pathStream.map(Path::toString).filter(s -> s.endsWith(".java")));
  ...
  public void basicTest() throws Throwable {
    ...
    try (BasicImageReader reader = BasicImageReader.open(jimage)) {
      ...
      Set<String> requires =
          ModuleDescriptor.read(new ByteArrayInputStream(b)).requires().stream()
              .map(mdr -> mdr.name())
      ...
      MethodHandle getVersion =
          MethodHandles.lookup().findVirtual(clazz, "getVersion", MethodType.methodType(int.class));
