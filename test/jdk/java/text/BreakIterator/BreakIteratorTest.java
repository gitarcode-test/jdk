...
public class BreakIteratorTest {
  ...
  private Vector testFirstAndNext(BreakIterator bi, String text) {
    ...
    if (p != 0) fail("first() returned " + p + " instead of 0");
  ...
  private Vector testLastAndPrevious(BreakIterator bi, String text) {
    ...
    if (p != text.length()) fail("last() returned " + p + " instead of " + text.length());
    while (p != BreakIterator.DONE) {
      p = bi.previous();
      if (p != BreakIterator.DONE) {
        if (p >= lastP)
  ...
  private void testIsBoundary(BreakIterator bi, String text, int[] boundaries) {
    ...
    for (int i = 0; i <= text.length(); i++) { // change to <= when new BI code goes in
      ...
      if (i == boundaries[p]) {
        if (!isB) fail("Wrong result from isBoundary() for " + i + ": expected true, got false");
        ++p;
      } else {
        if (isB) fail("Wrong result from isBoundary() for " + i + ": expected false, got true");
      }
    }
  }
    ...
    for (int i = 0; i < breaks.length(); i++) {
      ...
      for (int j = 0; j < testChars.length(); j++) {
        ...
        for (int k = 0; k < testChars.length(); k++) {
          ...
          if (type1 == Character.CONTROL
              || type1 == Character.FORMAT
              || type2 == Character.CONTROL
              || type2 == Character.FORMAT) {
            continue;
          }
          ...
          if (!seen2) {
            fail(
                "No break between U+"
                    + Integer.toHexString((int) (work.charAt(1)))
                    + " and U+"
                    + Integer.toHexString((int) (work.charAt(2))));
            errorCount++;
    ...
    for (int i = 0; i < testChars.length(); i++) {
      ...
      for (int j = 0; j < testChars.length(); j++) {
        ...
        for (int k = tb.first(); k != BreakIterator.DONE; k = tb.next())
          if (k == 2) {
            fail(
                "Break between CR and LF in string U+"
                    + Integer.toHexString((int) (work.charAt(0)))
                    + ", U+d U+a U+"
                    + Integer.toHexString((int) (work.charAt(3))));
            errorCount++;
            if (errorCount >= 75) return;
          }
      }
    ...
    for (int i = 0; i < testChars.length(); i++) {
      ...
      for (int j = 0; j < testChars.length(); j++) {
        ...
        if (Character.getType(c) != Character.NON_SPACING_MARK
            && Character.getType(c) != Character.ENCLOSING_MARK) continue;
        work.setCharAt(2, c);
  ...
  public void TestSentenceBreak() {
    ...
    sentenceSelectionData.addElement(
        "Harris thumbed down several, including \"Away We Go\" (which became the huge success"
            + " Oklahoma!). ");
  ...
  public void TestBug8264765() {
    ...
    sentenceSelectionData.addElement("Due to a problem (e.g., software bug), the server is down. ");
  ...
  */
  ...
  public void TestBug4153072() {
    ...
    for (int index = -1; index < begin + 1; ++index) {
      try {
        ...
        if (index < begin)
          fail("Didn't get exception with offset = " + index + " and begin index = " + begin);
      } catch (IllegalArgumentException e) {
        if (index >= begin)
          fail("Got exception with offset = " + index + " and begin index = " + begin);
      }
    }
  }
  ...
  private static final String
      cannedTestChars // characters fo the class Cc are ignorable for breaking
      = /*"\u0000\u0001\u0002\u0003\u0004*/
          " !\"#$%&()+-01234<=>ABCDE[]^_`abcde{}|\u00a0\u00a2"
              + "\u00a3\u00a4\u00a5\u00a6\u00a7\u00a8\u00a9\u00ab\u00ad\u00ae\u00af\u00b0\u00b2\u00b3"
              + "\u00b4\u00b9\u00bb\u00bc\u00bd\u02b0\u02b1\u02b2\u02b3\u02b4\u0300\u0301\u0302\u0303"
        ...
        for (int k = 0; k < testChars.length(); k++) {
          ...
          for (int l = e.first(); l != BreakIterator.DONE; l = e.next()) {
            if (l == 1 || l == 2) {
              ...
              fail(
                  "Got break between U+"
                      + Integer.toHexString((int) (work.charAt(l - 1)))
                      + " and U+"
                      + Integer.toHexString((int) (work.charAt(l))));
              errorCount++;
              if (errorCount >= 75) return;
            }
          }
  ...
  public void TestGetAvailableLocales() {
    ...
    if (locList.length == 0) fail("getAvailableLocales() returned an empty list!");
  ...
  public void TestJapaneseLineBreak() {
    ...
    for (int i = 0; i < precedingChars.length(); i++) {
      ...
      if (j != 1) {
        fail(
            "ja line break failure: failed to stop before '"
                + precedingChars.charAt(i)
                + "' (\\u"
      ...
      if (j != 3) {
        fail(
            "ja line break failure: failed to skip position after '"
                + precedingChars.charAt(i)
                + "' (\\u"
                + Integer.toString(precedingChars.charAt(i), 16)
                + ") at 3 and bounded at "
    ...
    for (int i = 0; i < followingChars.length(); i++) {
      ...
      if (j != 2) {
        fail(
            "ja line break failure: failed to skip position before '"
                + followingChars.charAt(i)
                + "' (\\u"
      ...
      if (j != 3) {
        fail(
            "ja line break failure: failed to stop after '"
                + followingChars.charAt(i)
                + "' (\\u"
                + Integer.toString(followingChars.charAt(i), 16)
                + ") at 3 and bounded at "
  ...
  public void TestLineBreakBasedOnUnicode3_0_0() {
    ...
    if (i != 2) {
      fail(
          "Mongolian line break failure: failed to skip position before \\u1806(Pd) at 2 and"
              + " bounded at "
              + i);
    }
    ...
    if (i != 3) {
      fail(
          "Khmer line break failure: failed to skip position after \\u17DB(Sc) at 3 and bounded at "
              + i);
    }
    ...
    if (i != 2) {
      fail(
          "Ogham line break failure: failed to skip postion before \\u1680(Zs) at 2 and bounded at "
              + i);
    }
  ...
  public void TestEndBehavior() {
    ...
    if (wb.first() != 0) fail("Didn't get break at beginning of string.");
    if (wb.next() != 3) fail("Didn't get break before period in \"boo.\"");
    if (wb.current() != 4 && wb.next() != 4) fail("Didn't get break at end of string.");
  }
