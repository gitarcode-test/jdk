...
public class MantissaDigits {
  ...
  private static final double[] NUMBERS = {
    1.1, 12.1, 123.1, 1234.1, 12345.1, 123456.1, -1.1, -12.1, -123.1, -1234.1, -12345.1, -123456.1,
    1, 12, 123, 1234, 12345, 123456, 1234567, -1, -12, -123, -1234, -12345, -123456, -1234567,
    1.1234, 1.1111, 1.412, 222.333, -771.2222
  };
  private static final DecimalFormatSymbols DFS = new DecimalFormatSymbols(Locale.US);
  private static final String ERRMSG =
      "%s formatted with %s gives %s, and "
  ...
  public void testMantissaDefinition(String pattern, int minDigits, int maxDigits) {
    ...
    for (double number : NUMBERS) {
      ...
      int originalNumDigits =
          (int) String.valueOf(number).chars().filter(Character::isDigit).count();
      ...
      Integer calculatedDigits = Math.min(Math.max(minDigits, originalNumDigits), maxDigits);
      assertEquals(
          mantissaDigits,
          calculatedDigits,
          String.format(ERRMSG, number, pattern, formattedNum, mantissaDigits, calculatedDigits));
    }
  }
