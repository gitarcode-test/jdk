...
public class ProxyGeneratorCombo extends ComboInstance<ProxyGeneratorCombo> {
  ...
  enum MethodsKind implements ComboParameter {
    ...
    TWO(
        "#{METHODACCESS} void one(#{ARG[0]} b);\n"
            + "#{METHODACCESS} void two(#{ARG[0]} a, #{ARG[1]} b);"),
    THREE(
        "#{METHODACCESS} void one(#{ARG[0]} a);\n"
            + "#{METHODACCESS} void two(#{ARG[0]} a, #{ARG[1]} b);\n"
  ...
  public static void main(String... args) throws Exception {
    ...
    new ComboTestHelper<ProxyGeneratorCombo>()
        ...
        .withDimension("METHODACCESS", new ClassAccessKind[] {ClassAccessKind.PUBLIC})
        .withDimension("METHODS", ProxyGeneratorCombo::saveMethod, MethodsKind.values())
        .withArrayDimension("ARG", ProxyGeneratorCombo::saveArg, 2, ArgumentKind.values())
        .withDimension(
            "EXCEPTION",
            ProxyGeneratorCombo::saveException,
            new ExceptionKind[] {ExceptionKind.NONE})
    ...
    new ComboTestHelper<ProxyGeneratorCombo>()
        ...
        .withDimension("METHODACCESS", new ClassAccessKind[] {ClassAccessKind.PUBLIC})
        .withDimension(
            "METHODS", ProxyGeneratorCombo::saveMethod, new MethodsKind[] {MethodsKind.ZERO})
        .withDimension("EXCEPTION", ProxyGeneratorCombo::saveException, ExceptionKind.values())
        .withDimension(
            "MULTI_INTERFACES",
            ProxyGeneratorCombo::saveInterface,
  ...
  boolean filter() {
    if ((currMethod == MethodsKind.NONE || currMethod == MethodsKind.ZERO)
        && currArgs.length >= 2) {
      return currArgs[0] == ArgumentKind.INT && currArgs[1] == ArgumentKind.INT;
    }
    if (currMethod == MethodsKind.ONE && currArgs.length >= 2) {
  ...
  /**
   * Generate the source file and compile. Generate a proxy for the interface and test the resulting
   * Proxy for the methods, exceptions and handling of a thrown exception
   *
  ...
  public void doWork() throws IOException {
    ...
    try {
      ...
      InvocationHandler handler = new ProxyHandler(currException.exceptionKindClass);

      // Construct array of interfaces for the proxy
      Class<?>[] interfaces = new Class<?>[currInterface.classes().length + 1];
      interfaces[0] = tc;
      System.arraycopy(currInterface.classes(), 0, interfaces, 1, currInterface.classes().length);
  ...
  /**
   * Check that the exceptions declared on the proxy match the declarations for exceptions from the
   * interfaces.
   *
    ...
    for (Method m : methods) {
      ...
      if (imethods != null) {
        ...
        if (!expectedEx.equals(exs)) {
          System.out.printf(
              "mismatch on exceptions for method %s:%nExpected: " + "%s%nActual:  %s%n",
              sig, expectedEx, exs);
  ...
  private static List<Class<?>> extractExceptions(List<Method> methods) {
    ...
    for (Method m : methods) {
      ...
      if (e.length == 0) return emptyClassList();
    ...
    for (Class<?> c : interfaces) {
      for (Method m : c.getMethods()) {
        if (!Modifier.isStatic(m.getModifiers())) {
          String sig = toShortSignature(m);
          methods.computeIfAbsent(sig, s -> new ArrayList<Method>()).add(m);
        }
      }
    }
    return methods;
  ...
  /**
   * The Handler for the proxy includes the method to invoke the proxy and the expected exception,
   * if any.
   */
  class ProxyHandler implements InvocationHandler {
    ...
    /**
     * Check that the expected exception was thrown. Special case is handled for Exception1 which
     * does not appear in the throws clause of the method so UndeclaredThrowableException is
     * expected.
     */
    void checkThrownException(Throwable thrown) {
      if (exceptionClass == Exception1.class
          ...
          && ((UndeclaredThrowableException) thrown).getUndeclaredThrowable()
              instanceof Exception1) {
      ...
      if (exceptionClass != null && !exceptionClass.equals(thrown.getClass())) {
        throw new RuntimeException(
            "Wrong exception thrown: expected: "
                + exceptionClass
                + ", actual: "
                + thrown.getClass());
      }
