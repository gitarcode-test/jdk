...
public class PublicMethodsTest {
  ...
  public static void main(String[] args) {
    ...
    int[] diffs = new int[1];
    try (Stream<Map.Entry<int[], Map<String, String>>> expected = expectedResults(c)) {
    ...
    if (diffs[0] > 0) {
      throw new RuntimeException("There were " + diffs[0] + " differences.");
    }
  }
  ...
  public static class Generate {
    public static void main(String[] args) {
      Case c = new Case1();
      dumpResults(generateResults(c)).forEach(System.out::println);
    }
  }
  ...
  interface Case {
    ...
    List<String> INTERFACE_METHODS =
        List.of("", "void m();", "default void m() {}", "static void m() {}");
  ...
  static class Case1 implements Case {
    ...
    private static final Map<String, List<String>> REPLACEMENTS =
        Map.of(
            "I", INTERFACE_METHODS,
            "J", INTERFACE_METHODS,
            "K", INTERFACE_METHODS,
    ...
    private static final List<String> REPLACEMENT_KEYS =
        REPLACEMENTS.keySet().stream().sorted().collect(Collectors.toList());
  ...
  static Stream<int[]> combinations(Case c) {
    int[] sizes =
        c.replacementKeys().stream().mapToInt(key -> c.replacements().get(key).size()).toArray();
  ...
  static TestClassLoader compile(String source) throws CompileException {
    ...
    StandardJavaFileManager standardJavaFileManager =
        javac.getStandardFileManager(errorsCollector, Locale.ROOT, Charset.forName("UTF-8"));
    try {
      standardJavaFileManager.setLocation(StandardLocation.CLASS_PATH, List.of());
    } catch (IOException e) {
      throw new UncheckedIOException(e);
    }
    TestFileManager testFileManager = new TestFileManager(standardJavaFileManager, source);
    ...
    try {
      javacTask =
          javac.getTask(
              ...
              null,
              List.of(
                  testFileManager.getJavaFileForInput(
    ...
    return new TestClassLoader(ClassLoader.getSystemClassLoader(), testFileManager);
  }

  static class CompileException extends Exception {
    CompileException(List<Diagnostic<?>> diagnostics) {
      super(diagnostics.stream().map(diag -> diag.toString()).collect(Collectors.joining("\n")));
    }
  }
  ...
  static class TestFileManager extends ForwardingJavaFileManager<StandardJavaFileManager> {
    ...
    private static class SourceFileObject extends SimpleJavaFileObject {
      ...
      SourceFileObject(String className, String source) {
        super(URI.create("memory:/src/" + className.replace('.', '/') + ".java"), Kind.SOURCE);
        this.source = source;
      }
    ...
    private static class ClassFileObject extends SimpleJavaFileObject {
      ...
      ClassFileObject(String className) {
        super(URI.create("memory:/out/" + className.replace('.', '/') + ".class"), Kind.CLASS);
      ...
      byte[] getBytes() {
        if (byteArrayOutputStream == null) {
          throw new IllegalStateException("No class file written for class: " + className);
        }
            ...
            comb -> {
              ...
              try {
                try (TestClassLoader cl = compile(src)) {
                  ...
                  return Stream.of(Map.entry(comb, generateResult(c, cl)));
      ...
      BufferedReader r =
          new BufferedReader(
              new InputStreamReader(
                  c.getClass().getResourceAsStream(c.getClass().getSimpleName() + ".results"),
                  "UTF-8"));
  ...
  static Stream<Map.Entry<int[], Map<String, String>>> parseResults(Stream<String> lines) {
    return lines
  ...
  static Stream<String> dumpResults(Stream<Map.Entry<int[], Map<String, String>>> results) {
    return results.map(
        le ->
            IntStream.of(le.getKey()).mapToObj(String::valueOf).collect(joining(","))
                + "#"
  ...
  static Stream<String> diffResults(
      Case c, Stream<Map.Entry<int[], Map<String, String>>> expectedResults) {
    return expectedResults.flatMap(
        exp -> {
          int[] comb = exp.getKey();
