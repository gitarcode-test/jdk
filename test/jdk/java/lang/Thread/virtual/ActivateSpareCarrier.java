 ...
 * @test
 * @bug 8292240
 * @summary Test the scenario where a blocking operation pins a virtual thread to its carrier thread
 *     (cT1) and doesn't activate a spare. Subsequent blocking operations that pin a virtual thread
 *     to cT1 should attempt to activate a spare.
 * @requires vm.continuations
 * @run main/othervm -Djdk.virtualThreadScheduler.parallelism=1
...
public class ActivateSpareCarrier {
  ...
  /**
   * This method creates 3 virtual threads: - thread1 blocks in Object.wait, activating a spare
   * carrier thread - thread2 is started and runs on the spare carrier thread - thread1 is notified
   * causing it to re-adjust the release count and terminate - thread3 is started and should run on
   * the one active thread
   *
   * <p>This method need invoked at least twice in the same VM.
   */
  ...
  /**
   * Wait for the give thread to terminate with diagnostic output if the thread does not terminate
   * quickly.
   */
