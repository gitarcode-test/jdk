...
public class ClassLoaderTest {
  ...
  /**
   * Test cases are based on the following logic, given: a policyFile in {none, valid, malformed}
   * and a classLoader in {SystemClassLoader, CustomClassLoader}: for (clientModule : {"NAMED",
   * "UNNAMED"}) { for (classLoaderModule : {"NAMED", "UNNAMED"}) { Create and run java command for
   * each possible Test case } }
   */
  private void processForPolicyFile() throws Exception {
    ...
    // NAMED-NAMED:
    System.out.println(
        "Case:- Modular Client and "
            + ((useSCL) ? "SystemClassLoader" : "Modular CustomClassLoader")
            + " "
            + smMsg);
    ...
    System.out.println(
        "Case:- Modular Client and "
            + ((useSCL) ? "SystemClassLoader" : "Unknown modular CustomClassLoader")
            + " "
            + smMsg);
    execute(
        new String[] {"--module-path", autoModCloc, "-cp", regLoaderLoc, "-m", "mc/c.TestClient"},
        "FAIL",
        MISSING_MODULE);

    // UNNAMED-NAMED:
    System.out.println(
        "Case:- Unknown modular Client and "
            + ((useSCL) ? "SystemClassLoader" : "Modular CustomClassLoader")
            + " "
            + smMsg);
    ...
    System.out.println(
        "Case:- Regular Client and "
            + ((useSCL) ? "SystemClassLoader" : "Unknown modular CustomClassLoader")
            + " inside --module-path "
            + smMsg);
    execute(
        "--module-path",
        regClientLoc + separator + regLoaderLoc,
        autoAddModArg,
        "-m",
        "c/c.TestClient");
  ...
  private static void setUp() throws Exception {
    ...
    updateModuleDescr(
        MCL_JAR, ModuleDescriptor.newModule("mcl").exports("cl").requires("java.base").build());
    ...
    updateModuleDescr(
        AMC_JAR,
        ModuleDescriptor.newModule("mc").exports("c").requires("java.base").requires("cl").build());
  }

  /** Update regular jars and include module-info.class inside it to make modular jars. */
