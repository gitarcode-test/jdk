...
public class Locks {
  ...
  private static String getLockName(Object lock) {
    ...
    return lock.getClass().getName() + '@' + Integer.toHexString(System.identityHashCode(lock));
  }
  ...
  private static void checkBlockedObject(Thread t, Object lock, Thread owner) {
    ...
    Predicate<String> p =
        (res) ->
            ((res != null && !res.equals(expectedLock)) || (res == null && expectedLock != null));
    ...
    if (p.test(lockName)) {
      ...
      while (p.test(lockName)) {
        if (retryCount++ > 500) {
          printStackTrace(t);
          throw new RuntimeException(
              "Thread " + t.getName() + " is blocked on " + expectedLock + " but got " + lockName);
        }
        goSleep(100);
    ...
    p =
        (res) ->
            ((res != null && !res.equals(expectedOwnerName))
                || (res == null && expectedOwnerName != null));
    if (p.test(lockOwnerName)) {
      printStackTrace(t);
      throw new RuntimeException(
          "Owner of " + lock + " should be " + expectedOwnerName + " but got " + lockOwnerName);
    }
  }
  ...
  private static ThreadInfo findOwnerInfo(ThreadInfo[] infos, String lock) throws Exception {
    ...
    for (ThreadInfo info : infos) {
      ...
      if (lock.equals(blockedLock)) {
        ...
        if (threadId == -1) {
          throw new RuntimeException("TEST FAILED: " + lock + " expected to have owner");
        }
  ...
  private static void findThreadsBlockedOn(Object o, long[] expectedThreads) throws Exception {
  ...
  private static void doCheck(ThreadInfo[] infos, String lock, long[] expectedThreads)
    ...
    if (ownerInfo == null) {
      throw new RuntimeException(
          "TEST FAILED: " + "Can't retrieve ThreadInfo for the blocked thread");
    }
    ...
    while (ownerInfo.getThreadState() == Thread.State.BLOCKED) {
      ...
      threads[count++] = ownerInfo.getThreadId();
      log(" Owner = %s  id = %d", ownerInfo.getThreadName(), ownerInfo.getThreadId());
      lock = ownerInfo.getLockName();
      log("%s Id = %d  blocked on %s", ownerInfo.getThreadName(), ownerInfo.getThreadId(), lock);
    }
    log("");

    if (count != expectedThreads.length) {
      throw new RuntimeException(
          "TEST FAILED: " + "Expected chain of threads not matched; current count =" + count);
    }
    int failures = 0;
