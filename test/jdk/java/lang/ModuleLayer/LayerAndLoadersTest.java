 ...
 * @build LayerAndLoadersTest jdk.test.lib.compiler.CompilerUtils jdk.test.lib.util.ModuleUtils
...
public class LayerAndLoadersTest {
  ...
  public void setup() throws Exception {
    ...
    assertTrue(
        CompilerUtils.compile(SRC_DIR, MODS_DIR, "--module-source-path", SRC_DIR.toString()));
  }

  /**
   * Basic test of ModuleLayer.defineModulesWithOneLoader
   *
   * <p>Test scenario: m1 requires m2 and m3
   */
  ...
  /**
   * Basic test of ModuleLayer.defineModulesWithOneLoader where one of the modules is a service
   * provider module.
   *
   * <p>Test scenario: m1 requires m2 and m3 m1 uses S m4 provides S with ...
   */
  ...
  /**
   * Basic test of ModuleLayer.defineModulesWithManyLoaders where one of the modules is a service
   * provider module.
   *
   * <p>Test scenario: m1 requires m2 and m3 m1 uses S m4 provides S with ...
   */
  ...
  /**
   * Tests that the class loaders created by defineModulesWithXXX delegate to the given parent class
   * loader.
   */
   ...
   *
   * <p>Test scenario: m1 exports p m2 exports p
   */
  public void testOverlappingPackages() {
    ModuleDescriptor descriptor1 = ModuleDescriptor.newModule("m1").exports("p").build();

    ModuleDescriptor descriptor2 = ModuleDescriptor.newModule("m2").exports("p").build();

    ModuleFinder finder = ModuleUtils.finderOf(descriptor1, descriptor2);

    Configuration cf =
        ModuleLayer.boot().configuration().resolve(finder, ModuleFinder.of(), Set.of("m1", "m2"));
   ...
   *
   * <p>Test scenario: layer1: m1 exports p, m2 exports p layer2: m3 reads m1, m4 reads m2
   */
  public void testSplitDelegation() {
    ...
    Configuration cf1 =
        ModuleLayer.boot().configuration().resolve(finder1, ModuleFinder.of(), Set.of("m1", "m2"));

    ModuleLayer layer1 = ModuleLayer.boot().defineModulesWithManyLoaders(cf1, null);
    checkLayer(layer1, "m1", "m2");

    ModuleDescriptor descriptor3 = ModuleDescriptor.newModule("m3").requires("m1").build();

    ModuleDescriptor descriptor4 = ModuleDescriptor.newModule("m4").requires("m2").build();
  ...
  /**
   * Test ModuleLayer.defineModulesWithXXX when the modules that override same named modules in the
   * parent layer.
   *
   * <p>Test scenario: layer1: m1, m2, m3 => same loader layer2: m1, m2, m4 => same loader
   */
  public void testOverriding1() throws Exception {
    ...
    Configuration cf2 = cf1.resolve(finder, ModuleFinder.of(), Set.of("m1"));
  ...
  /**
   * Test Layer defineModulesWithXXX when the modules that override same named modules in the parent
   * layer.
   *
   * <p>Test scenario: layer1: m1, m2, m3 => loader pool layer2: m1, m2, m3 => loader pool
   */
   ...
   * parent layer.
   *
   * <p>layer1: m1, m2, m3 => same loader layer2: m1, m3 => same loader
   */
   ...
   * layer.
   *
   * <p>layer1: m1, m2, m3 => loader pool layer2: m1, m3 => loader pool
   */
  ...
  /**
   * Basic test for locating resources with a class loader created by defineModulesWithOneLoader.
   */
  ...
  /**
   * Test locating resources with the class loader created by defineModulesWithOneLoader. The class
   * loader has the given class loader as its parent.
   */
  ...
  /**
   * Basic test for locating resources with class loaders created by defineModulesWithManyLoaders.
   */
  ...
  /**
   * Test locating resources with class loaders created by defineModulesWithManyLoaders. The class
   * loaders have the given class loader as their parent.
   */
  ...
  /** Resolve the given modules, by name, and returns the resulting Configuration. */
  ...
  /** Invokes the static void main(String[]) method on the given class in the given module. */
  ...
  /** Checks that the given layer contains exactly the expected modules (by name). */
  private void checkLayer(ModuleLayer layer, String... expected) {
    Set<String> names = layer.modules().stream().map(Module::getName).collect(Collectors.toSet());
  ...
  /** Test that a class can be loaded via the class loader of all modules in the given layer. */
  ...
  /**
   * Test that a class cannot be loaded via any of the class loaders of the modules in the given
   * layer.
   */
