...
public class PipelineTest {
  ...
  static void t1_simplePipeline() {
    try {
      ...
      verify(s1, s1, asList(new ProcessBuilder("cat")));
      verify(s1, s1, asList(new ProcessBuilder("cat"), new ProcessBuilder("cat")));
      verify(
          s1,
          s1,
  ...
  static void t2_translatePipeline() {
    try {
      ...
      verify(
          s2, r2, asList(new ProcessBuilder("tr", "e", "E"), new ProcessBuilder("tr", "o", "O")));
    } catch (Throwable t) {
      unexpected(t);
    }
  }
  ...
  /**
   * Test that redirectErrorStream sends standard error of the first process to the standard output.
   * The standard error of the first process should be empty. The standard output of the 2nd should
   * contain the error message including the bad file name.
   */
    ...
    try {
      ...
      List<Process> processes =
          ProcessBuilder.startPipeline(
              asList(
                  new ProcessBuilder("cat", "NON-EXISTENT-FILE")
      ...
      check(
          p2contents.contains("NON-EXISTENT-FILE"),
  ...
  /**
   * Test that no processes are left after a failed startPipeline. Test illegal combinations of
   * redirects.
   */
  static void t4_failStartPipeline() {
    ...
    THROWS(
        ...
        () -> {
          ...
          List<Process> processes =
              ProcessBuilder.startPipeline(
                  asList(
                      new ProcessBuilder("cat", "NON-EXISTENT-FILE1").redirectOutput(p1err),
        ...
        () -> {
          ...
          List<Process> processes =
              ProcessBuilder.startPipeline(
                  asList(
                      new ProcessBuilder("cat", "NON-EXISTENT-FILE2"),
    ...
    THROWS(
        ...
        () -> {
          List<Process> processes =
              ProcessBuilder.startPipeline(asList(new ProcessBuilder("cat", "a"), null));
        },
        () -> {
          List<Process> processes =
              ProcessBuilder.startPipeline(asList(null, new ProcessBuilder("cat", "b")));
        });
    ...
    THROWS(
        ...
        () -> {
          List<Process> processes =
              ProcessBuilder.startPipeline(
                  asList(
                      new ProcessBuilder("cat", "c"), new ProcessBuilder("NON-EXISTENT-COMMAND")));
        });
  ...
  static void verify(String input, String expected, List<ProcessBuilder> builders)
      throws IOException {
  ...
  static void print(ProcessHandle p) {
    System.out.printf("process: pid: %d, info: %s%n", p.pid(), p.info());
  }
