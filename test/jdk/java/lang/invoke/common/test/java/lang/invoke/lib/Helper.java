...
public class Helper {
  ...
  public static final boolean IS_VERBOSE = System.getProperty("verbose") != null;

  /**
   * Flag for thorough testing -- all test will be executed, true if {@code -Dthorough} specified.
   */
  public static final boolean IS_THOROUGH = System.getProperty("thorough") != null;
    ...
    try {
      AS_LIST =
          MethodHandles.lookup()
              .findStatic(
                  Arrays.class, "asList", MethodType.methodType(List.class, Object[].class));
  ...
  public static List<Class<?>> getParams(List<Class<?>> classes, boolean isVararg, int argsCount) {
    boolean unmodifiable = true;
    List<Class<?>> result = classes.subList(0, Math.min(argsCount, (MAX_ARITY / 2) - 1));
    int extra = 0;
  ...
  public static MethodHandle addTrailingArgs(
      MethodHandle target, int nargs, List<Class<?>> classes) {
  ...
  private static long nextArg(Class<?> kind) {
    if (kind == long.class || kind == Long.class || kind == double.class || kind == Double.class)
  ...
  public static MethodType randomMethodTypeGenerator(int arity) {
    ...
    if (arity > MAX_ARITY) {
      throw new IllegalArgumentException(String.format("Arity should not exceed %d!", MAX_ARITY));
    }
