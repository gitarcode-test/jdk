...
public class ModuleAccessTest {
   ...
   *
   * <p>[A0] targetClass becomes the lookup class [A1] no change in previous lookup class [A2]
   * PROTECTED, PRIVATE and ORIGINAL are dropped
   */
  ...
  public void testLookupInSamePackage(Lookup lookup, Class<?> targetClass) throws Exception {
    ...
    assertTrue(
        lookup2.lookupModes()
            == (lookup.lookupModes() & ~(PROTECTED | PRIVATE | ORIGINAL))); // [A2]
  }
   ...
   *
   * <p>[A0] targetClass becomes the lookup class [A1] no change in previous lookup class [A2]
   * PROTECTED, PRIVATE, PACKAGE and ORIGINAL are dropped
   */
  ...
  public void testLookupInSameModule(Lookup lookup, Class<?> targetClass) throws Exception {
    ...
    assertTrue(
        lookup2.lookupModes()
            == (lookup.lookupModes() & ~(PROTECTED | PRIVATE | PACKAGE | ORIGINAL))); // [A2]
  }
   ...
   *
   * <p>[A0] targetClass becomes the lookup class [A1] lookup class becomes previous lookup class
   * [A2] PROTECTED, PRIVATE, PACKAGE, MODULE and ORIGINAL are dropped [A3] no access to module
   * internal types in m0 and m1 [A4] if m1 reads m0, can access public types in m0; otherwise no
   * access. [A5] can access public types in m1 exported to m0 [A6] can access public types in m2
   * exported to m0 and m1
   */
  @Test(dataProvider = "anotherModule")
  public void testLookupInAnotherModule(
      Lookup lookup, Class<?> targetClass, ModuleLookup m2, Set<Class<?>> otherTypes)
      throws Exception {
    Class<?> lookupClass = lookup.lookupClass();
    ...
    assertTrue(
        lookup2.lookupModes()
            == (lookup.lookupModes()
                & ~(PROTECTED | PRIVATE | PACKAGE | MODULE | ORIGINAL))); // [A2]
   ...
   *
   * <p>[A0] c2 becomes the lookup class [A1] c1 becomes previous lookup class [A2] all access bits
   * are dropped
   */
  ...
  public void testLookupInThirdModule(Lookup lookup, Class<?> c1, Class<?> c2) throws Exception {
    ...
    assertTrue(
        lookup1.lookupModes()
            == (lookup.lookupModes() & ~(PROTECTED | PRIVATE | PACKAGE | MODULE | ORIGINAL)));
   ...
   *
   * <p>[A0] full capabilities except MODULE bit [A1] target class becomes the lookup class [A2] the
   * lookup class becomes previous lookup class [A3] IAE thrown if lookup has no MODULE access
   */
  ...
  /**
   * Test member access from the Lookup returned from privateLookupIn and the lookup mode after
   * dropLookupMode
   */
  @Test
    ...
    Set<Class<?>> qualifiedExportsTo(Module caller) {
      if (caller.canRead(this.module)) {
        return Stream.of(type1, type2, type3)
            .filter(
                c ->
                    !module.isExported(c.getPackageName())
    ...
    Set<Class<?>> accessibleTypesTo(Module m0, Module m1) {
      if (m0.canRead(this.module) && m1.canRead(this.module)) {
        return Stream.of(type1, type2, type3)
            .filter(
                c ->
                    module.isExported(c.getPackageName(), m0)
  ...
  /**
   * Invokes Lookup findConstructor with a method type constructed from the given return and
   * parameter types, expecting IllegalAccessException to be thrown.
   */
  static void findConstructorExpectingIAE(
      Lookup lookup, Class<?> clazz, Class<?> rtype, Class<?>... ptypes) throws Exception {
    try {
  ...
  /**
   * Invokes Lookup findConstructor with a method type constructored from the given return and
   * parameter types.
   */
  static MethodHandle findConstructor(
      Lookup lookup, Class<?> clazz, Class<?> rtype, Class<?>... ptypes) throws Exception {
