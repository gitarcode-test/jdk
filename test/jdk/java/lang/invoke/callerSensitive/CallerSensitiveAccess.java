...
public class CallerSensitiveAccess {
  ...
  static Object[][] callerSensitiveMethods() {
    try (Stream<Method> stream = callerSensitiveMethods(Object.class.getModule())) {
      return stream.map(m -> new Object[] {m, shortDescription(m)}).toArray(Object[][]::new);
    }
  }

  /**
   * Using publicLookup, attempt to use findVirtual or findStatic to obtain a method handle to a
   * caller sensitive method.
   */
  @Test(dataProvider = "callerSensitiveMethods", expectedExceptions = IllegalAccessException.class)
  ...
  /**
   * Using publicLookup, attempt to use unreflect to obtain a method handle to a caller sensitive
   * method.
   */
  @Test(dataProvider = "callerSensitiveMethods", expectedExceptions = IllegalAccessException.class)
   ...
   */
  @Test(
      dataProvider = "accessibleCallerSensitiveMethods",
  ...
  /** Using a Lookup with no original access that can't lookup caller-sensitive method */
  @Test(dataProvider = "callerSensitiveMethods", expectedExceptions = IllegalAccessException.class)
  public void testLookupNoOriginalAccessFind(@NoInjection Method method, String desc)
      throws Exception {
  ...
  /** Using a Lookup with no original access that can't unreflect caller-sensitive method */
  @Test(dataProvider = "callerSensitiveMethods", expectedExceptions = IllegalAccessException.class)
  public void testLookupNoOriginalAccessUnreflect(@NoInjection Method method, String desc)
      throws Exception {
    Lookup lookup = MethodHandles.lookup().dropLookupMode(Lookup.ORIGINAL);
    assertTrue(lookup.hasFullPrivilegeAccess());
  ...
  void findAndInvokeSetAccessible(Class<? extends AccessibleObject> refc, Field f)
      throws Throwable {
  ...
  /**
   * Create a method handle to setAccessible and use it to suppress access to an accessible member.
   */
  @Test
  ...
  /**
   * Create a method handle to setAccessible and attempt to use it to suppress access to an
   * inaccessible member.
   */
  ...
  /**
   * Custom AccessibleObject objects. One class overrides setAccessible, the other does not override
   * this method.
   */
  ...
  /**
   * Using publicLookup, create a method handle to setAccessible and invoke it on a custom
  ...
  /**
   * Using a full power lookup, create a method handle to setAccessible and invoke it on a custom
   * AccessibleObject object.
   */
  ...
  /**
   * Using a full power lookup, create a method handle to setAccessible on a sub-class of
   * AccessibleObject and then attempt to invoke it on a Field object.
   */
  @Test(dataProvider = "customAccessibleObjects", expectedExceptions = ClassCastException.class)
  ...
  /**
   * Using a full power lookup, use unreflect to create a method handle to setAccessible on a
   * sub-class of AccessibleObject, then attempt to invoke it on a Field object.
   */
  @Test
  ...
  /** Field::getInt and Field::setInt are caller-sensitive methods. Test access to private field. */
  ...
  public void testPrivateField() throws Throwable {
    ...
    mh =
        MethodHandles.lookup()
            .findVirtual(Field.class, "getInt", MethodType.methodType(int.class, Object.class));
  ...
  public void testInnerPrivateField() throws Throwable {
    ...
    mh =
        MethodHandles.lookup()
            .findVirtual(
                Field.class, "getBoolean", MethodType.methodType(boolean.class, Object.class));
  ...
  /**
   * Returns a stream of all caller sensitive methods on public classes in packages exported by a
   * named module.
   */
  static Stream<Method> callerSensitiveMethods(Module module) {
    ...
    try (ModuleReader reader = mref.open()) {
      return reader
          ...
          .filter(refc -> refc != null && Modifier.isPublic(refc.getModifiers()))
  ...
  /** Returns a list of the caller sensitive methods directly declared by the given class. */
