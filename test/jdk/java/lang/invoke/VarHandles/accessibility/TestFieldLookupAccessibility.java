...
public class TestFieldLookupAccessibility {
  ...
  enum FieldLookup {
    ...
    MH_UNREFLECT_SETTER() {
      ...
      boolean isAccessible(Field f) {
        return f.isAccessible() && !Modifier.isStatic(f.getModifiers())
  ...
  public Object[][] lookupProvider() throws Exception {
    Stream<List<Object>> baseCases =
        Stream.of(
            ...
            List.of(
                pkg.A.class,
                pkg.subpkg.B_extends_A.lookup(),
                pkg.subpkg.B_extends_A.inaccessibleFields()),
  ...
  public void test(
      FieldLookup fl, Class<?> src, MethodHandles.Lookup l, Set<String> inaccessibleFields) {
    ...
    Set<String> actualFieldNames =
        actual.keySet().stream().map(Field::getName).collect(Collectors.toSet());
