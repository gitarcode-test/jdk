...
public class LogGeneratedClassesTest {
  ...
  public void testNotLogging() throws Exception {
    ProcessBuilder pb =
        createLimitedTestJavaProcessBuilder(
            "-cp", CLASSES.toString(), "-Djava.security.manager=allow", "com.example.TestLambda");
  ...
  public void testLogging() throws Exception {
    ...
    assertEquals(
        Files.find(
                dumpDir,
                99,
                (p, a) -> p.startsWith(dumpDir.resolve("com/example")) && a.isRegularFile())
            .count(),
        2,
        "Two lambda captured");
  }
  ...
  public void testDumpDirNotWritable() throws Exception {
    ...
    Path dumpDir = testDir.resolve(DUMP_LAMBDA_PROXY_CLASS_FILES);
    Files.createDirectory(testDir);
    Files.createDirectory(dumpDir, asFileAttribute(fromString("r-xr-xr-x")));
    try {
      if (isWriteableDirectory(dumpDir)) {
        ...
        System.out.println(
            "WARNING: The dump directory is writeable. Skipping testDumpDirNotWritable test.");
        return;
      }
  ...
  public void testLoggingException() throws Exception {
    ...
    assertEquals(
        outputAnalyzer.asLines().stream().filter(s -> s.startsWith("WARNING: Exception")).count(),
        2,
        "show error each capture");
    ...
    Predicate<Path> filter =
        p -> p.getParent() == null || dumpPath.startsWith(p) || p.startsWith(dumpPath);
    boolean debug = true;
    if (debug) {
      Files.walk(dumpDir)
          .forEachOrdered(
