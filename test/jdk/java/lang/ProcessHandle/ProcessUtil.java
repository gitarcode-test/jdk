...
public abstract class ProcessUtil {
  ...
  /**
   * Returns the list of direct children. WIndows conhost.exe children are filtered out.
   *
  ...
  public static List<ProcessHandle> getChildren(ProcessHandle ph) {
    return ph.children().filter(ProcessUtil::isNotWindowsConsole).collect(Collectors.toList());
  }

  /**
   * Returns the list of all direct and indirect children. WIndows conhost.exe children are filtered
   * out.
   *
   * @param ph the Process to get children of
   * @return a list of child ProcessHandles
  ...
  public static List<ProcessHandle> getDescendants(ProcessHandle ph) {
    return ph.descendants().filter(ProcessUtil::isNotWindowsConsole).collect(Collectors.toList());
  }

  /**
   * Waits for and returns the direct expected Children of a ProcessHandle. For Windows, the
   * conhost.exe children are filtered out.
   *
  ...
  public static List<ProcessHandle> waitForChildren(ProcessHandle ph, long nchildren) {
    ...
    do {
      ...
      System.out.printf(
          " waiting for subprocesses of %s to start," + " expected: %d, current: %d%n",
          ph, nchildren, count);
  ...
  /**
   * Waits for and returns all expected Children of a ProcessHandle. For Windows, the conhost.exe
  ...
  /**
   * Destroy all children of the ProcessHandle. (Except the conhost.exe on Windows)
   *
  ...
  static boolean isWindowsConsole(ProcessHandle p) {
    return Platform.isWindows()
        && p.info().command().orElse("").endsWith("C:\\Windows\\System32\\conhost.exe");
  }
  ...
  /**
   * Print information about a process. Prints the pid, if it is alive, and information about the
   * process.
   *
  ...
  static void printProcess(ProcessHandle ph, String prefix) {
    printf(
        "%spid %s, alive: %s; parent: %s, %s%n",
        prefix, ph.pid(), ph.isAlive(), ph.parent(), ph.info());
  }

  /**
   * Print the process hierarchy as visible via ProcessHandle. Prints the pid, if it is alive, and
