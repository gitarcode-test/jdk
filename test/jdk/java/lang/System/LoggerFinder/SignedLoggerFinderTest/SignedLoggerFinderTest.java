...
public class SignedLoggerFinderTest {
  ...
  /**
   * This test triggers recursion in the broken JDK. The error can manifest in a few different ways.
   * One error seen is "java.lang.NoClassDefFoundError: Could not initialize class
   * jdk.internal.logger.LoggerFinderLoader$ErrorPolicy"
   *
   * <p>The original reported error was a StackOverflow (also seen in different iterations of this
   * run). Running test in signed and unsigned jar mode for sanity coverage. The current bug only
   * manifests when jars are signed.
   */
  ...
  private static final String CUSTOM_LOGGER_FINDER_NAME = "loggerfinder.SimpleLoggerFinder";
  private static final String CUSTOM_LOGGER_NAME = "loggerfinder.SimpleLoggerFinder$SimpleLogger";
  ...
  public static void main(String[] args) throws Throwable {
    ...
    if (init) {
      ...
      if (multiThreadLoad) {
        ...
        new Thread(
                runnableWithSleep(
                    ...
                    sleep,
                    "System.getLogger type: "))
            .start();
        new Thread(
                runnableWithSleep(
                    () -> System.LoggerFinder.getLoggerFinder(),
                    sleep,
                    "System.getLoggerFinder type: "))
            .start();
      }
      ...
      if (withCustomLoggerFinder) {
        assertEquals(
            System.LoggerFinder.getLoggerFinder().getClass().getName(), CUSTOM_LOGGER_FINDER_NAME);
        System.Logger testLogger = System.getLogger("jdk.event.security");
        assertEquals(testLogger.getClass().getName(), CUSTOM_LOGGER_NAME);
      } else {
        assertEquals(
            System.LoggerFinder.getLoggerFinder().getClass().getName(),
  ...
  private static void launchTest(boolean multiThreadLoad, boolean withCustomLoggerFinder) {
    ...
    if (withCustomLoggerFinder) {
      ...
      cmds.addAll(List.of("-classpath", System.getProperty("test.classes")));
    }
    ...
    cmds.addAll(List.of("SignedLoggerFinderTest", "no-init"));
    ...
    try {
      ...
      if (withCustomLoggerFinder && signJars) {
        ...
        outputAnalyzer.shouldContain(DNAME);
      }
  ...
  private static OutputAnalyzer jarsigner(List<String> extra) throws Throwable {
    JDKToolLauncher launcher =
        JDKToolLauncher.createUsingTestJDK("jarsigner")
