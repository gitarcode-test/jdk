...
public class BootstrapLoggerTest {
  ...
  static {
    try {
      ...
      isAlive = BootstrapLogger.class.getDeclaredMethod("isAlive");
      ...
      logManagerInitialized = BootstrapLogger.class.getDeclaredField("logManagerConfigured");
  ...
  public static void main(String[] args) throws Exception {
    ...
    int index = 0;
    logger.log(levels[index], "Early message #" + (index + 1));
    index++;
    logger.log(levels[index], "Early message #" + (index + 1));
    index++;
    ...
    if (!afterBoot.contains("INFO: LOG#4")) {
      throw new RuntimeException("System.err does not contain: " + "INFO: LOG#4");
    }
    ...
    try {
      ...
      Object fooLogger = loggerClass.getMethod("getLogger", String.class).invoke(null, "foo");
      loggerClass
      ...
      Object rootLogger = loggerClass.getMethod("getLogger", String.class).invoke(null, "");
      Object handlers = loggerClass.getMethod("getHandlers").invoke(rootLogger);
      handlerClass
          .getMethod("setLevel", levelClass)
          .invoke(Array.get(handlers, 0), levelClass.getField("ALL").get(null));

      hasJUL = true;
    } catch (ClassNotFoundException x) {
      LogStream.err.println("JUL is not present: class " + x.getMessage() + " not found");
      hasJUL = false;
    } finally {
      SimplePolicy.allowAll.set(Boolean.FALSE);
    }
    ...
    if (hasJUL) {
      ...
      if (!debug.contains("FINE: hi now!")) {
        throw new RuntimeException("System.err does not contain: " + "FINE: hi now!");
      }
    } else {
    ...
    try {
      bazbaz =
          java.lang.System.LoggerFinder.getLoggerFinder()
              .getLogger("foo.bar.baz.baz", BootstrapLoggerTest.class.getModule());
    } finally {
      SimplePolicy.allowAll.set(Boolean.FALSE);
    }
    if (!((Boolean) logManagerInitialized.get(null)).booleanValue()) {
    ...
    try {
      ...
      Stream<WeakReference<Thread>> stream =
          Thread.getAllStackTraces().keySet().stream()
              .filter((t) -> t.getName().startsWith("BootstrapMessageLoggerTask-"))
      ...
      if (test == TestCase.SECURE_AND_WAIT) {
        ...
        while (!threads.isEmpty()) {
          ...
          LogStream.err.println(
              "BootstrapMessageLoggerTask has been gc'ed: " + threads.size() + " remaining...");
        }
  ...
  static final class SimplePolicy extends Policy {
    static final ThreadLocal<Boolean> allowAll =
