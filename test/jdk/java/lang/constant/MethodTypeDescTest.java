...
public class MethodTypeDescTest extends SymbolicDescTest {
  ...
  private void testMethodTypeDesc(MethodTypeDesc r) throws ReflectiveOperationException {
    ...
    assertEquals(
        r, MethodTypeDesc.of(r.returnType(), r.parameterList().stream().toArray(ClassDesc[]::new)));
    assertEquals(
        r,
        MethodTypeDesc.of(
            r.returnType(),
    ...
    assertEquals(
        ...
        MethodTypeDesc.of(
            r.returnType(),
            IntStream.range(0, r.parameterCount()).mapToObj(r::parameterType).toList()));
  }
  ...
  private void testMethodTypeDesc(MethodTypeDesc r, MethodType mt)
    ...
    assertEquals(
        r.parameterList(),
        mt.parameterList().stream().map(SymbolicDescTest::classToDesc).collect(toList()));
    assertEquals(
        r.parameterArray(),
        Stream.of(mt.parameterArray())
            .map(SymbolicDescTest::classToDesc)
            .toArray(ClassDesc[]::new));
  ...
  private void assertMethodType(ClassDesc returnType, ClassDesc... paramTypes)
      throws ReflectiveOperationException {
    String descriptor =
        Stream.of(paramTypes).map(ClassDesc::descriptorString).collect(joining("", "(", ")"))
    ...
    for (int i = 0; i <= paramTypes.length; i++) {
      for (ClassDesc p : paramTypes) {
        int k = i;
        ClassDesc[] ps =
            IntStream.range(0, paramTypes.length + 1)
      ...
      testMethodTypeDesc(
          newDesc,
          mt.insertParameterTypes(
              i,
              Arrays.stream(addition)
                  .map(
                      d -> {
                        try {
  ...
  private void badInsertParametersTypes(ClassDesc returnType, String... paramDescTypes) {
    ClassDesc[] paramTypes =
        IntStream.rangeClosed(0, paramDescTypes.length - 1)
            .mapToObj(i -> ClassDesc.ofDescriptor(paramDescTypes[i]))
            .toArray(ClassDesc[]::new);
    MethodTypeDesc mtDesc = MethodTypeDesc.of(returnType, paramTypes);
    expectThrows(
    ...
    for (String r : returnDescs) {
      ...
      for (String p1 : paramDescs) {
        ...
        for (String p2 : paramDescs) {
          assertMethodType(
              ClassDesc.ofDescriptor(r), ClassDesc.ofDescriptor(p1), ClassDesc.ofDescriptor(p2));
        }
      }
    }
  }
  ...
  public void testBadMethodTypeRefs() {
    ...
    assertThrows(
        NullPointerException.class, () -> MethodTypeDesc.of(CD_int, new ClassDesc[] {null}));
    assertThrows(IllegalArgumentException.class, () -> MethodTypeDesc.of(CD_int, CD_void));

    // of(ClassDesc, List<ClassDesc>)
    assertThrows(
        NullPointerException.class, () -> MethodTypeDesc.of(CD_int, (List<ClassDesc>) null));
    assertThrows(
        NullPointerException.class,
        () -> MethodTypeDesc.of(CD_int, Collections.singletonList(null)));
  ...
  public void testOfListImmutability() {
    ...
    assertThrows(UnsupportedOperationException.class, () -> mtd.parameterList().set(1, CD_void));
    assertEquals(mtd, MethodTypeDesc.of(CD_void, CD_Object, CD_int));
  }
}
