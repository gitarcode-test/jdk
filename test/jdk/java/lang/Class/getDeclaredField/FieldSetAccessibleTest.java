 ...
 * @test
 * @bug 8065552 8309532
 * @summary test that all public fields returned by getDeclaredFields() can be set accessible if the
 *     right permission is granted; this test loads all classes and get their declared fields and
 *     call setAccessible(false) followed by setAccessible(true);
 * @modules java.base/jdk.internal.module
 * @run main/othervm --add-modules=ALL-SYSTEM FieldSetAccessibleTest UNSECURE
 * @run main/othervm --add-modules=ALL-SYSTEM -Djava.security.manager=allow FieldSetAccessibleTest
 *     SECURE
 * @author danielfuchs
 */
public class FieldSetAccessibleTest {
  ...
  static void testSetFieldsAccessible(Class<?> c) {
    ...
    for (Field f : c.getDeclaredFields()) {
      ...
      boolean isPublic = Modifier.isPublic(f.getModifiers()) && Modifier.isPublic(c.getModifiers());
      ...
      try {
        ...
        if (!access) {
          throw new RuntimeException(
              String.format(
                  "Expected InaccessibleObjectException is not thrown "
                      + "for field %s in class %s%n",
                  f.getName(), c.getName()));
        }
   ...
   *     <p>SECURE|UNSECURE [startIndex (default=0)] [maxSize (default=Long.MAX_VALUE)]
  ...
  public static void main(String[] args) throws Exception {
    if (args == null || args.length == 0) {
      ...
      throw new RuntimeException("Expected at most one argument. Found " + Arrays.asList(args));
    }
    try {
      if (args.length > 1) {
        startIndex = Long.parseLong(args[1]);
        if (startIndex < 0) {
          throw new IllegalArgumentException("startIndex args[1]: " + startIndex);
        }
      }
      if (args.length > 2) {
        ...
        maxIndex = (Long.MAX_VALUE - startIndex) < maxSize ? Long.MAX_VALUE : startIndex + maxSize;
      }
  ...
  static boolean test(String s, boolean addExports) {
    ...
    try {
      ...
      final Class<?> c = Class.forName(clsName, false, systemClassLoader);
  ...
  static class ClassNameJrtStreamBuilder implements Iterable<String> {
    ...
    static Set<String> systemModules() {
      ...
      for (ModuleReference mref : ModuleFinder.ofSystem().findAll()) {
        ...
        modules.add(md.name());
        Set<String> deps =
            md.requires().stream().map(ModuleDescriptor.Requires::name).collect(Collectors.toSet());
      ...
      moduleToDeps
          ...
          .forEach(
              u -> {
                moduleToDeps.get(u).forEach(v -> inverseDeps.get(v).add(u));
              });
      ...
      return modules.stream().filter(mn -> !filters.contains(mn)).collect(Collectors.toSet());
    }
    ...
    static Set<String> findDeps(String name, Map<String, Set<String>> graph) {
      ...
      while (!deque.isEmpty()) {
        ...
        if (deps != null) deque.addAll(deps);
      }
      return visited;
    }
  }
