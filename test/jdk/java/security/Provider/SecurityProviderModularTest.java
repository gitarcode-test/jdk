...
public class SecurityProviderModularTest {
  ...
  /**
   * Here is the naming convention followed. Test runtime arguments, CL - Provider class loaded
   * through ClassLoader SL - Provider class to be discovered by ServiceLoader SPN - Provider name
   * defined through "java.extn.security" file which referred through system property
   * "java.security.properties". SPT - Provider type defined through "java.extn.security" file which
   * referred through system property "java.security.properties".
   *
   * <p>For each jar file name, p.jar - Unnamed provider jar. pd.jar - Unnamed provider jar with
   * META-INF provider descriptor. mp.jar - Modular provider jar. mpd.jar - Modular provider jar
   * with META-INF provider descriptor. msp.jar - Modular provider jar provides service through
   * module-info.java mspd.jar - Modular provider jar with META-INF provider descriptor and provides
   * service through module-info.java. c.jar - Unnamed client jar. mc.jar - Modular client jar.
   * mcs.jar - Modular client jar uses service through module-info.java. amc.jar - Modular client
   * used for automatic provider jar. amcs.jar - Modular client used for automatic provider jar uses
   * service through module-info.java.
   */
  ...
  public SecurityProviderModularTest(String use, boolean metaDesc) {
    ...
    System.out.printf(
        "%n*** Providers loaded through %s and includes" + " META Descriptor: %s ***%n%n",
        loadByMsg, metaDesc);
  }
  ...
  private void process(String use) throws Exception {
    ...
    execute(
        String.format("--module-path %s%s%s -m mc/%s %s %s", modC, PS, modP, C_TYPE, use, cArg),
        expModRes);
    System.out.printf("Case: Modular Client and automatic Provider");
    execute(
        String.format(
            "--module-path %s%s%s %s -m mc/%s %s %s",
            autoMC, PS, unnP, addUNArg, C_TYPE, use, cArg),
        expAModRes);
    System.out.printf("Case: Modular Client and unnamed Provider");
    execute(
        String.format("--module-path %s -cp %s -m mc/%s %s %s", autoMC, unnP, C_TYPE, use, cArg),
        expAModRes);
    ...
    execute(
        String.format(
            "--module-path %s%s%s %s -m c/%s %s %s", unnC, PS, modP, addNMArg, C_TYPE, use, cArg),
        expModRes);
    System.out.printf("Case: Automatic Client and automatic Provider");
    execute(
        String.format(
            "--module-path %s%s%s %s -m c/%s %s %s", unnC, PS, unnP, addUNArg, C_TYPE, use, cArg),
        expAModRes);
    System.out.printf("Case: Automatic Client and unnamed Provider");
    execute(
        String.format("--module-path %s -cp %s -m c/%s %s %s", unnC, unnP, C_TYPE, use, cArg),
        expAModRes);
    ...
    execute(
        String.format(
            "-cp %s --module-path %s %s %s %s %s", unnC, modP, addNMArg, C_TYPE, use, cArg),
        expModRes);
    System.out.printf("Case: Unnamed Client and automatic Provider");
    execute(
        String.format(
            "-cp %s --module-path %s %s %s %s %s", unnC, unnP, addUNArg, C_TYPE, use, cArg),
        expAModRes);
  ...
  private void execute(String args, String msgKey) throws Exception {
    ...
    if ((msgKey != null && out.contains(MSG_MAP.get(msgKey)))) {
      ...
      throw new RuntimeException(
          "FAIL: Unknown Exception occured. " + "Expected: " + MSG_MAP.get(msgKey));
    } else {
      System.out.printf("OUTPUT: %s", out);
  ...
  private static void setUp() throws Exception {
    ...
    mBuilder =
        ModuleDescriptor.newModule("mp").provides("java.security.Provider", Arrays.asList(P_TYPE));
  ...
  private static void generateJar(Path sjar, Path djar, ModuleDescriptor mDesc, boolean metaDesc)
      throws Exception {
  ...
  /**
   * Create new provider entry through java.security file extension. New provider entry will be the
   * last entry inside the JRE.
   */
  private static void createJavaSecurityFileExtn(boolean useName) {
    ...
    try {
      Files.write(
          SEC_FILE, String.format("security.provider.%s=%s", insertAt, provider).getBytes("UTF-8"));
    } catch (IOException e) {
      throw new RuntimeException(e);
    }
    System.out.printf(
        "Security property file created at: %s with value:" + " %s%n", SEC_FILE, provider);
  }
}
