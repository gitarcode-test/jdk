...
/**
 * Basic test for ServiceLoader. The test make use of two service providers: 1. BananaScriptEngine -
 * a ScriptEngineFactory deployed as a module on the module path. It implementations a singleton via
 * the public static provider method. 2. PearScriptEngine - a ScriptEngineFactory deployed on the
 * class path with a service configuration file.
 */
public class ModulesTest {
  ...
  /**
   * Basic test of iterator() to ensure that providers located as modules and on the class path are
   * found.
   */
  @Test
  ...
  /**
   * Basic test of iterator() to test iteration order. Providers deployed as named modules should be
   * found before providers deployed on the class path.
   */
  @Test
  ...
  public void testProviderType() {
    Set<String> types =
        ServiceLoader.load(ScriptEngineFactory.class).stream()
            .map(Provider::type)
            .map(Class::getName)
            .collect(Collectors.toSet());
  ...
  /**
   * Basic test of the public static provider method. BananaScriptEngine defines a provider method
  ...
  /**
   * Basic test of stream() to ensure that elements for providers in named modules come before
   * elements for providers in unnamed modules.
   */
  @Test
  ...
  /**
   * Basic test ServiceLoader.load specifying the platform class loader. The providers on the module
   * path and class path should not be located.
   */
  @Test
  public void testWithPlatformClassLoader() {
    ...
    // iterator
    ServiceLoader<ScriptEngineFactory> loader = ServiceLoader.load(ScriptEngineFactory.class, pcl);
    Set<String> names =
        collectAll(loader).stream()
            .map(ScriptEngineFactory::getEngineName)
            .collect(Collectors.toSet());
    ...
    // stream
    names =
        ServiceLoader.load(ScriptEngineFactory.class, pcl).stream()
            .map(Provider::get)
            .map(ScriptEngineFactory::getEngineName)
            .collect(Collectors.toSet());
    assertFalse(names.contains("BananaScriptEngine"));
  ...
  public void testWithAutomaticModule() throws Exception {
    ...
    factory = ServiceLoader.load(ScriptEngineFactory.class, loader).findFirst().orElse(null);
    assertNotNull(factory);
    assertTrue(factory.getClass().getClassLoader() == loader);

    // load using the layer as context
    factory = ServiceLoader.load(layer, ScriptEngineFactory.class).findFirst().orElse(null);
    assertNotNull(factory);
    assertTrue(factory.getClass().getClassLoader() == loader);
  }

  /**
   * Basic test of ServiceLoader.load, using the class loader for a module in a custom layer as the
   * context.
   */
  @Test
  public void testWithCustomLayer1() {
    ...
    providers.remove(0);
    Set<String> names =
        providers.stream().map(ScriptEngineFactory::getEngineName).collect(Collectors.toSet());
    assertTrue(names.contains("BananaScriptEngine"));
    assertTrue(names.contains("PearScriptEngine"));
  }
  ...
  public void testWithCustomLayer2() {
    ...
    factories.remove(0);
    Set<String> names =
        factories.stream().map(ScriptEngineFactory::getEngineName).collect(Collectors.toSet());
    assertTrue(names.contains("BananaScriptEngine"));
    assertFalse(names.contains("PearScriptEngine"));
  }
   ...
   *
   * <p>Test scenario: - boot layer contains "bananascript", maybe other script engines - layer1,
   * with boot layer as parent, contains "bananascript" - layer2, with boot layer as parent,
   * contains "bananascript" - layer3, with layer1 ad layer as parents, contains "bananascript"
   *
   * <p>ServiceLoader should locate all 4 script engine factories in DFS order.
   */
  @Test
  public void testWithCustomLayer3() {
    ...
    Configuration cf3 =
        Configuration.resolveAndBind(finder, List.of(cf1, cf2), ModuleFinder.of(), Set.of());
    ModuleLayer layer3 =
  ...
  /**
   * Create a custom layer by resolving the given module names. The modules are located on the test
   * module path ({@code ${test.module.path}}).
   */
  ...
  private Path[] testModulePath() {
    ...
    return Stream.of(mp.split(File.pathSeparator)).map(Paths::get).toArray(Path[]::new);
  }
