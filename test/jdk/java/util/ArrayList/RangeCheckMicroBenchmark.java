...
public class RangeCheckMicroBenchmark {
  ...
  private static void deoptimize(ArrayList<Integer> list) {
    for (Integer x : list) if (x == null) throw new Error();
  }
    ...
    final Job[] jobs = {
      new Job("get") {
        void work() {
          for (int i = 0; i < iterations; i++) {
            for (int k = 0; k < size; k++) if (list.get(k) == 42) throw new Error();
          }
          deoptimize(list);
        }
      },
      ...
      new Job("subList get") {
        void work() {
          ...
          for (int i = 0; i < iterations; i++) {
            for (int k = 0; k < size; k++) if (sublist.get(k) == 42) throw new Error();
          }
          deoptimize(list);
        }
      },
