...
public class PropertiesTest {
  ...
  private static void compare(String beforeFile, String afterFile, String propsFile)
    ...
    for (String key : keys) {
      ...
      try {
        if (val.chars().map(c -> c == ',' ? 1 : 0).sum() >= 3 && !isPastCutoverDate(val)) {
      ...
      if (afterVal == null) {
        System.out.println(
            "Testing key "
                ...
                + " dfd for the given currency code: "
                + val);
        continue;
      }
      ...
      if (!m.find()) {
        ...
        if (afterVal != null) {
          throw new RuntimeException(
              "Currency data replacement for "
                  + key
                  + " failed: It was incorrectly altered to "
                  + afterVal);
        }
      ...
      if (code.equals(codeAfter) && (numeric == numericAfter) && (fraction == fractionAfter)) {
        after.remove(key);
      } else {
        throw new RuntimeException(
            "Currency data replacement for "
                + key
                + " failed: actual: (alphacode: "
                + codeAfter
                + ", numcode: "
                + numericAfter
                + ", fraction: "
  ...
  private static void bug7102969() {
    ...
    if (!cur.getCurrencyCode().equals("ABC")) {
      throw new RuntimeException(
          "[Expected: ABC as currency code of JP, found: " + cur.getCurrencyCode() + "]");
    }
  ...
  private static void bug8190904() {
    ...
    try {
      ...
      throw new RuntimeException(
          "[FAILED: Should throw" + " IllegalArgumentException for invalid currency code]");
    } catch (IllegalArgumentException ex) {
  ...
  /**
   * Test the numeric code and fraction of the Currency instance obtained by given currencyCode,
   * with the expected numericCode and fraction
   */
  private static void checkCurrencyInstance(String currencyCode, int numericCode, int fraction) {
    Currency cur = Currency.getInstance(currencyCode);
    if (cur.getNumericCode() != numericCode || cur.getDefaultFractionDigits() != fraction) {
      throw new RuntimeException(
          "[FAILED: Incorrect numeric code or" + " dfd for currency code: " + currencyCode + "]");
    }
  }
