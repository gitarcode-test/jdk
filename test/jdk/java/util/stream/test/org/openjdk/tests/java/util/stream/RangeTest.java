...
public class RangeTest extends OpTestCase {
  ...
  public void testIntRange() {
    ...
    for (int start : Arrays.asList(1, 10, -1, -10)) {
      ...
      for (int end : Arrays.asList(1, 10, -1, -10)) {
        ...
        withData(intRangeData(start, end)).stream(s -> s).expectedResult(exp).exercise();
      }
    }
    ...
    for (int start : Arrays.asList(1, 10, -1, -10)) {
      ...
      for (int end : Arrays.asList(1, 10, -1, -10)) {
        ...
        withData(intRangeClosedData(start, end)).stream(s -> s).expectedResult(exp).exercise();
      }
    }
    ...
    {
      Spliterator.OfInt s =
          IntStream.rangeClosed(Integer.MIN_VALUE, Integer.MAX_VALUE).spliterator();
  ...
  TestData.OfInt intRangeClosedData(int start, int end) {
    return TestData.Factory.ofIntSupplier(
        "int rangeClosed", () -> IntStream.rangeClosed(start, end));
  }

  public void tesIntRangeReduce() {
    withData(intRangeData(0, 10000)).terminal(s -> s.reduce(0, Integer::sum)).exercise();
  }
  ...
  public void testIntInfiniteRangeLimit() {
    withData(
        ...
        .terminal(s -> s.reduce(0, Integer::sum))
        .exercise();
  }
  ...
  public void testIntInfiniteRangeFindFirst() {
    ...
    assertEquals(
        first,
        IntStream.iterate(0, i -> i + 1).parallel().filter(i -> i > 10000).findFirst().getAsInt());
  }
  ...
  public void testLongRange() {
    ...
    for (long start : Arrays.asList(1, 1000, -1, -1000)) {
      ...
      for (long end : Arrays.asList(1, 1000, -1, -1000)) {
        ...
        withData(longRangeData(start, end)).stream(s -> s).expectedResult(exp).exercise();
      }
    }
    ...
    for (long start : Arrays.asList(1, 1000, -1, -1000)) {
      ...
      for (long end : Arrays.asList(1, 1000, -1, -1000)) {
        ...
        withData(longRangeClosedData(start, end)).stream(s -> s).expectedResult(exp).exercise();
      }
    }
  ...
  TestData.OfLong longRangeClosedData(long start, long end) {
    return TestData.Factory.ofLongSupplier(
        "long rangeClosed", () -> LongStream.rangeClosed(start, end));
  }

  public void testLongRangeReduce() {
    withData(longRangeData(0, 10000)).terminal(s -> s.reduce(0, Long::sum)).exercise();
  }
