...
public class SliceOpTest extends OpTestCase {
  ...
  private int sliceSize(int dataSize, int skip, int limit) {
    ...
    if (limit >= 0) size = Math.min(size, limit);
    return size;
  }
        ...
        TestData.Factory.ofSupplier(
            ...
            () ->
                StreamSupport.stream(
                    new NonSplittingNotSubsizedOrderedSpliterator<>(list.spliterator()), false));
  ...
  private ResultAsserter<Iterable<Integer>> sliceResultAsserter(
      Iterable<Integer> data, int expectedSize) {
  ...
  @SafeVarargs
  private final void testSliceMulti(
      TestData.OfRef<Integer> data,
      int expectedSize,
    ...
    for (int i = 0; i < ms.length; i++) {
      ...
      Collection<Integer> sr =
          withData(data).stream(m)
  ...
  public void testLimitShortCircuit() {
    for (int l : Arrays.asList(0, 10)) {
      ...
      countTo(100).stream().peek(i -> ai.getAndIncrement()).limit(l).toArray();
