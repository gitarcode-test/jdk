...
public class GatherersWindowFixedTest {
  ...
  public void throwsIAEWhenWindowSizeIsSmallerThanOne(int windowSize) {
    assertThrows(IllegalArgumentException.class, () -> Gatherers.windowFixed(windowSize));
  }
  ...
  public void behavesAsExpectedWhenWindowSizeIsSizeOfStream(Config config) {
    ...
    final var result = config.stream().gather(Gatherers.windowFixed(streamSize)).toList();
  ...
  public void toleratesNullElements() {
    assertEquals(
        ...
        Stream.of(null, null).gather(Gatherers.windowFixed(2)).toList());
  }

  @Test
  public void throwsUOEWhenWindowsAreAttemptedToBeModified() {
    var window = Stream.of(1).gather(Gatherers.windowFixed(1)).findFirst().get();
  ...
  public void behavesAsExpected(Config config) {
    ...
    for (var windowSize : List.of(1, 2, 3, 10)) {
      final var expectLastWindowSize =
          streamSize % windowSize == 0 ? windowSize : streamSize % windowSize;
      final var expectedSize = (streamSize / windowSize) + ((streamSize % windowSize == 0) ? 0 : 1);

      final var expected = config.stream().toList().iterator();

      final var result = config.stream().gather(Gatherers.windowFixed(windowSize)).toList();

      int currentWindow = 0;
      for (var window : result) {
        ++currentWindow;
        assertEquals(
            currentWindow < expectedSize ? windowSize : expectLastWindowSize, window.size());
        for (var element : window) assertEquals(expected.next(), element);
      }
