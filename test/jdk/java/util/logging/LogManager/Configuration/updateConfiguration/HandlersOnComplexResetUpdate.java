 ...
 * @test
 * @bug 8033661
 * @summary tests that FileHandlers configured on abstract nodes in logging.properties will be
 *     closed on reset and reopened on updateConfiguration(). Test a complex reconfiguration where a
 *     logger with handlers suddenly appears in the hierarchy between a child logger and the root
 *     logger.
...
public class HandlersOnComplexResetUpdate {
  ...
  /**
   * We will test the handling of abstract logger nodes with file handlers in two configurations:
   * UNSECURE: No security manager. SECURE: With the security manager present - and the required
   * permissions granted.
   */
  ...
  static {
    ...
    props1.setProperty(
        FileHandler.class.getName() + ".formatter", "java.util.logging.SimpleFormatter");
    ...
    properties =
        Collections.unmodifiableList(Arrays.asList(props1, props2, props3, props4, props5));
  }

  /**
   * This is the main test method. The rest is infrastructure. Creates a child of the 'com.foo'
   * logger (com.foo.child) and holds on to it.
   *
   * <p>Then applies all given configurations in sequence and verifies assumptions about the
   * handlers that com.foo should have, or not have. In the last configuration (props5) it also
   * verifies that the logger 'com' has been created and has now the expected handler.
   *
   * <p>Finally releases the child logger after all configurations have been applied.
   *
   * @param name
   * @param properties
   * @throws Exception
   */
  static void test(String name, List<Properties> properties) throws Exception {
    ...
    try {
      ...
      boolean checkHandlersOnParent =
          Boolean.parseBoolean(previousProps.getProperty("test.checkHandlersOnParent", "true"));
      ...
      for (int i = 1; i < properties.size(); i++) {
        ...
        checkHandlersOnParent =
            Boolean.parseBoolean(nextProps.getProperty("test.checkHandlersOnParent", "true"));
  ...
  public static void main(String... args) throws Exception {
    ...
    if (args == null || args.length == 0) {
      args =
          new String[] {
            TestCase.UNSECURE.name(), TestCase.SECURE.name(),
          };
    }
  ...
  static class Configure {
    ...
    static final ThreadLocal<AtomicBoolean> allowAll =
    ...
    static void updateConfigurationWith(Properties propertyFile, boolean append) {
      try {
        ...
        Function<String, BiFunction<String, String, String>> remapper =
            append ? (x) -> ((o, n) -> n == null ? o : n) : (x) -> ((o, n) -> n);
  ...
  public static class SimplePolicy extends Policy {
    ...
    public PermissionCollection getPermissions(CodeSource codesource) {
      return new PermissionsBuilder()
          .addAll(allowAll.get().get() ? allPermissions : permissions)
          .toPermissions();
    }
