 ...
 * @test
 * @bug 8245302
 * @summary tests the deprecation of threadID and a new field longThreadID, test should be run on
 *     jdk16 and subsequent versions
...
public class SerializeLogRecordTest {
   ...
   * can be deserialized in the current java version. (see Jdk11SerializedLog and Jdk16SerializedLog
   * below)
   */
  public abstract static class SerializedLog {
    ...
    /**
     * Deserializes the Base64 encoded string returned by {@link #getBase64()}, and checks that the
     * string representation obtained matches the original string representation returned by
     */
    ...
    /**
     * Check that the values of threadID and longThreadID are correct and consistent when
     * de-serialized from earlier jdk versions(jdk11)
     */
    protected void check(LogRecord r1) {
      ...
      if (loggerName.equals("test1") || loggerName.equals("test")) {
        ...
        if (tid != check
            || longThreadID != check
            || !str.contains("<thread>" + String.valueOf(longThreadID))) {
          throw new RuntimeException(this.getClass().getName());
        } else {
          System.out.println("Test Integer Check Passed");
        }
  ...
  public static class Jdk11SerializedLog extends SerializedLog {
    ...
    /**
     * Base64 encoded string for LogRecord object. It is generated using generate method and can be
     * copy pasted as base64 value, for generating this value, execute this class with an argument
     * value generate and the test will print out the base64 value that can be copy pasted below
     * Java version: 11.0.6
     */
    final String base64 =
            ...
            (x) -> {
              switch (x) {
                case GENERATE:
                  ...
                  break;
                case TESTJDK11:
                  Jdk11SerializedLog.test();
                  break;
                case TESTJDK16:
                  Jdk16SerializedLog.test();
                  break;
              }
            });
  }
