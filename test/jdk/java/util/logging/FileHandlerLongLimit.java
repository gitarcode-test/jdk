...
public class FileHandlerLongLimit {
  ...
  /**
   * We will test handling of limit and overflow of MeteredStream.written in two configurations.
  ...
  public static enum TestCase {
    ...
    public void run(Properties propertyFile) throws Exception {
      ...
      test(
          this.name() + " " + propertyFile.getProperty("test.name"),
          propertyFile,
  ...
  static {
    ...
    props2.setProperty(
        FileHandler.class.getName() + ".limit", String.valueOf(((long) Integer.MAX_VALUE) * 4));
    props3.setProperty("test.name", "with limit=Long.MAX_VALUE - 1024");
    props3.setProperty(FileHandler.class.getName() + ".pattern", PREFIX);
    props3.setProperty(
        FileHandler.class.getName() + ".limit", String.valueOf(Long.MAX_VALUE - 1024));
  ...
  private static TestOutputStream setTestOutputStream(OutputStream metered) throws Exception {
    return Configure.callPrivileged(
        (Callable<TestOutputStream>)
            () -> {
              outField.set(
                  metered, new TestOutputStream(OutputStream.class.cast(outField.get(metered))));
  ...
  public static FileHandler testFileHandlerLimit(FileHandlerSupplier supplier, long limit)
      throws Exception {
    Configure.doPrivileged(
        () -> {
          try {
    ...
    try {
      ...
      assertEquals(
          limit - 2 * offset + 10, setWritten(metered, limit - 2 * offset + 10), "written");
      w = getWritten(metered);
  ...
  public static class SimplePolicy extends Policy {
    ...
    public PermissionCollection getPermissions(CodeSource codesource) {
      return new PermissionsBuilder()
          .addAll(allowAll.get() ? allPermissions : permissions)
          .toPermissions();
    }
