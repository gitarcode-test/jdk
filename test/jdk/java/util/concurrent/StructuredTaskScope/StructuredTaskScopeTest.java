...
class StructuredTaskScopeTest {
  ...
  static void setup() throws Exception {
    ...
    if (value == null || value.equals("platform")) list.add(Thread.ofPlatform().factory());
    if (value == null || value.equals("virtual")) list.add(Thread.ofVirtual().factory());
  ...
  /**
   * Test that handleComplete method is not invoked for tasks that finish after shutdown or are
  ...
  /**
   * Test that the default handleComplete throws IllegalArgumentException if called with a running
   * task.
   */
  @Test
  ...
  void testShutdownOnFailure3(ThreadFactory factory) throws Throwable {
    try (var scope = new ShutdownOnFailure(null, factory)) {
      ...
      ex = assertThrows(FooException.class, () -> scope.throwIfFailed(e -> new FooException(e)));
      assertTrue(ex.getCause() instanceof ArithmeticException);
    }
  }
  ...
  /**
   * Interrupts a thread when it waits (timed or untimed) at location "{@code c.m}". {@code c} is
   * the fully qualified class name and {@code m} is the method name.
   */
  ...
  /**
   * Schedules the current thread to be interrupted when it waits (timed or untimed) at the given
   * location.
   */
  ...
  /**
   * Returns true if the given stack trace contains an element for the given class and method name.
   */
  private boolean contains(StackTraceElement[] stack, String className, String methodName) {
    return Arrays.stream(stack)
        .anyMatch(e -> className.equals(e.getClassName()) && methodName.equals(e.getMethodName()));
  }
}
