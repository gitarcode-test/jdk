...
public class LSRDataTest {
  ...
  private static final String LSR_FILE_PATH =
  ...
  public static void main(String[] args) throws IOException {
    ...
    String ranges =
        "Accept-Language: aam, adp, aue, bcg, cqu, ema,"
  ...
  private static String generateLangRanges() {
    return Stream.concat(singleLangEquivMap.keySet().stream(), multiLangEquivsMap.keySet().stream())
  ...
  private static void processDataAndGenerateMaps(
  ...
  /**
   * A faster alternative approach to String.replaceFirst(), if the given string is a literal
   * String, not a regex.
   */
  private static String replaceFirstSubStringMatch(
      String range, String substr, String replacement) {
    ...
    while (r.length() > 0) {
      if (singleLangEquivMap.containsKey(r)) {
        ...
        for (int i = 0; i < equivs.size(); i++) {
          result.add(i, replaceFirstSubStringMatch(range, r, equivs.get(i)));
        }
        return result;
      }
    ...
    for (String subtag : regionVariantEquivMap.keySet()) {
      ...
      if ((index = range.indexOf(subtag)) != -1) {
        ...
        if (range.length() == len || range.charAt(len) == HYPHEN) {
          return replaceFirstSubStringMatch(range, subtag, regionVariantEquivMap.get(subtag));
        }
      }
    }
  ...
  private static void checkEquality(List<LanguageRange> expected, List<LanguageRange> actual) {
    ...
    if (expectedSize != actualSize) {
      throw new RuntimeException(
          "[FAILED: Size of the priority list"
      ...
      for (int i = 0; i < expectedSize; i++) {
        ...
        if (!lr1.getRange().equals(lr2.getRange()) || lr1.getWeight() != lr2.getWeight()) {
          throw new RuntimeException(
              "[FAILED: Ranges at index "
                  + i
                  + " do not match Expected: range="
                  + lr1.getRange()
                  + ", weight="
                  + lr1.getWeight()
                  + ", Actual: range="
