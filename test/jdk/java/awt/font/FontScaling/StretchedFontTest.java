...
public final class StretchedFontTest {
  ...
  private static final Color[] FOREGROUNDS = {
    new Color(0xFF000000, true), new Color(0x7F000000, true)
  };
  ...
  public static void main(String[] args) {
    ...
    if (!errors.isEmpty()) {
      ...
      throw new Error(errors.size() + " failure(s) found;" + " the first one: " + errors.get(0));
    }
  }
  ...
  private static Stream<String> testFont(final Font font) {
    return Stream.of(
            VALUE_TEXT_ANTIALIAS_OFF, VALUE_TEXT_ANTIALIAS_ON, VALUE_TEXT_ANTIALIAS_LCD_HRGB)
        .flatMap(hint -> testFont(font, hint));
  }

  /**
   * Tests the font with the specified text antialiasing hint and a set of foreground colors.
   *
   * @param font the font to test
  ...
  private static Stream<String> testFont(final Font font, final Object hint) {
    return Stream.of(FOREGROUNDS).map(foreground -> testFont(font, hint, foreground));
  }

  /**
   * Tests the font with the specified text antialiasing hint and foreground color. In case of
   * failure, it saves the rendered image to a file.
   *
   * @param font the font to test
   * @param hint the text antialiasing hint to test
   * @param foreground the foreground color to use
   * @return {@code null} if the text rendered correctly; otherwise, a {@code String} with the font
   *     family name, the value of the rendering hint and the color in hex
   */
  private static String testFont(final Font font, final Object hint, final Color foreground) {
    final Dimension size = getTextSize(font);
    final BufferedImage image = new BufferedImage(size.width, size.height, TYPE_3BYTE_BGR);
  ...
  /**
   * Verifies the rendered image of the hieroglyph. The hieroglyph should be stretched across the
   * entire width of the image. If the right half of the image contains only pixels of the
   * background color, the hieroglyph isn't stretched correctly &mdash; it's a failure.
   *
   * @param image the image to verify
   * @return {@code true} if the hieroglyph is stretched correctly; or {@code false} if right half
   *     of the image contains only background-colored pixels, which means the hieroglyph isn't
   *     stretched.
   */
  ...
  private static final BufferedImage dummyImage = new BufferedImage(5, 5, TYPE_3BYTE_BGR);

  private static Dimension getTextSize(final Font font) {
    final Graphics g = dummyImage.getGraphics();
    try {
      return g.getFontMetrics(font).getStringBounds(TEXT, g).getBounds().getSize();
    } finally {
      g.dispose();
    }
  }

  private static void saveImage(final BufferedImage image, final String fileName) {
    try {
