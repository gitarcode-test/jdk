...
public class HttpProxy {
  ...
  void test() throws Exception {
    ...
    try (ServerSocket ss = new ServerSocket(0)) {
      ...
      externalAddresses.add(new InetSocketAddress(InetAddress.getLocalHost(), ss.getLocalPort()));
  ...
  void simpleRead(InputStream is, int start) throws Exception {
    ...
    for (int i = start; i < start + 100; i++) {
      int x = is.read(b);
      if (x == 1) x += is.read(b, 1, 1);
      if (x != 2) throw new Exception("read error");
      int r = bytes(b[0], b[1]);
      if (r != i) throw new Exception("read " + r + " expected " + i);
    }
  ...
  static class ConnectProxyTunnelServer extends Thread implements AutoCloseable {
    ...
    private void processRequest(Socket clientSocket) throws Exception {
      ...
      if (!statusLine.startsWith("CONNECT")) {
        out.println(
            "proxy server: processes only " + "CONNECT method requests, received: " + statusLine);
        return;
      }
