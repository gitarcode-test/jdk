...
public class UnreferencedSockets {
  ...
  static final ArrayDeque<NamedWeak> pendingSockets = new ArrayDeque<>(100);
  ...
  private static void extractRefs(FileDescriptor fileDescriptor, String name) {
    ...
    try {
      ...
      System.out.print(
          String.format("  fd: %s, fd: %d, cleanup: %s%n", fileDescriptor, rawfd, cleanup));
    }
  }
  ...
  private static void extractRefs(ServerSocket s, String name) {
    try {
      ...
      System.out.print(
          String.format(
              "  fd: %s, fd: %d, cleanup: %s, socket: %s%n", fileDescriptor, rawfd, cleanup, s));
