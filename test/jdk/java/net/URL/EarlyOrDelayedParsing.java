...
public class EarlyOrDelayedParsing {
  ...
  sealed interface URLArgTest permits OneArgTest, TwoArgsTest, ThreeArgsTest, FourArgsTest {
  ...
  sealed interface OneArgTest extends URLArgTest {
    ...
    default boolean early(int c) {
      return this instanceof OfHost && (c < 31 || c == 127);
    }
    ...
    static OneArgTest ofUserInfo(int c) {
      return new OfUserInfo(
          c, "http://user%sinfo@localhost:9999/".formatted(Character.toString(c)));
    }
  }
  ...
  sealed interface TwoArgsTest extends URLArgTest {
    ...
    @Override
    default boolean early(int c) {
      return this instanceof OfTwoArgsHost && (c < 31 || c == 127);
    }
    ...
    static TwoArgsTest ofOneArgTest(OneArgTest test) {
      ...
      throw new AssertionError("can't convert to TwoArgsTest: " + test.getClass());
    }
  }
  ...
  sealed interface ThreeArgsTest extends URLArgTest {
    ...
    default boolean acceptDelayedException(Throwable exception) {
      return URLArgTest.super.acceptDelayedException(exception)
          || "file".equalsIgnoreCase(scheme()) && exception instanceof IOException;
    }
  ...
  static Stream<OneArgTest> oneArgHostTests() {
    ...
    urls.addAll((UNWISE + EXCLUDED_DELIMS).chars().mapToObj(OneArgTest::ofHost).toList());
    urls.addAll(
        IntStream.concat(IntStream.range(0, 31), IntStream.of(127))
            .mapToObj(OneArgTest::ofHost)
            .toList());
  ...
  static Stream<OneArgTest> oneArgUserInfoTests() {
    ...
    urls.addAll(
        IntStream.concat(IntStream.range(0, 31), IntStream.of(127))
            .mapToObj(OneArgTest::ofUserInfo)
            .toList());
  ...
  static Stream<ThreeArgsTest> threeArgsTests() {
    ...
    urls.addAll(
        (UNWISE + EXCLUDED_DELIMS + DELIMS).chars().mapToObj(ThreeArgsTest::ofHostFile).toList());
    urls.addAll(
        IntStream.concat(IntStream.range(0, 31), IntStream.of(127))
            .mapToObj(ThreeArgsTest::ofHostFile)
            .toList());
  ...
  static Stream<FourArgsTest> fourArgsTests() {
    ...
    urls.addAll(
        (UNWISE + EXCLUDED_DELIMS + DELIMS)
            .chars()
            .mapToObj(FourArgsTest::ofHostPortFile)
            .toList());
    urls.addAll(
        IntStream.concat(IntStream.range(0, 31), IntStream.of(127))
            .mapToObj(FourArgsTest::ofHostPortFile)
