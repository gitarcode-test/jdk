...
public class Scoping {
  ...
  static List<ThrowingConsumer<InetSocketAddress>> targets =
      List.of(
          /* 0 */ (a) -> {
            Socket s = new Socket();
            s.bind(a);
          ...
          /* 11 */ (a) -> {
            dgSocketTest(a, false);
          },
          /* 12 */ (a) -> {
            dgSocketTest(a, true);
          },
          /* 13 */ (a) -> {
            dgChannelTest(a, false);
          },
          /* 14 */ (a) -> {
            dgChannelTest(a, true);
          });
  ...
  static void compare(InetAddress a, InetAddress b) {
    ...
    if (!a1.getHostAddress().equals(b1.getHostAddress())) {
      System.out.printf("%s != %s\n", a1, b1);
      if (a1.getScopeId() != b1.getScopeId()) throw new RuntimeException("Scope ids not the same");
    }
  }
  ...
  static void dgSocketTest(InetSocketAddress a, boolean connect) throws Exception {
    try (DatagramSocket s = new DatagramSocket(null)) {
      ...
      if (!t1.equals(t)) throw new RuntimeException("Packets not equal");
    }
  }
