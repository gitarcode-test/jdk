...
public class SetLoopbackMode {
  ...
  static boolean test(MulticastSocket mc, InetAddress grp) throws IOException {
    ...
    DatagramPacket p = new DatagramPacket(b, b.length, grp, mc.getLocalPort());
    mc.send(p);
    ...
    if (!gotPacket && !disabled) {
      System.out.println("Packet not received (packet excepted as loopback is enabled)");
      return FAILED;
    }
  ...
  public static void main(String args[]) throws Exception {
    ...
    try (MulticastSocket mc = new MulticastSocket()) {
      ...
      try {
        System.out.println(
            "NetworkInterface.getByInetAddress(grp): "
  ...
  static class DefaultInterface {
    ...
    static {
      try {
        GET_DEFAULT = Class.forName("java.net.DefaultInterface").getDeclaredMethod("getDefault");
    ...
    static NetworkInterface getDefault() {
      try {
        return (NetworkInterface) GET_DEFAULT.invoke(null);
      } catch (Exception x) {
