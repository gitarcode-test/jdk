 ...
 * @test
 * @library /test/lib
 * @modules java.management java.base/java.io:+open java.base/java.net:+open java.base/sun.net
...
public class UnreferencedMulticastSockets {
  ...
  static final ArrayDeque<NamedWeak> pendingSockets = new ArrayDeque<>(5);

  /** Queued objects when they are unreferenced. */
  static final ReferenceQueue<Object> pendingQueue = new ReferenceQueue<>();
  ...
  static class Server implements Runnable {
    ...
    Server() throws IOException {
      ...
      InetSocketAddress serverAddress = new InetSocketAddress(loopback, 0);
      ss = new MulticastSocket(serverAddress);
      port = ss.getLocalPort();
      System.out.printf("  DatagramServer addr: %s: %d%n", this.getHost(), this.getPort());
  ...
  public static void main(String args[]) throws Exception {
    ...
    System.out.printf(
        "  client bound port: %s:%d%n", client.getLocalAddress(), client.getLocalPort());
    ...
    try {
      ...
      if (!usePlainDatagramSocketImpl()) {
        ...
        FileDescriptor fileDescriptor =
            (FileDescriptor) fileDescriptorField.get(datagramSocketImpl);
        extractRefs(fileDescriptor, name);
      }
    } catch (Exception ex) {
      ex.printStackTrace();
  ...
  private static void extractRefs(FileDescriptor fileDescriptor, String name) {
    ...
    try {
      if (fileDescriptor != null) {
        ...
        pendingSockets.add(
            new NamedWeak(fileDescriptor, pendingQueue, name + "::fileDescriptor: " + rawfd));
      ...
      System.out.print(
          String.format(
              "  %s:: fd: %s, fd: %d, cleanup: %s%n", name, fileDescriptor, rawfd, cleanup));
    }
  }
