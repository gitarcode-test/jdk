...
public class HttpClientLocalAddrTest implements HttpServerAdapters {
  ...
  public static void beforeClass() throws Exception {
    ...
    HttpServerAdapters.HttpTestHandler handler =
        (exchange) -> {
  ...
  /**
   * Stops the server and returns (instead of throwing) any exception that might have occurred
   * during stop. If {@code prevException} is not null then any exception during stop of the {@code
   * server} will be added as a suppressed exception to the {@code prevException} and the {@code
   * prevException} will be returned.
   */
  private static Exception safeStop(
      HttpServerAdapters.HttpTestServer server, Exception prevException) {
    if (server == null) {
      return null;
    }
      ...
      ClientCloseable build() {
        ...
        ExecutorService virtualExecutor =
            Executors.newThreadPerTaskExecutor(
                Thread.ofVirtual().name("HttpClient-" + id + "-Worker", 0).factory());
  ...
  /**
   * Sends a GET request using the {@code client} and expects a 200 response. The returned response
   * body is then tested to see if the client address seen by the server side handler is the same
   * one as that is set on the {@code client}
   */
  @Test(dataProvider = "params")
  public void testSend(ClientProvider clientProvider, URI requestURI, InetAddress localAddress)
      throws Exception {
    try (var c = clientProvider.get()) {
      HttpClient client = c.client();
      System.out.println(
          "Testing using a HTTP client "
              + client.version()
              + " with local address "
              + localAddress
              + " against request URI "
  ...
  /**
   * Sends a GET request using the {@code sendAsync} method on the {@code client} and expects a 200
   * response. The returned response body is then tested to see if the client address seen by the
   * server side handler is the same one as that is set on the {@code client}
   */
  @Test(dataProvider = "params")
  public void testSendAsync(ClientProvider clientProvider, URI requestURI, InetAddress localAddress)
      throws Exception {
    try (var c = clientProvider.get()) {
      HttpClient client = c.client();
      System.out.println(
      ...
      // GET request
      var req = HttpRequest.newBuilder(requestURI).build();
      var cf = client.sendAsync(req, HttpResponse.BodyHandlers.ofByteArray());
      var resp = cf.get();
      Assert.assertEquals(resp.statusCode(), 200, "Unexpected status code");
      // verify the address only if a specific one was set on the client
      if (localAddress != null && !localAddress.isAnyLocalAddress()) {
  ...
  /**
   * Invokes the {@link #testSend} and {@link #testSendAsync} tests, concurrently in multiple
   * threads to verify that the correct local address is used when multiple concurrent threads are
   * involved in sending requests from the {@code client}
   */
  @Test(dataProvider = "params")
  public void testMultiSendRequests(
      ClientProvider clientProvider, URI requestURI, InetAddress localAddress) throws Exception {
    int numThreads = 4;
