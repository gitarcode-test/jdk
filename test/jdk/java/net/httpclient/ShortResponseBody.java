...
public abstract class ShortResponseBody {
  ...
  public Object[][] variants(ITestContext context) {
    String[][] cases =
        new String[][] {
          ...
          {
            httpURIVarLen + "?length=106", "chunked transfer encoding"
          }, // EOF during chunk header ( length )
          {
            httpURIVarLen + "?length=110", "chunked transfer encoding"
          }, // EOF during chunk response body data
  ...
  void assertExpectedMessage(HttpRequest request, Throwable t, String expected) {
    if (request.uri().getScheme().equalsIgnoreCase("https")
      ...
      assertTrue(msg.contains(expected), "exception msg:[" + msg + "]");
    }
  }
  ...
  void assertExpectedMessage(HttpRequest request, Throwable t, List<String> expected) {
    if (request.uri().getScheme().equalsIgnoreCase("https")
      ...
      String msg = t.getMessage();
      assertTrue(
          expected.stream().anyMatch(msg::contains),
  ...
  abstract static class Server extends Thread implements AutoCloseable {
  ...
  /**
   * A server that closes the connection immediately, without reading or writing, after completing
   * the SSL handshake.
   */
  ...
  /**
   * A server that replies with headers and a, possibly partial, reply, before closing the
   * connection. The number of bytes of written ( header + body), is controllable through the
   * "length" query string param in the requested URI.
   */
  abstract static class ReplyingServer extends Server {
    ...
    public void run() {
      while (!closed) {
        try (Socket s = ss.accept()) {
          ...
          if (query == null) {
            out.println(
                "Unexpected request without query string received. Got headers: [" + headers + "]");
