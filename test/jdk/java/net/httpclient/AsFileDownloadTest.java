...
public class AsFileDownloadTest {
  ...
  static final String[][] contentDispositionValues =
      new String[][] {
  ...
  void test(String uriString, String contentDispositionValue, String expectedFilename)
    ...
    try {
      ...
      BodyHandler bh =
          ofFileDownload(
              tempDir.resolve(uri.getPath().substring(1)), CREATE, TRUNCATE_EXISTING, WRITE);
      ...
      assertEquals(
          response.headers().firstValue("Content-Disposition").get(), contentDispositionValue);
  ...
  static final String[][] contentDispositionBADValues =
      new String[][] {
  ...
  void negativeTest(String uriString, String contentDispositionValue) throws Exception {
    ...
    try {
      URI uri = URI.create(uriString);
      HttpRequest request =
          HttpRequest.newBuilder(uri).POST(BodyPublishers.ofString("Does not matter")).build();
  ...
  static String contentDispositionValueFromURI(URI uri) {
    ...
    else throw new AssertionError("SERVER: UNEXPECTED query:" + queryIndex);
  ...
  static void caseInsensitivityOfHeaders(HttpHeaders headers) {
    try {
      for (Map.Entry<String, List<String>> entry : headers.map().entrySet()) {
        ...
        for (String name :
            List.of(headerName.toUpperCase(Locale.ROOT), headerName.toLowerCase(Locale.ROOT))) {
