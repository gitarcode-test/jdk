...
public class DatagramSocketMulticasting {
  ...
  static void testJoinGroup2(
      ProtocolFamily family, DatagramSocket s, InetAddress group, NetworkInterface ni)
      throws IOException {
   ...
   * socket option.
   */
  static void testNetworkInterface(DatagramSocket s, NetworkInterface ni) throws IOException {
    // default value
    assertNull(s.getOption(IP_MULTICAST_IF));
    ...
    while (true) {
      try {
        ...
        if (Arrays.equals(p.getData(), p.getOffset(), p.getLength(), message, 0, message.length)) {
          ...
          String messageReceived = new String(p.getData(), 0, p.getLength(), "UTF-8");
          System.out.format(
              "Received unexpected message %s from %s%n", messageReceived, p.getSocketAddress());
        }
