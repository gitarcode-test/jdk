...
public class Tests {
  ...
  /**
   * performs a simple exchange of data between the two sockets and throws an exception if there is
   * any problem.
   */
  ...
  /**
   * Send a packet from s1 to s2 (ia2/s2.localPort) and check it Send a packet from s2 to s1
   * (ia1/s1.localPort) and check it
   */
  public static void simpleDataExchange(
      DatagramSocket s1, InetAddress ia1, DatagramSocket s2, InetAddress ia2) throws Exception {
  ...
  /**
   * Send a packet from s1 to s2 (ia2/s2.localPort) and send same packet back from s2 to sender.
   * Check s1 receives original packet
   */
  public static void datagramEcho(DatagramSocket s1, DatagramSocket s2, InetAddress ia2)
      throws Exception {
  ...
  public static void comparePackets(DatagramPacket p1, DatagramPacket p2) throws Exception {
  ...
  public static void checkTime(long got, long start, long end) {
    ...
    if (got > upper || got < lower) {
      throw new RuntimeException(
          "checkTime failed: got " + got + ", expected between " + start + " and " + end);
    }
  }
