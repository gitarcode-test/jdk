...
public class Basic {
  ...
  static final Consumer<Result> KNOWN =
      r -> {
        if (r.exitValue != 0 || !withoutWarning(r.output).isEmpty())
          throw new RuntimeException("[" + r.output + "]");
      };
  static final Consumer<Result> UNKNOWN =
      r -> {
        if (r.exitValue == 0
  ...
  static final Consumer<Result> SCE =
      r -> {
        if (r.exitValue == 0 || !r.output.contains("java.util.ServiceConfigurationError")) {
          throw new RuntimeException("exitValue: " + r.exitValue + ", output:[" + r.output + "]");
        }
      };
  ...
  static void unknownProtocol(String protocol, Consumer<Result> resultChecker) {
    System.out.println("\nTesting " + protocol);
    Result r = java(Collections.emptyList(), asList(TEST_CLASSES), "Child", protocol);
    resultChecker.accept(r);
  }

  static void viaProvider(String protocol, Consumer<Result> resultChecker, String... sysProps)
      throws Exception {
    viaProviderWithTemplate(
        protocol, resultChecker, TEST_SRC.resolve("provider.template"), sysProps);
  }

  static void viaBadProvider(String protocol, Consumer<Result> resultChecker, String... sysProps)
      throws Exception {
    viaProviderWithTemplate(
        protocol, resultChecker, TEST_SRC.resolve("bad.provider.template"), sysProps);
  }

  static void viaProviderWithTemplate(
      String protocol, Consumer<Result> resultChecker, Path template, String... sysProps)
      throws Exception {
    System.out.println("\nTesting " + protocol);
    Path testRoot = Paths.get("URLStreamHandlerProvider-" + protocol);
    if (Files.exists(testRoot)) FileUtils.deleteFileTreeWithRetry(testRoot);
    ...
    for (String p : sysProps) props.add(p);

    Result r = java(props, asList(testJar, TEST_CLASSES), "Child", protocol);

    resultChecker.accept(r);
  }

  static String platformPath(String p) {
  ...
  static void createServices(Path dst, String protocol) throws IOException {
    Path services = Files.createDirectories(dst.resolve("META-INF").resolve("services"));
  ...
  static Path createProvider(String protocol, Path srcTemplate, Path dst) throws IOException {
  ...
  static void jar(Path jarName, Path jarRoot) {
    ...
    ProcessBuilder p =
        new ProcessBuilder(jar, "cf", jarName.toString(), "-C", jarRoot.toString(), ".");
    quickFail(run(p));
  }
  ...
  static void javac(Path dest, Path... sourceFiles) throws IOException {
    ...
    try (StandardJavaFileManager fileManager = compiler.getStandardFileManager(null, null, null)) {
      ...
      if (!passed) throw new RuntimeException("Error compiling " + files);
    }
  }

  static void quickFail(Result r) {
    if (r.exitValue != 0) throw new RuntimeException(r.output);
  }

  static Result java(
      List<String> sysProps, Collection<Path> classpath, String classname, String arg) {
  ...
  static Result run(ProcessBuilder pb) {
    ...
    try {
      ...
      throw new RuntimeException(format("Couldn't start process '%s'", pb.command()), e);
    }
    ...
    try {
      ...
      throw new RuntimeException(format("Couldn't read process output '%s'", pb.command()), e);
    }

    try {
      p.waitFor();
    } catch (InterruptedException e) {
      throw new RuntimeException(format("Process hasn't finished '%s'", pb.command()), e);
    }

    return new Result(p.exitValue(), output);
  }

  static final String DEFAULT_IMAGE_BIN =
  ...
  static String toString(InputStream... src) throws IOException {
    ...
    Reader concatenated =
        new InputStreamReader(new SequenceInputStream(Collections.enumeration(asList(src))));
  ...
  static void copy(Reader src, Writer dst) throws IOException {
    ...
    try {
      while ((len = src.read(buf)) != -1) dst.write(buf, 0, len);
    } finally {
      try {
        src.close();
