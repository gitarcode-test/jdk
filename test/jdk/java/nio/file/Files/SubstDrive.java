...
public class SubstDrive {
  ...
  /**
   * Setup for the test: + Create a temporary directory where all subsequently created temp
   * directories will be in. This directory and all of its contents will be deleted when the test
   * finishes. + Find a drive that is available for use with subst.
   */
  @BeforeClass
  ...
  /**
   * Each test method maps drive `SUBST_DRIVE` to a temporary directory, unmap the drive after every
   * test so that subsequent ones can reuse the drive.
   */
  @AfterMethod
  ...
  /**
   * Test if the attributes returned by the Files' APIs are consistent when using the actual path
   * and the substituted path.
   */
  @Test
  public void testAttributes() throws IOException {
    ...
    assertEquals(Files.isExecutable(tempDirectory), Files.isExecutable(SUBST_DRIVE));

    assertEquals(Files.isReadable(tempDirectory), Files.isReadable(SUBST_DRIVE));

    assertEquals(Files.isDirectory(tempDirectory), Files.isDirectory(SUBST_DRIVE));

    assertEquals(Files.isHidden(tempDirectory), Files.isHidden(SUBST_DRIVE));

    assertEquals(Files.isRegularFile(tempDirectory), Files.isRegularFile(SUBST_DRIVE));

    assertEquals(Files.isSymbolicLink(tempDirectory), Files.isSymbolicLink(SUBST_DRIVE));

    assertEquals(Files.getOwner(tempDirectory), Files.getOwner(SUBST_DRIVE));

    assertEquals(Files.isWritable(tempDirectory), Files.isWritable(SUBST_DRIVE));
  }

  /**
   * Test if setting attributes for a substituted path works the same way as it would for a real
   * path.
   */
  @Test
  ...
  public void testFileStore() throws IOException {
    ...
    assertEquals(fileStore1.getTotalSpace(), fileStore2.getTotalSpace());

    assertEquals(fileStore1.getBlockSize(), fileStore2.getBlockSize());

    assertEquals(fileStore1.name(), fileStore2.name());

    assertEquals(fileStore1.type(), fileStore2.type());
  ...
  /**
   * Test if Files.copy works correctly on a substituted drive, and that all of the attributes are
   * the same.
   */
  @Test
  public void testMoveAndCopySubstDrive() throws IOException {
    ...
    assertEquals(Files.isExecutable(SUBST_DRIVE), Files.isExecutable(tempDirectoryCopy));

    assertEquals(Files.isReadable(SUBST_DRIVE), Files.isReadable(tempDirectoryCopy));

    assertEquals(Files.isDirectory(SUBST_DRIVE), Files.isDirectory(tempDirectoryCopy));

    assertEquals(Files.isHidden(SUBST_DRIVE), Files.isHidden(tempDirectoryCopy));

    assertEquals(Files.isRegularFile(SUBST_DRIVE), Files.isRegularFile(tempDirectoryCopy));

    assertEquals(Files.isWritable(SUBST_DRIVE), Files.isWritable(tempDirectoryCopy));

    assertEquals(Files.getOwner(SUBST_DRIVE), Files.getOwner(tempDirectoryCopy));
  }

  /**
   * Test if the attributes of a resolved symlink are the same as its target's Note: requires
  ...
  /**
   * Test if files and directories can be created, moved, and cut when the substituted drive is a
   * symlink. Note: requires administrator privileges.
   */
  @Test
  public void testSubstWithSymlinkedDirectory() throws IOException {
    ...
    assertEquals(Files.readAttributes(SUBST_DRIVE, "*"), Files.readAttributes(tempDirectory, "*"));
  ...
  /**
   * When the substituted drive is a symlink, test if it has the same attributes as its target.
  ...
  /** Run a command and optionally prints stdout contents to `customOutputStream`. */
  private void runCmd(ProcessBuilder pb, PrintStream customOutputStream) {
    try {
      PrintStream ps = customOutputStream != null ? customOutputStream : System.out;
      OutputAnalyzer outputAnalyzer =
          ProcessTools.executeCommand(pb).outputTo(ps).errorTo(System.err);

      int exitCode = outputAnalyzer.getExitValue();
      assertEquals(
          exitCode /* actual value */,
          0 /* expected value */,
          String.format("Command `%s` failed with exit code %d", pb.command(), exitCode));
  ...
  /**
   * Helper to map a path to a drive letter using subst. For reference, see:
  ...
  private void substCreate(Path drive, Path path) {
    runCmd(
        new ProcessBuilder("cmd", "/c", "subst", drive.toString(), path.toString()),
        null /* customOutputStream */);
  }

  /**
   * Delete a drive mapping using subst. For reference, see:
   * https://docs.microsoft.com/en-us/windows-server/administration/windows-commands/subst
   */
  private void substDelete(Path drive) throws IOException {
    runCmd(
        new ProcessBuilder("cmd", "/c", "subst", drive.toString(), "/D"),
        null /* customOutputStream */);
  }

  /**
   * Return a list of strings that represents all the currently mapped drives. For instance, with
   * the following output of subst: A:\: => path1 B:\: => path2 T:\: => path3 X:\: => path4 The
   * function returns: ["A:\", "B:\", "T:\", "X:\"] For reference, see:
   * https://docs.microsoft.com/en-us/windows-server/administration/windows-commands/subst
   */
  ...
  /**
   * subst can fail if the drive to be mapped already exists. The method returns a drive that is
   * available.
   */
