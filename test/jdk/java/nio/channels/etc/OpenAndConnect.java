...
public class OpenAndConnect {
  ...
  /**
   * If the destination address is the wildcard, it is replaced by the alternate using the port
   * number from destination. Otherwise destination is returned. Only used by dcOpenAndConnect
   */
  ...
  public void scOpenAndConnect(
      ProtocolFamily sfam, InetAddress saddr, ProtocolFamily cfam, InetAddress caddr)
      throws IOException {
  ...
  private static ServerSocketChannel openSSC(ProtocolFamily fam) throws IOException {
    return fam == null ? ServerSocketChannel.open() : ServerSocketChannel.open(fam);
  }

  private static DatagramChannel openDC(ProtocolFamily fam) throws IOException {
    return fam == null ? DatagramChannel.open() : DatagramChannel.open(fam);
  }
  ...
  private static void initAddrs() throws IOException {
    ...
    IA4LOCAL = cfg.ip4Addresses().filter(a -> !a.isLoopbackAddress()).findFirst().orElse(null);
