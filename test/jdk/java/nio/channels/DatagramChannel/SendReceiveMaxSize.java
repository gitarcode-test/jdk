...
public class SendReceiveMaxSize {
  ...
  static DatagramChannelSupplier supplier(DatagramChannelSupplier supplier) {
    return supplier;
  }
  ...
  public Object[][] invariants() throws IOException {
    ...
    if (hasIPv4()) {
      ...
      testcases.add(
          new Object[] {
            supplier(() -> DatagramChannel.open()), IPSupport.getMaxUDPSendBufSizeIPv4(), IPv4Addr
          });
      testcases.add(
          new Object[] {
    ...
    if (!preferIPv4Stack() && hasIPv6()) {
      ...
      testcases.add(
          new Object[] {
            supplier(() -> DatagramChannel.open()), IPSupport.getMaxUDPSendBufSizeIPv6(), IPv6Addr
          });
      testcases.add(
          new Object[] {
  ...
  @Test(dataProvider = "invariants")
  public void testSendReceiveMaxSize(
      DatagramChannelSupplier supplier, int capacity, InetAddress host) throws IOException {
    try (var receiver = DatagramChannel.open()) {
      receiver.bind(new InetSocketAddress(host, 0));
      assertTrue(
          receiver.getOption(SO_RCVBUF) >= capacity,
      ...
      try (var sender = supplier.open()) {
        sender.bind(null);
        if (!Platform.isOSX()) {
          if (sender.getOption(SO_SNDBUF) < capacity) sender.setOption(SO_SNDBUF, capacity);
        }
