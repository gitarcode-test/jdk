...
public class MulticastSendReceiveTests {
  ...
  static int sendDatagram(InetAddress local, NetworkInterface nif, InetAddress group, int port)
      throws IOException {
   ...
   *
   * @param expectedSender - expected sender address, or null if no datagram expected
   * @param id - expected id of datagram
   */
  static void receiveDatagram(DatagramChannel dc, InetAddress expectedSender, int id)
      throws IOException {
  ...
  static void test(
      ProtocolFamily family, NetworkInterface nif, InetAddress group, InetAddress source)
      throws IOException {
    System.out.format("\nTest DatagramChannel to %s socket\n", family.name());
    try (DatagramChannel dc =
        (family == UNSPEC) ? DatagramChannel.open() : DatagramChannel.open(family)) {
      dc.setOption(StandardSocketOptions.SO_REUSEADDR, true).bind(new InetSocketAddress(0));
      ...
      System.out.format(
          ...
          group.getHostAddress(), nif.getName(), bogus.getHostAddress());
      try {
        // may throw UOE
        ...
        System.out.format(
            "join %s @ %s only-source %s\n",
            group.getHostAddress(), nif.getName(), source.getHostAddress());
  ...
  public static void main(String[] args) throws IOException {
    ...
    boolean canIPv6JoinIPv4Group = Platform.isLinux() || Platform.isOSX() || Platform.isWindows();
