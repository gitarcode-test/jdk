...
public class AdaptorMulticasting {
  ...
  public static void main(String[] args) throws IOException {
    ...
    List<NetworkInterface> ip4MulticastInterfaces =
        config.ip4MulticastInterfaces().collect(Collectors.toList());
    List<NetworkInterface> ip6MulticastInterfaces =
        config.ip6MulticastInterfaces().collect(Collectors.toList());
  ...
  static void test(ProtocolFamily family, InetAddress group, NetworkInterface ni)
      throws IOException {
    System.out.format(
        "Test family=%s, multicast group=%s, interface=%s%n", family.name(), group, ni.getName());
  ...
  /**
   * Creates a MulticastSocket. The SO_REUSEADDR socket option is set and it is bound to the
   * wildcard address.
   */
  static MulticastSocket create(ProtocolFamily family) throws IOException {
    DatagramChannel dc = (family == UNSPEC) ? DatagramChannel.open() : DatagramChannel.open(family);
    try {
  ...
  /** Test 1-arg joinGroup/leaveGroup */
  static void testJoinGroup1(
      ProtocolFamily family, MulticastSocket s, InetAddress group, NetworkInterface ni)
      throws IOException {
  ...
  static void testJoinGroup2(
    ...
    // already a member (exception not specified)
    assertThrows(SocketException.class, () -> s.joinGroup(new InetSocketAddress(group, 0), null));

    // leave
    s.leaveGroup(new InetSocketAddress(group, 0), null);

    // not a member (exception not specified)
    assertThrows(SocketException.class, () -> s.leaveGroup(new InetSocketAddress(group, 0), null));
    ...
    // already a member (exception not specified)
    assertThrows(SocketException.class, () -> s.joinGroup(new InetSocketAddress(group, 0), ni));

    // leave
    s.leaveGroup(new InetSocketAddress(group, 0), ni);

    // not a member (exception not specified)
    assertThrows(SocketException.class, () -> s.leaveGroup(new InetSocketAddress(group, 0), ni));
    ...
    var localHost = InetAddress.getLocalHost();
    assertThrows(
        SocketException.class, () -> s.joinGroup(new InetSocketAddress(localHost, 0), null));
    assertThrows(
        SocketException.class, () -> s.leaveGroup(new InetSocketAddress(localHost, 0), null));
    assertThrows(SocketException.class, () -> s.joinGroup(new InetSocketAddress(localHost, 0), ni));
    assertThrows(
        SocketException.class, () -> s.leaveGroup(new InetSocketAddress(localHost, 0), ni));

    // not an InetSocketAddress
    var customSocketAddress = new SocketAddress() {};
    assertThrows(IllegalArgumentException.class, () -> s.joinGroup(customSocketAddress, null));
    assertThrows(IllegalArgumentException.class, () -> s.leaveGroup(customSocketAddress, null));
    assertThrows(IllegalArgumentException.class, () -> s.joinGroup(customSocketAddress, ni));
    assertThrows(IllegalArgumentException.class, () -> s.leaveGroup(customSocketAddress, ni));
    ...
    if (family == INET) {
      ...
      assertThrows(
          SocketException.class, () -> s.leaveGroup(new InetSocketAddress(ip6Group, 0), null));
      assertThrows(
          SocketException.class, () -> s.leaveGroup(new InetSocketAddress(ip6Group, 0), ni));
    }
  ...
  /**
   * Test getNetworkInterface/setNetworkInterface/getInterface/setInterface and IP_MULTICAST_IF
   * socket option.
   */
  static void testNetworkInterface(MulticastSocket s, NetworkInterface ni) throws IOException {
    // default value
    ...
    assertTrue(
        s.getNetworkInterface().inetAddresses().filter(address::equals).findAny().isPresent());
    ...
    assertThrows(IllegalArgumentException.class, () -> s.setOption(IP_MULTICAST_IF, null));
    assertThrows(
        IllegalArgumentException.class,
        () -> s.setOption((SocketOption) IP_MULTICAST_IF, "badValue"));
  }

  /** Test getTimeToLive/setTimeToLive/getTTL/getTTL and IP_MULTICAST_TTL socket option. */
  static void testTimeToLive(MulticastSocket s) throws IOException {
    ...
    assertThrows(IllegalArgumentException.class, () -> s.setOption(IP_MULTICAST_TTL, -1));
    assertThrows(IllegalArgumentException.class, () -> s.setOption(IP_MULTICAST_TTL, null));
    assertThrows(
        IllegalArgumentException.class,
  ...
  static void testLoopbackMode(MulticastSocket s) throws IOException {
    ...
    assertThrows(IllegalArgumentException.class, () -> s.setOption(IP_MULTICAST_LOOP, null));
    assertThrows(
        IllegalArgumentException.class,
  ...
  /** Send a datagram to the given multicast group and check that it is not received. */
