...
public class EqualsCompareTest {
  ...
  abstract static class BufferType<T extends Buffer, E> {
    final BufferKind k;
    ...
    BufferType(BufferKind k, Class<T> bufferType, Class<?> elementType) {
      ...
      try {
        eq =
            lookup.findVirtual(
                bufferType, "equals", MethodType.methodType(boolean.class, Object.class));
        cmp =
            lookup.findVirtual(
                bufferType, "compareTo", MethodType.methodType(int.class, bufferType));
        mismtch =
            lookup.findVirtual(
                bufferType, "mismatch", MethodType.methodType(int.class, bufferType));

        getter =
            lookup.findVirtual(bufferType, "get", MethodType.methodType(elementType, int.class));
        setter =
            lookup.findVirtual(
                bufferType, "put", MethodType.methodType(bufferType, int.class, elementType));
      } catch (Exception e) {
    ...
    boolean pairWiseEquals(T a, T b) {
      ...
      for (int i = a.limit() - 1, j = b.limit() - 1; i >= p; i--, j--)
        if (!get(a, i).equals(get(b, j))) return false;
      return true;
    }
    ...
    static class Bytes extends BufferType<ByteBuffer, Byte> {
      ...
      Object convert(Object o) {
        return o instanceof Integer ? ((Integer) o).byteValue() : o;
      }
    }
    ...
    static class Chars extends BufferType<CharBuffer, Character> {
      ...
      CharBuffer construct(int length, ByteOrder bo) {
        switch (k) {
          case DIRECT:
            return ByteBuffer.allocateDirect(length * Character.BYTES).order(bo).asCharBuffer();
          case HEAP_VIEW:
            return ByteBuffer.allocate(length * Character.BYTES).order(bo).asCharBuffer();
          default:
          case HEAP:
      ...
      @Override
      Object convert(Object o) {
        return o instanceof Integer ? (char) ((Integer) o).intValue() : o;
      }
    ...
    static class Shorts extends BufferType<ShortBuffer, Short> {
      ...
      ShortBuffer construct(int length, ByteOrder bo) {
        switch (k) {
          case DIRECT:
            return ByteBuffer.allocateDirect(length * Short.BYTES).order(bo).asShortBuffer();
          case HEAP_VIEW:
            return ByteBuffer.allocate(length * Short.BYTES).order(bo).asShortBuffer();
          default:
          case HEAP:
      ...
      @Override
      Object convert(Object o) {
        return o instanceof Integer ? ((Integer) o).shortValue() : o;
      }
    }
    ...
    static class Ints extends BufferType<IntBuffer, Integer> {
      ...
      IntBuffer construct(int length, ByteOrder bo) {
        switch (k) {
          case DIRECT:
            return ByteBuffer.allocateDirect(length * Integer.BYTES).order(bo).asIntBuffer();
          case HEAP_VIEW:
            return ByteBuffer.allocate(length * Integer.BYTES).order(bo).asIntBuffer();
          default:
          case HEAP:
    ...
    static class Floats extends BufferType<FloatBuffer, Float> {
      ...
      FloatBuffer construct(int length, ByteOrder bo) {
        switch (k) {
          case DIRECT:
            return ByteBuffer.allocateDirect(length * Float.BYTES).order(bo).asFloatBuffer();
          case HEAP_VIEW:
            return ByteBuffer.allocate(length * Float.BYTES).order(bo).asFloatBuffer();
          default:
          case HEAP:
      ...
      @Override
      Object convert(Object o) {
        return o instanceof Integer ? ((Integer) o).floatValue() : o;
      }
      ...
      boolean pairWiseEquals(FloatBuffer a, FloatBuffer b) {
        ...
        for (int i = a.limit() - 1, j = b.limit() - 1; i >= p; i--, j--) {
          float av = a.get(i);
          float bv = b.get(j);
          if (av != bv && (!Float.isNaN(av) || !Float.isNaN(bv))) return false;
        }
        return true;
      }
    }
    ...
    static class Longs extends BufferType<LongBuffer, Long> {
      ...
      LongBuffer construct(int length, ByteOrder bo) {
        switch (k) {
          case DIRECT:
            return ByteBuffer.allocateDirect(length * Long.BYTES).order(bo).asLongBuffer();
          case HEAP_VIEW:
            return ByteBuffer.allocate(length * Long.BYTES).order(bo).asLongBuffer();
          default:
          case HEAP:
      ...
      @Override
      Object convert(Object o) {
        return o instanceof Integer ? ((Integer) o).longValue() : o;
      }
    }
    ...
    static class Doubles extends BufferType<DoubleBuffer, Double> {
      ...
      DoubleBuffer construct(int length, ByteOrder bo) {
        switch (k) {
          case DIRECT:
            return ByteBuffer.allocateDirect(length * Double.BYTES).order(bo).asDoubleBuffer();
          case HEAP_VIEW:
            return ByteBuffer.allocate(length * Double.BYTES).order(bo).asDoubleBuffer();
          default:
          case HEAP:
      ...
      @Override
      Object convert(Object o) {
        return o instanceof Integer ? ((Integer) o).doubleValue() : o;
      }
      ...
      boolean pairWiseEquals(DoubleBuffer a, DoubleBuffer b) {
        ...
        for (int i = a.limit() - 1, j = b.limit() - 1; i >= p; i--, j--) {
          double av = a.get(i);
          double bv = b.get(j);
          if (av != bv && (!Double.isNaN(av) || !Double.isNaN(bv))) return false;
        }
        return true;
      }
    }
    ...
    if (floatbufferTypes == null) {
      ...
      floatbufferTypes =
          new Object[][] {
            ...
            new Object[] {
              new BufferType.Floats(BufferKind.HEAP_VIEW), 0x7fc00000L, 0x7f800001L, bTof
            },
            ...
            new Object[] {
              new BufferType.Doubles(BufferKind.HEAP_VIEW), 0x0L, 0x8000000000000000L, bToD
            },
            new Object[] {
              new BufferType.Doubles(BufferKind.DIRECT), 0x0L, 0x8000000000000000L, bToD
            },
          };
    }
  ...
  <E> void testBuffers(BufferType<Buffer, E> bufferType) {
    // Test with buffers of the same byte order (BE)
    BiFunction<BufferType<Buffer, E>, Integer, Buffer> constructor =
        (at, s) -> {
    ...
    if (bufferType.elementType != byte.class
        && (bufferType.k == BufferKind.HEAP_VIEW || bufferType.k == BufferKind.DIRECT)) {

      BiFunction<BufferType<Buffer, E>, Integer, Buffer> leConstructor =
          (at, s) -> {
  ...
  public void testFloatBuffers(
    ...
    BiFunction<BufferType<Buffer, Float>, Integer, Buffer> allAs =
        (at, s) -> {
    ...
    BiFunction<BufferType<Buffer, Float>, Integer, Buffer> allBs =
        (at, s) -> {
          Buffer b = at.construct(s);
          for (int x = 0; x < s; x++) {
            at.set(b, x, bv);
    ...
    BiFunction<BufferType<Buffer, Float>, Integer, Buffer> halfBs =
        (at, s) -> {
          Buffer b = at.construct(s);
          for (int x = 0; x < s / 2; x++) {
            at.set(b, x, bv);
    ...
    Assert.assertTrue(
        bufferType.pairWiseEquals(allAs.apply(bufferType, size), allBs.apply(bufferType, size)));
    Assert.assertTrue(
        bufferType.equals(allAs.apply(bufferType, size), allBs.apply(bufferType, size)));
  ...
  public void testCharBuffers(BufferType.Chars charBufferType) {
    ...
    BiFunction<BufferType.Chars, Integer, CharBuffer> constructor =
        (at, s) -> {
    ...
    BiFunction<BufferType.Chars, Integer, CharBuffer> constructorX =
        constructor.andThen(charBufferType::transformToStringBuffer);

    testBufferType(charBufferType, constructor, constructorX);
  }

  <B extends Buffer, E, BT extends BufferType<B, E>> void testBufferType(
      BT bt, BiFunction<BT, Integer, B> aConstructor, BiFunction<BT, Integer, B> bConstructor) {
      ...
      for (int s : ranges(0, n)) {
        ...
        for (int aFrom : ranges(0, s)) {
          for (int aTo : ranges(aFrom, s)) {
            ...
            B as = aLength != s ? bt.slice(a, aFrom, aTo, dupOtherwiseSlice) : a;

            for (int bFrom : ranges(0, s)) {
              for (int bTo : ranges(bFrom, s)) {
                int bLength = bTo - bFrom;

                B bs = bLength != s ? bt.slice(b, bFrom, bTo, dupOtherwiseSlice) : b;
            ...
            if (aLength > 0 && !a.isReadOnly()) {
              for (int i = aFrom; i < aTo; i++) {
                ...
                B cs = aLength != s ? bt.slice(c, aFrom, aTo, dupOtherwiseSlice) : c;
