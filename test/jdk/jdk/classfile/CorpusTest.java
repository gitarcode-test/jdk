...
class CorpusTest {
  ...
  protected static final String testFilter =
      null; // "modules/java.base/java/util/function/Supplier.class";

  static void splitTableAttributes(String sourceClassFile, String targetClassFile)
      throws IOException, URISyntaxException {
    var root =
        Paths.get(URI.create(CorpusTest.class.getResource("CorpusTest.class").toString()))
            .getParent();
    var cc = ClassFile.of();
    Files.write(
        root.resolve(targetClassFile),
        cc.transformClass(
            cc.parse(root.resolve(sourceClassFile)),
            ClassTransform.transformingMethodBodies(
                (cob, coe) -> {
                  ...
                  switch (coe) {
                    case LineNumber ln ->
                        dcob.writeAttribute(
                            new UnboundAttribute.AdHocAttribute<>(Attributes.lineNumberTable()) {
                              @Override
                    ...
                    case LocalVariable lv ->
                        dcob.writeAttribute(
                            new UnboundAttribute.AdHocAttribute<>(Attributes.localVariableTable()) {
                              @Override
                              public void writeBody(BufWriter b) {
                                b.writeU2(1);
                                lv.writeTo(b);
                    ...
                    case LocalVariableType lvt ->
                        dcob.writeAttribute(
                            new UnboundAttribute.AdHocAttribute<>(
    ...
    //        ClassPrinter.toYaml(ClassModel.of(Files.readAllBytes(root.resolve(targetClassFile))),
    // ClassPrinter.Verbosity.TRACE_ALL, System.out::print);
  }
        ...
        .filter(
            p ->
                Files.isRegularFile(p)
                    ...
                    && !p.endsWith("DeadCodePattern.class"))
  ...
  void testNullAdaptations(Path path) throws Exception {
    ...
    for (Transforms.NoOpTransform m : Transforms.NoOpTransform.values()) {
      if (m == Transforms.NoOpTransform.ARRAYCOPY
          ...
          || m.name().startsWith("ASM")) continue;

      try {
        byte[] transformed =
            m.shared && m.classTransform != null
        ...
        if (oldRecord.isPresent() && newRecord.isPresent())
          assertEqualsDeep(
              newRecord.get(), oldRecord.get(), "Class[%s] with %s".formatted(path, m.name()));
        switch (m) {
          case SHARED_1, SHARED_2, SHARED_3, SHARED_3L, SHARED_3P:
            if (newDups.size() > baseDups.size()) {
              System.out.println(
                  String.format(
                      "Incremental dups in file %s (%s): %s / %s", path, m, baseDups, newDups));
            }
        ...
        System.err.printf(
            "Error processing %s with %s: %s.%s%n", path, m.name(), ex.getClass(), ex.getMessage());
    ...
    if (!errors.isEmpty()) {
      String msg =
          String.format("Failures for %s:%n", path)
    ...
    assertEqualsDeep(
        ClassRecord.ofClassModel(classModel),
        ClassRecord.ofStreamingElements(classModel),
        "ClassModel (actual) vs StreamingElements (expected)");

    byte[] newBytes = cc.build(classModel.thisClass().asSymbol(), classModel::forEach);
    var newModel = cc.parse(newBytes);
    assertEqualsDeep(
        ClassRecord.ofClassModel(newModel, CompatibilityFilter.By_ClassBuilder),
        ClassRecord.ofClassModel(classModel, CompatibilityFilter.By_ClassBuilder),
        "ClassModel[%s] transformed by ClassBuilder (actual) vs ClassModel before transformation (expected)"
            .formatted(path));

    assertEmpty(cc.verify(newModel));

    // testing maxStack and maxLocals are calculated identically by StackMapGenerator and
    // StackCounter
    byte[] noStackMaps =
        ClassFile.of(ClassFile.StackMapsOption.DROP_STACK_MAPS)
            .transformClass(
                newModel, ClassTransform.transformingMethodBodies(CodeTransform.ACCEPT_ALL));
    ...
    while (itStack.hasNext()) {
      ...
      var text1 =
          m1.methodName().stringValue()
              + m1.methodType().stringValue()
              + ": "
              + m1.code().map(c -> c.maxLocals() + " / " + c.maxStack()).orElse("-");
      var text2 =
          m2.methodName().stringValue()
              + m2.methodType().stringValue()
              + ": "
  ...
  //                                    .map(e -> String.format("%d -> %d", e.getKey(),
  // e.getValue()))
  //                                    .collect(joining(", "));
  //            System.out.println(String.format("Duplicate entries in input file %s: %s", path,
  // dupsString));
  //        }
  //    }
  ...
  private void compareCp(byte[] orig, byte[] transformed) {
    ...
    if (cp1.size() != cp2.size()) {
      StringBuilder failMsg =
          new StringBuilder("Extra entries in constant pool (" + (cp2.size() - cp1.size()) + "): ");
    ...
    for (int i = 1; i < pool.size(); i += pool.entryByIndex(i).width()) {
      ...
      if (entryStrings.contains(s)) {
        for (int j = 1; j < i; j += pool.entryByIndex(j).width()) {
          ...
          if (s.equals(cpiToString(e2))) dups.put(i, j);
        }
      }
      entryStrings.add(s);
    }
