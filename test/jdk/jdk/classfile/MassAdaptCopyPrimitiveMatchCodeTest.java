...
class MassAdaptCopyPrimitiveMatchCodeTest {
  ...
  static final List<Path> testClasses(Path which) {
    try {
  ...
  static final List<Path> testClasses =
      testClasses(
          // FileSystems.getFileSystem(URI.create("jrt:/")).getPath("modules/java.base/java/util")
          // Path.of("target", "classes")
          Paths.get(
                  URI.create(
                      MassAdaptCopyPrimitiveMatchCodeTest.class
                          .getResource("MassAdaptCopyPrimitiveMatchCodeTest.class")
                          .toString()))
              .getParent());
  ...
  public void testCodeMatch() throws Exception {
    for (Path path : testClasses) {
      try {
        copy(path.toString(), Files.readAllBytes(path));
        if (failure) {
  ...
  void copy(String name, byte[] bytes) throws Exception {
                ...
                cb.transformMethod(
                    ...
                    (mb, me) -> {
                      if (me instanceof CodeModel xm)
                        mb.transformCode(
                            xm, (xr, xe) -> InstructionModelToCodeBuilder.toBuilder(xe, xr));
                      else mb.with(me);
                    });
              } else cb.with(e);
            });
    ...
    if (cm.findAttribute(Attributes.record()).isPresent()) {
      System.err.printf(
          "MassAdaptCopyPrimitiveMatchCodeTest: Ignored because it is a record%n         - %s%n",
          name);
      return;
    }
    ...
    for (MethodModel rmm : rcm.methods()) {
      ...
      if (code.isPresent()) {
        ...
        if (!Arrays.equals(rbytes, obytes)) {
          System.err.printf(
              "Copy has mismatched bytecode -- Method: %s.%s%n",
              name, rmm.methodName().stringValue());
          ...
          for (int i = 0; i < len; ++i) {
            ...
            if (obytes[i] != rbytes[i]) {
              if (bciFirstBadInstruction < 0) {
                System.err.printf(
                    "  bytecode differs firstly at BCI [%d]; expected value is <%d> but was <%d>."
                        + " Instruction BCI %d%n",
          ...
          for (int i = 0; i < len; ++i) {
            ...
            if (i < olen) System.err.printf("%4d ", obytes[i] & 0xFF);
            else System.err.printf("     ");
            if (i < rlen) System.err.printf("%4d ", rbytes[i] & 0xFF);
            else System.err.printf("     ");
            CodeElement oim = oima[i];
            if (oim != null) System.err.printf("%s  ", oim);
            CodeElement rim = rima[i];
            if (rim != null) System.err.printf("---> %s  ", rim);
            System.err.printf("%n");
