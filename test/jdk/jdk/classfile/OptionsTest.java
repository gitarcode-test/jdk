...
class OptionsTest {
  ...
  void testAttributesProcessingOptionOnTransform(Path path) throws Exception {
    testNoUnstable(
        ...
        ClassFile.of()
            .parse(
                ClassFile.of(ClassFile.AttributesProcessingOption.DROP_UNSTABLE_ATRIBUTES)
                    .transformClass(
  ...
  static final AttributeMapper<StrangeAttribute> STRANGE_ATTRIBUTE_MAPPER =
      new AttributeMapper<>() {
        ...
        @Override
        public StrangeAttribute readAttribute(
  ...
  void testUnknownAttribute() throws Exception {
    var classBytes =
        ClassFile.of(
                ClassFile.AttributeMapperOption.of(
                    e -> {
                      return e.equalsString(STRANGE_ATTRIBUTE_MAPPER.name())
                          ? STRANGE_ATTRIBUTE_MAPPER
                          : null;
                    }))
    ...
    assertTrue(
        ClassFile.of()
            .parse(
                ClassFile.of(ClassFile.AttributesProcessingOption.DROP_UNKNOWN_ATTRIBUTES)
                    .transformClass(ClassFile.of().parse(classBytes), ClassTransform.ACCEPT_ALL))
            .attributes()
            .isEmpty());
  }
    ...
    if (e instanceof AttributedElement ae)
      ae.attributes()
          .forEach(
              a ->
                  assertTrue(
                      AttributeMapper.AttributeStability.UNSTABLE.ordinal()
                          >= a.attributeMapper().stability().ordinal(),
                      () -> "class " + path + " contains unexpected " + a));
    if (e instanceof CompoundElement ce)
      ce.forEach(ee -> testNoUnstable(path, (ClassFileElement) ee));
  }
}
