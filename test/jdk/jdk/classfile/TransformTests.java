...
class TransformTests {
  ...
  static final Path testClassPath =
      Paths.get(URI.create(ArrayTest.class.getResource(testClassName + ".class").toString()));
  static CodeTransform foo2foo = swapLdc("foo", "foo"),
  ...
  void testSeqN() throws Exception {
    ...
    assertEquals(invoke(bytes), "foo");
    assertEquals(
        invoke(cc.transformClass(cm, transformCode(foo2bar.andThen(bar2baz).andThen(baz2foo)))),
        "foo");
    assertEquals(
        invoke(cc.transformClass(cm, transformCode(foo2bar.andThen(bar2baz).andThen(baz2quux)))),
        "quux");
    assertEquals(
        invoke(cc.transformClass(cm, transformCode(foo2foo.andThen(foo2bar).andThen(bar2baz)))),
        "baz");
  }

  /**
   * Test to ensure class elements, such as field and methods defined with transform/with, are
   * visible to next transforms.
   */
  @Test
  ...
  /**
   * Test to ensure method elements, such as generated or transformed code, are visible to
   * transforms.
   */
  @Test
  void testMethodChaining() throws Exception {
    ...
    var cm =
        cf.parse(
            cf.build(
                ClassDesc.of("Temp"),
                clb -> clb.withMethod("baz", mtd, ACC_STATIC | ACC_NATIVE, _ -> {})));

    MethodTransform transform1 =
  ...
  /**
   * Test to ensure code elements, such as code block begin and end labels, are visible to
   * transforms.
   */
  @Test
  void testCodeChaining() throws Exception {
    ...
    cf.transformClass(
        ...
        ClassTransform.transformingMethods(
            MethodTransform.transformingCode(transform1.andThen(transform2))));
