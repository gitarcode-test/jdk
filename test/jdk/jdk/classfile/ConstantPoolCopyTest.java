...
class ConstantPoolCopyTest {
  ...
  void cloneConstantPool(ClassModel c) throws Exception {
    ...
    for (int i = 1; i < cp.size(); ) {
      ...
      assertTrue(
          representsTheSame(cp1i, cp2i),
          cp2i + " does not represent the same constant pool entry as " + cp1i);
      i += cp1i.width();
    }
  ...
  // This differs from a value-based equals in that the constant pool field can differ while the
  // constants still
  // represent the same
  boolean representsTheSame(PoolEntry first, PoolEntry second) {
    if (first instanceof Utf8Entry i) {
      ...
      if (second instanceof NameAndTypeEntry j) {
        return i.name().stringValue().equals(j.name().stringValue())
            && i.type().stringValue().equals(j.type().stringValue());
      } else {
        return false;
      }
      ...
      if (second instanceof FloatEntry j) {
        return Float.isNaN(i.floatValue())
            ? Float.isNaN(j.floatValue())
            : i.floatValue() == j.floatValue();
      } else {
        return false;
      }
      ...
      if (second instanceof DoubleEntry j) {
        return Double.isNaN(i.doubleValue())
            ? Double.isNaN(j.doubleValue())
            : i.doubleValue() == j.doubleValue();
      } else {
        return false;
      }
    }
  ...
  boolean sameBootstrap(BootstrapMethodEntry first, BootstrapMethodEntry second) {
    if (representsTheSame(first.bootstrapMethod(), second.bootstrapMethod())
      ...
      for (int i = 0; i < firstArgs.size(); ++i) {
        if (!representsTheSame(firstArgs.get(i), secondArgs.get(i))) return false;
      }
