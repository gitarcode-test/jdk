...
public class ExampleGallery {
  ...
  public byte[] overrideSuperclass(ClassModel cm, ClassDesc superclass) {
    return ClassFile.of()
        .transformClass(cm, ClassTransform.endHandler(cb -> cb.withSuperclass(superclass)));
  }
                ...
                () ->
                    new ClassTransform() {
                      ...
                      public void atEnd(ClassBuilder builder) {
                        if (!seen) builder.withInterfaceSymbols(newIntf);
                      }
                    }));
  }
                ...
                case SignatureAttribute sa -> {
                  ...
                  cb.with(
                      SignatureAttribute.of(
                          ClassSignature.parseFrom(result.replace("this/", "that/"))));
                }
                default -> cb.with(ce);
              }
            });
  ...
  public byte[] setSignature(ClassModel cm) {
    return ClassFile.of()
        .transformClass(
            cm,
            ClassTransform.dropping(e -> e instanceof SignatureAttribute)
                .andThen(
    ...
    return ClassFile.of()
        .transformClass(
            ...
            ClassTransform.dropping(
                e -> e instanceof FieldModel fm && filter.test(fm.fieldName().stringValue())));
  }
  ...
  public byte[] addField(ClassModel cm) {
    return ClassFile.of()
        .transformClass(
            cm,
            ClassTransform.endHandler(
                cb -> cb.withField("cool", ClassDesc.ofDescriptor("(I)D"), ClassFile.ACC_PUBLIC)));
  }
                ...
                (fb, fe) -> {
                  if (fe instanceof SignatureAttribute sa)
                    fb.with(
                        SignatureAttribute.of(
                            Signature.parseFrom(
                                sa.signature().stringValue().replace("this/", "that/"))));
                  else fb.with(fe);
                }));
  }
                ...
                (fb, fe) -> {
                  switch (fe) {
                    case AccessFlags a ->
                        fb.with(
                            AccessFlags.ofField(
                                a.flagsMask() & ~ClassFile.ACC_PUBLIC & ~ClassFile.ACC_PROTECTED));
                    default -> fb.with(fe);
                  }
                }));
  }
                              ...
                              if (!attr.exceptions().contains(newEx)) {
                                attr =
                                    ExceptionsAttribute.of(
                                        Stream.concat(attr.exceptions().stream(), Stream.of(newEx))
                                            .toList());
                              }
                              builder.with(attr);
                            }
