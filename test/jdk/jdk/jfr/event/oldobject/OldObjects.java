...
/**
 * Utility class to perform Old Object provocation/detection and stack trace/object verification for
 * the Old Object Sample JFR event
 */
public final class OldObjects {

  public static final int MIN_SIZE = 99901; // prime number
  public static final int LEAK_CONTEXT = 100; // length of chain assiociated with the object sample
  public static final int ROOT_CONTEXT = 100; // length of chain assoicated with the root
  public static final int MAX_CHAIN_LENGTH =
      LEAK_CONTEXT + ROOT_CONTEXT; // the VM should not construct chains longer than this
  ...
  /**
   * @param r A recording
   * @param expectedFrame A frame that must be found on the stack. Null if no check is required.
   * @param fieldType The object type (of the field). Null if no check is required.
   * @param fieldName The field name. Null if no check is required.
   * @param referrerType The class name. Null if no check is required.
   * @param minDuration The minimum duration of the event, -1 if not applicable.
   * @return The count of matching events
   * @throws IOException
   */
  public static long countMatchingEvents(
      Recording r,
      ...
      long minDuration)
      throws IOException {
    return countMatchingEvents(
        r, getFrames(expectedFrame), fieldType, fieldName, referrerType, minDuration);
  }

  /**
   * Gets the OldObjectSample events from the provided recording through a dump and counts how many
   * events matches the provided parameters.
   *
   * @param r A recording
   * @param expectedStack Some frames that must be found on the stack. Null if no check is required.
   * @param fieldType The object type (of the field). Null if no check is required.
   * @param fieldName The field name. Null if no check is required.
   * @param referrerType The class name. Null if no check is required.
   * @param minDuration The minimum duration of the event, -1 if not applicable.
  ...
  public static long countMatchingEvents(
      ...
      throws IOException {
    return countMatchingEvents(
        Events.fromRecording(r), fieldType, fieldName, referrerType, minDuration, expectedStack);
  }

  /**
   * @param events A list of RecordedEvent.
   * @param expectedFrame A frame that must be found on the stack. Null if no check is required.
   * @param fieldType The object type (of the field). Null if no check is required.
   * @param fieldName The field name. Null if no check is required.
   * @param referrerType The class name. Null if no check is required.
  ...
  public static long countMatchingEvents(
      ...
      throws IOException {
    return countMatchingEvents(
        events, fieldType, fieldName, referrerType, minDuration, getFrames(expectedFrame));
  }

  /**
   * @param events The list of events to find matching events in
   * @param expectedStack Some frames that must be found on the stack. Null if no check is required.
   * @param fieldType The object type (of the field). Null if no check is required.
   * @param fieldName The field name. Null if no check is required.
   * @param referrerType The class name. Null if no check is required.
  ...
  public static String getReferrerFieldName(RecordedEvent event) {
    RecordedObject referrer = event.getValue("object.referrer");
    return referrer != null && referrer.hasField("field.name")
        ? referrer.getValue("field.name")
        : null;
  }
  ...
  public static void validateReferenceChainLimit(RecordedEvent e, int maxLength) {
    ...
    if (length > maxLength) {
      throw new RuntimeException(
          "Reference chain max length not respected. Found a chain of length " + length);
    }
  }
}
