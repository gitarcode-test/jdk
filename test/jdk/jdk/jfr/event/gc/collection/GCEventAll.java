...
/**
 * Tests for event garbage_collection. The test function is called from TestGCEvent*.java, with
 * different worker threads. Groups all events belonging to the same garbage collection (the same
 * gcId). The group of events belonging to the same GC is called a batch.
 *
 * <p>This class contains the verifications done and the worker threads used to generate GCs. The
 * helper logic are in class GCHelper.
 *
 * <p>Summary of verifications: All gcIds in garbage_collection event are unique.
 *
 * <p>All events in batch has the same gcId.
 *
 * <p>Number of garbage_collection events == GarbageCollectionMXBean.getCollectionCount()
 *
 * <p>garbage_collection.sum_pause_time approximately equals
 * GarbageCollectionMXBean.getCollectionTime()
 *
 * <p>Batch contains expected events depending on garbage_collection.name
 *
 * <p>garbage_collection_start.timestamp == garbage_collection.startTime.
 *
 * <p>garbage_collection.timestamp >= timestamp for all other events in batch.
 *
 * <p>The start_garbage_collection and garbage_collection events must be synchronized. This means
 * that there may be multiple start_garbage_collection before a garbage_collection, but
 * garbage_collection.gcId must be equal to latest start_garbage_collection.gcId.
 *
 * <p>start_garbage_collection must be the first event in the batch, that means no event with same
 * gcId before garbage_collection_start event.
 *
 * <p>garbage_collection.name matches what is expected by the collectors specified in
 * initial_configuration.
 *
 * <p>Duration for event "vm/gc/phases/pause" >= 1. Duration for phase level events >= 0.
 */
public class GCEventAll {
  ...
  private void doSingleTest(Thread[] workerThreads) throws Throwable {
    ...
    // the end of the test. This extra System.gc() is only necessary when using
   ...
   * @param beanCounts Number of collections and sum pause time reported by
   *     GarbageCollectionMXBeans.
   * @param gcIds All used gcIds. Must be unique.
   * @throws Exception
   */
  private void verify(List<RecordedEvent> events, GCHelper.CollectionSummary beanCounts)
      throws Throwable {
    List<GCHelper.GcBatch> gcBatches = null;
    GCHelper.CollectionSummary eventCounts = null;

    // For some GC configurations, the JFR recording may have stopped before we received the last gc
    // event.
    try {
  ...
  /**
   * Verifies collection count reported by flight recorder events against the values reported by
   * GarbageCollectionMXBean. Number of collections should match exactly. Sum pause time are allowed
   * some margin of error because of rounding errors in measurements.
   */
  private void verifyCollectionCount(
      GCHelper.CollectionSummary eventCounts, GCHelper.CollectionSummary beanCounts) {
    verifyCollectionCount(
        youngCollector, eventCounts.collectionCountYoung, beanCounts.collectionCountYoung);
    verifyCollectionCount(
        oldCollector, eventCounts.collectionCountOld, beanCounts.collectionCountOld);
  }
  ...
  private void verifyCollectionCount(String collector, long eventCounts, long beanCounts) {
    ...
    Asserts.assertGreaterThanOrEqual(
        eventCounts, minCount, "Too few event counts for collector " + collector);
    Asserts.assertLessThanOrEqual(
        eventCounts, maxCount, "Too many event counts for collector " + collector);
  }

  /**
   * Verifies that all events belonging to a single GC are ok. A GcBatch contains all flight
   * recorder events that belong to a single GC.
   */
    ...
    for (GCHelper.GcBatch batch : batches) {
      ...
      try {
        ...
        for (RecordedEvent event : batch.getEvents()) {
          ...
          if (name.contains("AllocationRequiringGC")) {
            ...
            Asserts.assertLessThanOrEqual(
                event.getStartTime(),
                batchStartTime,
                "Timestamp in event after start event, should be sent before GC start");
          } else {
            Asserts.assertGreaterThanOrEqual(
                event.getStartTime(),
                batchStartTime,
                "startTime in event before batch start event, should be sent after GC start");
          }
          ...
          if (!EventNames.GCCPUTime.equals(name)) {
            Asserts.assertLessThanOrEqual(
                event.getEndTime(),
                batchEndTime,
                "endTime in event after batch end event, should be sent before GC end");
          }
        }
        ...
        Asserts.assertNotNull(
            requiredEvents, "No required events specified for " + batch.getName());
        for (String requiredEvent : requiredEvents) {
          boolean b = batch.containsEvent(requiredEvent);
          Asserts.assertTrue(
              b, String.format("%s does not contain event %s", batch, requiredEvent));
        }
      ...
      for (RecordedEvent event : batch.getEvents()) {
        if (event.getEventType().getName().contains(GCHelper.pauseLevelEvent)) {
          ...
          Asserts.assertGreaterThanOrEqual(
              event.getStartTime(),
              batchStartTime,
              "Phase startTime >= batch startTime. Event:" + event);

          // Duration for event "vm/gc/phases/pause" must be >= 1. Other phase event durations must
          // be >= 0.
          Duration minDuration =
              Duration.ofNanos(
                  GCHelper.event_phases_pause.equals(event.getEventType().getName()) ? 1 : 0);
  ...
  /**
   * Verifies that the collector name in initial configuration matches the name in garbage
   * configuration event. If the names are not equal, then we check if this is an expected collector
   * override. For example, if old collector in initial config is "G1Old" we allow both event
   * "G1Old" and "SerialOld".
   */
  private void verifyCollectorNames(List<GCHelper.GcBatch> batches) {
    for (GCHelper.GcBatch batch : batches) {
      ...
      if (!expectedName.equals(name)) {
        // Collector names not equal. Check if the collector has been overridden by an expected
        // collector.
        ...
        Asserts.assertTrue(
            isOverride,
            String.format(
                "Unexpected event name(%s) for collectors(%s, %s)",
                name, youngCollector, oldCollector));
      }
    }
  }

  /**
   * Verifies field "cause" in garbage_collection event. Only check that at cause is not null and
   * that at least 1 cause is "System.gc()" We might want to check more cause reasons later.
   */
  ...
  private void log(
      ...
      List<GCHelper.GcBatch> batches,
      GCHelper.CollectionSummary eventCounts,
      GCHelper.CollectionSummary beanCounts) {
  ...
  public static class SystemGcWaitRunner implements Runnable {
    ...
    public static SystemGcWaitRunner create(
        int deltaCollections, int minWaitCollections, long maxWaitMillis) {
