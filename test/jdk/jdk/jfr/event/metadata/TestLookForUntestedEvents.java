...
public class TestLookForUntestedEvents {
  ...
  private static final Set<String> hardToTestEvents =
      new HashSet<>(
          Arrays.asList(
              "DataLoss",
              "IntFlag",
  ...
  private static final Set<String> coveredGcEvents =
      new HashSet<>(
          Arrays.asList(
  ...
  private static final Set<String> coveredContainerEvents =
      new HashSet<>(
          Arrays.asList(
  ...
  private static final Set<String> coveredVirtualThreadEvents =
      Set.of("VirtualThreadPinned", "VirtualThreadSubmitFailed");
  ...
  // Experimental events
  private static final Set<String> experimentalEvents = Set.of("Flush", "SyncOnValueBasedClass");
  ...
  private static void checkEventNamesClass() throws Exception {
    ...
    if (!jfrEventTypes.equals(eventsFromEventNamesClass)) {
      String exceptionMsg =
          "Events declared in jdk.test.lib.jfr.EventNames differ "
              + "from events returned by FlightRecorder.getEventTypes()";
      print(MSG_SEPARATOR);
      print(exceptionMsg);
      print("");
      printSetDiff(
          jfrEventTypes, eventsFromEventNamesClass, "jfrEventTypes", "eventsFromEventNamesClass");
      print("");
  ...
  private static boolean findStringInFile(Path p, String searchTerm) throws IOException {
    ...
    try (Stream<String> stream = Files.lines(p)) {
      c = stream.filter(line -> line.contains(searchTerm)).count();
    }
    return (c != 0);
  }

  private static void printSetDiff(Set<String> a, Set<String> b, String setAName, String setBName) {
    if (a.size() > b.size()) {
      a.removeAll(b);
