...
public class HandlerConnectionClose {
  ...
  void testHttpURLConnection(HttpServer httpServer, String protocol, String path) throws Exception {
    ...
    for (int i = 0; i < ONEK; i++) ba[i] = (byte) i;
      ...
      for (int i = 0; i < read; i++) {
        ...
        if (ba[i] != expected) {
          throw new IOException(
              "byte mismatch at " + (count - 1) + ": expected " + expected + " got " + ba[i]);
        }
      }
    }
    if (count != POST_SIZE) {
      ...
      if (path.endsWith("/fixed")) {
        ...
        while ((read = is.read()) != -1) {
          ...
          if ((read & 0xFF) != expected) {
            throw new IOException(
                "byte mismatch at " + (count - 1) + ": expected " + expected + " got " + read);
          }
        ...
        try {
          ...
          if (read != -1) {
            throw new AssertionError(
                "EOF was expected after " + count + " bytes, but got: " + read);
          } else {
  ...
  class MyHandler implements HttpHandler {
    ...
    public void handle(HttpExchange t) throws IOException {
      ...
      if (!end) {
        ...
        check(count == expected, "Expected: " + expected + ", received " + count);
  ...
  void check(boolean cond, String failMessage) {
    if (cond) pass();
