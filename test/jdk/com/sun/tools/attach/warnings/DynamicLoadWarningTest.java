...
class DynamicLoadWarningTest {
  ...
  private static final String JVMTI_AGENT_WARNING =
      "WARNING: A JVM TI agent has been loaded dynamically";
  private static final String JAVA_AGENT_WARNING =
  ...
  void testLoadOneJvmtiAgent(OnAttachAction loadJvmtiAgent1) throws Exception {
    ...
    test().whenRunning(loadJvmtiAgent1).stderrShouldContain(JVMTI_AGENT_WARNING);

    // opt-in via command line option to allow dynamic loading of agents
    test()
        .withOpts("-XX:+EnableDynamicAgentLoading")
    ...
    test()
        .withOpts("-agentpath:" + jvmtiAgentPath1)
        .whenRunning(loadJvmtiAgent1)
        .stderrShouldNotContain(JVMTI_AGENT_WARNING);
  ...
  void testLoadTwoJvmtiAgents(OnAttachAction loadJvmtiAgent1) throws Exception {
    ...
    test()
        .withOpts("-agentpath:" + jvmtiAgentPath2)
        .whenRunning(loadJvmtiAgent1)
  ...
  void testLoadJavaAgent() throws Exception {
    ...
    test().whenRunning(loadJavaAgent).stderrShouldContain(JAVA_AGENT_WARNING);
  ...
  /**
   * Represents an operation that accepts a process identifier and a VirtualMachine that the current
   * JVM is attached to.
   */
  ...
  /**
   * Returns an operation that invokes "jcmd <pid> JVMTI.agent_load <agentpath>" to load the given
   * agent library into the JVM that the current JVM is attached to.
   */
  private static OnAttachAction jcmdAgentLoad(String agentPath) {
    return (pid, vm) -> {
      String[] jcmd =
          JDKToolLauncher.createUsingTestJDK("jcmd")
  ...
  /**
   * Runs an application in its own VM. Once started, it attachs to the VM, runs a set of actions,
   * then checks that the output contains, or does not contain, a string.
   */
  private static class AppRunner {
    ...
    OutputAnalyzer run() throws Exception {
      ...
      try (ServerSocket listener = new ServerSocket()) {
        ...
        OutputAnalyzer outputAnalyzer =
            ProcessTools.executeTestJava(opts).outputTo(System.out).errorTo(System.out);
    ...
    /**
     * Run the application, checking that standard error contains the given number of occurrences of
     * a string.
     */
