...
public class StorePasswords {
      ...
      try {
        ...
        if (algorithm.equals("default PBE algorithm")) {
          ...
          keystore.setEntry(
              ...
              new KeyStore.PasswordProtection(ENTRY_PWD, algorithm, null));
        }
        count++;
        ...
        if (inner instanceof UnrecoverableKeyException) {
          ...
          if (inner2 instanceof InvalidAlgorithmParameterException) {
            ...
            keystore.setEntry(
                "this entry is protected by " + algorithm,
                new KeyStore.SecretKeyEntry(key),
                new KeyStore.PasswordProtection(ENTRY_PWD, algorithm, specWithEightByteSalt));
            count++;
  ...
  private static int recover() throws Exception {
    ...
    for (String algorithm : PBE_ALGORITHMS) {
      ...
      if (key != null) {
        ...
        System.out.println(
            "Recovered user password '" + new String(pwd) + "' (protected by " + algorithm + ")");
  ...
  private static int storeByShell() throws Exception {
    ...
    for (String algorithm : PBE_ALGORITHMS) {
      ...
      String keyAlg = algorithm.equals("default PBE algorithm") ? "" : (" -keyalg " + algorithm);
