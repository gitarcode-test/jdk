...
public class FindSection {
  ...
  /**
   * {@link ManifestDigester.Entry#digestWorkaround} should not feed the trailing blank line into
   * the digester. Before resolution of 8217375 it fed the trailing blank line into the digest if
   * the second line break was at the end of the file due to
   *
   * <pre>
   * if (allBlank || (i == len-1)) {
   *     if (i == len-1)
   ...
   * </pre>
   *
   * in {@link ManifestDigester#findSection}. In that case at the end of the manifest file, {@link
   * ManifestDigester.Entry#digestWorkaround} would have produced the same digest as {@link
   * ManifestDigester.Entry#digest} which was wrong and without effect at best.
   *
   * <p>Once this fix is accepted, this flag can be removed along with {@link
   ...
   */
  static final boolean FIXED_8217375_STARTWITHBLANKLINE_ENDOFSECTION = FIXED_8217375;
  ...
  public static void setFindSectionAccessible() throws Exception {
    ...
    findSection = ManifestDigester.class.getDeclaredMethod("findSection", int.class, Position);
  ...
  Position findSection(byte[] manifestBytes) throws ReflectiveOperationException {
  ...
  Position findSection(String manifestString) throws ReflectiveOperationException {
    byte[] manifestBytes = manifestString.getBytes(UTF_8);
    byte[] manifestWithOffset = new byte[manifestBytes.length + offset];
    System.arraycopy(manifestBytes, 0, manifestWithOffset, offset, manifestBytes.length);
    return findSection(manifestWithOffset);
  }

  /**
   * Surprising, but the offset actually makes a difference in {@link ManifestDigester#findSection}
   * return value.
   */
  ...
  int actualEndOfSection8217375(int correctPosition, boolean eof, int lbl) {
    ...
    } else if (correctPosition == -1 && !FIXED_8217375_STARTWITHBLANKLINE_ENDOFSECTION) {
      return 0;
    } else {
      return correctPosition;
    }
  ...
  void assertPosition(Position pos, int endOfFirstLine, int endOfSection, int startOfNext) {
    AssertionError a = null;
    a =
        collectErrors(
            a, () -> assertEquals(pos.endOfFirstLine, endOfFirstLine + offset, "endOfFirstLine"));
    a =
        collectErrors(
            a, () -> assertEquals(pos.endOfSection, endOfSection + offset, "endOfSection"));
    a = collectErrors(a, () -> assertEquals(pos.startOfNext, startOfNext + offset, "startOfNext"));
    if (a != null) throw a;
  }

  void catchCrCausesIndexOutOfBoundsException(Callable<Position> test, Consumer<Position> asserts) {
    try {
  ...
  public void testOneLineBreakCr() throws Exception {
    catchCrCausesIndexOutOfBoundsException(
        () -> findSection("\r"),
        p -> assertPosition(p, -1, actualEndOfSection8217375(-1, false, 1), 1));
  }
  ...
  public void testSpaceAndLineBreakCr() throws Exception {
    catchCrCausesIndexOutOfBoundsException(
        () -> findSection("   \r"), p -> assertPosition(p, 2, 3, 4));
  }
  ...
  public void testOneLineBreakCrAndSpace() throws Exception {
    assertPosition(findSection("\r   "), -1, actualEndOfSection8217375(-1, false, 1), 1);
  }
  ...
  public void testCrEof() throws Exception {
    catchCrCausesIndexOutOfBoundsException(
        () -> findSection("abc\r"), p -> assertPosition(p, 2, 3, 4));
  }
  ...
  public void testCrCrEof() throws Exception {
    catchCrCausesIndexOutOfBoundsException(
        () -> findSection("abc\r\nxyz\r\r"),
        p -> assertPosition(p, 2, actualEndOfSection8217375(8, true, 1), 10));
  }
  ...
  public void testCrLfCrEof() throws Exception {
    catchCrCausesIndexOutOfBoundsException(
        () -> findSection("abc\r\nxyz\r\n\r"),
        p -> assertPosition(p, 2, actualEndOfSection8217375(9, true, 2), 11));
  }
  ...
  public void testCrLastCausesIndexOutOfBounds() throws Exception {
    catchCrCausesIndexOutOfBoundsException(
        () -> findSection("\r"),
        p -> assertPosition(p, -1, actualEndOfSection8217375(-1, true, 1), 1));
  }
        ...
        AssertionError a =
            new AssertionError(
                "offset = "
                    + offset
                    + ", lb = "
                    + Utils.escapeStringWithNumbers(lb)
                    + ", "
                    + "eof = "
                    + eof);
        ...
        a =
            collectErrors(
                ...
                () ->
                    assertEquals(
                        p.endOfSection,
                        actualEndOfSection8217375(2 + lb.length() + offset, eof, lb.length())));
            ...
            collectErrors(
                ...
                () ->
                    assertPosition(
                        p,
                        2,
                        actualEndOfSection8217375(2 + lb.length(), eof, lb.length()),
                        3 + lb.length() * 2));
      ...
      for (boolean blank : new boolean[] {false, true}) {
        ...
        try {
          ...
          assertPosition(
              p,
              2,
              actualEndOfSection8217375(5 + lb.length() * 2, true, blank ? lb.length() : 0),
              manifest.length());
        } catch (RuntimeException | ReflectiveOperationException e) {
          if ((e instanceof IndexOutOfBoundsException
                  || e.getCause() instanceof IndexOutOfBoundsException)
