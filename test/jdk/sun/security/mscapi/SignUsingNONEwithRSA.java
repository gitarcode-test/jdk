...
public class SignUsingNONEwithRSA {
  ...
  private static final List<byte[]> precomputedHashes =
      Arrays.asList(
          // A MD5 hash
          new byte[] {
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (providers == null) {
      System.out.println("No JCE providers support the " + "'Signature.NONEwithRSA' algorithm");
    ...
    System.out.println("Creating a temporary RSA keypair in the Windows-My store");
    ...
    ks.setKeyEntry(
        ...
        k,
        null,
        new X509Certificate[] {ckg.getSelfCertificate(new X500Name("cn=6578658,c=US"), 1000)});
    ks.store(null, null);
    ...
    try {
      ...
      System.out.println("Deleting temporary RSA keypair from Windows-My store");
  ...
  private static void signAllUsing(String providerName, PrivateKey privateKey) throws Exception {
    ...
    if (sig1.getProvider() != null) {
      System.out.println(
          "Using NONEwithRSA signer from the " + sig1.getProvider().getName() + " JCE provider");
    } else {
      System.out.println("Using NONEwithRSA signer from the internal JCE provider");
    }
      ...
      try {
        ...
        System.out.println(
            "\nGenerated RSA signature over a "
                + hash.length
                + "-byte hash (signature length: "
                + sigBytes.length * 8
                + " bits)");
        System.out.println(
            String.format(
                "0x%0" + (sigBytes.length * 2) + "x", new java.math.BigInteger(1, sigBytes)));
  ...
  private static void verifyAllUsing(String providerName, PublicKey publicKey) throws Exception {
    Signature sig1 = Signature.getInstance("NONEwithRSA", providerName);
    if (sig1.getProvider() != null) {
      System.out.println(
          "\nUsing NONEwithRSA verifier from the "
              + sig1.getProvider().getName()
              + " JCE provider");
    } else {
