...
public class ShortRSAKeyWithinTLS {
  ...
  private void checkKeySize(KeyStore ks) throws Exception {
    ...
    if (ks.containsAlias(keyAlias)) {
      ...
      if (privateKeySize != keySize) {
        throw new Exception(
            "Expected key size is " + keySize + ", but the private key size is " + privateKeySize);
      }

      int publicKeySize = KeyUtil.getKeySize(publicKey);
      if (publicKeySize != keySize) {
        throw new Exception(
            "Expected key size is " + keySize + ", but the public key size is " + publicKeySize);
      }
    }
  }
  ...
  class MyExtendedX509TM extends X509ExtendedTrustManager implements X509TrustManager {
    ...
    public X509Certificate[] getAcceptedIssuers() {
      ...
      try {
        for (X509Certificate c : tm.getAcceptedIssuers()) {
          if (serverKS.getCertificateAlias(c).equals(keyAlias)) certs.add(c);
        }
