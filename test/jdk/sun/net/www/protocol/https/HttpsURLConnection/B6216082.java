...
public class B6216082 {
  ...
  public static void main(String[] args) throws Exception {
    HostnameVerifier reservedHV = HttpsURLConnection.getDefaultHostnameVerifier();
    try {
      ...
      ProxySelector.setDefault(
          ProxySelector.of(new InetSocketAddress(firstNonLoAddress, proxy.getLocalPort())));
      makeHttpCall();
    } finally {
      if (proxy != null) {
        proxy.terminate();
  ...
  public static void makeHttpCall() throws Exception {
    ...
    URL url =
        new URL("https", firstNonLoAddress.getHostAddress(), server.getAddress().getPort(), "/");
    HttpURLConnection uc = (HttpURLConnection) url.openConnection();
    System.out.println(uc.getResponseCode());
    if (uc.getResponseCode() != 200) {
      uc.disconnect();
      throw new RuntimeException(
          "Test failed : bad http request with response code : " + uc.getResponseCode());
    }
    uc.disconnect();
  }
