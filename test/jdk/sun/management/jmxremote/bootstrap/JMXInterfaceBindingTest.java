 ...
 * @test
 * @bug 6425769
 * @summary Test JMX agent host address binding. Same ports but different interfaces to bind to
 *     (selecting plain or SSL sockets at random)
 * @library /test/lib
...
public class JMXInterfaceBindingTest {
  ...
  public static final String KEYSTORE_LOC =
      System.getProperty("test.src", ".") + File.separator + "ssl" + File.separator + "keystore";
  public static final String TRUSTSTORE_LOC =
      System.getProperty("test.src", ".") + File.separator + "ssl" + File.separator + "truststore";
  ...
  public static void main(String[] args) {
    ...
    if (addrs.isEmpty()) {
      System.out.println(
          "Ignoring test since no non-loopback IPs are available to bind to "
  ...
  private static class TestProcessThread extends Thread {
    ...
    public void run() {
      ...
      if (testFailed) {
        ...
        if (needRetry) {
          System.err.println(
              "Test FAILURE on " + name + " reason: run out of retries to pick free ports");
          ...
          System.out.println(
              "Test FAILURE on "
                  + name
                  + " reason: The expected line \""
                  + READY_MSG
                  + "\" is not present in the process output");
        } else {
          System.err.println(
              "Test FAILURE on " + name + " reason: Unexpected exit code => " + exitValue);
        }
    ...
    private Process createTestProcess() {
      ...
      String msg =
          String.format(
              "DEBUG: Launching java tester for triplet (HOSTNAME,JMX_PORT,RMI_PORT)"
                  + " == (%s,%d,%d)",
              address, jmxPort, rmiPort);
      ...
      try {
        ProcessBuilder builder =
            ProcessTools.createTestJavaProcessBuilder(args.toArray(new String[] {}));
    ...
    private boolean runTest() {
      ...
      if (output.getExitValue() == STOP_PROCESS_EXIT_VAL
          && output.getStdout().contains(READY_MSG)) {
        testFailed = false;
      } else if (output.getStderr().contains("Port already in use")) {
        System.out.println(
            "The test attempt for the test " + name + " failed due to the bind error");
        // Need to retry
        return true;
      }
      return false;
