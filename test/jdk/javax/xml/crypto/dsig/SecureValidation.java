 ...
 * @test
 * @bug 8241306
 * @summary Tests for the jdk.xml.dsig.secureValidationPolicy security property on the RSASSA-PSS
 *     signature method
 * @library /test/lib
...
public class SecureValidation {
  ...
  public static void main(String[] args) throws Exception {
    ...
    PSSParameterSpec pspec =
        new PSSParameterSpec("SHA-384", "MGF1", MGF1ParameterSpec.SHA512, 48, TRAILER_FIELD_BC);

    // Sign with PSS with SHA-384 and SHA-512
    Document signed =
        XMLUtils.signer(privateKey, cert)
    ...
    Utils.runAndCheckException(
        () -> validator.secureValidation(true).validate(signed),
        t ->
            Asserts.assertTrue(
                t instanceof MarshalException
                    && t.getMessage().contains("in MGF1")
                    && t.getMessage().contains(DigestMethod.SHA256),
                Objects.toString(t)));
