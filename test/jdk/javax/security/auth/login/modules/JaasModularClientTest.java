...
public class JaasModularClientTest {
  ...
  /**
   * Here is the naming convention followed. l.jar - Unnamed login module jar. ml.jar - Modular
   * login module jar. msl.jar - Modular login module jar provides login module service through
   * module-info c.jar - Unnamed client jar. mc.jar - Modular client jar. mcs.jar - Modular client
   * jar uses login module service through module-info. amc.jar - Modular client used for automatic
   * login module jar. amcs.jar - Modular client used for automatic login module jar and uses login
   * module service through module-info.
   */
  ...
  public JaasModularClientTest(boolean service) {
    ...
    System.out.printf(
        "%n*** Login Module defined as service in " + "module-info: %s ***%n%n", service);
    List<String> argList = new LinkedList<>();
    argList.add("-Djava.security.auth.login.config=" + toAbsPath(SRC.resolve("jaas.conf")));
  ...
  private void process() throws Exception {
    ...
    execute(String.format("--module-path %s%s%s -m mc/%s", modC, PS, modL, C_TYPE));
    System.out.println("Case: Modular Client and automatic Login module.");
    execute(
        String.format("--module-path %s%s%s --add-modules=l -m mc/%s", autoMC, PS, unnL, C_TYPE));
    System.out.println("Case: Modular Client and unnamed Login module.");
    execute(String.format("--module-path %s -cp %s -m mc/%s", autoMC, unnL, C_TYPE));
    ...
    execute(String.format("--module-path %s%s%s --add-modules=ml -m c/%s", unnC, PS, modL, C_TYPE));
    System.out.println("Case: Automatic Client and automatic Login module");
    execute(String.format("--module-path %s%s%s --add-modules=l -m c/%s", unnC, PS, unnL, C_TYPE));
    System.out.println("Case: Automatic Client and unnamed Login module.");
    execute(String.format("--module-path %s -cp %s -m c/%s", unnC, unnL, C_TYPE));

    // Case: UNNAMED-NAMED, UNNAMED-AUTOMATIC, UNNAMED-UNNAMED
    System.out.println("Case: Unnamed Client and modular Login module.");
    execute(String.format("-cp %s --module-path %s --add-modules=ml %s", unnC, modL, C_TYPE));
    System.out.println("Case: Unnamed Client and automatic Login module.");
    execute(String.format("-cp %s --module-path %s --add-modules=l %s", unnC, unnL, C_TYPE));
    ...
    System.out.println("Case: Unnamed Client and Login module from modulepath.");
    execute(String.format("--module-path %s%s%s --add-modules=l -m c/%s", unnC, PS, unnL, C_TYPE));
    System.out.println("Case: Modular Client and Login module in classpath.");
  ...
  private static void setUp() throws Exception {
    ...
    JarUtils.createJarFile(L_JAR, TEST_CLASSES, "login/TestLoginModule.class");
    // Generate unnamed client jar.
    JarUtils.createJarFile(
        C_JAR,
        TEST_CLASSES,
        "client/JaasClient.class",
        "client/JaasClient$MyCallbackHandler.class");

    Builder mBuilder = ModuleDescriptor.newModule("ml").requires("jdk.security.auth");
    // Modular jar exports package to let the login module type accessible.
    generateJar(L_JAR, ML_JAR, mBuilder.exports("login").build());

    mBuilder =
        ModuleDescriptor.newModule("ml")
            .requires("jdk.security.auth")
            .provides("javax.security.auth.spi.LoginModule", Arrays.asList(L_TYPE));
    ...
    mBuilder = ModuleDescriptor.newModule("mc").exports("client").requires("jdk.security.auth");
  ...
  private static void generateJar(Path sjar, Path djar, ModuleDescriptor mDesc) throws Exception {
