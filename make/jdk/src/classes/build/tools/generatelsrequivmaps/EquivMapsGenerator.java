 ...
 * http://www.iana.org/assignments/language-subtag-registry, which is specified in the command line
 * and generates a .java source file as specified in command line. The generated .java source file
 * contains equivalent language maps. These equivalent language maps are used by LocaleMatcher.java
 * for the locale matching mechanism specified in RFC 4647 "Matching of Language Tags".
 */
public class EquivMapsGenerator {
  ...
  private static Map<String, StringBuilder> initialLanguageMap = new TreeMap<>();
  private static Map<String, StringBuilder> initialRegionVariantMap = new TreeMap<>();
  ...
  private static Map<String, String> sortedRegionVariantMap = new TreeMap<>();
  ...
  private static void processDeprecatedData(
      String type, String tag, String preferred, String prefix) {
    StringBuilder sb;
  ...
  private static void generateEquivalentMap() {
    ...
    for (String preferred : initialLanguageMap.keySet()) {
      ...
      subtags =
          Arrays.stream(initialLanguageMap.get(preferred).toString().split(","))
              .distinct()
              .toList()
    ...
    for (String preferred : initialRegionVariantMap.keySet()) {
      subtags = initialRegionVariantMap.get(preferred).toString().split(",");
  ...
  /**
   * The input lsr data file is in UTF-8, so theoretically for the characters beyond US-ASCII, the
   * generated Java String literals need to be Unicode escaped (\\uXXXX) while writing to a file.
   * But as of now, there is not the case since we don't use "description", "comment" or alike.
   */
  ...
  private static String getOpenJDKCopyright() {
    return String.format(
        Locale.US, (jdkHeaderTemplate != null ? jdkHeaderTemplate : COPYRIGHT), copyrightYear);
  }
  ...
  private static String getMapsText() {
    return """
           ...
           """
        .formatted(
            sortedLanguageMap1.size(), sortedLanguageMap2.size(), sortedRegionVariantMap.size());
  }
  ...
  private static void writeEquiv(BufferedWriter writer, String name, Map<String, String> map)
      throws IOException {
    for (String key : map.keySet()) {
      String value = map.get(key);
      writer.write(String.format("        %s.put(\"%s\", \"%s\");", name, key, value));
      writer.newLine();
    }
  }
