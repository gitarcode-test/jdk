      ...
      for (String key : map.keySet()) {
        if (useJava) {
          ...
          if (value == null) {
            ...
            if (type == BundleType.TIMEZONE && !key.startsWith(CLDRConverter.EXEMPLAR_CITY_PREFIX)
  ...
  public void generateMetaInfo(Map<String, SortedSet<String>> metaInfo) throws IOException {
    String dirName =
        CLDRConverter.DESTINATION_DIR
            ...
            + (CLDRConverter.isBaseModule
                ? "cldr" + File.separator + File.separator
                : "resources"
                    + File.separator
                    + "cldr"
                    + File.separator
    ...
    try (PrintWriter out = new PrintWriter(file, "us-ascii")) {
      ...
      out.printf(
          ...
          """,
          CLDRConverter.isBaseModule ? "cldr" : "resources.cldr.provider", className);
...
"""
        ...
        for (String key : metaInfo.keySet()) {
          if (key.startsWith(CLDRConverter.PARENT_LOCALE_PREFIX)) {
            ...
            if ("root".equals(parentTag)) {
              ...
              out.printf(
                  "        parentLocalesMap.put(Locale.forLanguageTag(\"%s\"),\n", parentTag);
            }
        ...
        CLDRConverter.handlerSupplMeta
            .getLanguageAliasData()
            .forEach(
                (key, value) -> {
...
""",
            ...
            metaInfo.keySet().stream()
                .filter(k -> k.startsWith(CLDRConverter.LIKELY_SCRIPT_PREFIX))
                .count());
...
""");
  ...
  private static void generateStringArray(SortedSet<String> set, PrintWriter out)
      throws IOException {
