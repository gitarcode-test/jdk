...
/**
 * Generate the DOT file for a module graph for each module in the JDK after transitive reduction.
 */
public class GenGraphs {
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (ModuleReference mref : finder.findAll()) {
      ...
      if (genGraphs.accept(name, mref.descriptor())) {
        configurations.put(
            name, Configuration.empty().resolve(finder, ModuleFinder.of(), Set.of(name)));
      }
    }
  ...
  static class ModuleGraphAttributes extends ModuleDotGraph.DotGraphAttributes {
    ...
    public int weightOf(String s, String t) {
      ...
      if (w != 1) return w;
      if (s.startsWith("java.") && t.startsWith("java.")) return 10;
      return 1;
    }
  ...
  /**
   * Returns true for any name if generating graph for non-spec; otherwise, returns true except
   * "jdk" and name with "jdk.internal." prefix
   */
