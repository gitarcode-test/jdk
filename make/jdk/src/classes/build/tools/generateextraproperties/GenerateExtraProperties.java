...
/**
 * Parses extra properties files of UCD, and replaces the placeholders in the given template source
 * file with the generated conditions, then emits .java files. For example, if the properties file
 * has:
 *
 * <blockquote>
 *
 * 0009..000D ; Type (; Value) 0020 ; Type (; Value) 2000..200A ; Type (; Value)
 *
 * </blockquote>
 *
 ...
 * <blockquote>
 *
 * (cp >= 0x0009 && cp <= 0x000D) || cp == 0x0020 || (cp >= 0x2000 && cp <= 0x200A);
 *
 * </blockquote>
 *
 * Note that those in parentheses in the properties file and the template file are optional.
 *
 * <p>Arguments to this utility: args[0]: Full path string to the template file args[1]: Full path
 * string to the properties file args[2]: Full path string to the generated .java file args[3...]:
 * Names of the property to generate the conditions
 */
public class GenerateExtraProperties {
  ...
  static class Range implements Comparable<Range> {
    ...
    Range(String input) {
      ...
      last =
          input.contains("..")
              ? toInt(input.replaceFirst(".*\\.\\.", "").replaceFirst(";.*", "").trim())
              : start;
    }
