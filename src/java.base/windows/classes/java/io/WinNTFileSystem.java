...
final class WinNTFileSystem extends FileSystem {
  ...
  public String normalize(String path) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (c == altSlash) return normalize(path, n, (prev == slash) ? i - 1 : i);
      if ((c == slash) && (prev == slash) && (i > 1)) return normalize(path, n, i - 1);
      if ((c == ':') && (i > 1)) return normalize(path, n, 0);
      prev = c;
    }
  ...
  */
  ...
  public int prefixLength(String path) {
    ...
    if (isLetter(c0) && (c1 == ':')) {
      if ((n > 2) && (path.charAt(2) == slash)) return 3; /* Absolute local pathname "z:\\foo" */
  ...
  public String resolve(String parent, String child) {
    ...
    if ((cn > 1) && (c.charAt(0) == slash)) {
      ...
      if (cn == childStart) { // Child is double slash
        if (parent.charAt(pn - 1) == slash) return parent.substring(0, pn - 1);
        return parent;
      }
    }

    if (parent.charAt(pn - 1) == slash) parentEnd--;
  ...
  public String fromURIPath(String path) {
    ...
    if ((p.length() > 2) && (p.charAt(2) == ':')) {
      ...
      p = p.substring(1);
      // "c:/foo/" --> "c:/foo", but "c:/" --> "c:/"
      if ((p.length() > 3) && p.endsWith("/")) p = p.substring(0, p.length() - 1);
  ...
  public boolean isAbsolute(File f) {
    ...
    return (((pl == 2) && (f.getPath().charAt(0) == slash)) || (pl == 3));
  }

  @Override
  public boolean isInvalid(File f) {
    if (f.getPath().indexOf('\u0000') >= 0) return true;
    ...
    if (lastColon < 0 || (lastColon == 1 && isLetter(pathname.charAt(0)))) return false;
  ...
  public String resolve(File f) {
    ...
    int pl = f.getPrefixLength();
    if ((pl == 2) && (path.charAt(0) == slash)) return path; /* UNC */
