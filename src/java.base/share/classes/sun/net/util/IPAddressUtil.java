...
public class IPAddressUtil {
  ...
  /**
   * Validates if input string is a valid IPv4 address literal. If the
   * "jdk.net.allowAmbiguousIPAddressLiterals" system property is set to {@code false}, or is not
   * set then validation of the address string is performed as follows: If string can't be parsed by
   * following IETF IPv4 address string literals formatting style rules (default one), but can be
   * parsed by following BSD formatting style rules, the IPv4 address string content is treated as
   * ambiguous and either {@code IllegalArgumentException} is thrown, or {@code null} is returned.
   *
   * @param src input string
   * @param throwIAE {@code true} - throw {@code IllegalArgumentException} when cannot be parsed as
   *     IPv4 address string; {@code false} - throw {@code IllegalArgumentException} only when IPv4
   *     address string is ambiguous.
   ...
   *     {@code false}, IPv4 address string {@code src} is ambiguous, or when address string cannot
   *     be parsed as an IPv4 address string and {@code throwIAE} is set to {@code true}.
   */
  ...
  /**
   * Mapping from unscoped local Inet(6)Address to the same address including the correct scope-id,
   * determined from NetworkInterface.
   */
  private static final ConcurrentHashMap<InetAddress, InetAddress> cache =
      new ConcurrentHashMap<>();

  /**
   * Returns a scoped version of the supplied local, link-local ipv6 address if that scope-id can be
   * determined from local NetworkInterfaces. If the address already has a scope-id or if the
   * address is not local, ipv6 or link local, then the original address is returned.
   *
   * @param address
   * @exception SocketException if the given ipv6 link local address is found on more than one local
   *     interface
   * @return
   */
    ...
    try {
      ...
      if (sz > 1)
        throw new UncheckedIOException(
            new SocketException("Duplicate link local addresses: must specify scope-id"));
      return result.get(0);
  ...
  private static final char[] OTHERS = {
    8263, 8264, 8265, 8448, 8449, 8453, 8454, 10868, 65109, 65110, 65119, 65131, 65283, 65295,
    65306, 65311, 65312
  };
  ...
  public static String checkUserInfo(String str) {
    ...
    int index = scan(str, MASKS.L_USERINFO_MASK, MASKS.H_USERINFO_MASK);
    if (index >= 0) {
  ...
  private static String checkHost(String str) {
    ...
    if (str.startsWith("[") && str.endsWith("]")) {
      ...
      if (isIPv6LiteralAddress(str)) {
        index = str.indexOf('%');
        if (index >= 0) {
          index = scan(str = str.substring(index), MASKS.L_SCOPE_MASK, MASKS.H_SCOPE_MASK);
          if (index >= 0) {
      ...
      index =
          scan(str, L_EXCLUDE | MASKS.L_HOSTNAME_MASK, H_EXCLUDE | MASKS.H_HOSTNAME_MASK, OTHERS);
      if (index >= 0) {
  ...
  public static String checkExternalForm(URL url) {
    ...
    if (earlyURLParsing) {
      ...
      int index = scan(s = userInfo, L_NON_PRINTABLE | L_SLASH, H_NON_PRINTABLE | H_SLASH);
      if (index >= 0) {
  ...
  public static String checkHostString(String host) {
    ...
    if (earlyURLParsing()) {
      ...
      int index = scan(host, MASKS.L_HOSTNAME_MASK, MASKS.H_HOSTNAME_MASK, OTHERS);
      if (index >= 0) {
   ...
   * @return {@code true} if input string is parsable as IPv4 address literal, {@code false}
   *     otherwise.
   */
   ...
   * @return a byte array representing the IPv4 numeric address if input string is a parsable POSIX
   *     formatted IPv4 address literal, {@code null} otherwise.
   */
   ...
   *
   * <p>This method supports three {@code "radix"} values to decode field values in {@code
   * "HEXADECIMAL (radix=16)"}, {@code "DECIMAL (radix=10)"} and {@code "OCTAL (radix=8)"} radixes.
   *
   * <p>If {@code -1} value is returned the char buffer position is reset to the value it was before
   * it was called.
   *
   * <p>Method returns {@code -2} if formatting illegal for all supported {@code radix} values is
   * observed, and there is no point in checking other radix values. That includes the following
   * cases:
   *
   * <ul>
   ...
   *   <li>Character is not a valid digit for the requested {@code radix} value, given that a field
   *       has the radix specific prefix
   * </ul>
   *
   ...
   *     {@code TERMINAL_PARSE_ERROR} if field can't be parsed and the whole parse process should be
   *     terminated. Parsed field value otherwise.
   */
  ...
  private static final String ALLOW_AMBIGUOUS_IPADDRESS_LITERALS_SP =
      "jdk.net.allowAmbiguousIPAddressLiterals";
  ...
  private static class MASKS {
    ...
    static {
      var value = GetPropertyAction.privilegedGetProperty(DELAY_URL_PARSING_SP, "false");
      DELAY_URL_PARSING_SP_VALUE = value.isEmpty() || Boolean.parseBoolean(value);
      if (DELAY_URL_PARSING_SP_VALUE) {
        ...
        L_SCOPE_MASK =
            L_NON_PRINTABLE | L_IPV6_DELIMS | L_SLASH | L_BACKSLASH | L_FRAGMENT | L_QUERY;
        H_SCOPE_MASK =
            H_NON_PRINTABLE | H_IPV6_DELIMS | H_SLASH | H_BACKSLASH | H_FRAGMENT | H_QUERY;
      }
    }
  }
}
