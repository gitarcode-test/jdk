...
public class LocaleResources {
  ...
  private static final Pattern VALID_SKELETON_PATTERN =
      Pattern.compile(
          "(?<date>"
              + "G{0,5}"
              + // Era
  ...
  public String getCalendarData(String key) {
    ...
    if (data == null || ((caldata = (String) data.get()) == null)) {
      ...
      cache.put(cacheKey, new ResourceReference(cacheKey, caldata, referenceQueue));
    }
  ...
  public Object[] getDecimalFormatSymbolsData() {
    ...
    if (data == null || ((dfsdata = (Object[]) data.get()) == null)) {
      ...
      cache.put(
          DECIMAL_FORMAT_SYMBOLS_DATA_CACHEKEY,
    ...
    if (Objects.isNull(data) || Objects.isNull(val = data.get())) {
      ...
      if (key.startsWith(TZNB_EXCITY_PREFIX)) {
        ...
        if (names != null) {
          names[0] = key;
          trace(
              "tznb: %s key: %s, names: %s, %s, %s, %s, %s, %s, %s\n",
  ...
  Set<String> getZoneIDs() {
    ...
    if (data == null || ((zoneIDs = (Set<String>) data.get()) == null)) {
      ...
      zoneIDs = rb.keySet();
      cache.put(
          ZONE_IDS_CACHEKEY, new ResourceReference(ZONE_IDS_CACHEKEY, zoneIDs, referenceQueue));
    }
  ...
  String[][] getZoneStrings() {
    ...
    if (type == LocaleProviderAdapter.Type.CLDR) {
      ...
      tzIds.stream()
          .filter(
              i -> (!i.startsWith("Etc/GMT") && !i.startsWith("GMT") && !i.startsWith("SystemV")))
          .forEach(
              tzid -> {
                ...
                if (keyset.contains(tzid)) {
                  ...
                  var canonID = TimeZoneNameUtility.canonicalTZID(tzid).orElse(tzid);
  ...
  String[] getCalendarNames(String key) {
    ...
    if (data == null || ((names = (String[]) data.get()) == null)) {
      ...
      if (rb.containsKey(key)) {
        ...
        cache.put(cacheKey, new ResourceReference(cacheKey, names, referenceQueue));
      }
    }
   ...
   *
   * @param timeStyle style of time; one of FULL, LONG, MEDIUM, SHORT in DateFormat, or -1 if not
   *     required
   * @param dateStyle style of time; one of FULL, LONG, MEDIUM, SHORT in DateFormat, or -1 if not
   *     required
  ...
  private String getDateTimePattern(String prefix, int timeStyle, int dateStyle, String calType) {
    ...
    if (timeStyle >= 0) {
      if (dateStyle >= 0) {
        ...
        pattern =
            switch (Objects.requireNonNull(dateTimePattern)) {
              case "{1} {0}" -> datePattern + " " + timePattern;
              case "{0} {1}" -> timePattern + " " + datePattern;
              default ->
                  MessageFormat.format(
                      dateTimePattern.replaceAll("'", "''"), timePattern, datePattern);
            };
      } else {
        pattern = timePattern;
      }
  ...
  /**
   * Returns the FormatData resource bundle of this LocaleResources. The FormatData should be used
   * only for accessing extra resources required by JSR 310.
   */
  ...
  /**
   * Returns the actual format pattern string based on the requested template and calendar type for
   * this locale.
   *
  ...
  private String getLocalizedPatternImpl(String requestedTemplate, String calType) {
    ...
    if (!matcher.matches()) {
      throw new IllegalArgumentException(
          "Requested template \"%s\" is invalid".formatted(requestedTemplate)
              + (requestedTemplate.equals(skeleton)
                  ? "."
                  : ", which translated into \"%s\"".formatted(skeleton)
    ...
    if (matched == null) {
      ...
      if (dateMatched != null && timeMatched != null) {
        ...
        var style =
            switch (requestedTemplate.replaceAll("[^M]+", "").length()) {
        ...
        matched =
            MessageFormat.format(dateTimePattern.replaceAll("'", "''"), timeMatched, dateMatched);
      }
    }

    trace(
        "requested: %s, locale: %s, calType: %s, matched: %s\n",
  ...
  private void initSkeletonIfNeeded() {
          ...
          .forEach(
              type -> {
                ...
                if (r.containsKey(inputRegionsKey)) {
                  p.matcher(r.getString(inputRegionsKey))
                      .results()
                      .forEach(
                          mr ->
    ...
    if (jPattern == null) {
      ...
      // hack: "allowed" contains reversed order for hour/period, e.g, "hB" which should be "Bh" as
      // a skeleton
  ...
  /**
   * Resolve locale specific input skeletons. Fall back method is different from usual resource
   * bundle's, as it has to be "lang-region" -> "region" -> "lang-001" -> "001"
   *
  ...
  private String resolveInputSkeleton(String type) {
    ...
    return regionToSkeletonMap.getOrDefault(
        locale.getLanguage() + "-" + locale.getCountry(),
        regionToSkeletonMap.getOrDefault(
            locale.getCountry(),
            regionToSkeletonMap.getOrDefault(
                locale.getLanguage() + "-001", regionToSkeletonMap.getOrDefault("001", "h"))));
  }

  /**
   * Replace 'j' and 'C' input skeletons with locale specific patterns. Note that 'j' is guaranteed
   * to be replaced with one char [hkHK], while 'C' may be replaced with multiple chars. Repeat each
   * as much as 'C' count.
   *
  ...
  /**
   * Inferring the possible format styles in priority order, based on the original skeleton length.
   *
  ...
  private String getDateTimePattern(
      String prefix, String key, int styleIndex, String calendarType) {
    ...
    if (data == null || ((value = data.get()) == null)) {
      ResourceBundle r =
          (prefix != null) ? getJavaTimeFormatData() : localeData.getDateFormatData(locale);
  ...
  public String[] getListPatterns(ListFormat.Type type, ListFormat.Style style) {
    ...
    if (data == null || ((lpArray = (String[]) data.get()) == null)) {
      var rbKey =
          "ListPatterns_" + typeStr + (style == ListFormat.Style.FULL ? "" : "-" + styleStr);
