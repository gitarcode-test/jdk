...
public final class Main {
  ...
  private static final DisabledAlgorithmConstraints LEGACY_CHECK =
      new DisabledAlgorithmConstraints(DisabledAlgorithmConstraints.PROPERTY_SECURITY_LEGACY_ALGS);
  ...
  enum Command {
    ...
    STOREPASSWD(
        ...
        PROVIDERPATH,
        V),
    SHOWINFO("showinfo.command.help", TLS, V),
    ...
    public static Command getCommand(String cmd) {
      for (Command c : Command.values()) {
        if (collator.compare(cmd, c.name) == 0
            || (c.altName != null && collator.compare(cmd, c.altName) == 0)) {
          return c;
        }
      }
      return null;
  ...
  private static final java.util.ResourceBundle rb =
      java.util.ResourceBundle.getBundle("sun.security.tools.keytool.Resources");
  ...
  private static class ExitException extends RuntimeException {
    @java.io.Serial static final long serialVersionUID = 0L;
  ...
  String[] parseArgs(String[] args) throws Exception {
        ...
        if (optionsSet.contains(lowerFlags)) {
          switch (lowerFlags) {
            ...
            case "-providerarg":
              ...
              break;
            default:
              weakWarnings.add(String.format(rb.getString("option.1.set.twice"), lowerFlags));
          }
        } else {
        ...
        if (collator.compare(flags, "-conf") == 0) {
          ...
          if (c != null) {
            if (command == null) {
              ...
              throw new Exception(
                  String.format(rb.getString("multiple.commands.1.2"), command.name, c.name));
            }
          }
        }
      }
    ...
    if (confFile != null && command != null) {
      args =
          KeyStoreUtil.expandArgs(
              "keytool", confFile, command.toString(), command.getAltName(), args);
    }
    ...
    for (i = 0; (i < args.length) && args[i].startsWith("-"); i++) {
      ...
      if (c != null) {
        ...
        if (new File(ksfname)
            .getCanonicalPath()
            .equals(new File(KeyStoreUtil.getCacerts()).getCanonicalPath())) {
  ...
  void doCommands(PrintStream out) throws Exception {
    ...
    if (token && !nullStream) {
      System.err.println(
          MessageFormat.format(
              rb.getString(".keystore.must.be.NONE.if.storetype.is.{0}"), storetype));
      System.err.println();
      tinyHelp();
    }
    ...
    if (token && (command == KEYPASSWD || command == STOREPASSWD)) {
      throw new UnsupportedOperationException(
          MessageFormat.format(
              rb.getString(".storepasswd.and.keypasswd.commands.not.supported.if.storetype.is.{0}"),
              storetype));
    }
    ...
    if (token && (keyPass != null || newPass != null || destKeyPass != null)) {
      throw new IllegalArgumentException(
          MessageFormat.format(
              rb.getString(".keypass.and.new.can.not.be.specified.if.storetype.is.{0}"),
              storetype));
    }
    ...
    if (providerClasses != null) {
      ...
      for (Pair<String, String> provider : providerClasses) {
        try {
          ...
          throw new Exception(
              String.format(rb.getString("provclass.not.a.provider"), provider.fst));
        } catch (IllegalArgumentException e) {
          throw new Exception(
              String.format(rb.getString("provider.class.not.found"), provider.fst), e.getCause());
        }
      }
    }
    ...
    if (ksfile != null && ksStream != null && providerName == null && !inplaceImport) {
      // existing keystore
      if (storetype == null) {
        ...
        if (command == CERTREQ
          ...
          do {
            ...
            if (!nullStream && (storePass == null || storePass.length < 6)) {
              System.err.println(
                  rb.getString("Keystore.password.is.too.short.must.be.at.least.6.characters"));
              storePass = null;
            }
    ...
    if (command != IMPORTCERT && command != PRINTCERT && command != PRINTCRL) {
      trustcacerts = false;
    }
    ...
    if (command == CERTREQ) {
      ...
      if (verbose && filename != null) {
        MessageFormat form =
            new MessageFormat(rb.getString("Certification.request.stored.in.file.filename."));
      ...
      if (filename != null) {
        MessageFormat form =
            new MessageFormat(rb.getString("Certificate.stored.in.file.filename."));
        Object[] source = {filename};
        System.err.println(form.format(source));
      }
    } else if (command == GENKEYPAIR) {
      if (keyAlgName == null) {
        throw new Exception(rb.getString("keyalg.option.missing.error"));
      }
      doGenKeyPair(alias, dname, keyAlgName, keysize, groupName, sigAlgName, signerAlias);
      kssave = true;
    ...
    if (isKeyStoreRelated(command) && !token && !nullStream && ksfname != null) {
      ...
      if (f.exists()) {
        ...
        try {
          ...
          if (realType.equalsIgnoreCase("JKS") || realType.equalsIgnoreCase("JCEKS")) {
            ...
            for (String a : Collections.list(keyStore.aliases())) {
              if (!keyStore.entryInstanceOf(a, TrustedCertificateEntry.class)) {
                allCerts = false;
                break;
              }
            }
            ...
            if (!allCerts) {
              weakWarnings.add(
                  String.format(rb.getString("jks.storetype.warning"), realType, ksfname));
            }
          }
        ...
        if (inplaceImport) {
          ...
          try {
            realSourceStoreType =
                KeyStore.getInstance(new File(inplaceBackupName), srcstorePass).getType();
          } catch (KeyStoreException e) {
          ...
          weakWarnings.add(
              String.format(format, srcksfname, realSourceStoreType, inplaceBackupName, realType));
        }
      }
    }
  }
  ...
  /**
   * Generate a certificate: Read PKCS10 request from in, and print certificate to out. Use alias as
   * CA, sigAlgName as the signature type.
   */
  private void doGenCert(String alias, String sigAlgName, InputStream in, PrintStream out)
    ...
    CertPathConstraintsParameters cpcp =
        new CertPathConstraintsParameters(req.getSubjectPublicKeyInfo(), null, null, null);
    ...
    CertificateExtensions ext =
        createV3Extensions(reqex, null, v3ext, subjectPubKey, signerSubjectKeyId);
    info.setExtensions(ext);
    X509CertImpl cert = X509CertImpl.newSigned(info, privateKey, sigAlgName);
    dumpCert(cert, out);
    ...
    checkWeakConstraint(rb.getString("the.issuer"), keyStore.getCertificateChain(alias));
    cpcp = buildCertPathConstraint(cert, null);
    checkWeakConstraint(rb.getString("the.generated.certificate"), cert, cpcp);
  }
  ...
  private void doGenCRL(PrintStream out) throws Exception {
    ...
    CertPathConstraintsParameters cpcp =
        new CertPathConstraintsParameters(privateKey, null, null, null);
    checkWeakConstraint(rb.getString("the.generated.crl"), crl, privateKey, cpcp);
  }

  /**
   * Creates a PKCS#10 cert signing request, corresponding to the keys (and name) associated with a
   * given alias.
   */
  private void doCertReq(String alias, String sigAlgName, PrintStream out) throws Exception {
    ...
    CertPathConstraintsParameters cpcp =
        new CertPathConstraintsParameters(request.getSubjectPublicKeyInfo(), null, null, null);
    checkWeakConstraint(rb.getString("the.generated.certificate.request"), request, cpcp);
  }
  ...
  private void doExportCert(String alias, PrintStream out) throws Exception {
    ...
    dumpCert(cert, out);
    CertPathConstraintsParameters cpcp = buildCertPathConstraint(cert, null);
    ...
    if (origPass != null && P12KEYSTORE.equalsIgnoreCase(storetype)) {
      ...
      for (count = 0; count < 3; count++) {
        ...
        if (entered == null && origPass != null) {
          ...
          System.err.println(
              rb.getString("Key.password.is.too.short.must.be.at.least.6.characters"));
        }
      }
      if (count == 3) {
        if (command == KEYCLONE) {
  ...
  private void doGenSecretKey(String alias, String keyAlgName, int keysize) throws Exception {
    ...
    if (keyStore.containsAlias(alias)) {
      MessageFormat form =
          new MessageFormat(rb.getString("Secret.key.not.generated.alias.alias.already.exists"));
      Object[] source = {alias};
      throw new Exception(form.format(source));
    }
    ...
    if (keyAlgName.toUpperCase(Locale.ENGLISH).startsWith("PBE")) {
      ...
      SecretKeyConstraintsParameters skcp = new SecretKeyConstraintsParameters(secKey);
      checkWeakConstraint(rb.getString("the.generated.secretkey"), keyAlgName, skcp);
      ...
      MessageFormat form =
          new MessageFormat(rb.getString("Generated.keysize.bit.keyAlgName.secret.key"));
      Object[] source = {keysize, secKey.getAlgorithm()};
      System.err.println(form.format(source));

      SecretKeyConstraintsParameters skcp = new SecretKeyConstraintsParameters(secKey);
      checkWeakConstraint(rb.getString("the.generated.secretkey"), secKey, skcp);
    }
    ...
    if (useDefaultPBEAlgorithm) {
      ...
      keyStore.setEntry(
          alias,
          new KeyStore.SecretKeyEntry(secKey),
          new KeyStore.PasswordProtection(keyPass, keyAlgName, null));
    }
  }

  /**
   * If no signature algorithm was specified at the command line, we choose one that is compatible
  ...
  private void doGenKeyPair(
    ...
    if (groupName != null) {
      ...
      if (keysize == -1) {
        ...
        if ("EC".equalsIgnoreCase(keyAlgName)) {
          weakWarnings.add(
              String.format(rb.getString("deprecate.keysize.for.ec"), ecGroupNameForSize(keysize)));
        }
      }
    }
    ...
    if (keyStore.containsAlias(alias)) {
      MessageFormat form =
          new MessageFormat(rb.getString("Key.pair.not.generated.alias.alias.already.exists"));
      Object[] source = {alias};
      throw new Exception(form.format(source));
    }
    ...
    if (signerAlias != null) {
      ...
      keypair =
          new CertAndKeyGen(
              keyAlgName, sigAlgName, providerName, signerPrivateKey, signerSubjectName);
    ...
    CertificateExtensions ext =
        createV3Extensions(null, null, v3ext, keypair.getPublicKeyAnyway(), signerSubjectKeyId);
    ...
    checkWeakConstraint(rb.getString("the.generated.certificate"), finalChain);
  ...
  /**
   * Imports a JDK 1.1-style identity database. We can only store one certificate per identity,
   * because we use the identity's name as the alias (which references a keystore entry), and
  ...
  private void doPrintEntry(String label, String alias, PrintStream out) throws Exception {
    ...
    if (keyStore.entryInstanceOf(alias, KeyStore.SecretKeyEntry.class)) {
      ...
      if (chain != null) {
        if (verbose || rfc || debug) {
          ...
          for (int i = 0; i < chain.length; i++) {
            ...
            if (i == 0 && ((X509Certificate) chain[0]).getBasicConstraints() == -1) {
              // this is an EE
              cpcp = buildCertPathConstraint((X509Certificate) chain[i], anchor);
            } else {
              cpcp =
                  new CertPathConstraintsParameters((X509Certificate) chain[i], null, anchor, null);
            }
        ...
        out.println(rb.getString("Certificate.chain.length.") + 0);
      }
      ...
      if (verbose && (cert instanceof X509Certificate)) {
        ...
        out.println(
            rb.getString("Certificate.fingerprint.SHA.256.") + getCertFingerPrint("SHA-256", cert));
      }
  ...
  boolean inplaceImportCheck() throws Exception {
    ...
    if (srcksfname != null) {
      ...
      if (srcksfile.getCanonicalFile().equals(new File(ksfname).getCanonicalFile())) {
        return true;
      } else {
        // Informational, especially if destkeystore is not
        // provided, which default to ~/.keystore.
        System.err.println(
            String.format(rb.getString("importing.keystore.status"), srcksfname, ksfname));
        return false;
      }
    } else {
  ...
  KeyStore loadSourceKeyStore() throws Exception {
    ...
    if (P11KEYSTORE.equalsIgnoreCase(srcstoretype)
      ...
      if (!NONE.equals(srcksfname)) {
        System.err.println(
            MessageFormat.format(
                rb.getString(".keystore.must.be.NONE.if.storetype.is.{0}"), srcstoretype));
        System.err.println();
        tinyHelp();
      }
    } else {
    ...
    if (srcstorePass == null
      ...
      System.err.println(
          rb.getString(".The.integrity.of.the.information.stored.in.the.srckeystore."));
      System.err.println(rb.getString(".WARNING.WARNING.WARNING."));
      System.err.println();
    }
  ...
  /**
   * import all keys and certs from importkeystore. keep alias unchanged if no name conflict,
   * otherwise, prompt. keep keypass unchanged for keys
   */
   ...
   *
   * @return 1 if the import action succeed 0 if user choose to ignore an alias-dumplicated entry 2
   *     if setEntry throws Exception
   */
  private int doImportKeyStoreSingle(KeyStore srckeystore, String alias) throws Exception {
    ...
    if (keyStore.containsAlias(newAlias)) {
      ...
      if (noprompt) {
        ...
        String reply =
            getYesNoReply(
                new MessageFormat(rb.getString("Existing.entry.alias.alias.exists.overwrite.no."))
                    .format(source));
        if ("NO".equals(reply)) {
          newAlias =
          ...
          if ("".equals(newAlias)) {
            System.err.println(
                new MessageFormat(rb.getString("Entry.for.alias.alias.not.imported."))
                    .format(source));
            return 0;
          }
        }
    ...
    try {
      ...
      if (c != null) {
        CertPathConstraintsParameters cpcp = buildCertPathConstraint((X509Certificate) c, null);
        checkWeakConstraint("<" + newAlias + ">", c, cpcp);
      } else {
        try {
          Key key = keyStore.getKey(newAlias, newPass);
          SecretKeyConstraintsParameters skcp = new SecretKeyConstraintsParameters(key);
  ...
  private void doImportKeyStoreAll(KeyStore srckeystore) throws Exception {
    ...
    for (Enumeration<String> e = srckeystore.aliases(); e.hasMoreElements(); ) {
      ...
      if (result == 1) {
        ...
        Object[] source = {alias};
        MessageFormat form =
            new MessageFormat(rb.getString("Entry.for.alias.alias.successfully.imported."));
        System.err.println(form.format(source));
      } else if (result == 2) {
        if (!noprompt) {
  ...
  private void doPrintEntries(PrintStream out) throws Exception {
    ...
    form =
        (keyStore.size() == 1)
            ? new MessageFormat(rb.getString("Your.keystore.contains.keyStore.size.entry"))
   ...
   *
   * @param src the source, which means System.in if null, or a URI, or a bare file path name
   */
  public static Collection<? extends CRL> loadCRLs(String src) throws Exception {
    ...
    if (in != null) {
      try {
        ...
        return CertificateFactory.getInstance("X509").generateCRLs(new ByteArrayInputStream(bytes));
      } finally {
        if (in != System.in) {
          in.close();
        }
  ...
  /**
   * Returns CRLs described in a X509Certificate's CRLDistributionPoints Extension. Only those
   * containing a general name of type URI are read.
   */
    ...
    for (CRL crl : loadCRLs(src)) {
      ...
      if (caks != null) {
        ...
        if (issuer != null) {
          ...
          out.printf(
              rb.getString("verified.by.s.in.s.weak"),
              issuer,
              "cacerts",
      ...
      if (signer != null) {
        CertPathConstraintsParameters cpcp =
            buildCertPathConstraint((X509Certificate) signer, null);
        checkWeakConstraint(rb.getString("the.crl"), crl, signer.getPublicKey(), cpcp);
      } else {
  ...
  private void doPrintCertReq(InputStream in, PrintStream out) throws Exception {
    ...
    CertPathConstraintsParameters cpcp = new CertPathConstraintsParameters(pkey, null, null, null);
    out.printf(
        rb.getString("PKCS.10.with.weak"),
        req.getSubjectName(),
        pkey.getFormat(),
    ...
    for (PKCS10Attribute attr : req.getAttributes().getAttributes()) {
      ...
      if (oid.equals(PKCS9Attribute.EXTENSION_REQUEST_OID)) {
        ...
        PKCS9Attribute pkcs9Attr =
            new PKCS9Attribute(attr.getAttributeId(), attr.getAttributeValue());
    ...
    CertPathConstraintsParameters cpcp1 =
        new CertPathConstraintsParameters(req.getSubjectPublicKeyInfo(), null, null, null);
    checkWeakConstraint(rb.getString("the.certificate.request"), req, cpcp1);
  }
  ...
  private void printCertFromStream(InputStream in, PrintStream out) throws Exception {
    ...
    for (int i = 0; i < certs.length; i++) {
      ...
      CertPathConstraintsParameters cpcp;
      if (i == 0 && x509Cert.getBasicConstraints() == -1) {
        // this is an EE
        cpcp = buildCertPathConstraint(x509Cert, anchor);
      } else {
        cpcp = new CertPathConstraintsParameters(x509Cert, null, anchor, null);
      }
      checkWeakConstraint(
          oneInMany(rb.getString("the.certificate"), i, certs.length), x509Cert, cpcp);
    }
  }
  ...
  private Certificate generateCertificate(InputStream in) throws CertificateException, IOException {
    byte[] data = in.readAllBytes();
    try {
      return CertificateFactory.getInstance("X.509")
  ...
  private static String oneInManys(
      String label, int certNo, int certCnt, int signerNo, int signerCnt) {
    ...
    if (jarfile != null) {
      ...
      for (CodeSigner signer : ss) {
        ...
        for (Certificate cert : certs) {
          ...
          if (cc == 0 && x.getBasicConstraints() == -1) {
            ...
            cpcp = new CertPathConstraintsParameters(x, null, anchor, null);
          }
          checkWeakConstraint(
              oneInManys(rb.getString("the.certificate"), cc, certs.size(), pos, ss.size()),
              x,
              cpcp);
        }
        ...
        if (ts != null) {
          ...
          @SuppressWarnings("unchecked")
          List<X509Certificate> tschain = (List<X509Certificate>) certs;
          ...
          for (Certificate cert : certs) {
            ...
            checkWeakConstraint(
                oneInManys(rb.getString("the.tsa.certificate"), cc, certs.size(), pos, ss.size()),
                x,
                cpcp);
          }
        }
      ...
      for (Certificate cert : chain) {
        try {
          ...
          checkWeakConstraint(
              oneInMany(rb.getString("the.certificate"), i++, chain.size()), x, cpcp);
        } catch (Exception e) {
          if (debug) {
            e.printStackTrace();
          }
    ...
    if (P12KEYSTORE.equalsIgnoreCase(storetype)) {
      ...
      for (String alias : Collections.list(keyStore.aliases())) {
        if (!keyStore.isCertificateEntry(alias)) {
          ...
          Pair<Entry, char[]> objs = recoverEntry(keyStore, alias, storePass, keyPass);
          keyStore.setEntry(alias, objs.fst, new PasswordProtection(storePassNew));
        }
      }
    }
  }
  ...
  private void doSelfCert(String alias, String dname, String sigAlgName) throws Exception {
    ...
    certInfo.setSerialNumber(CertificateSerialNumber.newRandom64bit(new SecureRandom()));
    ...
    CertificateExtensions ext =
        createV3Extensions(null, certInfo.getExtensions(), v3ext, oldCert.getPublicKey(), null);
    certInfo.setExtensions(ext);
    // Sign the new certificate
    X509CertImpl newCert = X509CertImpl.newSigned(certInfo, privKey, sigAlgName);

    // Store the new certificate as a single-element certificate chain
    keyStore.setKeyEntry(
        alias, privKey, (keyPass != null) ? keyPass : storePass, new Certificate[] {newCert});
   ...
   *
   * <p>Builds a certificate chain on top of the certificate reply, using trusted certificates from
   * the keystore. The chain is complete after a self-signed certificate has been encountered. The
   * self-signed certificate is considered a root certificate authority, and is stored at the end of
   * the chain.
   *
   * <p>The newly generated chain replaces the old chain associated with the key entry.
   *
  ...
  private boolean addTrustedCert(String alias, InputStream in) throws Exception {
    ...
    if (keyStore.containsAlias(alias)) {
      MessageFormat form =
          new MessageFormat(rb.getString("Certificate.not.imported.alias.alias.already.exists"));
      Object[] source = {alias};
      throw new Exception(form.format(source));
    }
  ...
  /**
   * Prompts user for key password. User may select to choose the same password (<code>otherKeyPass
   * </code>) as for <code>otherAlias</code>.
   */
  private char[] getKeyPasswd(String alias, String otherAlias, char[] otherKeyPass)
      throws Exception {
    int count = 0;
    char[] keyPass;
  ...
  private String withWeakConstraint(String alg, CertPathConstraintsParameters cpcp) {
    try {
  ...
  private String withWeakConstraint(Key key, CertPathConstraintsParameters cpcp) {
  ...
  /**
   * Locates a signer for a given certificate from a given keystore and returns the signer's
   * certificate.
   *
   * @param cert the certificate whose signer is searched, not null
   * @param ks the keystore to search with, not null
   * @return <code>cert</code> itself if it's already inside <code>ks</code>, or a certificate
   *     inside <code>ks</code> who signs <code>cert</code>, or null otherwise. A label is added.
   */
  private static Pair<String, Certificate> getSigner(Certificate cert, KeyStore ks)
      throws Exception {
  ...
  private X500Name getX500Name() throws IOException {
    ...
    do {
      ...
      organizationalUnit =
          inputString(
              in,
              rb.getString("What.is.the.name.of.your.organizational.unit."),
              organizationalUnit);
      organization =
          inputString(in, rb.getString("What.is.the.name.of.your.organization."), organization);
      ...
      country =
          inputString(
              in, rb.getString("What.is.the.two.letter.country.code.for.this.unit."), country);
      name =
          new X500Name(
      ...
      if (name.isEmpty()) {
        ...
        needRepeat =
            collator.compare(userInput, rb.getString("yes")) != 0
  ...
  private String inputString(BufferedReader in, String prompt, String defaultValue)
      throws IOException {
   ...
   *
   * @return an array of objects, where the 1st element in the array is the recovered private key,
   *     and the 2nd element is the password used to recover it.
   */
  private Pair<Key, char[]> recoverKey(String alias, char[] storePass, char[] keyPass)
      throws Exception {
    Key key;
   ...
   *
   * @return an array of objects, where the 1st element in the array is the recovered entry, and the
   *     2nd element is the password used to recover it (null if no password).
   */
  private Pair<Entry, char[]> recoverEntry(KeyStore ks, String alias, char[] pstore, char[] pkey)
      throws Exception {
  ...
  /**
   * Validates chain in certification reply, and returns the ordered elements of the chain (with
   * user certificate first, and root certificate last in the array).
   *
   * @param alias the alias name
   * @param userCert the user certificate of the alias
   * @param replyCerts the chain provided in the reply
   */
  private Certificate[] validateReply(String alias, Certificate userCert, Certificate[] replyCerts)
      throws Exception {
    ...
    if (root == null) {
      ...
      if (root.snd != topCert) {
        ...
        replyCerts = tmpCerts;
        CertPathConstraintsParameters cpcp =
            buildCertPathConstraint((X509Certificate) root.snd, null);
        checkWeakConstraint(
            String.format(
                fromKeyStore ? rb.getString("alias.in.keystore") : rb.getString("alias.in.cacerts"),
                root.fst),
            root.snd,
            cpcp);
      }
  ...
  /**
   * Establishes a certificate chain (using trusted certificates in the keystore and cacerts),
   * starting with the reply (certToVerify) and ending at a self-signed certificate found in the
   * keystore.
   *
   * @param userCert optional existing certificate, mostly likely be the original self-signed cert
   *     created by -genkeypair. It must have the same public key as certToVerify but cannot be the
   *     same cert.
  ...
  private Certificate[] establishCertChain(Certificate userCert, Certificate certToVerify)
    ...
    if (userCert != null) {
      ...
      if (certToVerify.equals(userCert)) {
        throw new Exception(
            rb.getString("Certificate.reply.and.certificate.in.keystore.are.identical"));
      }
    }
    ...
    if (buildChain(
        new Pair<>(rb.getString("the.input"), (X509Certificate) certToVerify), chain, certs)) {
      for (Pair<String, X509Certificate> p : chain) {
        CertPathConstraintsParameters cpcp = buildCertPathConstraint(p.snd, null);
  ...
  /**
   * Recursively tries to establish chain from pool of certs starting from certToVerify until a
   * self-signed cert is found, and fill the certs found into chain. Each cert in the chain signs
   * the next one.
   *
   * <p>This method is able to recover from an error, say, if certToVerify is signed by certA but
   * certA has no issuer in certs and itself is not self-signed, the method can try another certB
   * that also signs certToVerify and look for signer of certB, etc., etc.
   *
   * <p>Each cert in chain comes with a label showing its origin. The label is used in the warning
   * message when the cert is considered a risk.
   *
  ...
  private boolean buildChain(
      Pair<String, X509Certificate> certToVerify,
  ...
  /**
   * Stores the (leaf) certificates of a keystore in a hashtable. All certs belonging to the same CA
   * are stored in a vector that in turn is stored in the hashtable, keyed by the CA's subject DN.
   * Each cert comes with a string label that shows its origin and alias.
   */
  private void keystorecerts2Hashtable(
      KeyStore ks, Hashtable<Principal, Vector<Pair<String, X509Certificate>>> hash)
      throws Exception {
    ...
    for (Enumeration<String> aliases = ks.aliases(); aliases.hasMoreElements(); ) {
      ...
      if (cert != null) {
        ...
        Pair<String, X509Certificate> pair =
            new Pair<>(
                String.format(
                    rb.getString(ks == caks ? "alias.in.cacerts" : "alias.in.keystore"), alias),
                (X509Certificate) cert);
   ...
   *
   * @param s the command provided by user
   * @param list the legal command set. If there is a null, commands after it are regarded
   *     experimental, which means they are supported but their existence should not be revealed to
   *     user.
   * @return the position of a single match, or -1 if none matched
   * @throws Exception if s is ambiguous
   */
  private static int oneOf(String s, String... list) throws Exception {
    ...
    res =
        oneOfMatch((a, b) -> a.equals(b.replaceAll("(?<!^)[a-z]", "")) || b.startsWith(a), s, list);
    if (res >= 0) {
      return res;
    }
    ...
    return oneOfMatch(
        (a, b) ->
            a.equalsIgnoreCase(b.replaceAll("(?<!^)[a-z]", ""))
   ...
   *
   * @param matcher a BiFunction which returns {@code true} if the 1st argument (user input) matches
   *     the 2nd one (full command)
   * @param s the command provided by user
   * @param list the legal command set
   * @return the position of a single match, or -1 if none matched
   * @throws Exception if s is ambiguous
   */
  private static int oneOfMatch(
      BiFunction<String, String, Boolean> matcher, String s, String... list) throws Exception {
   ...
   * @return which one
   */
  private GeneralName createGeneralName(String t, String v, int exttype) throws Exception {
   ...
   * SubjectKeyIdentifierExtension will always be created non-critical besides the extension
   * requested in the <code>extstr</code> argument.
   *
  ...
  private CertificateExtensions createV3Extensions(
    ...
    try {
      ...
      if (aSubjectKeyId != null) {
        setExt(result, new AuthorityKeyIdentifierExtension(aSubjectKeyId, null, null));
      }
      ...
      if (requestedEx != null) {
        ...
        for (String extstr : extstrs) {
          if (extstr.toLowerCase(Locale.ENGLISH).startsWith("honored=")) {
            List<String> list =
                Arrays.asList(extstr.toLowerCase(Locale.ENGLISH).substring(8).split(","));
                ...
                if (!e.isCritical() && action == 0 || e.isCritical() && action == 1) {
                  e =
                      Extension.newExtension(
                          e.getExtensionId(), !e.isCritical(), e.getExtensionValue());
                }
                setExt(result, e);
              } else {
                result.delete(n);
      ...
      for (String extstr : extstrs) {
        ...
        switch (exttype) {
          ...
          case 1: // KU
            if (value != null) {
              ...
              setExt(
                  result,
                  Extension.newExtension(
                      kue.getExtensionId(), isCritical, kue.getExtensionValue()));
            } else {
              throw new Exception(rb.getString("Illegal.value.") + extstr);
            }
            break;
          ...
          case 6: // AIA, always non-critical
            if (isCritical) {
              throw new Exception(
                  rb.getString("This.extension.cannot.be.marked.as.critical.") + extstr);
            }
            if (value != null) {
              ...
              for (String item : ps) {
                ...
                accessDescriptions.add(
                    new AccessDescription(oid, createGeneralName(t, v, exttype)));
              }
              if (exttype == 5) {
          ...
          case -1:
            ...
            setExt(
                result,
                new Extension(
                    oid, isCritical, new DerValue(DerValue.tag_OctetString, data).toByteArray()));
            break;
          default:
  ...
  private Date getLastDate(Date firstDate, long validity) throws Exception {
    ...
    if (c.get(Calendar.YEAR) > 9999) {
      throw new Exception(
          "Validity period ends at calendar year "
              + c.get(Calendar.YEAR)
              + " which is greater than 9999");
    }
  ...
  private void checkWeakConstraint(
      String label, String sigAlg, Key key, CertPathConstraintsParameters cpcp) throws Exception {
    if (sigAlg != null) {
      try {
        ...
        try {
          ...
          weakWarnings.add(String.format(rb.getString("whose.sigalg.weak"), label, sigAlg));
        }
      } catch (CertPathValidatorException e) {
        ...
        if (eMessage.contains("denyAfter constraint check failed")
          ...
          try {
            denyAfterDate = eMessage.substring(startSepPos + startSeparator.length(), endSepPos);
          } catch (IndexOutOfBoundsException e1) {
            throw new Exception(
                rb.getString("Unable.to.parse.denyAfter.string.in.exception.message"));
          }

          weakWarnings.add(
              String.format(
                  rb.getString("whose.sigalg.usagesignedjar"), label, sigAlg, denyAfterDate));
        } else {
          weakWarnings.add(String.format(rb.getString("whose.sigalg.disabled"), label, sigAlg));
        }
        if (debug) {
          e.printStackTrace();
        }
    ...
    if (key != null) {
      try {
        ...
        try {
          ...
          weakWarnings.add(
              String.format(
                  rb.getString("whose.key.weak"),
                  label,
                  String.format(
                      rb.getString("key.bit"),
        ...
        weakWarnings.add(
            String.format(
                rb.getString("whose.key.disabled"),
                label,
                String.format(
                    rb.getString("key.bit"),
                    KeyUtil.getKeySize(key),
  ...
  private void checkWeakConstraint(String label, Certificate[] certs) throws Exception {
    X509Certificate[] xcerts = convertCerts(certs);
    List<X509Certificate> chain = Arrays.asList(xcerts);
    TrustAnchor anchor = findTrustAnchor(chain);
    for (int i = 0; i < certs.length; i++) {
      ...
      if (cert instanceof X509Certificate xc) {
        ...
        if (i == 0 && xc.getBasicConstraints() == -1) {
          ...
          cpcp = new CertPathConstraintsParameters(xc, null, anchor, null);
        }
  ...
  private void checkWeakConstraint(
      String label, Certificate cert, CertPathConstraintsParameters cpcp) throws Exception {
    if (cert instanceof X509Certificate xc) {
  ...
  private void checkWeakConstraint(
      String label, SecretKey secKey, SecretKeyConstraintsParameters skcp) {
     ...
     */
    if (secKeyAlg.startsWith("PBE")) return;
    ...
    try {
      ...
      if (eMessage.contains("constraints check failed on keysize limits")
        ...
        weakWarnings.add(
            String.format(
                rb.getString("key.size.weak"),
                label,
                String.format(rb.getString("key.bit"), KeyUtil.getKeySize(secKey), secKeyAlg)));
      } else {
        weakWarnings.add(String.format(rb.getString("key.algorithm.weak"), label, secKeyAlg));
      }
    }
  }

  private void checkWeakConstraint(String label, PKCS10 p10, CertPathConstraintsParameters cpcp)
      throws Exception {
    checkWeakConstraint(label, p10.getSigAlg(), p10.getSubjectPublicKeyInfo(), cpcp);
  }

  private void checkWeakConstraint(
      String label, CRL crl, Key key, CertPathConstraintsParameters cpcp) throws Exception {
  ...
  private void checkWeakConstraint(
      String label, String keyAlg, SecretKeyConstraintsParameters skcp) {
    try {
      LEGACY_CHECK.permits(keyAlg, skcp, false);
    } catch (CertPathValidatorException e) {
      weakWarnings.add(String.format(rb.getString("key.algorithm.weak"), label, keyAlg));
    }
  }
  ...
  private CertPathConstraintsParameters buildCertPathConstraint(
    ...
    if (eku.contains(KnownOIDs.codeSigning.value())) {
      return new CertPathConstraintsParameters(xcert, Validator.VAR_CODE_SIGNING, anchor, null);
    } else if (eku.contains(KnownOIDs.clientAuth.value())) {
      return new CertPathConstraintsParameters(xcert, Validator.VAR_TLS_CLIENT, anchor, null);
    } else if (eku.contains(KnownOIDs.serverAuth.value())) {
      return new CertPathConstraintsParameters(xcert, Validator.VAR_TLS_SERVER, anchor, null);
    } else if (eku.contains(KnownOIDs.KP_TimeStamping.value())) {
      return new CertPathConstraintsParameters(xcert, Validator.VAR_TSA_SERVER, anchor, null);
    }
    return new CertPathConstraintsParameters(xcert, Validator.VAR_GENERIC, anchor, null);
  }
  ...
  private void errorNeedArgument(String flag) {
    Object[] source = {flag};
    System.err.println(
        new MessageFormat(rb.getString("Command.option.flag.needs.an.argument.")).format(source));
    tinyHelp();
  }

  private char[] getPass(String modifier, String arg) {
    char[] output = KeyStoreUtil.getPassWithModifier(modifier, arg, rb, collator);
