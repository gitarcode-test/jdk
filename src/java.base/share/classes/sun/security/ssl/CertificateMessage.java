...
final class CertificateMessage {
   ...
   *
   * <p>In server mode, the certificate handshake message is sent whenever the agreed-upon key
   * exchange method uses certificates for authentication. In client mode, this message is only sent
   * if the server requests a certificate for client authentication.
   *
   * <p>opaque ASN.1Cert<1..2^24-1>;
   *
   * <p>SSL 3.0: struct { ASN.1Cert certificate_list<1..2^24-1>; } Certificate; Note: For SSL 3.0
   * client authentication, if no suitable certificate is available, the client should send a
   * no_certificate alert instead. This alert is only a warning; however, the server may respond
   * with a fatal handshake failure alert if client authentication is required.
   *
   * <p>TLS 1.0/1.1/1.2: struct { ASN.1Cert certificate_list<0..2^24-1>; } Certificate;
   */
  static final class T12CertificateMessage extends HandshakeMessage {
    final List<byte[]> encodedCertChain;

    T12CertificateMessage(HandshakeContext handshakeContext, X509Certificate[] certChain)
        throws SSLException {
      ...
      for (X509Certificate cert : certChain) {
        try {
          ...
          throw handshakeContext.conContext.fatal(
              ...
              "Could not encode certificate (" + cert.getSubjectX500Principal() + ")",
              cee);
        }
      }
    ...
    T12CertificateMessage(HandshakeContext handshakeContext, ByteBuffer m) throws IOException {
      ...
      if (listLen > m.remaining()) {
        throw handshakeContext.conContext.fatal(
            Alert.ILLEGAL_PARAMETER, "Error parsing certificate message:no sufficient data");
      }
      if (listLen > 0) {
  ...
  /**
   * The "Certificate" handshake message producer for TLS 1.2 and previous SSL/TLS protocol
   * versions.
   */
  private static final class T12CertificateProducer implements HandshakeProducer {
    ...
    private byte[] onProduceCertificate(
        ServerHandshakeContext shc, SSLHandshake.HandshakeMessage message) throws IOException {
      ...
      if (x509Possession == null) { // unlikely
        throw shc.conContext.fatal(
            Alert.INTERNAL_ERROR, "No expected X.509 certificate for server authentication");
      }
    ...
    private byte[] onProduceCertificate(
        ClientHandshakeContext chc, SSLHandshake.HandshakeMessage message) throws IOException {
      X509Possession x509Possession = null;
      ...
      if (x509Possession == null) {
        if (chc.negotiatedProtocol.useTLS10PlusSpec()) {
          ...
          x509Possession = new X509Possession(null, new X509Certificate[0]);
        } else {
          if (SSLLogger.isOn && SSLLogger.isOn("ssl,handshake")) {
            SSLLogger.fine(
                "No X.509 certificate for client authentication, " + "send a no_certificate alert");
          }
  ...
  /**
   * The "Certificate" handshake message consumer for TLS 1.2 and previous SSL/TLS protocol
   * versions.
   */
  static final class T12CertificateConsumer implements SSLConsumer {
    ...
    private void onCertificate(ServerHandshakeContext shc, T12CertificateMessage certificateMessage)
        throws IOException {
      ...
      if (encodedCerts == null || encodedCerts.isEmpty()) {
        ...
        if (shc.sslConfig.clientAuthType != ClientAuthType.CLIENT_AUTH_REQUESTED) {
          ...
          throw shc.conContext.fatal(Alert.HANDSHAKE_FAILURE, "Empty client certificate chain");
        } else {
          return;
        }
      }
      ...
      try {
        ...
        for (byte[] encodedCert : encodedCerts) {
          x509Certs[i++] =
              (X509Certificate) cf.generateCertificate(new ByteArrayInputStream(encodedCert));
        }
      } catch (CertificateException ce) {
        throw shc.conContext.fatal(
            Alert.BAD_CERTIFICATE, "Failed to parse client certificates", ce);
      }
      ...
      // update
      //
      shc.handshakeCredentials.add(new X509Credentials(x509Certs[0].getPublicKey(), x509Certs));
      shc.handshakeSession.setPeerCertificates(x509Certs);
    }

    private void onCertificate(ClientHandshakeContext chc, T12CertificateMessage certificateMessage)
        throws IOException {
      List<byte[]> encodedCerts = certificateMessage.encodedCertChain;
      if (encodedCerts == null || encodedCerts.isEmpty()) {
      ...
      try {
        ...
        throw chc.conContext.fatal(
            Alert.BAD_CERTIFICATE, "Failed to parse server certificates", ce);
      }
      ...
      if (chc.reservedServerCerts != null && !chc.handshakeSession.useExtendedMasterSecret) {
        ...
        if ((identityAlg == null || identityAlg.isEmpty())
            && !isIdentityEquivalent(x509Certs[0], chc.reservedServerCerts[0])) {
          throw chc.conContext.fatal(
      ...
      // update
      //
      chc.handshakeCredentials.add(new X509Credentials(x509Certs[0].getPublicKey(), x509Certs));
     ...
     */
    private static boolean isIdentityEquivalent(
        X509Certificate thisCert, X509Certificate prevCert) {
      ...
      try {
        ...
        if (SSLLogger.isOn && SSLLogger.isOn("handshake")) {
          SSLLogger.fine("Attempt to obtain subjectAltNames extension failed!");
        }
      }
      ...
      if (thisSubjectAltNames != null && prevSubjectAltNames != null) {
        ...
        if (thisSubAltIPAddrs != null
            && prevSubAltIPAddrs != null
        ...
        if (thisSubAltDnsNames != null
            && prevSubAltDnsNames != null
    ...
    private static boolean isEquivalent(
        Collection<String> thisSubAltNames, Collection<String> prevSubAltNames) {
     ...
     *
     * @param certs an array of {@code X509Certificate} objects presented by the server in the
     *     ServerCertificate message.
     * @throws IOException if a failure occurs during validation or the trust manager associated
     *     with the {@code SSLContext} is not an {@code X509ExtendedTrustManager}.
     */
    static void checkServerCerts(ClientHandshakeContext chc, X509Certificate[] certs)
        throws IOException {
      ...
      if (chc.negotiatedCipherSuite.keyExchange == CipherSuite.KeyExchange.K_RSA_EXPORT
          || chc.negotiatedCipherSuite.keyExchange == CipherSuite.KeyExchange.K_DHE_RSA_EXPORT) {
        ...
        if (tm instanceof X509ExtendedTrustManager) {
          if (chc.conContext.transport instanceof SSLEngine engine) {
            ((X509ExtendedTrustManager) tm)
                .checkServerTrusted(certs.clone(), keyExchangeString, engine);
          } else {
            SSLSocket socket = (SSLSocket) chc.conContext.transport;
            ((X509ExtendedTrustManager) tm)
                .checkServerTrusted(certs.clone(), keyExchangeString, socket);
          }
        } else {
     ...
     * determine what TLS alert description to use.
     *
    ...
    private static Alert getCertificateAlert(
      ...
      if (baseCause instanceof CertPathValidatorException cpve) {
        ...
        } else if (reason == BasicReason.INVALID_SIGNATURE || reason == BasicReason.NOT_YET_VALID) {
...
'}',""",
      ...
      try {
        ...
        CertificateFactory cf = CertificateFactory.getInstance("X.509");
        x509Certs = cf.generateCertificate(new ByteArrayInputStream(encoded));
      } catch (CertificateException ce) {
        // no X.509 certificate factory service
        x509Certs = encoded;
      }
      ...
      Object[] messageFields = {
        SSLLogger.toString(x509Certs), Utilities.indent(extensions.toString(), "    ")
      };
  ...
  static final class T13CertificateMessage extends HandshakeMessage {
    ...
    T13CertificateMessage(
        HandshakeContext context, byte[] requestContext, X509Certificate[] certificates)
    ...
    T13CertificateMessage(HandshakeContext handshakeContext, ByteBuffer m) throws IOException {
      ...
      if (m.remaining() < 4) {
        throw new SSLProtocolException(
            "Invalid Certificate message: " + "insufficient data (length=" + m.remaining() + ")");
      }
      ...
      if (listLen != m.remaining()) {
        throw new SSLProtocolException(
            "Invalid Certificate message: " + "incorrect list length (length=" + listLen + ")");
      }

      SSLExtension[] enabledExtensions =
          handshakeContext.sslConfig.getEnabledExtensions(SSLHandshake.CERTIFICATE);
    ...
    public String toString() {
      ...
      Object[] messageFields = {
        Utilities.toHexString(requestContext), Utilities.indent(builder.toString())
      };
  ...
  private static final class T13CertificateProducer implements HandshakeProducer {
    ...
    private byte[] onProduceCertificate(ServerHandshakeContext shc, HandshakeMessage message)
        throws IOException {
      ...
      if (!(pos instanceof X509Possession x509Possession)) {
        throw shc.conContext.fatal(
            Alert.HANDSHAKE_FAILURE, "No X.509 certificate for server authentication");
      }
      ...
      try {
        ...
        throw shc.conContext.fatal(
            Alert.HANDSHAKE_FAILURE, "Failed to produce server Certificate message", ce);
      }
      ...
      SSLExtension[] enabledCTExts =
          shc.sslConfig.getEnabledExtensions(
              SSLHandshake.CERTIFICATE, Arrays.asList(ProtocolVersion.PROTOCOLS_OF_13));
    ...
    private static SSLPossession choosePossession(
        HandshakeContext hc, ClientHelloMessage clientHello) {
      if (hc.peerRequestedCertSignSchemes == null || hc.peerRequestedCertSignSchemes.isEmpty()) {
        if (SSLLogger.isOn && SSLLogger.isOn("ssl,handshake")) {
          SSLLogger.warning("No signature_algorithms(_cert) in ClientHello");
        }
        return null;
      }
    ...
    private byte[] onProduceCertificate(ClientHandshakeContext chc, HandshakeMessage message)
        throws IOException {
      ClientHelloMessage clientHello = (ClientHelloMessage) message;
      ...
      if (pos == null) {
        ...
        if (!(pos instanceof X509Possession x509Possession)) {
          if (SSLLogger.isOn && SSLLogger.isOn("ssl,handshake")) {
            SSLLogger.fine("No X.509 certificate for client authentication");
          }
          localCerts = new X509Certificate[0];
        } else {
      ...
      try {
        ...
        throw chc.conContext.fatal(
            Alert.HANDSHAKE_FAILURE, "Failed to produce client Certificate message", ce);
      }
      if (SSLLogger.isOn && SSLLogger.isOn("ssl,handshake")) {
  ...
  private static final class T13CertificateConsumer implements SSLConsumer {
    ...
    private void onConsumeCertificate(
        ServerHandshakeContext shc, T13CertificateMessage certificateMessage) throws IOException {
      if (certificateMessage.certEntries == null || certificateMessage.certEntries.isEmpty()) {
        ...
        if (shc.sslConfig.clientAuthType == CLIENT_AUTH_REQUIRED) {
          throw shc.conContext.fatal(Alert.CERTIFICATE_REQUIRED, "Empty client certificate chain");
        } else {
      ...
      // update
      //
      shc.handshakeCredentials.add(new X509Credentials(cliCerts[0].getPublicKey(), cliCerts));
      shc.handshakeSession.setPeerCertificates(cliCerts);
    }

    private void onConsumeCertificate(
        ClientHandshakeContext chc, T13CertificateMessage certificateMessage) throws IOException {
      if (certificateMessage.certEntries == null || certificateMessage.certEntries.isEmpty()) {
        throw chc.conContext.fatal(Alert.DECODE_ERROR, "Empty server certificate chain");
      }
      ...
      // update
      //
      chc.handshakeCredentials.add(new X509Credentials(srvCerts[0].getPublicKey(), srvCerts));
      chc.handshakeSession.setPeerCertificates(srvCerts);
    }

    private static X509Certificate[] checkClientCerts(
        ServerHandshakeContext shc, List<CertificateEntry> certEntries) throws IOException {
      ...
      try {
        ...
        for (CertificateEntry entry : certEntries) {
          certs[i++] =
              (X509Certificate) cf.generateCertificate(new ByteArrayInputStream(entry.encoded));
        }
      } catch (CertificateException ce) {
        throw shc.conContext.fatal(
            Alert.BAD_CERTIFICATE, "Failed to parse client certificates", ce);
    ...
    private static X509Certificate[] checkServerCerts(
        ClientHandshakeContext chc, List<CertificateEntry> certEntries) throws IOException {
      X509Certificate[] certs = new X509Certificate[certEntries.size()];
      try {
        CertificateFactory cf = CertificateFactory.getInstance("X.509");
        int i = 0;
