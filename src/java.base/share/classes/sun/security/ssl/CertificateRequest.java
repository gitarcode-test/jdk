...
final class CertificateRequest {
  ...
  private enum ClientCertificateType {
    ...
    // RFC 4492 and 8442
    ECDSA_SIGN((byte) 0x40, "ecdsa_sign", List.of("EC", "EdDSA"), JsseJce.isEcAvailable()),
    ...
    private static final byte[] CERT_TYPES =
        JsseJce.isEcAvailable()
            ? new byte[] {ECDSA_SIGN.id, RSA_SIGN.id, DSS_SIGN.id}
            : new byte[] {RSA_SIGN.id, DSS_SIGN.id};
    ...
    ClientCertificateType(byte id, String name, List<String> keyAlgorithm, boolean isAvailable) {
      this.id = id;
      this.name = name;
  ...
  static final class T10CertificateRequestMessage extends HandshakeMessage {
    ...
    T10CertificateRequestMessage(HandshakeContext handshakeContext, ByteBuffer m)
        throws IOException {
      super(handshakeContext);
      ...
      if (m.remaining() < 4) {
        throw handshakeContext.conContext.fatal(
            Alert.ILLEGAL_PARAMETER, "Incorrect CertificateRequest message: no sufficient data");
      }
  ...
  private static final class T10CertificateRequestProducer implements HandshakeProducer {
    ...
    public byte[] produce(ConnectionContext context, HandshakeMessage message) throws IOException {
      ...
      T10CertificateRequestMessage crm =
          new T10CertificateRequestMessage(shc, caCerts, shc.negotiatedCipherSuite.keyExchange);
      ...
      shc.handshakeConsumers.put(
          SSLHandshake.CERTIFICATE_VERIFY.id, SSLHandshake.CERTIFICATE_VERIFY);
    ...
    public void consume(ConnectionContext context, ByteBuffer message) throws IOException {
      ...
      if (chc.handshakeConsumers.containsKey(SSLHandshake.CERTIFICATE.id)) {
        throw chc.conContext.fatal(
            Alert.UNEXPECTED_MESSAGE, "Unexpected CertificateRequest handshake message");
      }

      SSLConsumer certStatCons = chc.handshakeConsumers.remove(SSLHandshake.CERTIFICATE_STATUS.id);
      ...
      try {
        if (chc.conContext.transport instanceof SSLSocketImpl) {
          clientAlias =
              km.chooseClientAlias(
                  crm.getKeyTypes(), crm.getAuthorities(), (SSLSocket) chc.conContext.transport);
        } else if (chc.conContext.transport instanceof SSLEngineImpl) {
          clientAlias =
              km.chooseEngineClientAlias(
                  crm.getKeyTypes(), crm.getAuthorities(), (SSLEngine) chc.conContext.transport);
        }
      } catch (IllegalArgumentException iae) {
        chc.conContext.fatal(
            Alert.DECODE_ERROR,
      ...
      chc.handshakePossessions.add(new X509Possession(clientPrivateKey, clientCerts));
  ...
  static final class T12CertificateRequestMessage extends HandshakeMessage {
    ...
    T12CertificateRequestMessage(
      ...
      if (signatureSchemes == null || signatureSchemes.isEmpty()) {
        throw handshakeContext.conContext.fatal(
            ...
            "No signature algorithms specified for " + "CertificateRequest handshake message");
      }
    ...
    T12CertificateRequestMessage(HandshakeContext handshakeContext, ByteBuffer m)
        throws IOException {
      super(handshakeContext);
      ...
      if (m.remaining() < 8) {
        throw handshakeContext.conContext.fatal(
            Alert.ILLEGAL_PARAMETER,
            "Invalid CertificateRequest handshake message: " + "no sufficient data");
      }
      this.types = Record.getBytes8(m);
      ...
      if (algs.length == 0 || (algs.length & 0x01) != 0) {
        throw handshakeContext.conContext.fatal(
            Alert.ILLEGAL_PARAMETER,
            "Invalid CertificateRequest handshake message: " + "incomplete signature algorithms");
      }
      ...
      if (listLen > m.remaining()) {
        throw handshakeContext.conContext.fatal(
            Alert.ILLEGAL_PARAMETER, "Invalid CertificateRequest message: no sufficient data");
      }
  ...
  private static final class T12CertificateRequestProducer implements HandshakeProducer {
    ...
    public byte[] produce(ConnectionContext context, HandshakeMessage message) throws IOException {
      ...
      if (shc.localSupportedSignAlgs == null) {
        shc.localSupportedSignAlgs =
            SignatureScheme.getSupportedAlgorithms(
                shc.sslConfig, shc.algorithmConstraints, shc.activeProtocols);
      }
      ...
      T12CertificateRequestMessage crm =
          new T12CertificateRequestMessage(
  ...
  private static final class T12CertificateRequestConsumer implements SSLConsumer {
    ...
    public void consume(ConnectionContext context, ByteBuffer message) throws IOException {
      ...
      List<SignatureScheme> sss =
          SignatureScheme.getSupportedAlgorithms(
              chc.sslConfig, chc.algorithmConstraints, chc.negotiatedProtocol, crm.algorithmIds);
      if (sss.isEmpty()) {
      ...
      try {
        ...
        chc.conContext.fatal(
            ...
            "The "
                ...
                + "authorities could not be parsed",
            iae);
      }
    ...
    private static SSLPossession choosePossession(
        HandshakeContext hc, T12CertificateRequestMessage crm) {
      if (hc.peerRequestedCertSignSchemes == null || hc.peerRequestedCertSignSchemes.isEmpty()) {
        if (SSLLogger.isOn && SSLLogger.isOn("ssl,handshake")) {
          SSLLogger.warning("No signature and hash algorithms " + "in CertificateRequest");
        }
        return null;
      }

      // Put the CR key type into a more friendly format for searching
      List<String> crKeyTypes = new ArrayList<>(Arrays.asList(crm.getKeyTypes()));
  ...
  static final class T13CertificateRequestMessage extends HandshakeMessage {
    ...
    T13CertificateRequestMessage(HandshakeContext handshakeContext, ByteBuffer m)
        throws IOException {
      super(handshakeContext);
      ...
      if (m.remaining() < 4) {
        throw handshakeContext.conContext.fatal(
            Alert.ILLEGAL_PARAMETER,
            "Invalid CertificateRequest handshake message: " + "no sufficient extensions data");
      }
      SSLExtension[] enabledExtensions =
          handshakeContext.sslConfig.getEnabledExtensions(SSLHandshake.CERTIFICATE_REQUEST);
  ...
  private static final class T13CertificateRequestConsumer implements SSLConsumer {
    ...
    public void consume(ConnectionContext context, ByteBuffer message) throws IOException {
      ...
      //
      SSLExtension[] extTypes =
          chc.sslConfig.getEnabledExtensions(SSLHandshake.CERTIFICATE_REQUEST);
