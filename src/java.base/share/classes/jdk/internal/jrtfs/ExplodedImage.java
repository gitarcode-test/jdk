...
class ExplodedImage extends SystemImage {
  ...
  private final class PathNode extends Node {
    ...
    public boolean isDirectory() {
      return children != null || (link == null && getFileAttributes().isDirectory());
    }
    ...
    public List<Node> getChildren() {
      if (!isDirectory()) throw new IllegalArgumentException("not a directory: " + getNameString());
      if (children == null) {
    ...
    if (p != null) {
      try {
        ...
        if (attrs.isRegularFile()) {
          ...
          if (f.toString().startsWith("_the.")) return null;
        }
  ...
  private void initNodes() throws IOException {
    ...
    // "nodes" map contains only /modules/<foo> nodes only so far and so add all as children of
    // /modules
