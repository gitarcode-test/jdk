...
class JrtFileSystem extends FileSystem {
  ...
  public void close() throws IOException {
    if (!isClosable) throw new UnsupportedOperationException();
    cleanup();
  }
  ...
  private static final Set<String> supportedFileAttributeViews =
      Collections.unmodifiableSet(new HashSet<String>(Arrays.asList("basic", "jrt")));
  ...
  Iterator<Path> iteratorOf(JrtPath path, DirectoryStream.Filter<? super Path> filter)
    ...
    if (filter == null) {
      return node.getChildren().stream()
          .map(
              child ->
                  (Path) (path.resolve(new JrtPath(this, child.getNameString()).getFileName())))
          .iterator();
    }
  ...
  final FileChannel newFileChannel(
      JrtPath path, Set<? extends OpenOption> options, FileAttribute<?>... attrs)
      throws IOException {
