...
class ModulePathValidator {
  ...
  /**
   * Scans and the validates all modules on the module path. The module path comprises the upgrade
   * module path, system modules, and the application module path.
   *
  ...
  static int scanAllModules(PrintStream out) {
    ...
    if (value != null) {
      Stream.of(value.split(File.pathSeparator)).map(Path::of).forEach(validator::scan);
    }
  ...
  private void printModule(ModuleReference mref) {
    ...
    if (descriptor.isAutomatic()) out.print(" automatic");
    out.println();
  }

  /**
   * Prints the module location and name, checks if the module is shadowed by a previously seen
   * module, and finally checks for package conflicts with previously seen modules.
   */
    ...
    if (previous != null) {
      ...
      for (String pkg : mref.descriptor().packages()) {
        ...
        if (previous != null) {
          ...
          out.println(INDENT + "contains " + pkg + " conflicts with module " + mn);
        }
      }
    }
  }
  ...
  /**
   * Scan an element on a module path. The element is a directory of modules, an exploded module, or
   * a JAR file.
   */
      ...
      for (Path entry : stream) {
        ...
        if (mref != null) {
          ...
          if (previous != null) {
            ...
            printModule(mref);
            out.println(INDENT + "contains same module as " + previous.getFileName());
            errorCount++;
          } else {
            process(mref);
          }
