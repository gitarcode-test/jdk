...
public final class ModulePatcher {
  ...
  /**
   * Initialize the module patcher with the given map. The map key is the module name, the value is
  ...
  public ModulePatcher(Map<String, List<String>> input) {
    if (input.isEmpty()) {
      ...
      for (Map.Entry<String, List<String>> e : input.entrySet()) {
        String mn = e.getKey();
        List<Path> paths = e.getValue().stream().map(Paths::get).toList();
        map.put(mn, paths);
      }
      this.map = map;
    }
  ...
  /**
   * Returns a module reference that interposes on the given module if needed. If there are no
   * patches for the given module then the module reference is simply returned. Otherwise the
   * patches for the module are scanned (to find any new packages) and a new module reference is
   * returned.
   *
  ...
  public ModuleReference patchIfNeeded(ModuleReference mref) {
      ...
      for (Path file : paths) {
        if (Files.isRegularFile(file)) {
          ...
          try (JarFile jf = new JarFile(file.toString())) {
            jf.stream()
                .filter(e -> !e.isDirectory() && (!isAutomatic || e.getName().endsWith(".class")))
          ...
          try (Stream<Path> stream =
              Files.find(top, Integer.MAX_VALUE, ((path, attrs) -> attrs.isRegularFile()))) {
            stream
                .filter(
                    path -> (!isAutomatic || path.toString().endsWith(".class")) && !isHidden(path))
    ...
    if (!packages.isEmpty()) {
      Builder builder =
          JLMA.newModuleBuilder(
              descriptor.name(), /*strict*/ descriptor.isAutomatic(), descriptor.modifiers());
   ...
   *
   * <p>This class is public so as to expose the findResource method to the built-in class loaders
   * and avoid locating the resource twice during class loading (once to locate the resource, the
   * second to gets the URL for the CodeSource).
   */
  public static class PatchedModuleReader implements ModuleReader {
    ...
    private static URL codeSourceURL(ModuleReference mref) {
      try {
        ...
        if (ouri.isPresent()) return ouri.get().toURL();
    ...
    /**
     * Finds a resources in the patch locations. Returns null if not found or the name is
     * "module-info.class" as that cannot be overridden.
     */
