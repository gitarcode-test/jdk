...
/**
 * A helper class for creating and updating modules. This class is intended to support command-line
 * options, tests, and the instrumentation API. It is also used by the VM to load modules or add
 * read edges when agents are instrumenting code that need to link to supporting classes.
 *
 * <p>The parameters that are package names in this API are the fully-qualified names of the
 * packages as defined in section 6.5.3 of <cite>The Java Language Specification </cite>, for
...
public class Modules {
  ...
  /**
   * Creates a new Module. The module has the given ModuleDescriptor and is defined to the given
   * class loader.
   *
   * <p>The resulting Module is in a larval state in that it does not read any other module and does
   * not have any exports.
   *
   * <p>The URI is for information purposes only.
   */
  ...
  /**
   * Updates module m1 to open a package to module m2. Same as m1.addOpens(pn, m2) but without a
   * caller check.
   */
  ...
  /**
   * Resolves a collection of root modules, with service binding and the empty Configuration as the
   * parent to create a Configuration for the boot layer.
   *
   * <p>This method is intended to be used to create the Configuration for the boot layer during
   * startup or at a link-time.
   */
  public static Configuration newBootLayerConfiguration(
      ModuleFinder finder, Collection<String> roots, PrintStream traceOutput) {
    return JLMA.resolveAndBind(finder, roots, traceOutput);
  }

  /**
   * Called by the VM when code in the given Module has been transformed by an agent and so may have
   * been instrumented to call into supporting classes on the boot class path or application class
   * path.
   */
   ...
   * "jdk.management.agent". If the module is not loaded then it is resolved and loaded (along with
   * any dependences that weren't previously loaded) into a child layer.
   */
  public static synchronized Module loadModule(String name) {
    ModuleLayer top = topLayer;
    if (top == null) top = ModuleLayer.boot();
    ...
    Map<String, Module> map =
        newLayer.modules().stream().collect(Collectors.toMap(Module::getName, Function.identity()));
  ...
  /**
   * Finds the module with the given name in the boot layer or any child layers created to load the
   * "java.instrument" or "jdk.management.agent" modules into a running VM.
   */
