...
public final class ModuleHashes {
  ...
  /**
   * Returns the hash for the given module name, {@code null} if there is no hash recorded for the
   * module.
   */
  ...
  /**
   * Computes the hash from the names and content of a set of modules. Returns a {@code
   * ModuleHashes} to encapsulate the result.
   *
  ...
  public boolean equals(Object obj) {
    if (!(obj instanceof ModuleHashes)) return false;
    ModuleHashes other = (ModuleHashes) obj;
    if (!algorithm.equals(other.algorithm) || nameToHash.size() != other.nameToHash.size())
      return false;
    for (Map.Entry<String, byte[]> e : nameToHash.entrySet()) {
      String name = e.getKey();
      byte[] hash = e.getValue();
      if (!Arrays.equals(hash, other.nameToHash.get(name))) return false;
    }
    return true;
  }
  ...
  /** This is used by jdk.internal.module.SystemModules class generated at link time. */
