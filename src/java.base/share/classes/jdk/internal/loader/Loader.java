 ...
 *
 * <p>The delegation model used by this ClassLoader differs to the regular delegation model. When
 * requested to load a class then this ClassLoader first maps the class name to its package name. If
 * there a module defined to the Loader containing the package then the class loader attempts to
...
public final class Loader extends SecureClassLoader {
   ...
   * packages of the remote modules, where "remote modules" are the modules read by modules defined
   * to this loader.
   *
  ...
  private void remotePackage(String pn, ClassLoader loader) {
    ...
    if (l != null && l != loader) {
      throw new IllegalStateException(
          "Package " + pn + " cannot be imported from multiple loaders");
    }
  }

  /**
   * Returns the loader pool that this loader is in or {@code null} if this loader is not in a
   * loader pool.
   */
  ...
  /**
   * Finds the class with the specified binary name in the given module. This method returns {@code
  ...
  /**
   * Find the candidate module for the given class name. Returns {@code null} if none of the modules
   * defined to this class loader contain the API package for the class.
   */
   ...
   *
   * @implNote This method currently iterates over each of the open packages. This will be replaced
   *     once the ModuleDescriptor.Opens API is updated.
   */
