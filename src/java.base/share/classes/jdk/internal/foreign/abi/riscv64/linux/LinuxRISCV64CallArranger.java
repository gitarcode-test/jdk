...
/**
 * For the RISCV64 C ABI specifically, this class uses CallingSequenceBuilder to translate a C
...
public class LinuxRISCV64CallArranger {
  ...
  abstract static class BindingCalculator {
    ...
    abstract List<Binding> getBindings(
        Class<?> carrier, MemoryLayout layout, boolean isVariadicArg);
  ...
  static final class UnboxBindingCalculator extends BindingCalculator {
    ...
    List<Binding> getBindings(
        Class<?> carrier, MemoryLayout layout, TypeClass argumentClass, boolean isVariadicArg) {
      ...
      switch (argumentClass) {
        ...
        case STRUCT_REGISTER_X -> {
          ...
          while (offset < layout.byteSize()) {
            ...
            bindings.bufferLoad(offset, type, (int) copy).vmStore(storage, type);
            offset += copy;
          }
        }
        case STRUCT_REGISTER_F -> {
          ...
          if (storageCalculator.regsAvailable(0, descs.size())) {
            for (int i = 0; i < descs.size(); i++) {
              ...
              bindings.bufferLoad(desc.offset(), type).vmStore(storage, type);
            }
          } else {
            // If there is not enough register can be used, then fall back to integer calling
            // convention.
      ...
      switch (argumentClass) {
        ...
        case STRUCT_REGISTER_X -> {
          ...
          while (offset < layout.byteSize()) {
            ...
            VMStorage storage = locations[locIndex++];
            Class<?> type = SharedUtils.primitiveCarrierForSize(copy, false);
            bindings.dup().vmLoad(storage, type).bufferStore(offset, type, (int) copy);
            offset += copy;
          }
        }
        case STRUCT_REGISTER_F -> {
          ...
          if (storageCalculator.regsAvailable(0, descs.size())) {
            for (FlattenedFieldDesc desc : descs) {
              Class<?> type = desc.layout().carrier();
              VMStorage storage = storageCalculator.getStorage(StorageType.FLOAT);
              bindings.dup().vmLoad(storage, type).bufferStore(desc.offset(), type);
            }
          } else {
            return getBindings(carrier, layout, STRUCT_REGISTER_X, isVariadicArg);
          }
