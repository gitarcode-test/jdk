...
/**
 * For the SysV x64 C ABI specifically, this class uses namely CallingSequenceBuilder to translate a
 * C FunctionDescriptor into a CallingSequence, which can then be turned into a MethodHandle.
 *
...
public class CallArranger {
  ...
  private static final ABIDescriptor CSysV =
  ...
  public record Bindings(
      CallingSequence callingSequence, boolean isInMemoryReturn, int nVectorArgs) {}
  ...
  public static Bindings getBindings(
    ...
    if (!forUpcall && options.isVariadicFunction()) {
      ...
      csb.addArgumentBindings(long.class, C_LONG, List.of(vmStore(rax, long.class)));
    }
  ...
  public static MethodHandle arrangeDowncall(
    ...
    MethodHandle handle =
        new DowncallLinker(CSysV, bindings.callingSequence).getBoundMethodHandle();
    if (options.isVariadicFunction()) {
      handle =
          MethodHandles.insertArguments(
              handle, handle.type().parameterCount() - 1, bindings.nVectorArgs);
    }
  ...
  public static UpcallStubFactory arrangeUpcall(
    ...
    return SharedUtils.arrangeUpcallHelper(
        mt, bindings.isInMemoryReturn, dropReturn, CSysV, bindings.callingSequence);
  }
  ...
  static class StorageCalculator {
    ...
    VMStorage nextStorage(int type) {
      ...
      if (registerCount < maxRegisterArguments(type)) {
        VMStorage[] source = (forArguments ? CSysV.inputStorage : CSysV.outputStorage)[type];
      ...
      switch (argumentClass.kind()) {
        case STRUCT -> {
          ...
          while (offset < layout.byteSize()) {
            ...
            boolean useFloat = storage.type() == StorageType.VECTOR;
            Class<?> type = SharedUtils.primitiveCarrierForSize(copy, useFloat);
            bindings.vmLoad(storage, type).bufferStore(offset, type, (int) copy);
            offset += copy;
          }
        }
        case POINTER -> {
