...
/**
 * For the PPC64 C ABI specifically, this class uses CallingSequenceBuilder to translate a C
 ...
 * 'in-memory' returns.
 *
 * <p>There are minor differences between the ABIs implemented on Linux and AIX which are handled in
 * sub-classes. Clients should access these through the provided public constants
 * CallArranger.ABIv1/2.
 */
public abstract class CallArranger {
  ...
  public Bindings getBindings(
    ...
    BindingCalculator argCalc =
        forUpcall
            ...
            : new UnboxBindingCalculator(true, options.allowsHeapAccess());
  ...
  public UpcallStubFactory arrangeUpcall(
    ...
    return SharedUtils.arrangeUpcallHelper(
        mt, bindings.isInMemoryReturn, dropReturn, C, bindings.callingSequence);
  }
  ...
  class StorageCalculator {
    ...
    VMStorage regAlloc(int type) {
      ...
      if (type == StorageType.FLOAT
          && nRegs[StorageType.FLOAT] + fpRegCnt > MAX_FLOAT_REGISTER_ARGUMENTS) {
        type = StorageType.INTEGER; // Try gp reg.
      }
      if (type == StorageType.INTEGER
          && nRegs[StorageType.INTEGER] + gpRegCnt > MAX_REGISTER_ARGUMENTS) return null;
    ...
    HfaRegs hfaAlloc(List<MemoryLayout> scalarLayouts) {
      ...
      if (elementCarrier == float.class) {
        ...
        for (int i = 0; i < fpRegCnt; i++) {
          ...
          result[i] =
              new VMStorage(
                  StorageType.FLOAT, PPC64Architecture.REG32_MASK, sourceReg.indexOrOffset());
        }
      } else {
        for (int i = 0; i < fpRegCnt; i++) {
      ...
      int gpRegCnt = (elementCarrier == float.class) ? ((fpRegCnt + 1) / 2) : fpRegCnt;
      nRegs[StorageType.INTEGER] += gpRegCnt;
      ...
      if (needOverlapping) {
        ...
        if (nRegs[StorageType.INTEGER] <= MAX_REGISTER_ARGUMENTS) {
          VMStorage allocatedGpReg =
              C.inputStorage[StorageType.INTEGER][nRegs[StorageType.INTEGER] - 1];
          overlappingReg =
              new VMStorage(
                  StorageType.INTEGER,
                  PPC64Architecture.REG64_MASK,
                  allocatedGpReg.indexOrOffset());
        } else {
          overlappingReg =
              new VMStorage(StorageType.STACK, (short) STACK_SLOT_SIZE, (int) stackOffset - 4);
    ...
    List<Binding> getBindings(Class<?> carrier, MemoryLayout layout) {
      ...
      switch (argumentClass) {
        case STRUCT_REGISTER -> {
          ...
          final boolean isLargeABIv1Struct =
              !useABIv2 && (isAIX || layout.byteSize() > MAX_COPY_SIZE);
          long offset = 0;
          for (VMStorage storage : regs) {
            ...
            if (shiftAmount != 0) {
              bindings.shiftLeft(shiftAmount, type).vmStore(storage, long.class);
            } else {
        ...
        case STRUCT_HFA -> {
          ...
          for (int index = 0; index < regs.first().length; index++) {
            ...
            long size =
                (baseSize == 4
                        && (storage.type() == StorageType.FLOAT || layout.byteSize() - offset < 8))
                    ? 4
                    : 8;
            Class<?> type =
                SharedUtils.primitiveCarrierForSize(size, storage.type() == StorageType.FLOAT);
            if (offset + size < layout.byteSize()) {
              bindings.dup();
            }
            bindings.bufferLoad(offset, type).vmStore(storage, type);
            ...
            if (storage2 != null) {
              ...
              bindings.bufferLoad(offset, long.class).vmStore(storage2, long.class);
            }
            offset += size;
          }
        }
        ...
        case FLOAT -> {
          VMStorage storage =
              storageCalculator.nextStorage(StorageType.FLOAT, carrier == float.class);
          bindings.vmStore(storage, carrier);
        }
      ...
      switch (argumentClass) {
        case STRUCT_REGISTER -> {
          ...
          for (VMStorage storage : regs) {
            ...
            bindings.dup();
            if (shiftAmount != 0) {
              bindings.vmLoad(storage, long.class).shiftRight(shiftAmount, type);
            } else {
        ...
        case STRUCT_HFA -> {
          ...
          for (int index = 0; index < regs.first().length; index++) {
            ...
            VMStorage storage =
                regs.second()[index] == null ? regs.first()[index] : regs.second()[index];
            // Floats are 4 Bytes, Double, GP reg and stack slots 8 Bytes (except maybe last slot).
            final long size =
                (baseSize == 4
                        && (storage.type() == StorageType.FLOAT || layout.byteSize() - offset < 8))
            ...
            Class<?> type =
                SharedUtils.primitiveCarrierForSize(size, storage.type() == StorageType.FLOAT);
            bindings.dup().vmLoad(storage, type).bufferStore(offset, type);
            offset += size;
          }
        }
        case POINTER -> {
        ...
        case INTEGER -> {
          // We could use carrier != long.class for BoxBindingCalculator, but C always uses 64 bit
          // slots.
          VMStorage storage = storageCalculator.nextStorage(StorageType.INTEGER, false);
