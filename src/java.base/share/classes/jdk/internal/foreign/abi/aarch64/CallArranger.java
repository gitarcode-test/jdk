...
/**
 * For the AArch64 C ABI specifically, this class uses CallingSequenceBuilder to translate a C
 * FunctionDescriptor into a CallingSequence, which can then be turned into a MethodHandle.
 *
 * <p>This includes taking care of synthetic arguments like pointers to return buffers for
 * 'in-memory' returns.
 *
 * <p>There are minor differences between the ABIs implemented on Linux, macOS, and Windows which
 * are handled in sub-classes. Clients should access these through the provided public constants
 * CallArranger.LINUX, CallArranger.MACOS, and CallArranger.WINDOWS.
 */
public abstract class CallArranger {
  ...
  protected static final ABIDescriptor C =
      abiFor(
  ...
  /**
   * Are variadic arguments assigned to registers as in the standard calling convention, or always
   * passed on the stack?
   *
  ...
  /**
   * {@return true if this ABI requires sub-slot (smaller than STACK_SLOT_SIZE) packing of arguments
   * on the stack.}
   */
  protected abstract boolean requiresSubSlotStackPacking();

  /**
   * Are floating point arguments to variadic functions passed in general purpose registers instead
   * of floating point registers?
   *
   * <p>{@return true if this ABI uses general purpose registers for variadic floating point
   * arguments.}
   */
  protected abstract boolean useIntRegsForVariadicFloatingPointArgs();

  /**
   * Should some fields of structs that assigned to registers be passed in registers when there are
   * not enough registers for all the fields of the struct?
   *
   * <p>{@return true if this ABI passes some fields of a struct in registers.}
   */
  ...
  protected TypeClass getArgumentClassForBindings(
      MemoryLayout layout, boolean forVariadicFunction) {
  ...
  public Bindings getBindings(
      MethodType mt, FunctionDescriptor cDesc, boolean forUpcall, LinkerOptions options) {
    ...
    BindingCalculator argCalc =
        forUpcall
            ? new BoxBindingCalculator(true)
            : new UnboxBindingCalculator(true, forVariadicFunction, options.allowsHeapAccess());
    BindingCalculator retCalc =
        forUpcall
            ? new UnboxBindingCalculator(false, forVariadicFunction, false)
            : new BoxBindingCalculator(false);

    boolean returnInMemory = isInMemoryReturn(cDesc.returnLayout());
    if (returnInMemory) {
      csb.addArgumentBindings(
          MemorySegment.class, SharedUtils.C_POINTER, argCalc.getIndirectBindings());
  ...
  public MethodHandle arrangeDowncall(
      MethodType mt, FunctionDescriptor cDesc, LinkerOptions options) {
    Bindings bindings = getBindings(mt, cDesc, false, options);

    MethodHandle handle =
        new DowncallLinker(abiDescriptor(), bindings.callingSequence).getBoundMethodHandle();
  ...
  public UpcallStubFactory arrangeUpcall(
      MethodType mt, FunctionDescriptor cDesc, LinkerOptions options) {
    Bindings bindings = getBindings(mt, cDesc, true, options);
    final boolean dropReturn = true; /* drop return, since we don't have bindings for it */
    return SharedUtils.arrangeUpcallHelper(
        mt, bindings.isInMemoryReturn, dropReturn, abiDescriptor(), bindings.callingSequence);
  }
  ...
  class StorageCalculator {
    ...
    private VMStorage stackAlloc(ValueLayout layout) {
      ...
      long stackSlotAlignment =
          requiresSubSlotStackPacking() && !forVarArgs
  ...
  class UnboxBindingCalculator extends BindingCalculator {
    ...
    UnboxBindingCalculator(
        boolean forArguments, boolean forVariadicFunction, boolean useAddressPairs) {
    ...
    List<Binding> getIndirectBindings() {
      return Binding.builder().unboxAddress().vmStore(INDIRECT_RESULT, long.class).build();
    }
      ...
      switch (argumentClass) {
        case STRUCT_REGISTER, STRUCT_HFA -> {
          ...
          for (int i = 0; i < structStorages.length; i++) {
            ...
            bindings
                .bufferLoad(
                    structStorage.offset(), structStorage.carrier(), structStorage.byteWidth())
                .vmStore(structStorage.storage(), structStorage.carrier());
          }
        }
        case STRUCT_REFERENCE -> {
          assert carrier == MemorySegment.class;
          bindings.copy(layout).unboxAddress();
          VMStorage storage =
              storageCalculator.nextStorage(StorageType.INTEGER, SharedUtils.C_POINTER);
      ...
      switch (argumentClass) {
        case STRUCT_REGISTER, STRUCT_HFA -> {
          ...
          for (StorageCalculator.StructStorage structStorage : structStorages) {
            bindings.dup();
            bindings
                .vmLoad(structStorage.storage(), structStorage.carrier())
                .bufferStore(
                    structStorage.offset(), structStorage.carrier(), structStorage.byteWidth());
          }
        }
        case STRUCT_REFERENCE -> {
          ...
          VMStorage storage =
              storageCalculator.nextStorage(StorageType.INTEGER, SharedUtils.C_POINTER);
          bindings.vmLoad(storage, long.class).boxAddress(layout);
        }
        case POINTER -> {
          ...
          bindings
              ...
              .boxAddressRaw(
                  Utils.pointeeByteSize(addressLayout), Utils.pointeeByteAlign(addressLayout));
        }
        case INTEGER -> {
          VMStorage storage =
              storageCalculator.nextStorage(StorageType.INTEGER, (ValueLayout) layout);
