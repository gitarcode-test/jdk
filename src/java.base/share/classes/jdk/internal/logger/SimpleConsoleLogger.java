 ...
 * SimpleConsoleLoggers are also used when JUL is not present and no DefaultLoggerFinder is
 * installed.
 */
public class SimpleConsoleLogger extends LoggerConfiguration
  ...
  /**
   * Default platform logging support - output messages to System.err - equivalent to ConsoleHandler
  ...
  private synchronized String format(
      Enum<?> level, String msg, Throwable thrown, String callerInfo) {

    ZonedDateTime zdt = ZonedDateTime.now();
    String throwable = toString(thrown);

    return String.format(
        getSimpleFormatString(), zdt, callerInfo, name, level.name(), msg, throwable);
  }
  ...
  public final void log(
      PlatformLogger.Level level, Throwable thrown, Supplier<String> msgSupplier) {
    if (isLoggable(level)) {
  ...
  @Override
  public final void logrb(
      PlatformLogger.Level level, ResourceBundle bundle, String key, Object... params) {
    if (isLoggable(level)) {
      String msg = bundle == null ? key : getString(bundle, key);
      publish(getCallerInfo(), logLevel(level), msg, params);
    }
  ...
  @Override
  public final void logrb(
      PlatformLogger.Level level, ResourceBundle bundle, String key, Throwable thrown) {
    if (isLoggable(level)) {
      String msg = bundle == null ? key : getString(bundle, key);
      publish(getCallerInfo(), logLevel(level), msg, thrown);
    }
