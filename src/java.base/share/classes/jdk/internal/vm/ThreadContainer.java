...
public abstract class ThreadContainer extends StackableScope {
   ...
   *
   * @param shared true for a shared container, false for a container owned by the current thread
   */
   ...
   *
   * <p>For a platform thread, this method is invoked by the thread itself when it terminates. For a
   * virtual thread, this method is invoked on its carrier after the virtual thread has terminated.
   *
   * <p>If starting the Thread failed then this method is invoked on the thread that invoked
   * onStart.
   */
