...
/**
 * An ExecutorService that starts a new thread for each task. The number of threads is unbounded.
 */
class ThreadPerTaskExecutor extends ThreadContainer implements ExecutorService {
  ...
  /**
   * Attempts to terminate if already shutdown. If this method terminates the executor then it
   * signals any threads that are waiting for termination.
   */
  private void tryTerminate() {
    assert state >= SHUTDOWN;
    if (threads.isEmpty() && STATE.compareAndSet(this, SHUTDOWN, TERMINATED)) {
  ...
  /**
   * Attempts to shutdown and terminate the executor. If interruptThreads is true then all running
   * threads are interrupted.
   */
  private void tryShutdownAndTerminate(boolean interruptThreads) {
    if (STATE.compareAndSet(this, RUNNING, SHUTDOWN)) tryTerminate();
    if (interruptThreads) {
  ...
  public void shutdown() {
    checkPermission();
    if (!isShutdown()) tryShutdownAndTerminate(false);
  }

  @Override
  public List<Runnable> shutdownNow() {
    checkPermission();
    if (!isTerminated()) tryShutdownAndTerminate(true);
    return List.of();
  }
  ...
  private Thread newThread(Runnable task) {
    ...
    if (thread == null) throw new RejectedExecutionException();
    return thread;
  }

  /**
   * Notify the executor that the task executed by the given thread is complete. If the executor has
   * been shutdown then this method will attempt to terminate the executor.
   */
  ...
  /**
   * A Future for a task that runs in its own thread. The thread is created (but not started) when
   * the Future is created. The thread is interrupted when the future is cancelled. The executor is
  ...
  private <T> T invokeAny(
      Collection<? extends Callable<T>> tasks, boolean timed, long timeout, TimeUnit unit)
      throws InterruptedException, ExecutionException, TimeoutException {
  ...
  /**
   * An object for use by invokeAny to hold the result of the first task to complete normally and/or
   * the first exception thrown. The object also maintains a count of the number of tasks that
   * attempted to complete up to when the first tasks completes normally.
   */
  private static class AnyResultHolder<T> {
    ...
    /**
     * Complete with the given result if not already completed. The winner unparks the owner thread.
     */
    ...
    /**
     * Complete with the given exception. If the result is not already set then it unparks the owner
     * thread.
     */
    void completeExceptionally(Throwable exc) {
      if (result == null) {
        if (exception == null) EXCEPTION.compareAndSet(this, null, exc);
    ...
    /**
     * Returns non-null if a task completed successfully. The result is NULL if completed with null.
     */
    T result() {
      return result;
    }
     ...
     *
     * @apiNote The result() method should be used to test if there is a result before invoking the
     *     exception method.
     */
    ...
    /**
     * Returns the number of tasks that terminated with an exception before a task completed
     * normally.
     */
    int exceptionCount() {
