 ...
 *
 * <p>This class is designed to work with (though does not require) {@linkplain java.util.stream
 * streams}. For example, you can compute summary statistics on a stream of doubles with:
 *
 * <pre>{@code
 ...
 * java.util.stream.Stream#collect(Collector) reduction} target for a {@linkplain
 * java.util.stream.Stream stream}. For example:
 *
 * <pre>{@code
 ...
 *
 * @implNote This implementation is not thread safe. However, it is safe to use {@link
 ...
 *     {@link java.util.stream.Stream#collect Stream.collect()} provides the necessary partitioning,
...
public class DoubleSummaryStatistics implements DoubleConsumer {
   ...
   * </ul>
   *
   * @apiNote The enforcement of argument correctness means that the retrieved set of recorded
   *     values obtained from a {@code DoubleSummaryStatistics} source instance may not be a legal
   *     set of arguments for this constructor due to arithmetic overflow of the source's recorded
   *     count of values. The consistent argument conditions are not sufficient to prevent the
   *     creation of an internally inconsistent instance. An example of such a state would be an
   *     instance with: {@code count} = 2, {@code min} = 1, {@code max} = 2, and {@code sum} = 0.
   ...
   * values.
   *
   * <p>Because of the unspecified order of operations and the possibility of using differing
