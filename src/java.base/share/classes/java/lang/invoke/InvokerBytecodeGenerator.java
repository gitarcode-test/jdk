...
class InvokerBytecodeGenerator {
  ...
  private static final ClassDesc CD_CasesHolder =
      ReferenceClassDescImpl.ofValidated("Ljava/lang/invoke/MethodHandleImpl$CasesHolder;");

  private static final ClassDesc CD_DirectMethodHandle =
      ReferenceClassDescImpl.ofValidated("Ljava/lang/invoke/DirectMethodHandle;");
  private static final ClassDesc CD_MethodHandleImpl =
      ReferenceClassDescImpl.ofValidated("Ljava/lang/invoke/MethodHandleImpl;");
  private static final ClassDesc CD_LambdaForm =
      ReferenceClassDescImpl.ofValidated("Ljava/lang/invoke/LambdaForm;");
  ...
  private static final ClassDesc CD_MethodHandle_array =
      ReferenceClassDescImpl.ofValidated("[Ljava/lang/invoke/MethodHandle;");
  private static final ClassDesc CD_MethodHandle_array2 =
      ReferenceClassDescImpl.ofValidated("[[Ljava/lang/invoke/MethodHandle;");

  private static final MethodTypeDesc MTD_boolean_Object =
      MethodTypeDescImpl.ofValidated(CD_boolean, CD_Object);
  private static final MethodTypeDesc MTD_Object_int =
      MethodTypeDescImpl.ofValidated(CD_Object, CD_int);
  private static final MethodTypeDesc MTD_Object_Class =
      MethodTypeDescImpl.ofValidated(CD_Object, CD_Class);
  private static final MethodTypeDesc MTD_Object_Object =
      MethodTypeDescImpl.ofValidated(CD_Object, CD_Object);
  ...
  private static final Consumer<FieldBuilder> STATIC_FINAL_FIELD =
  ...
  private InvokerBytecodeGenerator(
    ...
    this.localsMap =
        new int[localsMapSize + 1]; // last entry of localsMap is count of allocated local slots
  ...
  InvokerBytecodeGenerator(
      String name, String invokerName, LambdaForm form, MethodType invokerType) {
  ...
  private MemberName loadMethod(byte[] classFile) {
    Class<?> invokerClass =
        LOOKUP
            .makeHiddenClassDefiner(className, classFile, Set.of(), dumper())
            .defineClass(true, classDataValues());
    return resolveInvokerMember(invokerClass, invokerName, invokerType);
  }

  private static MemberName resolveInvokerMember(
      Class<?> invokerClass, String name, MethodType type) {
    MemberName member = new MemberName(invokerClass, name, type, REF_invokeStatic);
    try {
      member =
          MEMBERNAME_FACTORY.resolveOrFail(
              REF_invokeStatic, member, HOST_CLASS, LM_TRUSTED, ReflectiveOperationException.class);
                  ...
                  clb.withFlags(ACC_FINAL | ACC_SUPER)
                      ...
                      .with(
                          SourceFileAttribute.of(
                              clb.constantPool().utf8Entry(SOURCE_PREFIX + name)));
                  config.accept(clb);
                }
              });
   ...
   * The classData is loaded in the <clinit> method of the generated class. If the class data
   * contains only one single object, this method returns that single object. If the class data
   * contains more than one objects, this method returns a List.
   *
   * <p>This method returns null if no class data.
   */
            ...
            new Consumer<CodeBuilder>() {
              ...
              public void accept(CodeBuilder cob) {
                ...
                if (classData.size() == 1) {
                  ...
                  cob.checkcast(p.desc).putstatic(classDesc, p.name, p.desc);
                } else {
                  cob.checkcast(CD_List).astore(0);
                  int index = 0;
                  var listGet =
                      cob.constantPool().interfaceMethodRefEntry(CD_List, "get", MTD_Object_int);
                  for (ClassData p : classData) {
  ...
  /**
   * Emit an implicit conversion for an argument which must be of the given pclass. This is usually
   * a no-op, except when pclass is a subword type or a reference other than Object or an interface.
   *
   * @param ptype type of value present on stack
   * @param pclass type of value required on stack
   * @param arg compile-time representation of value on stack (Node, constant) or null if none
   */
  private void emitImplicitConversion(
      CodeBuilder cob, BasicType ptype, Class<?> pclass, Object arg) {
    assert (basicType(pclass) == ptype); // boxing/unboxing handled by caller
    if (pclass == ptype.basicTypeClass() && ptype != L_TYPE) return; // nothing to do
    switch (ptype) {
      case L_TYPE:
        if (VerifyType.isNullConversion(Object.class, pclass, false)) {
          if (PROFILE_LEVEL > 0) emitReferenceCast(cob, Object.class, arg);
          return;
        }
  ...
  private void emitReferenceCast(CodeBuilder cob, Class<?> cls, Object arg) {
    ...
    if (isStaticallyNameable(cls)) {
      ...
      if (Object[].class.isAssignableFrom(cls)) cob.checkcast(CD_Object_array);
      else if (PROFILE_LEVEL > 0) cob.checkcast(CD_Object);
    }
    if (writeBack != null) {
      cob.dup();
  ...
  private static MemberName resolveFrom(String name, MethodType type, Class<?> holder) {
    ...
    MemberName resolvedMember =
        MemberName.getFactory().resolveOrNull(REF_invokeStatic, member, holder, LM_TRUSTED);
  ...
  private boolean checkActualReceiver(CodeBuilder cob) {
    ...
    cob.dup()
        ...
        .invokestatic(
            CD_MethodHandleImpl,
            "assertSame",
            MethodTypeDescImpl.ofValidated(CD_void, CD_Object, CD_Object));
    return true;
  }

  static final Annotation DONTINLINE =
      Annotation.of(ReferenceClassDescImpl.ofValidated("Ljdk/internal/vm/annotation/DontInline;"));
  static final Annotation FORCEINLINE =
      Annotation.of(ReferenceClassDescImpl.ofValidated("Ljdk/internal/vm/annotation/ForceInline;"));
  static final Annotation HIDDEN =
      Annotation.of(ReferenceClassDescImpl.ofValidated("Ljdk/internal/vm/annotation/Hidden;"));
  static final Annotation INJECTEDPROFILE =
      Annotation.of(ReferenceClassDescImpl.ofValidated("Ljava/lang/invoke/InjectedProfile;"));
  static final Annotation LF_COMPILED =
      Annotation.of(ReferenceClassDescImpl.ofValidated("Ljava/lang/invoke/LambdaForm$Compiled;"));
    ...
    methodSetup(
        ...
        new Consumer<MethodBuilder>() {
          ...
          public void accept(MethodBuilder mb) {
            ...
            classData(
                lambdaForm); // keep LambdaForm instance & its compiled form lifetime tightly
                             // coupled.
                ...
                new Consumer<CodeBuilder>() {
                  ...
                  public void accept(CodeBuilder cob) {
                    if (lambdaForm.customized != null) {
                      // Since LambdaForm is customized for a particular MethodHandle, it's safe to
                      // substitute
                      // receiver MethodHandle (at slot #0) with an embedded constant and use it
                      // instead.
                      // It enables more efficient code generation in some situations, since
                      // embedded constants
                      ...
                      switch (intr) {
                        case SELECT_ALTERNATIVE:
                          ...
                          if (PROFILE_GWT) {
                            ...
                            mb.with(
                                RuntimeVisibleAnnotationsAttribute.of(List.of(INJECTEDPROFILE)));
                          }
  ...
  private static final Class<?>[] STATICALLY_INVOCABLE_PACKAGES = {
    ...
    java.lang.Object.class, java.util.Arrays.class, jdk.internal.misc.Unsafe.class
  ...
  static boolean isStaticallyInvocable(MemberName member) {
    ...
    if (cls.isArray() || cls.isPrimitive()) return false; // FIXME
    if (cls.isAnonymousClass() || cls.isLocalClass()) return false; // inner class of some sort
    if (cls.getClassLoader() != MethodHandle.class.getClassLoader()) return false; // not on BCP
    if (cls.isHidden()) return false;
    if (!isStaticallyInvocableType(member.getMethodOrFieldType())) return false;
  ...
  private static boolean isStaticallyInvocableType(MethodType mtype) {
    ...
    for (Class<?> ptype : mtype.ptypes()) if (!isStaticallyNameable(ptype)) return false;
    return true;
  }

  static boolean isStaticallyNameable(Class<?> cls) {
    if (cls == Object.class) return true;
    if (MethodHandle.class.isAssignableFrom(cls)) {
      assert (!cls.isHidden());
      return true;
    }
    while (cls.isArray()) cls = cls.getComponentType();
    if (cls.isPrimitive()) return true; // int[].class, for example
    if (cls.isHidden()) return false;
   ...
   *
   * <p>The pattern looks like (Cf. MethodHandleImpl.makeGuardWithTest):
   *
   * <blockquote>
   *
   * <pre>{@code
  ...
  private Name emitSelectAlternative(
    ...
    emitStoreInsn(
        cob, TypeKind.ReferenceType, receiver.index()); // store the MH in the receiver slot
   ...
   *
   * <p>The pattern looks like (Cf. MethodHandleImpl.makeGuardWithCatch):
   *
   * <blockquote>
   *
   * <pre>{@code
  ...
  private Name emitGuardWithCatch(CodeBuilder cob, int pos) {
    ...
    MethodType type =
        args.function.resolvedHandle().type().dropParameterTypes(0, 1).changeReturnType(returnType);
   ...
   *
   * <p>The pattern looks like (Cf. MethodHandleImpl.makeTryFinally):
   *
   * <blockquote>
   *
   * <pre>{@code
   ...
   * </blockquote>
   *
   * <p>It is compiled into bytecode equivalent to the following code:
   *
   * <blockquote>
   *
   * <pre>{@code
   ...
   * </blockquote>
   *
   * <p>Specifically, the bytecode will have the following form (the stack effects are given for the
   * beginnings of blocks, and for the situations after executing the given instruction - the code
   * will have a slightly different shape if the return type is {@code void}):
   *
   * <blockquote>
   *
   * <pre>{@code
   ...
   *
   * <p>The pattern looks like (Cf. MethodHandleImpl.loop):
   *
   * <blockquote>
   *
   * <pre>{@code
   ...
   * </blockquote>
   *
   * <p>It is compiled into bytecode equivalent to the code seen in {@link
   * MethodHandleImpl#loop(BasicType[], MethodHandleImpl.LoopClauses, Object...)}, with the
   * difference that no arrays will be used for local state storage. Instead, the local state will
   * be mapped to actual stack slots.
   *
   * <p>Bytecode generation applies an unrolling scheme to enable better bytecode generation
   * regarding local state type handling. The generated bytecode will have the following form
   * ({@code void} types are ignored for convenience). Assume there are {@code C} clauses in the
   * loop.
   *
   * <blockquote>
   *
   ...
   * </blockquote>
   *
   * <p>The {@code INIT_SEQ_x} sequence for clause {@code x} (with {@code x} ranging from {@code 0}
   * to {@code C-1}) has the following shape. Assume slot {@code vx} is used to hold the state for
   * clause {@code x}.
   *
   * <blockquote>
   *
   ...
   * </blockquote>
   *
   * <p>The {@code LOOP_SEQ_x} sequence for clause {@code x} (with {@code x} ranging from {@code 0}
   * to {@code C-1}) has the following shape. Again, assume slot {@code vx} is used to hold the
   * state for clause {@code x}.
   *
   * <blockquote>
   *
   * <pre>{@code
  ...
  private Name emitLoop(CodeBuilder cob, int pos) {
    ...
    for (int c = 0, state = 0; c < nClauses; ++c) {
      ...
      if (cInitType.returnType() != void.class) {
        emitStoreInsn(
            cob,
            BasicType.basicType(cInitType.returnType()).basicTypeKind(),
            firstLoopStateIndex + state);
        ++state;
      }
    }
    ...
    for (int c = 0, state = 0; c < nClauses; ++c) {
      ...
      if (!isVoid) {
        emitStoreInsn(
            cob,
            BasicType.basicType(stepType.returnType()).basicTypeKind(),
            firstLoopStateIndex + state);
        ++state;
      }
  ...
  private void emitLoopHandleInvoke(
      CodeBuilder cob,
      Name holder,
      int handles,
      int clause,
      Name args,
      boolean pushLocalState,
      MethodType type,
      Class<?>[] loopLocalStateTypes,
      int clauseDataSlot,
    ...
    if (pushLocalState) {
      for (int s = 0; s < loopLocalStateTypes.length; ++s) {
        emitLoadInsn(
            cob,
            BasicType.basicType(loopLocalStateTypes[s]).basicTypeKind(),
            firstLoopStateSlot + s);
      }
    }
  ...
  /**
   * Emits a return statement from a LF invoker. If required, the result type is cast to the correct
   * return type.
   */
  ...
  private void emitPrimCast(CodeBuilder cob, TypeKind from, TypeKind to) {
    ...
    if (from != to && from != TypeKind.BooleanType)
      try {
                      ...
                      public void accept(MethodBuilder mb) {
                        ...
                        mb.with(
                            RuntimeVisibleAnnotationsAttribute.of(
                                List.of(
                                    HIDDEN, // Suppress this method in backtraces displayed to the
                                            // user.
                        ...
                        mb.withCode(
                            new Consumer<CodeBuilder>() {
                              ...
                              public void accept(CodeBuilder cob) {
                                ...
                                cob
                                    .areturn(); // NOTE: NamedFunction invokers always return a
                                                // reference value.
                              }
                            });
                      }
  ...
  /**
   * Emit a bogus method that just loads some string constants. This is to get the constants into
   * the constant pool for debugging purposes.
   */
  ...
  static ClassDesc classDesc(Class<?> cls) {
    ...
    return cls.isPrimitive()
        ? Wrapper.forPrimitiveType(cls).basicClassDescriptor()
        : cls == MethodHandle.class
            ? CD_MethodHandle
            : cls == DirectMethodHandle.class
