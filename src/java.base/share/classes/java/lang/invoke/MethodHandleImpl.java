...
abstract class MethodHandleImpl {
  ...
  static MethodHandle makeArrayElementAccessor(Class<?> arrayClass, ArrayAccess access) {
    ...
    if (mh.type() != correctType) {
      ...
      /* if access == SET */ assert (access != ArrayAccess.SET
          || mh.type().parameterType(2) == Object.class);
  ...
  enum ArrayAccess {
    ...
    // As ArrayAccess and ArrayAccessor have a circular dependency, the ArrayAccess properties
    // cannot be stored in
    // final fields.
  ...
  static final class ArrayAccessor {
    ...
    static {
      ...
      cache[GETTER_INDEX] =
          OBJECT_ARRAY_GETTER =
              makeIntrinsic(getAccessor(Object[].class, ArrayAccess.GET), Intrinsic.ARRAY_LOAD);
      cache[SETTER_INDEX] =
          OBJECT_ARRAY_SETTER =
              makeIntrinsic(getAccessor(Object[].class, ArrayAccess.SET), Intrinsic.ARRAY_STORE);
      cache[LENGTH_INDEX] =
          OBJECT_ARRAY_LENGTH =
              makeIntrinsic(
                  getAccessor(Object[].class, ArrayAccess.LENGTH), Intrinsic.ARRAY_LENGTH);

      assert (InvokerBytecodeGenerator.isStaticallyInvocable(
          ArrayAccessor.OBJECT_ARRAY_GETTER.internalMemberName()));
      assert (InvokerBytecodeGenerator.isStaticallyInvocable(
          ArrayAccessor.OBJECT_ARRAY_SETTER.internalMemberName()));
      assert (InvokerBytecodeGenerator.isStaticallyInvocable(
          ArrayAccessor.OBJECT_ARRAY_LENGTH.internalMemberName()));
    }
  ...
  /**
   * Create a JVM-level adapter method handle to conform the given method handle to the similar
   * newType, using only pairwise argument conversions. For each argument, convert incoming argument
   * to the exact type needed. The argument conversions allowed are casting, boxing and unboxing,
   ...
   */
  static MethodHandle makePairwiseConvert(
      MethodHandle target, MethodType srcType, boolean strict, boolean monobox) {
  ...
  static MethodHandle makePairwiseConvertByEditor(
      MethodHandle target, MethodType srcType, boolean strict, boolean monobox) {
    ...
    if (convCount == 0) return target.viewAsType(srcType, strict);
    ...
    if (convSpec != null) {
      MethodHandle fn;
      if (convSpec instanceof Class) {
        if (convSpec == void.class) fn = null;
        else fn = getConstantHandle(MH_cast).bindTo(convSpec);
      } else {
        fn = (MethodHandle) convSpec;
      }
  ...
  static Object[] computeValueConversions(
      MethodType srcType, MethodType dstType, boolean strict, boolean monobox) {
  ...
  /**
   * Find a conversion function from the given source to the given destination. This conversion
   * function will be used as a LF NamedFunction. Return a Class object if a simple cast is needed.
   * Return void.class if void is involved.
   */
  static Object valueConversion(Class<?> src, Class<?> dst, boolean strict, boolean monobox) {
    assert (!VerifyType.isNullConversion(
        src, dst, /* keepInterfaces= */ strict)); // caller responsibility
    if (dst == void.class) return dst;
    MethodHandle fn;
    if (src.isPrimitive()) {
      if (src == void.class) {
        ...
        if (!VerifyType.isNullConversion(wsrc.wrapperType(), dst, strict)) {
          ...
          if (strict) fn = fn.asType(mt);
          else fn = MethodHandleImpl.makePairwiseConvert(fn, mt, /* strict= */ false);
        }
      }
    } else if (dst.isPrimitive()) {
  ...
  static final class AsVarargsCollector extends DelegatingMethodHandle {
    ...
    public MethodHandle asTypeUncached(MethodType newType) {
      ...
      if (acc != null && acc.type().parameterCount() == newArity) return acc.asType(newType);
    ...
    boolean viewAsTypeChecks(MethodType newType, boolean strict) {
      ...
      assert (type()
              .lastParameterType()
              .getComponentType()
              .isAssignableFrom(newType.lastParameterType().getComponentType()))
    ...
    public Object invokeWithArguments(Object... arguments) throws Throwable {
      ...
      Object collArgs =
          (elemType == Object.class)
              ? new Object[collected]
              : Array.newInstance(elemType, collected);
  ...
  static MethodHandle selectAlternative(
      boolean testResult, MethodHandle target, MethodHandle fallback) {
    if (testResult) {
      return target;
    } else {
      return fallback;
  ...
  static MethodHandle makeGuardWithTest(
      MethodHandle test, MethodHandle target, MethodHandle fallback) {
    MethodType type = target.type();
    assert (test.type().equals(type.changeReturnType(boolean.class))
        && fallback.type().equals(type));
  ...
  static MethodHandle makeBlockInliningWrapper(MethodHandle target) {
    ...
    return new CountingWrapper(
        target,
        lform,
        Makers.PRODUCE_BLOCK_INLINING_FORM,
  ...
  private static final class Makers {
     ...
     * method handle
     */
    static final Function<MethodHandle, LambdaForm> PRODUCE_BLOCK_INLINING_FORM =
        new Function<MethodHandle, LambdaForm>() {
          @Override
          public LambdaForm apply(MethodHandle target) {
            return DelegatingMethodHandle.makeReinvokerForm(
                target,
                MethodTypeForm.LF_DELEGATE_BLOCK_INLINING,
                CountingWrapper.class,
                false,
                DelegatingMethodHandle.NF_getTarget,
                CountingWrapper.NF_maybeStopCounting);
          }
        };

    /** Constructs simple reinvoker lambda form for a particular method handle */
    static final Function<MethodHandle, LambdaForm> PRODUCE_REINVOKER_FORM =
        new Function<MethodHandle, LambdaForm>() {
          @Override
          public LambdaForm apply(MethodHandle target) {
            return DelegatingMethodHandle.makeReinvokerForm(
    ...
    static final ClassValue<MethodHandle[]> TYPED_COLLECTORS =
        new ClassValue<MethodHandle[]>() {
          @Override
          protected MethodHandle[] computeValue(Class<?> type) {
  ...
  /**
   * Counting method handle. It has 2 states: counting and non-counting. It is in counting state for
   * the first n invocations and then transitions to non-counting state. Behavior in counting and
   * non-counting states is determined by lambda forms produced by countingFormProducer &
   * nonCountingFormProducer respectively.
   */
  static final class CountingWrapper extends DelegatingMethodHandle {
    ...
    boolean countDown() {
      ...
      if (c <= 1) {
        // Try to limit number of updates. MethodHandle.updateForm() doesn't guarantee LF update
        // visibility.
        if (isCounting) {
          isCounting = false;
          return true;
        ...
        wrapper.updateForm(
            new Function<>() {
              public LambdaForm apply(LambdaForm oldForm) {
                ...
                lform
                    .compileToBytecode(); // speed up warmup by avoiding LF interpretation again
                                          // after transition
                return lform;
              }
            });
    ...
    static {
      ...
      try {
        NF_maybeStopCounting =
            new NamedFunction(THIS_CLASS.getDeclaredMethod("maybeStopCounting", Object.class));
      } catch (ReflectiveOperationException ex) {
  ...
  static LambdaForm makeGuardWithTestForm(MethodType basicType) {
    ...
    if (PROFILE != -1) {
      names[PROFILE] =
          new Name(getFunction(NF_profileBoolean), names[CALL_TEST], names[GET_COUNTERS]);
    }
    ...
    names[SELECT_ALT] =
        new Name(
            new NamedFunction(
                makeIntrinsic(
                    getConstantHandle(MH_selectAlternative), Intrinsic.SELECT_ALTERNATIVE)),
            names[TEST],
            names[GET_TARGET],
            names[GET_FALLBACK]);
    ...
    lform =
        LambdaForm.create(lambdaType.parameterCount(), names, /* forceInline= */ true, Kind.GUARD);
   ...
   *
   * <p>Having t8 and t10 passed outside and not hardcoded into a lambda form allows to share lambda
  ...
  private static LambdaForm makeGuardWithCatchForm(MethodType basicType) {
    ...
    names[BOXED_ARGS] =
        new Name(new NamedFunction(makeIntrinsic(invokeBasic, Intrinsic.GUARD_WITH_CATCH)), args);

    // t_{i+1}:L=MethodHandleImpl.guardWithCatch(target:L,exType:L,catcher:L,t_{i}:L);
    Object[] gwcArgs =
        new Object[] {names[GET_TARGET], names[GET_CLASS], names[GET_CATCHER], names[BOXED_ARGS]};
    names[TRY_CATCH] = new Name(getFunction(NF_guardWithCatch), gwcArgs);

    // t_{i+2}:I=MethodHandle.invokeBasic(unbox:L,t_{i+1}:L);
    MethodHandle invokeBasicUnbox =
        MethodHandles.basicInvoker(MethodType.methodType(basicType.rtype(), Object.class));
  ...
  static MethodHandle makeGuardWithCatch(
  ...
  /**
   * Intrinsified during LambdaForm compilation (see {@link
   * InvokerBytecodeGenerator#emitGuardWithCatch emitGuardWithCatch}).
   */
  @Hidden
  static Object guardWithCatch(
      MethodHandle target, Class<? extends Throwable> exType, MethodHandle catcher, Object... av)
      throws Throwable {
  ...
  static MethodHandle fakeMethodHandleInvoke(MemberName method) {
    ...
    MethodType type =
        MethodType.methodType(
            Object.class, UnsupportedOperationException.class,
  ...
  static MethodHandle fakeVarHandleInvoke(MemberName method) {
    ...
    MethodType type =
        MethodType.methodType(
            method.getMethodType().returnType(),
  ...
  /**
   * Create an alias for the method handle which, when called, appears to be called from the same
   * class loader and protection domain as hostClass. This is an expensive no-op unless the method
   * which is called is sensitive to its caller. A small number of system methods are in this
   * category, including Class.forName and Method.invoke.
   */
  ...
  private static class BindCaller {
    ...
    private static MethodHandle restoreToType(
        MethodHandle vamh, MethodHandle original, Class<?> hostClass) {
    ...
    private static boolean checkInjectedInvoker(Class<?> hostClass, Class<?> invokerClass) {
      assert (hostClass.getClassLoader() == invokerClass.getClassLoader())
          : hostClass.getName() + " (CL)";
      try {
        assert (hostClass.getProtectionDomain() == invokerClass.getProtectionDomain())
            : hostClass.getName() + " (PD)";
      ...
      try {
        MH_checkCallerClass =
            IMPL_LOOKUP.findStatic(
                THIS_CLASS, "checkCallerClass", MethodType.methodType(boolean.class, Class.class));
    ...
    private static boolean checkCallerClass(Class<?> expected) {
      // This method is called via MH_checkCallerClass and so it's correct to ask for the immediate
      // caller here.
    ...
    private static byte[] generateInvokerTemplate() {
      ...
      //     @Hidden
      //     static Object reflect_invoke_V(MethodHandle vamh, Object target, Object[] args) throws
      // Throwable {
      ...
      return ClassFile.of()
          .build(
              ReferenceClassDescImpl.ofValidated("LInjectedInvoker;"),
              clb ->
                  clb.withFlags(ACC_PRIVATE | ACC_SUPER)
                      .withMethodBody(
                          "invoke_V",
                          MethodTypeDescImpl.ofValidated(
                              CD_Object, CD_MethodHandle, CD_Object_array),
                          ACC_STATIC,
                          cob ->
                              cob.aload(0)
                                  .aload(1)
                                  .invokevirtual(
                                      CD_MethodHandle,
                                      "invokeExact",
                                      MethodTypeDescImpl.ofValidated(CD_Object, CD_Object_array))
                                  .areturn())
                      .withMethodBody(
                          "reflect_invoke_V",
                          MethodTypeDescImpl.ofValidated(
                              CD_Object, CD_MethodHandle, CD_Object, CD_Object_array),
                          ACC_STATIC,
                          cob ->
                              cob.aload(0)
                                  .aload(1)
                                  ...
                                  .invokevirtual(
                                      ...
                                      "invokeExact",
                                      MethodTypeDescImpl.ofValidated(
                                          CD_Object, CD_Object, CD_Object_array))
                                  .areturn()));
    }
  }
  ...
  static MethodHandle makeWrappedMember(
      MethodHandle target, MemberName member, boolean isInvokeSpecial) {
  ...
  /**
   * Mark arbitrary method handle as intrinsic. InvokerBytecodeGenerator uses this info to produce
   * more efficient bytecode shape.
   */
  ...
  static MethodHandle makeIntrinsic(
      MethodHandle target, Intrinsic intrinsicName, Object intrinsicData) {
    if (intrinsicName == target.intrinsicName()) return target;
    ...
    if (mh1 != mh2) {
      String msg =
          String.format(
              ...
              mh1, ((MethodHandle) mh1).form, mh2, ((MethodHandle) mh2).form);
    ...
    try {
      return switch (func) {
        case NF_checkSpreadArgument ->
            new NamedFunction(
                MethodHandleImpl.class.getDeclaredMethod(
                    "checkSpreadArgument", Object.class, int.class));
        ...
        case NF_tryFinally ->
            new NamedFunction(
                MethodHandleImpl.class.getDeclaredMethod(
                    "tryFinally", MethodHandle.class, MethodHandle.class, Object[].class));
        case NF_loop ->
            new NamedFunction(
                MethodHandleImpl.class.getDeclaredMethod(
                    "loop", BasicType[].class, LoopClauses.class, Object[].class));
        case NF_throwException ->
            new NamedFunction(
                MethodHandleImpl.class.getDeclaredMethod("throwException", Throwable.class));
        case NF_profileBoolean ->
            new NamedFunction(
                MethodHandleImpl.class.getDeclaredMethod(
                    "profileBoolean", boolean.class, int[].class));
        case NF_tableSwitch ->
            new NamedFunction(
                MethodHandleImpl.class.getDeclaredMethod(
                    "tableSwitch",
  ...
  static {
    SharedSecrets.setJavaLangInvokeAccess(
        new JavaLangInvokeAccess() {
          @Override
          ...
          @Override
          public VarHandle memorySegmentViewHandle(
          ...
          @Override
          public VarHandle permuteCoordinates(
          ...
          public MethodHandle unreflectField(Field field, boolean isSetter)
              throws IllegalAccessException {
            return isSetter
                ? IMPL_LOOKUP.unreflectSetter(field)
                : IMPL_LOOKUP.unreflectGetter(field);
          }
  ...
  /**
   * Result unboxing: ValueConversions.unbox() OR ValueConversions.identity() OR
  ...
  static MethodHandle makeLoop(
    ...
    LoopClauses clauseData =
        new LoopClauses(
            new MethodHandle[][] {toArray(init), toArray(step), toArray(pred), toArray(fini)});
    ...
    try {
      mh =
          (BoundMethodHandle)
              data.factory()
                  .invokeBasic(
                      type, form, (Object) clauseData, (Object) collectArgs, (Object) unboxResult);
    } catch (Throwable ex) {
      throw uncaughtException(ex);
    }
    assert (mh.type() == type);
  ...
  /**
   * Loops introduce some complexity as they can have additional local state. Hence, LambdaForms for
   * loops are generated from a template. The LambdaForm template shape for the loop combinator is
   * as follows (assuming one reference parameter passed in {@code a1}, and a reference return type,
   * with the return value represented by {@code t12}):
   *
   * <blockquote>
   *
   * <pre>{@code
   ...
   * </blockquote>
   *
   * <p>{@code argL0} is a LoopClauses instance holding, in a 2-dimensional array, the init, step,
   * pred, and fini method handles. {@code argL1} and {@code argL2} are auxiliary method handles:
   * {@code argL1} boxes arguments and wraps them into {@code Object[]} ({@code
   * ValueConversions.array()}), and {@code argL2} unboxes the result if necessary ({@code
   * ValueConversions.unbox()}).
   *
   * <p>Having {@code t3} and {@code t4} passed in via a BMH and not hardcoded in the lambda form
   * allows to share lambda forms among loop combinators with the same basic type.
   *
   * <p>The above template is instantiated by using the {@link LambdaFormEditor} to replace the
   * {@code null} argument to the {@code loop} invocation with the {@code BasicType} array
   * describing the loop clause types. This argument is ignored in the loop invoker, but will be
   * extracted and used in {@linkplain InvokerBytecodeGenerator#emitLoop(int) bytecode generation}.
   */
  private static LambdaForm makeLoopForm(MethodType basicType, BasicType[] localVarTypes) {
    ...
    if (lform == null) {
      ...
      System.arraycopy(names, ARG_BASE, args, 1, ARG_LIMIT - ARG_BASE);
      names[BOXED_ARGS] =
          new Name(new NamedFunction(makeIntrinsic(invokeBasic, Intrinsic.LOOP)), args);

      // t_{i+1}:L=MethodHandleImpl.loop(localTypes:L,clauses:L,t_{i}:L);
      Object[] lArgs =
          new Object[] {
            null, // placeholder for BasicType[] localTypes - will be added by LambdaFormEditor
            names[GET_CLAUSE_DATA],
            names[BOXED_ARGS]
          };
  ...
  /**
   * Intrinsified during LambdaForm compilation (see {@link
   * InvokerBytecodeGenerator#emitLoop(int)}).
   */
  @Hidden
  static Object loop(BasicType[] localTypes, LoopClauses clauseData, Object... av)
      throws Throwable {
  ...
  /**
   * This method is bound as the predicate in {@linkplain MethodHandles#countedLoop(MethodHandle,
   * MethodHandle, MethodHandle) counting loops}.
   *
  ...
  /**
   * This method is bound as the step function in {@linkplain
   * MethodHandles#countedLoop(MethodHandle, MethodHandle, MethodHandle) counting loops} to
  ...
  /**
   * This is bound to initialize the loop-local iterator in {@linkplain MethodHandles#iteratedLoop
   * iterating loops}.
   *
  ...
  /**
   * This method is bound as the predicate in {@linkplain MethodHandles#iteratedLoop iterating
   * loops}.
   *
  ...
  /**
   * This method is bound as the step for retrieving the current value from the iterator in
   * {@linkplain MethodHandles#iteratedLoop iterating loops}.
   *
   ...
   */
  static MethodHandle makeTryFinally(
      MethodHandle target, MethodHandle cleanup, Class<?> rtype, Class<?>[] argTypes) {
  ...
  /**
   * The LambdaForm shape for the tryFinally combinator is as follows (assuming one reference
   * parameter passed in {@code a1}, and a reference return type, with the return value represented
   * by {@code t8}):
   *
   * <blockquote>
   *
   ...
   * </blockquote>
   *
   * <p>{@code argL0} and {@code argL1} are the target and cleanup method handles. {@code argL2} and
   * {@code argL3} are auxiliary method handles: {@code argL2} boxes arguments and wraps them into
   * {@code Object[]} ({@code ValueConversions.array()}), and {@code argL3} unboxes the result if
   * necessary ({@code ValueConversions.unbox()}).
   *
   * <p>Having {@code t4} and {@code t5} passed in via a BMH and not hardcoded in the lambda form
   * allows to share lambda forms among tryFinally combinators with the same basic type.
   */
  private static LambdaForm makeTryFinallyForm(MethodType basicType) {
    ...
    System.arraycopy(names, ARG_BASE, args, 1, ARG_LIMIT - ARG_BASE);
    names[BOXED_ARGS] =
        new Name(new NamedFunction(makeIntrinsic(invokeBasic, Intrinsic.TRY_FINALLY)), args);
  ...
  /**
   * Intrinsified during LambdaForm compilation (see {@link InvokerBytecodeGenerator#emitTryFinally
   * emitTryFinally}).
   */
  @Hidden
  static Object tryFinally(MethodHandle target, MethodHandle cleanup, Object... av)
      throws Throwable {
    Throwable t = null;
    Object r = null;
    try {
  ...
  private static MethodHandle makeCollector(Class<?> arrayType, int parameterCount) {
    MethodType type =
        MethodType.methodType(
            arrayType, Collections.nCopies(parameterCount, arrayType.componentType()));
  ...
  private static LambdaForm makeCollectorForm(MethodType basicType, Class<?> arrayType) {
    ...
    MethodHandle invokeBasic =
        MethodHandles.basicInvoker(MethodType.methodType(Object.class, int.class));
    names[CALL_NEW_ARRAY] =
        new Name(new NamedFunction(invokeBasic), names[GET_NEW_ARRAY], parameterCount);
    for (int storeIndex = 0, storeNameCursor = STORE_ELEMENT_BASE, argCursor = ARG_BASE;
        storeNameCursor < STORE_ELEMENT_LIMIT;
        storeIndex++, storeNameCursor++, argCursor++) {

      names[storeNameCursor] =
          new Name(
              new NamedFunction(makeIntrinsic(storeFunc, Intrinsic.ARRAY_STORE)),
              names[CALL_NEW_ARRAY],
              storeIndex,
              names[argCursor]);
    }
    ...
    LambdaForm lform =
        LambdaForm.create(lambdaType.parameterCount(), names, CALL_NEW_ARRAY, Kind.COLLECTOR);
    if (isSharedLambdaForm) {
  ...
  static class CasesHolder {
    @Stable final MethodHandle[] cases;
  ...
  static MethodHandle makeTableSwitch(
      MethodType type, MethodHandle defaultCase, MethodHandle[] caseActions) {
    MethodType varargsType = type.changeReturnType(Object[].class);
    MethodHandle collectArgs = varargsArray(type.parameterCount()).asType(varargsType);
  ...
  private static LambdaForm makeTableSwitchForm(
      MethodType basicType, BoundMethodHandle.SpeciesData data, int numCases) {
    MethodType lambdaType = basicType.invokerType();
...

    {
      ...
      names[BOXED_ARGS] =
          new Name(
              new NamedFunction(makeIntrinsic(invokeBasic, Intrinsic.TABLE_SWITCH, numCases)),
              args);
    }
  ...
  @Hidden
  static Object tableSwitch(int input, MethodHandle defaultCase, CasesHolder holder, Object[] args)
      throws Throwable {
  ...
  private static MethodHandle makeConstantHandle(int idx) {
    try {
      switch (idx) {
        case MH_cast:
          return IMPL_LOOKUP.findVirtual(
              Class.class, "cast", MethodType.methodType(Object.class, Object.class));
        case MH_selectAlternative:
          return IMPL_LOOKUP.findStatic(
              MethodHandleImpl.class,
              "selectAlternative",
              MethodType.methodType(
                  MethodHandle.class, boolean.class, MethodHandle.class, MethodHandle.class));
