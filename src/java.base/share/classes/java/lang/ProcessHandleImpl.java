...
final class ProcessHandleImpl implements ProcessHandle {
  ...
  @Native private static final int NOT_A_CHILD = -2;
              ...
              () -> {
                ...
                final long stackSize =
                    Boolean.getBoolean("jdk.lang.processReaperUseDefaultStackSize")
                        ? 0
                        : REAPER_DEFAULT_STACKSIZE + debugDelta;

                ThreadFactory threadFactory =
                    grimReaper -> {
                      Thread t =
                          InnocuousThread.newSystemThread(
                              "process reaper", grimReaper, stackSize, Thread.MAX_PRIORITY);
  ...
  static CompletableFuture<Integer> completion(long pid, boolean shouldReap) {
    ...
    while (completion == null || (shouldReap && !completion.isReaping)) {
      ...
      if (completion == null) {
        ...
        completion =
            completions.replace(pid, completion, newCompletion) ? null : completions.get(pid);
      }
      if (completion == null) {
  ...
  /**
   * The start time of this process. If STARTTIME_ANY, the start time of the process is not
   * available from the os. If greater than zero, the start time of the process.
   */
   ...
   * @return The ProcessHandle for the pid if the process is alive; or {@code null} if the process
   *     ID does not exist in the native system.
  ...
  /**
   * Returns a ProcessHandle for an existing native process known to be alive. The startTime of the
   * process is retrieved and stored in the ProcessHandle. It does not perform a security check
   * since it is called from ProcessImpl.
   *
  ...
  /**
   * Returns the native process ID. A {@code long} is used to be able to fit the system specific
   * binary values for the process.
   *
   ...
   *
   * @return a ProcessHandle of the parent process; {@code null} is returned if the child process
   *     does not have a parent
   * @throws SecurityException if permission is not granted by the security policy
   */
   ...
   *
   * @param pid if {@code pid} equals zero, then all known processes are returned; otherwise only
   *     direct child process pids are returned
   ...
   * @return if greater than or equal to zero is the number of pids in the array; if greater than
   *     the length of the arrays, the arrays are too small
   */
  private static native int getProcessPids0(long pid, long[] pids, long[] ppids, long[] starttimes);

  /**
   * Destroy the process for this ProcessHandle. The native code checks the start time before
   * sending the termination request.
   *
   * @param force {@code true} if the process should be terminated forcibly; else {@code false} for
  ...
  /**
   * Signal the process to terminate. The process is signaled only if its start time matches the
   * known start time.
   *
   ...
   *
   * @return {@code true} if the process represented by this {@code ProcessHandle} object has not
   *     yet terminated.
   * @since 9
   */
  @Override
   ...
   * @return the start time in milliseconds since 1970, 0 if the start time cannot be determined, -1
  ...
  /**
   * Implementation of ProcessHandle.Info. Information snapshot about a process. The attributes of a
   * process vary by operating system and are not available in all implementations. Additionally,
   * information about other processes is limited by the operating system privileges of the process
   * making the request. If a value is not available, either a {@code null} or {@code -1} is stored.
  ...
  static class Info implements ProcessHandle.Info {
    ...
    /**
     * Fill in this Info instance with information about the native process. If values are not
     * available the native code does not modify the field.
     *
    ...
    /**
     * Returns the Info object with the fields from the process. Whatever fields are provided by
     * native are returned. If the startTime of the process does not match the provided startTime
     * then an empty Info is returned.
     *
     * @param pid the native process identifier
     * @param startTime the startTime of the process being queried
     * @return ProcessHandle.Info non-null; individual fields may be null or -1 if not available.
     */
