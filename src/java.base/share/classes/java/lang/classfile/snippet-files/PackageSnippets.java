...
class PackageSnippets {
  ...
  void enumerateFieldsMethods1(byte[] bytes) {
    ...
    for (FieldModel fm : cm.fields()) System.out.printf("Field %s%n", fm.fieldName().stringValue());
                                ...
                                mb.withCode(
                                    cob ->
                                        cob.getstatic(CD_System, "out", CD_PrintStream)
                                            .ldc("Hello World")
                                            .invokevirtual(
                                                CD_PrintStream, "println", MTD_void_String)
                                            .return_())));
    // @end
  }
    ...
    ClassTransform ct =
        (builder, element) -> {
          if (!(element instanceof MethodModel mm
              && mm.methodName().stringValue().startsWith("debug"))) builder.with(element);
        };
    var cc = ClassFile.of();
  ...
  void stripDebugMethods3(byte[] bytes) {
    ...
    ClassTransform ct =
        ClassTransform.dropping(
            element ->
                element instanceof MethodModel mm
                    && mm.methodName().stringValue().startsWith("debug"));
    // @end
  }
    ...
    CodeTransform fooToBar =
        (b, e) -> {
          switch (e) {
            ...
            default ->
                throw new IllegalArgumentException(
                    "An unknown instruction could not be handled by this transformation");
          }
        };
    // @end
  }
                            ...
                            instrumentorClassRemapper
                                ...
                                .andThen(
                                    (codeBuilder, instrumentorCodeElement) -> {
                                      // all invocations of target methods from instrumentor are
                                      // inlined
                                      if (instrumentorCodeElement instanceof InvokeInstruction inv
                                        ...
                                        // inlined target locals must be shifted based on the actual
                                        // instrumentor locals
                                        codeBuilder.block(
                                            inlinedBlockBuilder ->
                                                inlinedBlockBuilder.transform(
                                                    ...
                                                    CodeLocalsShifter.of(
                                                            mm.flags(), mm.methodTypeSymbol())
                                                        .andThen(CodeRelabeler.of())
                                                        .andThen(
                                                            (innerBuilder, shiftedTargetCode) -> {
                                                              // returns must be replaced with jump
                                                              // to the end of the inlined method
  ...
  void resolverExample() {
    ...
    MethodHandles.Lookup lookup =
        MethodHandles.lookup(); // @replace regex="MethodHandles\.lookup\(\)" replacement="..."
