...
class PackageSnippets {
  ...
  void assertContains(ClassPrinter.Node node, ConstantDesc key, ConstantDesc value) {
    if (!node.walk()
        .anyMatch(
            n ->
                n instanceof ClassPrinter.LeafNode ln
                    && ln.name().equals(key)
  ...
  void singleClassRemap(ClassModel... allMyClasses) {
    ...
    var classRemapper = ClassRemapper.of(Map.of(CD_Foo, CD_Bar));
    var cc = ClassFile.of();
        ...
        ClassRemapper.of(
            cd ->
                ClassDesc.ofDescriptor(
                    cd.descriptorString().replace("Lcom/oldpackage/", "Lcom/newpackage/")));
    var cc = ClassFile.of();
    for (var classModel : allMyClasses) {
      byte[] newBytes = classRemapper.remapClass(cc, classModel);
    }
        ...
        ClassFile.of()
            .transformClass(
                ...
                (classBuilder, classElement) -> {
                  ...
                  else classBuilder.accept(classElement);
                });
    // @end
  }
