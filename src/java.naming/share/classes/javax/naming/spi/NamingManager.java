 ...
 *
 * <p>Except as otherwise noted, a {@code Name} or environment parameter passed to any method is
...
public class NamingManager {
   ...
   * @throws SecurityException builder cannot be installed for security reasons.
   * @throws NamingException builder cannot be installed for a non-security-related reason.
   ...
   *
   * <ol>
   *   <li>If {@code refInfo} is a {@code Reference} or {@code Referenceable} containing a factory
   *       class name, use the named factory to create the object. Return {@code refInfo} if the
   *       factory cannot be created. Under JDK 1.1, if the factory class must be loaded from a
   *       location specified in the reference, a {@code SecurityManager} must have been installed
   *       or the factory creation will fail. If an exception is encountered while creating the
   *       factory, it is passed up to the caller.
   *   <li>If {@code refInfo} is a {@code Reference} or {@code Referenceable} with no factory class
   *       name, and the address or addresses are {@code StringRefAddr}s with address type "URL",
   *       try the URL context factory corresponding to each URL's scheme id to create the object
   *       (see {@code getURLContext()}). If that fails, continue to the next step.
   *   <li>Use the object factories specified in the {@code Context.OBJECT_FACTORIES} property of
   *       the environment, and of the provider resource file associated with {@code nameCtx}, in
   ...
   *       used. If none of the factories can be loaded, return {@code refInfo}. If an exception is
   *       encountered while creating the object, the exception is passed up to the caller.
   * </ol>
   *
   * <p>Service providers that implement the {@code DirContext} interface should use {@code
   * DirectoryManager.getObjectInstance()}, not this method. Service providers that implement only
   * the {@code Context} interface should use this method.
   *
   * <p>Note that an object factory (an object that implements the ObjectFactory interface) must be
   * public and must have a public constructor that accepts no arguments. In cases where the factory
   * is in a named module then it must be in a package which is exported by that module to the
   * {@code java.naming} module.
   *
   * <p>The {@code name} and {@code nameCtx} parameters may optionally be used to specify the name
   * of the object being created. {@code name} is the name of the object, relative to context {@code
   * nameCtx}. This information could be useful to the object factory or to the object
   * implementation. If there are several possible contexts from which the object could be named --
   * as will often be the case -- it is up to the caller to select one. A good rule of thumb is to
   * select the "deepest" context available. If {@code nameCtx} is null, {@code name} is relative to
   * the default initial context. If no name is being specified, the {@code name} parameter should
   * be null.
   *
   * @param refInfo The possibly null object for which to create an object.
   * @param name The name of this object relative to {@code nameCtx}. Specifying a name is optional;
   *     if it is omitted, {@code name} should be null.
   * @param nameCtx The context relative to which the {@code name} parameter is specified. If null,
   ...
   * @return An object created using {@code refInfo}; or {@code refInfo} if an object cannot be
   *     created using the algorithm described above.
   * @throws NamingException if a naming exception was encountered while attempting to get a URL
   *     context, or if one of the factories accessed throws a NamingException.
   * @throws Exception if one of the factories accessed throws an exception, or if an error was
  ...
  public static Object getObjectInstance(
    ...
    return NamingManagerHelper.getObjectInstance(
        refInfo, name, nameCtx, environment, ObjectFactoriesFilter::checkGlobalFilter);
  }

  /**
   * Retrieves a context identified by {@code obj}, using the specified environment. Used by
   * ContinuationContext.
   *
   * @param obj The object identifying the context.
   * @param name The name of the context being returned, relative to {@code nameCtx}, or null if no
   *     name is being specified. See the {@code getObjectInstance} method for details.
   * @param nameCtx The context relative to which {@code name} is specified, or null for the default
   *     initial context. See the {@code getObjectInstance} method for details.
   ...
   *
   * <p>The resulting context is for resolving URLs of the scheme {@code scheme}. The resulting
   ...
   * package specified as follows. The {@code Context.URL_PKG_PREFIXES} environment property (which
   * may contain values taken from system properties, or application resource files) contains a
   * colon-separated list of package prefixes. Each package prefix in the property is tried in the
   * order specified to load the factory class. The default package prefix is "com.sun.jndi.url" (if
   * none of the specified packages work, this default is tried). The complete package name is
   * constructed using the package prefix, concatenated with the scheme id.
   *
   * <p>For example, if the scheme id is "ldap", and the {@code Context.URL_PKG_PREFIXES} property
   ...
   * resulting context.
   *
   * <p>{@code factory.getObjectInstance(null, environment);}
   *
   ...
   * @return A context for resolving URLs with the scheme id {@code scheme}; {@code null} if the
   *     factory for creating the context is not found.
   * @throws NamingException If a naming exception occurs while creating the context.
   * @see #getObjectInstance
   * @see ObjectFactory#getObjectInstance
   */
   ...
   *
   * <ul>
   *   <li>If an InitialContextFactoryBuilder has been installed, it is used to create the factory
   *       for creating the initial context
   *   <li>Otherwise, the class specified in the {@code Context.INITIAL_CONTEXT_FACTORY} environment
   *       property is used
   *       <ul>
   *         <li>First, the {@linkplain java.util.ServiceLoader ServiceLoader} mechanism tries to
   *             locate an {@code InitialContextFactory} provider using the current thread's context
   *             class loader
   *         <li>Failing that, this implementation tries to locate a suitable {@code
   *             InitialContextFactory} using a built-in mechanism <br>
   *             (Note that an initial context factory (an object that implements the
   *             InitialContextFactory interface) must be public and must have a public constructor
   *             that accepts no arguments. In cases where the factory is in a named module then it
   *             must be in a package which is exported by that module to the {@code java.naming}
   *             module.)
   *       </ul>
   * </ul>
   *
   ...
   * @throws NoInitialContextException If the {@code Context.INITIAL_CONTEXT_FACTORY} property is
   *     not found or names a nonexistent class or a class that cannot be instantiated, or if the
  ...
  private static InitialContextFactory getFactory(String className) {
    ...
    try {
      ...
      NoInitialContextException ne =
          new NoInitialContextException(
              "Cannot load initial context factory " + "'" + className + "'");
      ne.setRootCause(e);
  ...
  /**
   * Constant that holds the name of the environment property into which {@code
   * getContinuationContext()} stores the value of its {@code CannotProceedException} parameter.
   * This property is inherited by the continuation context, and may be used by that context's
   * service provider to inspect the fields of the exception.
   *
   ...
   * implementation does this by first constructing a {@code CannotProceedException} containing
   * information pinpointing how far it has proceeded. It then obtains a continuation context from
   * JNDI by calling {@code getContinuationContext}. The context implementation should then resume
   * the context operation by invoking the same operation on the continuation context, using the
   * remainder of the name that has not yet been resolved.
   *
   * <p>Before making use of the {@code cpe} parameter, this method updates the environment
   * associated with that object by setting the value of the property <a href="#CPE">{@code CPE}</a>
   * to {@code cpe}. This property will be inherited by the continuation context, and may be used by
   ...
   *
   * <p>Service providers that implement the {@code DirContext} interface should use {@code
   * DirectoryManager.getStateToBind()}, not this method. Service providers that implement only the
   * {@code Context} interface should use this method.
   *
   * <p>This method uses the specified state factories in the {@code Context.STATE_FACTORIES}
   * property from the environment properties, and from the provider resource file associated with
   * {@code nameCtx}, in that order. The value of this property is a colon-separated list of factory
   * class names that are tried in order, and the first one that succeeds in returning the object's
   * state is the one used. If no object's state can be retrieved in this way, return the object
   * itself. If an exception is encountered while retrieving the state, the exception is passed up
   * to the caller.
   *
   ...
   *
   * <p>The {@code name} and {@code nameCtx} parameters may optionally be used to specify the name
   * of the object being created. See the description of "Name and Context Parameters" in {@link
   * ObjectFactory#getObjectInstance ObjectFactory.getObjectInstance()} for details.
   *
   * <p>This method may return a {@code Referenceable} object. The service provider obtaining this
   * object may choose to store it directly, or to extract its reference (using {@code
   * Referenceable.getReference()}) and store that instead.
   *
   * @param obj The non-null object for which to get state to bind.
   * @param name The name of this object relative to {@code nameCtx}, or null if no name is
   *     specified.
   * @param nameCtx The context relative to which the {@code name} parameter is specified, or null
   *     if {@code name} is relative to the default initial context.
   * @param environment The possibly null environment to be used in the creation of the state
   *     factory and the object's state.
   * @return The non-null object representing {@code obj}'s state for binding. It could be the
   *     object ({@code obj}) itself.
   * @throws NamingException If one of the factories accessed throws an exception, or if an error
   *     was encountered while loading and instantiating the factory and object classes. A factory
   *     should only throw an exception if it does not want other factories to be used in an attempt
   *     to create an object. See {@code StateFactory.getStateToBind()}.
   * @see StateFactory
  ...
  /**
   * Thrown when an error is encountered while loading and instantiating the context factory
   * classes.
   */
  private static class FactoryInitializationError extends Error {
    @java.io.Serial static final long serialVersionUID = -5805552256848841560L;
