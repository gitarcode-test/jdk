 ...
 *
 * <p>Implementation is not thread-safe. Caller must sync as per JNDI spec. Members that are used
 * directly or indirectly by internal worker threads (Connection, EventQueue, NamingEventNotifier)
 * must be thread-safe. Connection - calls LdapClient.processUnsolicited(), which in turn calls
 * LdapCtx.convertControls() and LdapCtx.fireUnsolicited(). convertControls() - no sync; reads
 * envprops and 'this' fireUnsolicited() - sync on eventSupport for all references to 'unsolicited'
 * (even those in other methods); don't sync on LdapCtx in case caller is already sync'ing on it -
 * this would prevent Unsol events from firing and the Connection thread to block (thus preventing
 * any other data from being read from the connection) References to 'eventSupport' need not be
 * sync'ed because these methods can only be called after eventSupport has been set first (via
 * addNamingListener()). EventQueue - no direct or indirect calls to LdapCtx NamingEventNotifier -
 * calls newInstance() to get instance for run() to use; no sync needed for methods invoked on new
 * instance;
 *
 * <p>LdapAttribute links to LdapCtx in order to process getAttributeDefinition() and
 * getAttributeSyntaxDefinition() calls. It invokes LdapCtx.getSchema(), which uses schemaTrees (a
 * Hashtable - already sync). Potential conflict of duplicating construction of tree for same
 * subschemasubentry but no inconsistency problems.
 *
 * <p>NamingEnumerations link to LdapCtx for the following: 1. increment/decrement enum count so
 * that ctx doesn't close the underlying connection 2. LdapClient handle to get next batch of
 * results 3. Sets LdapCtx's response controls 4. Process return code 5. For narrowing response
 * controls (using ctx's factories) Since processing of NamingEnumeration by client is treated the
 * same as method invocation on LdapCtx, caller is responsible for locking.
 *
 * @author Vincent Ryan
 * @author Rosanna Lee
 */
public final class LdapCtx extends ComponentDirContext implements EventDirContext, LdapContext {
  ...
  static final String DEFAULT_SSL_FACTORY = "javax.net.ssl.SSLSocketFactory"; // use Sun's SSL
  ...
  LdapCtx(LdapCtx existing, String newDN) throws NamingException {
    ...
    parentIsLdapCtx =
        ((newDN == null || newDN.equals(existing.currentDN)) ? existing.parentIsLdapCtx : true);
  ...
  protected void c_bind(Name name, Object obj, Attributes attrs, Continuation cont)
    ...
    try {
      ...
      if (obj == null) {
        ...
        attrs =
            Obj.determineBindAttrs(
                addrEncodingSeparator, obj, attrs, false, name, this, envprops); // not cloned
      }
      ...
      if (handleReferrals == LdapClient.LDAP_REF_THROW) throw cont.fillInException(e);
  ...
  protected void c_rebind(Name name, Object obj, Attributes attrs, Continuation cont)
    ...
    try {
      ...
      if (attrs == null) {
        ...
        Attribute origObjectClass = origAttrs.get(Obj.JAVA_ATTRIBUTES[Obj.OBJECT_CLASS]);
      ...
      if (obj != null) {
        attrs =
            Obj.determineBindAttrs(
                addrEncodingSeparator, obj, attrs, inputAttrs != attrs, name, this, envprops);
      }
      ...
      if ((addEx != null && !(addEx instanceof LdapReferralException))
        ...
        LdapResult answer2 = clnt.add(new LdapEntry(newDN, keepAttrs), reqCtls);
  ...
  protected void c_rename(Name oldName, Name newName, Continuation cont) throws NamingException {
    ...
    try {
      ...
      if (!oldParent.equals(newParent)) {
        if (!clnt.isLdapv3) {
          throw new InvalidNameException(
              "LDAPv2 doesn't support changing " + "the parent as a result of a rename");
        } else {
      ...
      LdapResult answer =
          clnt.moddn(fullyQualifiedName(oldName), newRDN, deleteRDN, newSuperior, reqCtls);
      respCtls = answer.resControls; // retrieve response controls
  ...
  protected DirContext c_createSubcontext(Name name, Attributes attrs, Continuation cont)
    ...
    try {
      ...
      if (attrs == null) {
        ...
        Attribute oc =
            new BasicAttribute(
                Obj.JAVA_ATTRIBUTES[Obj.OBJECT_CLASS], Obj.JAVA_OBJECT_CLASSES[Obj.STRUCTURAL]);
        oc.add("top");
  ...
  /**
   * Adds attributes from RDN to attrs if not already present. Note that if attrs already contains
   * an attribute by the same name, or if the distinguished name is empty, then leave attrs
   * unchanged.
   *
   ...
   */
  private static Attributes addRdnAttributes(String dn, Attributes attrs, boolean directUpdate)
      throws NamingException {
    ...
    while (enum_.hasMore()) {
      ...
      if (attrs.get(nameAttr.getID()) == null) {
        ...
        /**
         * When attrs.isCaseIgnored() is false, attrs.get() will return null when the case
         * mis-matches for otherwise equal attrIDs. As the attrIDs' case is irrelevant for LDAP,
         * ignore the case of attrIDs even when attrs.isCaseIgnored() is false. This is done by
         * explicitly comparing the elements in the enumeration of IDs with their case ignored.
         */
  ...
  private static boolean containsIgnoreCase(NamingEnumeration<String> enumStr, String str)
      throws NamingException {
    String strEntry;
  ...
  private void adjustDeleteStatus(String fname, LdapResult answer) {
    if (answer.status == LdapClient.LDAP_NO_SUCH_OBJECT && answer.matchedDN != null) {
      try {
        // %%% RL: are there any implications for referrals?

        Name orig = parser.parse(fname);
        Name matched = parser.parse(answer.matchedDN);
        if ((orig.size() - matched.size()) == 1) answer.status = LdapClient.LDAP_SUCCESS;
      } catch (NamingException e) {
      }
    }
  }
  ...
  protected Object c_lookup(Name name, Continuation cont) throws NamingException {
    ...
    try {
      return NamingManagerHelper.getDirObjectInstance(
          obj, name, this, envprops, attrs, ObjectFactoriesFilter::checkLdapFilter);
  ...
  protected NamingEnumeration<NameClassPair> c_list(Name name, Continuation cont)
    ...
    try {
      ...
      if ((answer.status != LdapClient.LDAP_SUCCESS) || (answer.referrals != null)) {
        processReturnCode(answer, name);
      }
      ...
      LdapNamingEnumeration res = new LdapNamingEnumeration(this, answer, name, cont);
  ...
  protected NamingEnumeration<Binding> c_listBindings(Name name, Continuation cont)
    ...
    try {
      ...
      LdapBindingEnumeration res = new LdapBindingEnumeration(this, answer, name, cont);
  ...
  private String fullyQualifiedName(Name rel) {
    return rel.isEmpty() ? currentDN : fullyQualifiedName(rel.get(0));
  }
  ...
  protected DirContext c_getSchemaClassDefinition(Name name, Continuation cont)
    ...
    try {
      ...
      Attribute objectClassAttr =
          c_getAttributes(name, new String[] {"objectclass"}, cont).get("objectclass");
      ...
      Context ocSchema =
          (Context) c_getSchema(name, cont).lookup(LdapSchemaParser.OBJECTCLASS_DEFINITION_NAME);
  ...
  private DirContext getSchemaTree(Name name) throws NamingException {
    ...
    if (schemaTree == null) {
      if (debug) {
        System.err.println("LdapCtx: building new schema tree " + this);
      }
  ...
  private DirContext buildSchemaTree(String subschemasubentry) throws NamingException {
    ...
    NamingEnumeration<SearchResult> results =
        searchAux(sse, "(objectClass=subschema)", constraints, false, true, new Continuation());
    ...
    return LdapSchemaCtx.createSchemaTree(
        envprops,
        subschemasubentry,
  ...
  private String getSchemaEntry(Name name, boolean relative) throws NamingException {
    ...
    NamingEnumeration<SearchResult> results;
    try {
      results = searchAux(name, "objectclass=*", constraints, relative, true, new Continuation());
  ...
  // --------------------- Searches -----------------------------
  protected NamingEnumeration<SearchResult> c_search(
      Name name, Attributes matchingAttributes, Continuation cont) throws NamingException {
    return c_search(name, matchingAttributes, null, cont);
  }

  protected NamingEnumeration<SearchResult> c_search(
      Name name, Attributes matchingAttributes, String[] attributesToReturn, Continuation cont)
      throws NamingException {
    SearchControls cons = new SearchControls();
  ...
  protected NamingEnumeration<SearchResult> c_search(
      Name name, String filter, SearchControls cons, Continuation cont) throws NamingException {
    return searchAux(name, filter, cloneSearchControls(cons), true, waitForReply, cont);
  }

  protected NamingEnumeration<SearchResult> c_search(
      Name name, String filterExpr, Object[] filterArgs, SearchControls cons, Continuation cont)
      throws NamingException {
    String strfilter;
    try {
  ...
  NamingEnumeration<SearchResult> searchAux(
    ...
    if (cons.getReturningObjFlag()) {
      if (reqAttrs != null) {
        ...
        if (!hasWildcard) {
          String[] totalAttrs = new String[reqAttrs.length + Obj.JAVA_ATTRIBUTES.length];
          System.arraycopy(reqAttrs, 0, totalAttrs, 0, reqAttrs.length);
          System.arraycopy(
              Obj.JAVA_ATTRIBUTES, 0, totalAttrs, reqAttrs.length, Obj.JAVA_ATTRIBUTES.length);
    ...
    LdapCtx.SearchArgs args = new LdapCtx.SearchArgs(name, filter, cons, reqAttrs);
    ...
    try {
      ...
      LdapSearchEnumeration res =
          new LdapSearchEnumeration(this, answer, fullyQualifiedName(name), args, cont);
  ...
  private LdapResult doSearchOnce(Name name, String filter, SearchControls cons, boolean relative)
      throws NamingException {
  ...
  private LdapResult doSearch(
      Name name, String filter, SearchControls cons, boolean relative, boolean waitForReply)
      throws NamingException {
    ensureOpen();
    try {
      int scope;
      ...
      String nm = (relative ? fullyQualifiedName(name) : (name.isEmpty() ? "" : name.get(0)));
  ...
  /**
   * Sets the domain name for the context in the com.sun.jndi.ldap.domainname property. Used for
   * hostname verification by Start TLS
   */
  ...
  private void setDeleteRDN(String deleteRDNProp) {
    if ((deleteRDNProp != null) && (deleteRDNProp.equalsIgnoreCase("false"))) {
      deleteRDN = false;
    } else {
      deleteRDN = DEFAULT_DELETE_RDN;
    }
  ...
  private void setTypesOnly(String typesOnlyProp) {
    if ((typesOnlyProp != null) && (typesOnlyProp.equalsIgnoreCase("true"))) {
      typesOnly = true;
    } else {
      typesOnly = DEFAULT_TYPES_ONLY;
    }
  ...
  /**
   * Sets the referral mode of this context to 'follow', 'throw' or 'ignore'. If referral mode is
   * 'ignore' then activate the manageReferral control.
   */
    ...
    if (deref != null) {
      switch (deref) {
        ...
        case "always":
          ...
          break;
        default:
          throw new IllegalArgumentException("Illegal value for " + DEREF_ALIASES + " property.");
      }
    } else {
      derefAliases = DEFAULT_DEREF_ALIASES;
    }
  ...
  private void setReferralLimit(String referralLimitProp) {
    ...
    if (referralLimitProp != null) {
      ...
      if (referralHopLimit == 0) referralHopLimit = Integer.MAX_VALUE;
    } else {
      referralHopLimit = DEFAULT_REFERRAL_LIMIT;
    }
  }
  ...
  private void setWaitForReply(String waitForReplyProp) {
    if (waitForReplyProp != null && (waitForReplyProp.equalsIgnoreCase("false"))) {
      waitForReply = false;
    } else {
      waitForReply = true;
    }
  ...
  public void close() throws NamingException {
    ...
    try {
      ...
      if (enumCount > 0) {
        if (debug) System.err.println("LdapCtx: close deferred");
        closeRequested = true;
        return;
      }
      closeConnection(SOFT_CLOSE);
  ...
  // Returns true if TLS connection opened using "ldaps" scheme, or using "ldap" and then upgraded
  // with
  ...
  private void ensureCanTransmitCredentials(String authMechanism) throws NamingException {
    ...
    // or 'mechsAllowedToSendCredentials' is set to any value via system/context environment
    // properties
    if (!isConnectionEncrypted() && (contextSeenStartTlsEnabled || anyPropertyIsSet)) {
      ...
      Set<String> allowedAuthMechs =
          useSpMechsCache
              ? MECHS_ALLOWED_BY_SP
              : getMechsFromPropertyValue(allowedMechanismsOrTrue);
  ...
  private void connect(boolean startTLS) throws NamingException {
    if (debug) {
      System.err.println("LdapCtx: Connecting " + this);
    }
    ...
    if (envprops != null) {
      ...
      secProtocol = useSsl ? "ssl" : (String) envprops.get(Context.SECURITY_PROTOCOL);
      socketFactory = (String) envprops.get(SOCKET_FACTORY);
      authMechanism = (String) envprops.get(Context.SECURITY_AUTHENTICATION);

      usePool = "true".equalsIgnoreCase((String) envprops.get(ENABLE_POOL));
    }

    if (socketFactory == null) {
      socketFactory = "ssl".equals(secProtocol) ? DEFAULT_SSL_FACTORY : null;
    }
    ...
    try {
      ...
      if (initial) {
        ldapVersion = (ver != null) ? Integer.parseInt(ver) : DEFAULT_LDAP_VERSION;
        ...
        /** Pooled connections are preauthenticated; newly created ones are not. */
      ...
      try {
        ...
        answer =
            clnt.authenticate(
                initial, user, passwd, ldapVersion, authMechanism, bindCtls, envprops);
      } finally {
        startTlsLock.unlock();
      }
      ...
      if (handleReferrals == LdapClient.LDAP_REF_THROW) throw e;
  ...
  protected void processReturnCode(
      LdapResult res,
      Name resolvedName,
      Object resolvedObj,
      Name remainName,
      ...
      case LdapClient.LDAP_SUCCESS:
        ...
        if (res.referrals != null) {
          ...
          if (hopCount > referralHopLimit) {
            NamingException lee = new LimitExceededException("Referral limit exceeded");
            lee.setRootCause(r);
            throw lee;
          }
        }
  ...
  public ExtendedResponse extendedOperation(ExtendedRequest request) throws NamingException {
    ...
    try {
      ...
      LdapResult answer =
          clnt.extendedOp(request.getID(), request.getEncodedValue(), reqCtls, startTLS);
      respCtls = answer.resControls; // retrieve response controls
      ...
      int len = (answer.extensionValue == null) ? 0 : answer.extensionValue.length;

      ExtendedResponse er =
          request.createExtendedResponse(answer.extensionId, answer.extensionValue, 0, len);

      if (er instanceof StartTlsResponseImpl) {
        // Pass the connection handle to StartTlsResponseImpl
        String domainName = (String) (envprops != null ? envprops.get(DOMAIN_NAME) : null);
  ...
  public void addNamingListener(String nm, int scope, NamingListener l) throws NamingException {
    if (eventSupport == null) eventSupport = new EventSupport(this);
    eventSupport.addNamingListener(getTargetName(new CompositeName(nm)), scope, l);
  ...
  public void removeNamingListener(NamingListener l) throws NamingException {
    if (eventSupport == null) return; // no activity before, so just return

    eventSupport.removeNamingListener(l);

    // If removing an Unsol listener and it is the last one, let clnt know
    if (l instanceof UnsolicitedNotificationListener && !eventSupport.hasUnsolicited()) {
      removeUnsolicited();
    }
  }
  ...
  public void addNamingListener(String nm, String filter, SearchControls ctls, NamingListener l)
    ...
    eventSupport.addNamingListener(
        getTargetName(new CompositeName(nm)), filter, cloneSearchControls(ctls), l);
  ...
  public void addNamingListener(
      Name nm, String filter, Object[] filterArgs, SearchControls ctls, NamingListener l)
      throws NamingException {
    addNamingListener(getTargetName(nm), filter, filterArgs, ctls, l);
  }

  public void addNamingListener(
      String nm, String filterExpr, Object[] filterArgs, SearchControls ctls, NamingListener l)
      throws NamingException {
  ...
  /**
   * Retrieves the target name for which the listener is registering. If nm is a CompositeName, use
   * its first and only component. It cannot have more than one components because a target be
   * outside of this namespace. If nm is not a CompositeName, then treat it as a compound name.
   *
  ...
  /**
   * Registers this context with the underlying LdapClient. When the underlying LdapClient receives
   * an unsolicited notification, it will invoke LdapCtx.fireUnsolicited() so that this context can
   * (using EventSupport) notified any registered listeners. This method is called by EventSupport
  ...
  /**
   * Removes this context from registering interest in unsolicited notifications from the underlying
   * LdapClient. This method is called under any one of the following conditions:
   *
   * <ul>
   ...
   * </ul>
   *
   * After this method has been called, this context will not pass on any events related to
   * unsolicited notifications to EventSupport and and its listeners.
   */
  ...
  /**
   * Uses EventSupport to fire an event related to an unsolicited notification. Called by LdapClient
   * when LdapClient receives an unsolicited notification.
   */
