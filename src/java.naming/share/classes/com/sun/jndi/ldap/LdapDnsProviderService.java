...
/**
 * The {@code LdapDnsProviderService} is responsible for creating and providing access to the
 * registered {@code LdapDnsProvider}s. The {@link ServiceLoader} is used to find and register any
 * implementations of {@link LdapDnsProvider}.
 *
...
final class LdapDnsProviderService {
  ...
  private LdapDnsProviderService() {
    ...
    if (sm == null) {
      providers = ServiceLoader.load(LdapDnsProvider.class, ClassLoader.getSystemClassLoader());
    } else {
      final PrivilegedAction<ServiceLoader<LdapDnsProvider>> pa =
          () -> ServiceLoader.load(LdapDnsProvider.class, ClassLoader.getSystemClassLoader());
  ...
  /**
   * Retrieves result from the first provider that successfully resolves the endpoints. If no
   * results are found when calling installed subclasses of {@code LdapDnsProvider} then this method
   * will fall back to the {@code DefaultLdapDnsProvider}.
   *
   * @throws NamingException if the {@code url} is not valid or an error occurred while performing
   *     the lookup.
   */
  LdapDnsProviderResult lookupEndpoints(String url, Hashtable<?, ?> env) throws NamingException {
    ...
    if (result == null) {
      return new DefaultLdapDnsProvider()
          .lookupEndpoints(url, env)
