...
/**
 * There is a single global LogManager object that is used to maintain a set of shared state about
 * Loggers and log services.
 *
 * <p>This LogManager object:
 *
 * <ul>
 *   <li>Manages a hierarchical namespace of Logger objects. All named Loggers are stored in this
 *       namespace.
 *   <li>Manages a set of logging control properties. These are simple key-value pairs that can be
 *       used by Handlers and other logging objects to configure themselves.
 * </ul>
 *
 * <p>The global LogManager object can be retrieved using LogManager.getLogManager(). The LogManager
 * object is created during class initialization and cannot subsequently be changed.
 *
 * <p>At startup the LogManager class is located using the java.util.logging.manager system
 * property.
 *
 * <h2>LogManager Configuration</h2>
 *
 * A LogManager initializes the logging configuration via the {@link #readConfiguration()} method
 * during LogManager initialization. By default, LogManager default configuration is used. The
 * logging configuration read by LogManager must be in the {@linkplain Properties properties file}
 * format.
 *
 * <p>The LogManager defines two optional system properties that allow control over the initial
 * configuration, as specified in the {@link #readConfiguration()} method:
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>These two system properties may be specified on the command line to the "java" command, or as
 * system property definitions passed to JNI_CreateJavaVM.
 *
 * <p>The {@linkplain Properties properties} for loggers and Handlers will have names starting with
 * the dot-separated name for the handler or logger.<br>
 * The global logging properties may include:
 *
 * <ul>
 *   <li>A property "handlers". This defines a whitespace or comma separated list of class names for
 *       handler classes to load and register as handlers on the root Logger (the Logger named "").
 *       Each class name must be for a Handler class which has a default constructor. Note that
 *       these Handlers may be created lazily, when they are first used.
 *   <li>A property "&lt;logger&gt;.handlers". This defines a whitespace or comma separated list of
 *       class names for handlers classes to load and register as handlers to the specified logger.
 *       Each class name must be for a Handler class which has a default constructor. Note that
 *       these Handlers may be created lazily, when they are first used.
 *   <li>A property "&lt;logger&gt;.handlers.ensureCloseOnReset". This defines a a boolean value. If
 *       "&lt;logger&gt;.handlers" is not defined or is empty, this property is ignored. Otherwise
 *       it defaults to {@code true}. When the value is {@code true}, the handlers associated with
 *       the logger are guaranteed to be closed on {@linkplain #reset} and shutdown. This can be
 *       turned off by explicitly setting "&lt;logger&gt;.handlers.ensureCloseOnReset=false" in the
 *       configuration. Note that turning this property off causes the risk of introducing a
 *       resource leak, as the logger may get garbage collected before {@code reset()} is called,
 *       thus preventing its handlers from being closed on {@code reset()}. In that case it is the
 *       responsibility of the application to ensure that the handlers are closed before the logger
 *       is garbage collected.
 *   <li>A property "&lt;logger&gt;.useParentHandlers". This defines a boolean value. By default
 *       every logger calls its parent in addition to handling the logging message itself, this
 *       often result in messages being handled by the root logger as well. When setting this
 *       property to false a Handler needs to be configured for this logger otherwise no logging
 *       messages are delivered.
 *   <li>A property "config". This property is intended to allow arbitrary configuration code to be
 *       run. The property defines a whitespace or comma separated list of class names. A new
 *       instance will be created for each named class. The default constructor of each class may
 *       execute arbitrary code to update the logging configuration, such as setting logger levels,
 *       adding handlers, adding filters, etc.
 * </ul>
 *
 * <p>Note that all classes loaded during LogManager configuration are first searched on the system
 * class path before any user class path. That includes the LogManager class, any config classes,
 * and any handler classes.
 *
 * <p>Loggers are organized into a naming hierarchy based on their dot separated names. Thus "a.b.c"
 * is a child of "a.b", but "a.b1" and a.b2" are peers.
 *
 * <p>All properties whose names end with ".level" are assumed to define log levels for Loggers.
 * Thus "foo.level" defines a log level for the logger called "foo" and (recursively) for any of its
 * children in the naming hierarchy. Log Levels are applied in the order they are defined in the
 * properties file. Thus level settings for child nodes in the tree should come after settings for
 * their parents. The property name ".level" can be used to set the level for the root of the tree.
 *
 * <p>All methods on the LogManager object are multi-thread safe.
 *
 * @since 1.4
 */
public class LogManager {
  ...
  private static final int STATE_INITIALIZED = 0, // initial state
  ...
  /**
   * Protected constructor. This is protected so that container applications (such as J2EE
   * containers) can subclass the object. It is non-public as it is intended that there only be one
   * LogManager object, whose value is retrieved by calling LogManager.getLogManager.
   */
  ...
  /**
   * Lazy initialization: if this instance of manager is the global manager then this method will
   * read the initial configuration and add the root logger and global logger by calling
   * addLogger().
   *
   * <p>Note that it is subtly different from what we do in LoggerContext. In LoggerContext we're
   * patching up the logger context tree in order to add the root and global logger *to the context
   * tree*.
   *
   * <p>For this to work, addLogger() must have already have been called once on the LogManager
   * instance for the default logger being added.
   *
   * <p>This is why ensureLogManagerInitialized() needs to be called before any logger is added to
   * any logger context.
   */
  ...
  class LoggerContext {
    ...
    Logger findLogger(String name) {
      ...
      // fall through.
      if (logger != null
          || (ref == null && !name.isEmpty() && !name.equals(Logger.GLOBAL_LOGGER_NAME))) {
        return logger;
      }
    ...
    @SuppressWarnings("removal")
    private void processParentHandlers(
        final Logger logger, final String name, Predicate<Logger> visited) {
      final LogManager owner = getOwner();
      AccessController.doPrivileged(
          new PrivilegedAction<Void>() {
            @Override
  ...
  private void loadLoggerHandlers(
      final Logger logger, final String name, final String handlersPropertyName) {
    AccessController.doPrivileged(
        new PrivilegedAction<Void>() {
          @Override
          public Void run() {
  ...
  private void setLoggerHandlers(
      final Logger logger,
      final String name,
      final String handlersPropertyName,
      List<Handler> handlers) {
    final boolean ensureCloseOnReset =
        !handlers.isEmpty()
   ...
   *
   * <p>Note: Loggers may be added dynamically as new classes are loaded. This method only reports
   * on the loggers that are currently registered. It is also important to note that this method
   * only returns the name of a Logger, not a strong reference to the Logger itself. The returned
   * String does nothing to prevent the Logger from being garbage collected. In particular, if the
   * returned name is passed to {@code LogManager.getLogger()}, then the caller must check the
   * return value from {@code LogManager.getLogger()} for null to properly handle the case where the
   * Logger has been garbage collected in the time since its name was returned by this method.
   *
   ...
   *
   * <p>If the "java.util.logging.config.class" system property is set, then the property value is
   * treated as a class name. The given class will be loaded, an object will be instantiated, and
   * that object's constructor is responsible for reading in the initial configuration. (That object
   * may use other system properties to control its configuration.) The alternate configuration
   * class can use {@code readConfiguration(InputStream)} to define properties in the LogManager.
   *
   * <p>If "java.util.logging.config.class" system property is <b>not</b> set, then this method will
   * read the initial configuration from a properties file and calls the {@link
   * #readConfiguration(InputStream)} method to initialize the configuration. The
   * "java.util.logging.config.file" system property can be used to specify the properties file that
   * will be read as the initial configuration; if not set, then the LogManager default
   * configuration is used. The default configuration is typically loaded from the properties file
   * "{@code conf/logging.properties}" in the Java installation directory.
   *
   * <p>Any {@linkplain #addConfigurationListener registered configuration listener} will be invoked
   * after the properties are read.
   *
   * @apiNote This {@code readConfiguration} method should only be used for initializing the
   *     configuration during LogManager initialization or used with the
   *     "java.util.logging.config.class" property. When this method is called after loggers have
   *     been created, and the "java.util.logging.config.class" system property is not set, all
   *     existing loggers will be {@linkplain #reset() reset}. Then any existing loggers that have a
   *     level property specified in the new configuration stream will be {@linkplain
   *     Logger#setLevel(java.util.logging.Level) set} to the specified log level.
   *     <p>To properly update the logging configuration, use the {@link
   *     #updateConfiguration(java.util.function.Function)} or {@link
   *     #updateConfiguration(java.io.InputStream, java.util.function.Function)} methods instead.
   ...
   * the root logger) sets the level to {@code null}. The root logger's level is set to {@code
   * Level.INFO}.
   *
   * @apiNote Calling this method also clears the LogManager {@linkplain
   *     #getProperty(java.lang.String) properties}. The {@link
   *     #updateConfiguration(java.util.function.Function) updateConfiguration(Function)} or {@link
   *     #updateConfiguration(java.io.InputStream, java.util.function.Function)
   *     updateConfiguration(InputStream, Function)} method can be used to properly update to a new
   *     configuration.
   * @throws SecurityException if a security manager exists and if the caller does not have
   *     LoggingPermission("control").
   */
   ...
   *     configuration during LogManager initialization or used with the
   *     "java.util.logging.config.class" property. When this method is called after loggers have
   *     been created, all existing loggers will be {@linkplain #reset() reset}. Then any existing
   *     loggers that have a level property specified in the given input stream will be {@linkplain
   *     Logger#setLevel(java.util.logging.Level) set} to the specified log level.
   *     <p>To properly update the logging configuration, use the {@link
   *     #updateConfiguration(java.util.function.Function)} or {@link
   *     #updateConfiguration(java.io.InputStream, java.util.function.Function)} method instead.
   * @param ins stream to read properties from
   * @throws SecurityException if a security manager exists and if the caller does not have
   *     LoggingPermission("control").
   * @throws IOException if there are problems reading from the stream, or the given stream is not
   *     in the {@linkplain java.util.Properties properties file} format.
   */
  ...
  static enum ConfigProperty {
    ...
    /**
     * If the property is one that should be updated on existing loggers by updateConfiguration,
     * returns the name of the logger for which the property is configured. Otherwise, returns null.
     *
     * @param property a property key in 'props'
     * @return the name of the logger on which the property is to be set, if the property is one
     *     that should be updated on existing loggers, {@code null} otherwise.
     */
     ...
     *
     * @param property a property key in 'props'
     * @return An optional containing a ConfigProperty object, if the property is one that should be
     *     updated on existing loggers, empty otherwise.
     */
    ...
    /**
     * Returns true if the given property is one that should be updated on existing loggers. Used to
     * filter property name streams.
     *
    ...
    /**
     * Returns true if the new property value is different from the old, and therefore needs to be
     * updated on existing loggers.
     *
     * @param k a property key in the configuration
     * @param previous the old configuration
     * @param next the new configuration
     * @return true if the property is changing value between the two configurations.
     */
    ...
    /**
     * Applies the mapping function for the given key to the next configuration. If the mapping
     * function is null then this method does nothing. Otherwise, it calls the mapping function to
     * compute the value that should be associated with {@code key} in the resulting configuration,
     * and applies it to {@code next}. If the mapping function returns {@code null} the key is
     * removed from {@code next}.
     *
     * @param k a property key in the configuration
     * @param previous the old configuration
  ...
  /**
   * An object that keep track of loggers we have already visited. Used when updating configuration,
   * to avoid processing the same logger twice.
   */
   ...
   *
   * <p>If the "java.util.logging.config.file" system property is set, then the property value
   * specifies the properties file to be read as the new configuration. Otherwise, the LogManager
   * default configuration is used. <br>
   * The default configuration is typically loaded from the properties file "{@code
   * conf/logging.properties}" in the Java installation directory.
   *
   * <p>This method reads the new configuration and calls the {@link
   * #updateConfiguration(java.io.InputStream, java.util.function.Function) updateConfiguration(ins,
   * mapper)} method to update the configuration.
   *
   * @apiNote This method updates the logging configuration from reading a properties file and
   *     ignores the "java.util.logging.config.class" system property. The
   *     "java.util.logging.config.class" property is only used by the {@link #readConfiguration()}
   *     method to load a custom configuration class as an initial configuration.
   * @param mapper a functional interface that takes a configuration key <i>k</i> and returns a
   *     function <i>f(o,n)</i> whose returned value will be applied to the resulting configuration.
   *     The function <i>f</i> may return {@code null} to indicate that the property <i>k</i> will
   *     not be added to the resulting configuration. <br>
   *     If {@code mapper} is {@code null} then {@code (k) -> ((o, n) -> n)} is assumed. <br>
   *     For each <i>k</i>, the mapped function <i>f</i> will be invoked with the value associated
   *     with <i>k</i> in the old configuration (i.e <i>o</i>) and the value associated with
   *     <i>k</i> in the new configuration (i.e. <i>n</i>). <br>
   *     A {@code null} value for <i>o</i> or <i>n</i> indicates that no value was present for
   *     <i>k</i> in the corresponding configuration.
   * @throws SecurityException if a security manager exists and if the caller does not have
   *     LoggingPermission("control"), or does not have the permissions required to set up the
   *     configuration (e.g. open file specified for FileHandlers etc...)
   * @throws NullPointerException if {@code mapper} returns a {@code null} function when invoked.
   * @throws IOException if there are problems reading from the logging configuration file.
   ...
   * Updates the logging configuration.
   *
   * <p>For each configuration key in the {@linkplain #getProperty(java.lang.String) existing
   * configuration} and the given input stream configuration, the given {@code mapper} function is
   * invoked to map from the configuration key to a function, <i>f(o,n)</i>, that takes the old
   * value and new value and returns the resulting value to be applied in the resulting
   * configuration, as specified in the table below.
   *
   * <p>Let <i>k</i> be a configuration key in the old or new configuration, <i>o</i> be the old
   * value (i.e. the value associated with <i>k</i> in the old configuration), <i>n</i> be the new
   * value (i.e. the value associated with <i>k</i> in the new configuration), and <i>f</i> be the
   * function returned by {@code mapper.apply(}<i>k</i>{@code )}: then <i>v = f(o,n)</i> is the
   * resulting value. If <i>v</i> is not {@code null}, then a property <i>k</i> with value <i>v</i>
   * will be added to the resulting configuration. Otherwise, it will be omitted. <br>
   * A {@code null} value may be passed to function <i>f</i> to indicate that the corresponding
   * configuration has no configuration key <i>k</i>. The function <i>f</i> may return {@code null}
   * to indicate that there will be no value associated with <i>k</i> in the resulting
   * configuration.
   *
   * <p>If {@code mapper} is {@code null}, then <i>v</i> will be set to <i>n</i>.
   *
   * <p>LogManager {@linkplain #getProperty(java.lang.String) properties} are updated with the
   * resulting value in the resulting configuration.
   *
   * <p>The registered {@linkplain #addConfigurationListener configuration listeners} will be
   * invoked after the configuration is successfully updated. <br>
   * <br>
   *
   * <table class="striped">
   ...
   * </table>
   *
   * <p><em>Example mapper functions:</em> <br>
   * <br>
   *
   * <ul>
   *   <li>Replace all logging properties with the new configuration: <br>
   *       <br>
   *       {@code (k) -> ((o, n) -> n)}: <br>
   *       <br>
   *       this is equivalent to passing a null {@code mapper} parameter.
   *   <li>Merge the new configuration and old configuration and use the new value if <i>k</i>
   *       exists in the new configuration: <br>
   *       <br>
   *       {@code (k) -> ((o, n) -> n == null ? o : n)}: <br>
   *       <br>
   *       as if merging two collections as follows: {@code result.putAll(oldc);
   *       result.putAll(newc)}.<br>
   *   <li>Merge the new configuration and old configuration and use the old value if <i>k</i>
   *       exists in the old configuration: <br>
   *       <br>
   *       {@code (k) -> ((o, n) -> o == null ? n : o)}: <br>
   *       <br>
   *       as if merging two collections as follows: {@code result.putAll(newc);
   *       result.putAll(oldc)}.<br>
   *   <li>Replace all properties with the new configuration except the handler property to
   *       configure Logger's handler that is not root logger: <br>
   *       <pre>{@code (k) -> k.endsWith(".handlers")}
   *      {@code     ? ((o, n) -> (o == null ? n : o))}
   *      {@code     : ((o, n) -> n)}</pre>
   * </ul>
   *
   * <p>To completely reinitialize a configuration, an application can first call {@link #reset()
   * reset} to fully remove the old configuration, followed by {@code updateConfiguration} to
   ...
   * @throws NullPointerException if {@code ins} is null or if {@code mapper} returns a null
  ...
  public void updateConfiguration(
    ...
    try {
      ...
      updatePropertyNames =
          Stream.concat(
                  previous.stringPropertyNames().stream(), next.stringPropertyNames().stream())
      ...
      if (mapper != null) {
        ...
        updatePropertyNames.stream()
            .forEachOrdered(
                k ->
                    ConfigProperty.merge(
                        k, previous, next, Objects.requireNonNull(mapper.apply(k))));
      }
      ...
      final List<Logger> loggers =
          cxs.isEmpty() ? Collections.emptyList() : new ArrayList<>(cxs.size());
  ...
  /**
   * Check that the current context is trusted to modify the logging configuration. This requires
   * LoggingPermission("control").
   *
   * <p>If the check fails we throw a SecurityException, otherwise we return normally.
   *
   * @throws SecurityException if a security manager exists and if the caller does not have
   *     LoggingPermission("control").
   * @deprecated This method is only useful in conjunction with {@linkplain SecurityManager the
   *     Security Manager}, which is deprecated and subject to removal in a future release.
   *     Consequently, this method is also deprecated and subject to removal. There is no
  ...
  /**
   * String representation of the {@link java.management/javax.management.ObjectName} for the
   * management interface for the logging facility.
   *
   ...
   * @deprecated {@code java.util.logging.LoggingMXBean} is deprecated and replaced with {@code
   *     java.lang.management.PlatformLoggingMXBean}. Use {@link
   *     java.management/java.lang.management.ManagementFactory#getPlatformMXBean(Class)
   *     ManagementFactory.getPlatformMXBean}(PlatformLoggingMXBean.class) instead.
   * @see java.management/java.lang.management.PlatformLoggingMXBean
   * @since 1.5
   */
  @Deprecated(since = "9")
  ...
  /**
   * Adds a configuration listener to be invoked each time the logging configuration is read. If the
   * listener is already registered the method does nothing.
   *
   * <p>The listener is invoked with privileges that are restricted by the calling context of this
   * method. The order in which the listeners are invoked is unspecified.
   *
   * <p>It is recommended that listeners do not throw errors or exceptions.
   *
   * <p>If a listener terminates with an uncaught error or exception then the first exception will
   * be propagated to the caller of {@link #readConfiguration()} (or {@link
   * #readConfiguration(java.io.InputStream)}) after all listeners have been invoked.
   *
   * @implNote If more than one listener terminates with an uncaught error or exception, an
   *     implementation may record the additional errors or exceptions as {@linkplain
   *     Throwable#addSuppressed(java.lang.Throwable) suppressed exceptions}.
   * @param listener A configuration listener that will be invoked after the configuration changed.
   ...
   *
   * <p>Returns silently if the listener is not found.
   *
  ...
  /**
   * This class allows the {@link LoggingProviderImpl} to demand loggers on behalf of system and
   * application classes.
   */
  private static final class LoggingProviderAccess
      implements LoggingProviderImpl.LogManagerAccess, PrivilegedAction<Void> {
     ...
     *
     * <p>If a named logger suitable for the given module is found returns it. Otherwise, creates a
     * new logger suitable for the given module.
     *
     ...
     *     null}.
     * @throws IllegalArgumentException if {@code manager} is not the default LogManager.
     * @throws SecurityException if a security manager is present and the calling code doesn't have
     *     the {@link LoggingPermission LoggingPermission("demandLogger", null)}.
     */
    @Override
