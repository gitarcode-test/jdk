...
public class ServerCompilerScheduler implements Scheduler {
  ...
  private static class Node {
    ...
    public static final String WARNING_BLOCK_PROJECTION_WITH_MULTIPLE_SUCCS =
        "Block projection with multiple successors";
    public static final String WARNING_REGION_WITH_MULTIPLE_SUCCS =
        "Region with multiple successors";
    public static final String WARNING_PHI_INPUT_WITHOUT_REGION =
        "Phi input without associated region";
    public static final String WARNING_REGION_WITHOUT_CONTROL_INPUT =
        "Region without control input";
    public static final String WARNING_PHI_WITH_REGIONLESS_INPUTS =
        "Phi node with input nodes without associated region";
    public static final String WARNING_NOT_MARKED_WITH_BLOCK_START =
        "Region not marked with is_block_start";
    public static final String WARNING_CFG_AND_INPUT_TO_PHI = "CFG node is a phi input";
    public static final String WARNING_PHI_NON_DOMINATING_INPUTS =
        "Phi input that does not dominate the phi's input block";
    public static final String WARNING_CONTROL_UNREACHABLE_CFG = "Control-unreachable CFG node";
    public static final String WARNING_CFG_WITHOUT_SUCCESSORS =
        "CFG node without control successors";
  ...
  private static final Comparator<InputEdge> edgeComparator =
      Comparator.comparingInt(InputEdge::getToIndex);
  ...
  private void scheduleLocal() {
    ...
    for (InputBlock b : blocks) {
      for (InputNode in : b.getNodes()) {
        ...
        Predicate<Node> excludePredecessors = node -> isPhi(node) || node.isBlockStart;
  ...
  private static final Comparator<Node> schedulePriority =
      (n1, n2) -> {
  ...
  public void schedulePinned() {
    Set<Node> reachable = reachableNodes();
    for (Node n : nodes) {
      if (!reachable.contains(n) || n.block != null) {
        continue;
      }
    ...
    for (Node n : nodes) {
      ...
      if (category.equals("control") || category.equals("mixed")) {
        ...
        n.isCFG = true;
          ...
          && n.preds.size() > 0
          && n.preds.get(0) != null
          && n.preds.get(0).inputNode.getProperties().get("category").equals("control")) {
