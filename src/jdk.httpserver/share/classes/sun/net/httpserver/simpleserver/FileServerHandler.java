 ...
 *
 * <p>Must be given an absolute pathname to the directory to be served. Supports only HEAD and GET
 * requests. Directory listings and files can be served, content types are supported on a best-guess
 * basis.
 */
public final class FileServerHandler implements HttpHandler {
  ...
  private FileServerHandler(Path root, UnaryOperator<String> mimeTable) {
    ...
    if (securityManager != null) securityManager.checkRead(pathForSecurityCheck(root.toString()));

    if (!Files.exists(root)) throw new IllegalArgumentException("Path does not exist: " + root);
    if (!root.isAbsolute()) throw new IllegalArgumentException("Path is not absolute: " + root);
  ...
  private static final HttpHandler NOT_IMPLEMENTED_HANDLER = HttpHandlers.of(501, Headers.of(), "");
  ...
  public static HttpHandler create(Path root, UnaryOperator<String> mimeTable) {
    ...
    return HttpHandlers.handleOrElse(
        ...
        new FileServerHandler(root, mimeTable),
        fallbackHandler);
  }
  ...
  private static void checkRequestWithinContext(String requestPath, String contextPath) {
    if (requestPath.equals(contextPath)) {
      return; // context path requested, e.g. context /foo, request /foo
    }
    String contextPathWithTrailingSlash =
  ...
  private Path mapToPath(HttpExchange exchange, Path root) {
    try {
      ...
      logger.log(
          System.Logger.Level.TRACE, "FileServerHandler: request URI path resolution failed", e);
  ...
  private void serveDefaultFavIcon(HttpExchange exchange, boolean writeBody) throws IOException {
  ...
  private void listFiles(HttpExchange exchange, Path path, boolean writeBody) throws IOException {
    var respHdrs = exchange.getResponseHeaders();
