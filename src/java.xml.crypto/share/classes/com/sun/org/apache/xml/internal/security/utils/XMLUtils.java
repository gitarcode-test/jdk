...
public final class XMLUtils {
  ...
  private static boolean ignoreLineBreaks =
      AccessController.doPrivileged(
          (PrivilegedAction<Boolean>)
              () ->
                  Boolean.getBoolean("com.sun.org.apache.xml.internal.security.ignoreLineBreaks"));
          ...
          (PrivilegedAction<XMLParser>)
              () -> {
                String xmlParserClass =
                    System.getProperty("com.sun.org.apache.xml.internal.security.XMLParser");
                if (xmlParserClass != null) {
                  try {
                    return (XMLParser)
                        JavaUtils.newInstanceWithEmptyConstructor(
  ...
  private static void getSetRec(
      final Node rootNode, final Set<Node> result, final Node exclude, final boolean comments) {
    if (rootNode == exclude) {
      return;
    }
  ...
  public static void outputDOM(Node contextNode, File outputFile) throws IOException {
    try (OutputStream os =
        new BufferedOutputStream(Files.newOutputStream(outputFile.toPath()), 8192)) {
  ...
  /**
   * Outputs a DOM tree to an {@link OutputStream}. <I>If an Exception is thrown during execution,
   * it's StackTrace is output to System.out, but the Exception is not re-thrown.</I>
   *
   * @param contextNode root node of the DOM tree
   * @param os the {@link OutputStream}
   * @param addPreamble
  ...
  public static void outputDOM(Node contextNode, OutputStream os, boolean addPreamble) {
    try {
      ...
      Canonicalizer.getInstance(Canonicalizer.ALGO_ID_C14N_PHYSICAL)
          .canonicalizeSubtree(contextNode, os);
  ...
  /**
   * Serializes the {@code contextNode} into the OutputStream, <I>but suppresses all Exceptions</I>.
   *
   * <p>NOTE: <I>This should only be used for debugging purposes, NOT in a production environment;
   * this method ignores all exceptions, so you won't notice if something goes wrong. If you're
   * asking what is to be used in a production environment, simply use the code inside the {@code
   * try{}} statement, but handle the Exceptions appropriately.</I>
   *
   * @param contextNode
   * @param os
   */
  ...
  /**
   * Returns true if the element is in XML Signature namespace and the local name equals the
   * supplied one.
   *
   * @param element
   * @param localName
   * @return true if the element is in XML Signature namespace and the local name equals the
   *     supplied one
   */
  ...
  /**
   * Returns true if the element is in XML Signature 1.1 namespace and the local name equals the
   * supplied one.
   *
   * @param element
   * @param localName
  ...
  /**
   * Returns true if the element is in XML Encryption namespace and the local name equals the
   * supplied one.
   *
   * @param element
   * @param localName
   * @return true if the element is in XML Encryption namespace and the local name equals the
   *     supplied one
   */
  ...
  /**
   * Returns true if the element is in XML Encryption 1.1 namespace and the local name equals the
   * supplied one.
   *
   * @param element
   * @param localName
   * @return true if the element is in XML Encryption 1.1 namespace and the local name equals the
   *     supplied one
   */
  ...
  /**
   * This method returns the owner document of a particular node. This method is necessary because
   * it <I>always</I> returns a {@link Document}. {@link Node#getOwnerDocument} returns {@code null}
  ...
  public static Document getOwnerDocument(Node node) {
    ...
    try {
      ...
      throw new NullPointerException(
          I18n.translate("endorsed.jdk1.4.0")
  ...
  /**
   * This method returns the first non-null owner document of the Nodes in this Set. This method is
   * necessary because it <I>always</I> returns a {@link Document}. {@link Node#getOwnerDocument}
   * returns {@code null} if the {@link Node} is a {@link Document}.
   *
   * @param xpathNodeSet
   ...
   *
   * @param doc
   * @see <A HREF="http://nagoya.apache.org/bugzilla/show_bug.cgi?id=2650"> Namespace axis
   *     resolution is not XPath compliant </A>
   */
  ...
  /**
   * This method is a tree-search to help prevent against wrapping attacks. It checks that no two
   * Elements have ID Attributes that match the "value" argument, if this is the case then "false"
   * is returned. Note that a return value of "true" does not necessarily mean that a matching
   ...
   * argument, which is the ID value of "knownElement". If this is the case then "false" is
   * returned.
   */
   ...
   * @throws IOException
   */
  public static Document read(File file, boolean disallowDocTypeDeclarations)
      throws XMLParserException, IOException {
    try (InputStream inputStream =
        new BufferedInputStream(Files.newInputStream(file.toPath()), 8192)) {
   ...
   */
  public static Document readResource(
      String name, ClassLoader loader, boolean disallowDocTypeDeclarations)
      throws XMLParserException, IOException {
  ...
  /**
   * Reads a document from the input stream. Caller is responsible for closing the stream.
   *
   * @param inputStream
   * @param disallowDocTypeDeclarations
   * @return {@link Document}
  ...
  /**
   * Returns a byte-array representation of a {@code {@link BigInteger}}. No sign-bit is output.
   *
   * <p><b>N.B.:</B> {@code {@link BigInteger}}'s toByteArray returns eventually longer arrays
   * because of the leading sign-bit.
   *
