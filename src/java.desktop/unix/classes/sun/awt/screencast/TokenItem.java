...
/**
 * Helper class used by {@link TokenStorage} as restore token record with its associated screen
 * boundaries.
 *
 * <p>It helps in serialization/deserialization of screen boundaries to/from string format.
 *
 * <p>The screen boundary is represented as {@code _x_y_width_height} and can be repeated several
 * times.
 */
final class TokenItem {
  ...
  public TokenItem(String token, int[] allowedScreenBounds) {
    ...
    this.allowedScreensBounds =
        IntStream.iterate(0, i -> i < allowedScreenBounds.length, i -> i + 4)
            .mapToObj(
                i ->
                    new Rectangle(
  ...
  public boolean hasAllScreensOfSameSize(List<Dimension> screenSizes) {
    ...
    List<Dimension> tokenSizes =
        allowedScreensBounds.stream()
            .map(bounds -> new Dimension(bounds.width, bounds.height))
            .collect(Collectors.toCollection(ArrayList::new));

    return screenSizes.size() == screenSizes.stream().filter(tokenSizes::remove).count();
  }
  ...
  public String dump() {
    ...
    for (Rectangle bounds : allowedScreensBounds) {
      sb.append("_%d_%d_%d_%d".formatted(bounds.x, bounds.y, bounds.width, bounds.height));
    }
    return sb.toString();
  }
