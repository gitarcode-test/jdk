...
/**
 * Helper class for persistent storage of ScreenCast restore tokens and associated screen
 * boundaries.
 *
 * <p>The restore token allows the ScreenCast session to be restored with previously granted screen
 * access permissions.
 */
...
final class TokenStorage {
  ...
  private static final String REL_NAME = ".awt/robot/screencast-tokens.properties";
  ...
  private static Path setupPath() {
    ...
    try {
      ...
      if (SCREENCAST_DEBUG) {
        System.err.printf(
            "Token storage: cannot set permissions " + "for directory %s %s\n", workdir, e);
      }
    }
  ...
  private static boolean setFilePermission(Path path) {
    try {
      ...
      if (SCREENCAST_DEBUG) {
        System.err.printf(
            "Token storage: failed to set " + "property file permission %s %s\n", path, e);
      }
    }
    return false;
  }
    ...
    public void run() {
      ...
      for (; ; ) {
        ...
        for (WatchEvent<?> event : key.pollEvents()) {
          ...
          if (kind == OVERFLOW || !event.context().equals(PROP_FILENAME)) {
            continue;
          }
    ...
    doPrivilegedRunnable(
        () -> {
          try {
            ...
            PROPS_PATH.getParent().register(watchService, ENTRY_CREATE, ENTRY_DELETE, ENTRY_MODIFY);
  ...
  static Set<TokenItem> getTokens(List<Rectangle> affectedScreenBounds) {
          ...
          PROPS.entrySet().stream()
              .map(
                  o -> {
                    ...
                    TokenItem tokenItem = TokenItem.parse(token, o.getValue());
                    if (tokenItem == null) {
                      malformed.add(token);
                    }
                    return tokenItem;
              ...
              .sorted(
                  (t1, t2) -> // Token with more screens preferred
                  t2.allowedScreensBounds.size() - t1.allowedScreensBounds.size())
              .toList();
    }
    ...
    for (TokenItem tokenItem : allTokenItems) {
      if (tokenItem != null && tokenItem.hasAllScreensWithExactMatch(affectedScreenBounds)) {
    ...
    List<Dimension> dimensions =
        affectedScreenBounds.stream()
            .map(rectangle -> new Dimension(rectangle.width, rectangle.height))
            .toList();

    for (TokenItem tokenItem : allTokenItems) {
      if (tokenItem != null && tokenItem.hasAllScreensOfSameSize(dimensions)) {
  ...
  private static void removeMalformedRecords(Set<String> malformedRecords) {
    if (!isWritable() || malformedRecords == null || malformedRecords.isEmpty()) {
      return;
    }
  ...
  private static boolean isWritable() {
    if (PROPS_PATH == null || (Files.exists(PROPS_PATH) && !Files.isWritable(PROPS_PATH))) {
