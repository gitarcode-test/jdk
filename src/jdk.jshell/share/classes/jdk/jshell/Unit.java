...
final class Unit {
  ...
  Unit(JShell state, Snippet si, Snippet causalSnippet, DiagList generatedDiagnostics) {
    ...
    this.siOld = isDependency ? si : state.maps.getSnippet(si.key());
    ...
    this.prevStatus = (isNew || isDependency) ? si.status() : siOld.status();
  ...
  public boolean equals(Object o) {
    return (o instanceof Unit) ? si.equals(((Unit) o).si) : false;
  }
  ...
  void setWrap(Collection<Unit> exceptUnit, Collection<Unit> plusUnfiltered) {
    if (isImport()) {
      ...
      if (snippet().kind() == Kind.METHOD) {
        ...
        units =
            plusUnfiltered.stream()
                .filter(
                    u ->
                        u.snippet().kind() == Kind.METHOD
      ...
      Set<Key> except = exceptUnit.stream().map(u -> u.snippet().key()).collect(toSet());
      // Snippets to add to imports
      Collection<Snippet> plus =
          plusUnfiltered.stream().filter(u -> !units.contains(u)).map(Unit::snippet).toList();
      // Snippets to wrap in an outer
      List<Snippet> snippets = units.stream().map(Unit::snippet).toList();
      // Snippet wraps to wrap in an outer
      List<Wrap> wraps = units.stream().map(u -> u.activeGuts).toList();
  ...
  void setDiagnostics(DiagList diags) {
    ...
    state.debug(
        DBG_GEN,
        "++setCompilationInfo() %s\n%s\n-- diags: %s\n",
        si,
        si.outerWrap().wrapped(),
        diags);
  }
  ...
  boolean corralIfNeeded(Collection<Unit> working) {
    if (isRecoverable() && si.corralled() != null) {
  ...
  boolean doRedefines() {
    ...
    ClassBytecodes[] cbcs =
        toRedefine.stream().map(ClassInfo::toClassBytecodes).toArray(ClassBytecodes[]::new);
    try {
  ...
  Stream<Unit> dependents() {
    return state.maps.getDependents(si).stream()
  ...
  private void markOldDeclarationOverwritten() {
    if (si != siOld && siOld != null && siOld.status().isActive()) {
      ...
      replaceOldEvent = new SnippetEvent(siOld, siOld.status(), OVERWRITTEN, false, si, null, null);
  ...
  private void recordCompilation() {
    ...
    state.debug(
        DBG_GEN, "recordCompilation: %s -- status %s, unresolved %s\n", si, status, unresolved);
  }
  ...
  private void checkForOverwrite(AnalyzeTask at) {
    ...
    if (si.kind() == Kind.METHOD && status.isDefined()) {
      ...
      msi.setQualifiedParameterTypes(computeQualifiedParameterTypes(at, msi));
  ...
  private Status overwriteMatchingMethod(MethodSnippet msi) {
    ...
    for (MethodSnippet sn : matching) {
      ...
      SnippetEvent se =
          new SnippetEvent(sn, overwrittenStatus, OVERWRITTEN, false, msi, null, null);
      sn.setOverwritten();
      secondaryEvents.add(se);
      state.debug(
          DBG_EVNT,
          "Overwrite event #%d -- key: %s before: %s status: %s sig: %b cause: %s\n",
          secondaryEvents.size(),
          se.snippet(),
          se.previousStatus(),
          se.status(),
          se.isSignatureChange(),
          se.causeSnippet());
    }
  ...
  SnippetEvent event(String value, JShellException exception) {
    ...
    state.debug(
        DBG_EVNT,
        "Snippet: %s id: %s before: %s status: %s sig: %b cause: %s\n",
        si,
        si.id(),
        prevStatus,
        si.status(),
    ...
    return new SnippetEvent(
        si, prevStatus, si.status(), wasSignatureChanged, causalSnippet, value, exception);
  }

  List<SnippetEvent> secondaryEvents() {
    return secondaryEvents == null ? Collections.emptyList() : secondaryEvents;
  }
  ...
  /**
   * Separate out the unresolvedDependencies errors from both the other corralling errors and the
   * overall errors.
   */
