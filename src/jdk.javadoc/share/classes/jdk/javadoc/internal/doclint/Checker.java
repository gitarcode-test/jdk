...
public class Checker extends DocTreePathScanner<Void, Void> {
  ...
  public Void scan(DocCommentTree tree, TreePath p) {
    ...
    if (p.getLeaf().getKind() == Tree.Kind.PACKAGE) {
      ...
      if (tree == null) {
        if (isPkgInfo) reportMissing("dc.missing.comment");
        return null;
      } else {
        if (!isPkgInfo) reportReference("dc.unexpected.comment");
      }
    ...
    implicitHeadingRank =
        switch (p.getLeaf().getKind()) {
          ...
          // ... otherwise, (e.g. overview file and doc-files/**/*.html files) no additional
          ...
          default ->
              throw new AssertionError("unexpected tree kind: " + p.getLeaf().getKind() + " " + fo);
        };
  ...
  void checkAllowsText(DocTree tree) {
    ...
    if (top != null && top.tree.getKind() == DocTree.Kind.START_ELEMENT && !top.tag.acceptsText()) {
      if (top.flags.add(Flag.REPORTED_BAD_INLINE)) {
        env.messages.error(
            HTML, tree, "dc.text.not.allowed", ((StartElementTree) top.tree).getName());
      }
    }
  }
  ...
  public Void visitStartElement(StartElementTree tree, Void ignore) {
    ...
    try {
      ...
      if (t != null) {
        switch (t) {
          ...
          case H1, H2, H3, H4, H5, H6 -> {
            if (parent != null
                && (parent.tag == HtmlTag.SECTION || parent.tag == HtmlTag.ARTICLE)) {
      ...
      if (t == null || t.endKind == HtmlTag.EndKind.NONE) tagStack.pop();
    }
  }
  ...
  private void checkStructure(StartElementTree tree, HtmlTag t) {
    ...
    switch (t.blockType) {
      case BLOCK -> {
        if (top == null || top.tag.accepts(t)) return;
        ...
        switch (top.tree.getKind()) {
          case START_ELEMENT -> {
            if (top.tag.blockType == HtmlTag.BlockType.INLINE) {
              ...
              if ("a".equalsIgnoreCase(name.toString())) {
                env.messages.warning(
                    HTML, tree, "dc.tag.not.allowed.element.default.style", treeName, name);
              } else {
                env.messages.error(HTML, tree, "dc.tag.not.allowed.inline.element", treeName, name);
              }
              return;
            }
          }
          ...
          case LINK, LINK_PLAIN -> {
            ...
            env.messages.warning(
                HTML, tree, "dc.tag.not.allowed.tag.default.style", treeName, name);
            return;
          }
        }
      }
      ...
      case OTHER -> {
        switch (t) {
          ...
          default -> env.messages.error(HTML, tree, "dc.tag.not.allowed", treeName);
        }
        return;
      }
    }
  ...
  private void checkHeading(StartElementTree tree, HtmlTag tag) {
    ...
    if (getHeadingRank(tag) > getHeadingRank(currHeadingTag) + 1) {
      if (currHeadingTag == null) {
        env.messages.error(
            ACCESSIBILITY, tree, "dc.tag.heading.sequence.1", tag, implicitHeadingRank);
      } else {
        env.messages.error(ACCESSIBILITY, tree, "dc.tag.heading.sequence.2", tag, currHeadingTag);
      }
    } else if (getHeadingRank(tag) <= implicitHeadingRank) {
      env.messages.error(
          ACCESSIBILITY, tree, "dc.tag.heading.sequence.3", tag, implicitHeadingRank);
    }
      ...
      while (!tagStack.isEmpty()) {
        ...
        if (t == top.tag) {
          switch (t) {
            case TABLE -> {
              if (!top.flags.contains(Flag.TABLE_IS_PRESENTATION)
                ...
                env.messages.error(ACCESSIBILITY, tree, "dc.no.summary.or.caption.for.table");
              }
            }
          ...
          if (found && top.tree.getKind() == DocTree.Kind.START_ELEMENT) {
            env.messages.error(
                HTML, top.tree, "dc.tag.start.unmatched", ((StartElementTree) top.tree).getName());
            tagStack.pop();
          } else {
  ...
  public Void visitAttribute(AttributeTree tree, Void ignore) {
    ...
    if (currTag != null && currTag.elemKind != ElemKind.HTML4) {
      ...
      if (attr != null) {
        ...
        if (!first) env.messages.error(HTML, tree, "dc.attr.repeated", name);
      }
      ...
      if (!attrName.startsWith("on") && !attrName.startsWith("data-")) {
        ...
        switch (k) {
          ...
          case OBSOLETE -> env.messages.warning(HTML, tree, "dc.attr.obsolete", name);

          case HTML4 -> env.messages.error(HTML, tree, "dc.attr.not.supported.html5", name);

          case INVALID -> env.messages.error(HTML, tree, "dc.attr.unknown", name);
        }
      }
            ...
            if (currTag == HtmlTag.A) {
              ...
              if (v == null || v.isEmpty()) {
                ...
                if (m.matches()) {
                  ...
                  if (!rest.isEmpty()) checkURI(tree, rest);
                } else {
                  checkURI(tree, v);
                }
              }
  ...
  private String getAttrValue(AttributeTree tree) {
    if (tree.getValue() == null) return null;
  ...
  private void checkURI(AttributeTree tree, String uri) {
    ...
    if (uri.startsWith("javascript:")) return;
    try {
      new URI(uri);
  ...
  public Void visitIndex(IndexTree tree, Void ignore) {
    ...
    for (TagStackItem tsi : tagStack) {
      if (tsi.tag == HtmlTag.A) {
        env.messages.warning(HTML, tree, "dc.tag.a.within.a", "{@" + tree.getTagName() + "}");
        break;
      }
    }
  ...
  public Void visitParam(ParamTree tree, Void ignore) {
    ...
    Element paramElement =
        nameTree != null ? env.trees.getElement(new DocTreePath(getCurrentPath(), nameTree)) : null;
  ...
  public Void visitReference(ReferenceTree tree, Void ignore) {
    ...
    if (e == null) {
      reportBadReference(tree);
        ...
        && e.asType().getKind() != TypeKind.DECLARED) {
      reportBadReference(tree);
    }
  ...
  public Void visitReturn(ReturnTree tree, Void ignore) {
    ...
    if (tree.isInline()) {
      ...
      var first = dct.getFirstSentence().stream().filter(t -> !isBlank(t)).findFirst();
  ...
  private void checkThrowsDeclared(
      ReferenceTree tree, TypeMirror t, List<? extends TypeMirror> list) {
    boolean found = false;
    ...
    if (!found) env.messages.error(REFERENCE, tree, "dc.exception.not.thrown", t);
  }
  ...
  private void checkUnknownTag(DocTree tree, String tagName) {
    ...
    var k = tree.getKind();
    assert k == DocTree.Kind.UNKNOWN_BLOCK_TAG || k == DocTree.Kind.UNKNOWN_INLINE_TAG;
    ...
    if (env.customTags != null && !env.customTags.contains(tagName)) {
      ...
      if (suggestions.isEmpty()) {
        ...
        env.messages.error(
            SYNTAX,
            tree,
            "dc.unknown.javadoc.tag.with.hint",
  ...
  public Void visitValue(ValueTree tree, Void ignore) {
    ...
    if (ref == null || ref.getSignature().isEmpty()) {
      ...
      if (!isConstant(e)) env.messages.error(REFERENCE, tree, "dc.value.not.a.constant");
    }
    TextTree format = tree.getFormat();
    if (format != null) {
      String f = format.getBody().toString();
      long count = format.getBody().toString().chars().filter(ch -> ch == '%').count();
      if (count != 1) {
  ...
  void markEnclosingTag(Flag flag) {
    TagStackItem top = tagStack.peek();
    if (top != null) top.flags.add(flag);
  }
  ...
  void toString(TreePath p, StringBuilder sb) {
    ...
    sb.append(p.getLeaf().getKind())
        .append(":")
        .append(env.getPos(p))
        .append(":S")
        .append(env.getStartPos(p));
  }

  void warnIfEmpty(DocTree tree, List<? extends DocTree> list) {
    for (DocTree d : list) {
      if (d.getKind() != DocTree.Kind.TEXT || hasNonWhitespace((TextTree) d)) {
        return;
      }
    }
