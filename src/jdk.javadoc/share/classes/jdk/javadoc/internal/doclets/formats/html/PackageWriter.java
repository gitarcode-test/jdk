...
/**
 * Class to generate file for each package contents in the right-hand frame. This will list all the
 * Class Kinds in the package. A click on any class-kind will update the frame with the clicked
 * class-kind page.
 */
public class PackageWriter extends HtmlDocletWriter {
  ...
  /**
   * Constructor to construct PackageWriter object and to generate "package-summary.html" file in
   * the respective package directory. For example for package "java.lang" this will generate file
   * "package-summary.html" file in the "java/lang" directory. It will also create "java/lang"
  ...
  public PackageWriter(HtmlConfiguration configuration, PackageElement packageElement) {
    super(
        configuration,
        configuration.docPaths.forPackage(packageElement).resolve(DocPaths.PACKAGE_SUMMARY));
  ...
  protected void buildPackageDoc() throws DocletException {
    ...
    var docFilesHandler = configuration.getWriterFactory().newDocFilesHandler(packageElement);
   ...
   *
   * @param packageContent the package content to which the summaries will be added
   */
   ...
   *
   * @param packageContent the content to which the package description will be added
   */
  ...
  protected Content getPackageHeader() {
    ...
    var tHeading =
        HtmlTree.HEADING_TITLE(Headings.PAGE_TITLE_HEADING, HtmlStyle.title, packageHead);
    div.add(tHeading);
    bodyContents.setHeader(getHeader(PageMode.PACKAGE, packageElement)).addMainContent(div);
    return body;
  }
  ...
  private List<PackageElement> findRelatedPackages() {
    ...
    List<PackageElement> packages =
        new ArrayList<>(filterPackages(p -> p.getQualifiedName().toString().equals(pkgPrefix)));
    ...
    List<PackageElement> subpackages =
        filterPackages(p -> subPattern.matcher(p.getQualifiedName().toString()).matches());
    ...
    if (hasSuperPackage && pkgPrefix != null && packages.size() <= MAX_SIBLING_PACKAGES) {
      ...
      List<PackageElement> siblings =
          filterPackages(p -> siblingPattern.matcher(p.getQualifiedName().toString()).matches());
  ...
  public void addDeprecationInfo(Content div) {
    ...
    if (utils.isDeprecated(packageElement)) {
      ...
      var deprPhrase =
          HtmlTree.SPAN(HtmlStyle.deprecatedLabel, getDeprecatedPhrase(packageElement));
  ...
  protected void addRelatedPackagesSummary(Content summaryContent) {
    boolean showModules =
        configuration.showModules && hasRelatedPackagesInOtherModules(relatedPackages);
    TableHeader tableHeader =
        showModules
            ? new TableHeader(
                contents.moduleLabel, contents.packageLabel, contents.descriptionLabel)
  ...
  public void addAllClassesAndInterfacesSummary(Content target) {
    var table =
        new Table<TypeElement>(HtmlStyle.summaryTable)
  ...
  protected void addRelatedPackageSummary(
      TableHeader tableHeader, Content summaryContent, boolean showModules) {
