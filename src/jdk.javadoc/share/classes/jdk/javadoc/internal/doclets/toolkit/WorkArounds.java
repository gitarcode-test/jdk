...
/**
 * A quarantine class to isolate all the workarounds and bridges to a locality. This class should
 * eventually disappear once all the standard APIs support the needed interfaces.
 */
public class WorkArounds {
  ...
  public Location getLocationForModule(ModuleElement mdle) {
    ...
    return msym.sourceLocation != null ? msym.sourceLocation : msym.classLocation;
  }
  ...
  private NewSerializedForm getSerializedForm(TypeElement typeElem) {
    return serializedForms.computeIfAbsent(
        typeElem, te -> new NewSerializedForm(utils, configuration.docEnv.getElementUtils(), te));
  }
  ...
  static class NewSerializedForm {
    ...
    private VarSymbol getDefinedSerializableFields(ClassSymbol def) {
      ...
      for (Symbol sym : def.members().getSymbolsByName(names.fromString(SERIALIZABLE_FIELDS))) {
        if (sym.kind == VAR) {
          ...
          if ((f.flags() & Flags.STATIC) != 0 && (f.flags() & Flags.PRIVATE) != 0) {
            return f;
          }
        }
      }
    ...
    public ExecutableElement findMethod(
        TypeElement te, String methodName, List<String> paramTypes) {
  ...
  /**
   * Returns whether or not to permit dynamically loaded components to access part of the javadoc
   * internal API. The flag is the same (hidden) compiler option that allows javac plugins and
   * annotation processors to access javac internal API.
   *
   * <p>As with all workarounds, it is better to consider updating the public API, rather than
   * relying on undocumented features like this, that may be withdrawn at any time, without notice.
   *
  ...
  /**
   * Returns a map containing {@code jdk.internal.javac.PreviewFeature.JEP} element values
   * associated with the {@code jdk.internal.javac.PreviewFeature.Feature} enum constant identified
   * by {@code feature}.
   *
   * <p>This method uses internal javac features (although only reflectively).
   *
