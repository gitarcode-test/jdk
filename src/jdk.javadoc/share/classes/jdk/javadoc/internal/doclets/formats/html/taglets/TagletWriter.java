...
public class TagletWriter {
  ...
  /**
   * A class that provides the information about the enclosing context for a series of {@code
   * DocTree} nodes. This context may be used to determine the content that should be generated from
   * the tree nodes.
   */
  public static class Context {
    /**
     * Whether the trees are appearing in a context of just the first sentence, such as in the
     * summary table of the enclosing element.
     */
    public final boolean isFirstSentence;

    /** Whether the trees are appearing in the "summary" section of the page for a declaration. */
     ...
     *
     * @param isFirstSentence {@code true} if the trees are appearing in a context of just the first
     *     sentence and {@code false} otherwise
     * @param inSummary {@code true} if the trees are appearing in the "summary" section of the page
     *     for a declaration and {@code false} otherwise
     */
    ...
    /**
     * Creates a new {@code Context} that includes an extra tag kind in the set of enclosing kinds
     * of tags.
     *
   ...
   * @param isFirstSentence {@code true} if this taglet writer is being used for a "first sentence"
   *     summary
   */
   ...
   * @param htmlWriter the {@code HtmlDocletWriter} for the page
   * @param isFirstSentence {@code true} if this taglet writer is being used for a "first sentence"
   *     summary, and {@code false} otherwise
   * @param inSummary {@code true} if this taglet writer is being used for the content of a {@code
   *     {@summary ...}} tag, and {@code false} otherwise
   */
  ...
  /**
   * Returns the content generated from the block tags for a given element. The content is generated
   * according to the order of the list of taglets. The result is a possibly-empty list of the
   * output generated by each of the given taglets for all of the tags they individually support.
   *
   ...
   * @return the content
   */
  public Content getBlockTagOutput(
      TagletManager tagletManager, Element element, List<Taglet> taglets) {
  ...
  /**
   * Returns the content generated from an inline tag in the doc comment for a given element, or
   * {@code null} if the tag is not supported or does not return any output.
   *
   ...
   */
  public Content getInlineTagOutput(Element holder, InlineTagTree inlineTag) {
   ...
   * @param trees array of text tags and inline tags (often alternating) present in the text of
   *     interest for this doc
   * @param isFirstSentence true if this is the first sentence
   * @return the generated content
   */
  public Content commentTagsToOutput(
      Element element, DocTree holder, List<? extends DocTree> trees, boolean isFirstSentence) {
    return htmlWriter.commentTagsToContent(
        element, trees, holder == null ? context : context.within(holder));
  }

  public Content createAnchorAndSearchIndex(
      Element element, String tagText, String desc, DocTree tree) {
    return createAnchorAndSearchIndex(element, tagText, Text.of(tagText), desc, tree);
  }

  @SuppressWarnings("preview")
  Content createAnchorAndSearchIndex(
      Element element, String tagText, Content tagContent, String desc, DocTree tree) {
    Content result;
    if (context.isFirstSentence && context.inSummary
      ...
      if (options.createIndex() && !tagText.isEmpty()) {
        ...
        IndexItem item =
            IndexItem.of(
                element, tree, tagText, holder, desc, new DocLink(htmlWriter.path, id.name()));
  ...
  public String getHolderName(Element element) {
    return new SimpleElementVisitor14<String, Void>() {
      ...
      public String visitModule(ModuleElement e, Void p) {
        return resources.getText("doclet.module") + " " + utils.getFullyQualifiedName(e);
      }

      @Override
      public String visitPackage(PackageElement e, Void p) {
        return resources.getText("doclet.package") + " " + utils.getFullyQualifiedName(e);
      }

      @Override
      public String visitType(TypeElement e, Void p) {
        return utils.getTypeElementKindName(e, true) + " " + utils.getFullyQualifiedName(e);
      }
