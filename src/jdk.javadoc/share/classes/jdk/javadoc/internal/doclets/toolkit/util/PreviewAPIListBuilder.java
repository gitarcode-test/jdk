...
public class PreviewAPIListBuilder extends SummaryAPIListBuilder {
  ...
  protected boolean belongsToSummary(Element element) {
    ...
    String feature =
        Objects.requireNonNull(utils.getPreviewFeature(element), "Preview feature not specified")
            .toString();
    JEP jep =
        jeps.computeIfAbsent(
            feature,
            featureName -> {
              ...
              if (!jepInfo.isEmpty()) {
                ...
                String title = "";
                String status =
                    "Preview"; // Default value is not returned by the method we used above.
