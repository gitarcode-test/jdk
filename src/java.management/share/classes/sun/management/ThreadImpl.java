...
/**
 * Implementation for java.lang.management.ThreadMXBean as well as providing the supporting method
 * for com.sun.management.ThreadMXBean. The supporting method for com.sun.management.ThreadMXBean
 * can be moved to jdk.management in the future.
 */
public class ThreadImpl implements ThreadMXBean {
  ...
  /**
   * Returns an array of thread identifiers for the threads in the given array. Returns {@code null}
   * if {@code threads} is null or the array of threads is empty.
   */
  ...
  private static long[] threadIds(Thread[] threads) {
    return Stream.of(threads).mapToLong(Thread::threadId).toArray();
  }
}
