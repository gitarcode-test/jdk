...
/**
 * The {@code ManagementFactory} class is a factory class for getting managed beans for the Java
 * platform. This class consists of static methods each of which returns one or more <i>platform
 * MXBeans</i> representing the management interface of a component of the Java virtual machine.
 *
 * <h2><a id="MXBean">Platform MXBeans</a></h2>
 *
 * <p>A platform MXBean is a <i>managed bean</i> that conforms to the <a
 * href="../../../javax/management/package-summary.html">JMX</a> Instrumentation Specification and
 * only uses a set of basic data types. A JMX management application and the {@linkplain
 * #getPlatformMBeanServer platform MBeanServer} can interoperate without requiring classes for
 * MXBean specific data types. The data types being transmitted between the JMX connector server and
 * the connector client are {@linkplain javax.management.openmbean.OpenType open types} and this
 * allows interoperation across versions. See <a
 * href="../../../javax/management/MXBean.html#MXBean-spec">the specification of MXBeans</a> for
 * details.
 *
 * <p><a id="MXBeanNames"></a>
 *
 * <p>Each platform MXBean is a {@link PlatformManagedObject} and it has a unique {@link
 * javax.management.ObjectName ObjectName} for registration in the platform {@code MBeanServer} as
 * returned by by the {@link PlatformManagedObject#getObjectName getObjectName} method.
 *
 * <p>An application can access a platform MXBean in the following ways:
 *
 ...
 *
 * <ul>
 *   <li>Get an MXBean instance by calling the {@link #getPlatformMXBean(Class) getPlatformMXBean}
 *       or {@link #getPlatformMXBeans(Class) getPlatformMXBeans} method and access the MXBean
 *       locally in the running virtual machine.
 *   <li>Construct an MXBean proxy instance that forwards the method calls to a given {@link
 *       MBeanServer MBeanServer} by calling the {@link #getPlatformMXBean(MBeanServerConnection,
 *       Class)} or {@link #getPlatformMXBeans(MBeanServerConnection, Class)} method. The {@link
 *       #newPlatformMXBeanProxy newPlatformMXBeanProxy} method can also be used to construct an
 *       MXBean proxy instance of a given {@code ObjectName}. A proxy is typically constructed to
 *       remotely access an MXBean of another running virtual machine.
 * </ul>
 *
 * <h3>2. Indirect access to an MXBean interface via MBeanServer</h3>
 *
 * <ul>
 *   <li>Go through the platform {@code MBeanServer} to access MXBeans locally or a specific {@code
 *       MBeanServerConnection} to access MXBeans remotely. The attributes and operations of an
 *       MXBean use only <em>JMX open types</em> which include basic data types, {@link
 *       javax.management.openmbean.CompositeData CompositeData}, and {@link
 *       javax.management.openmbean.TabularData TabularData} defined in {@link
 *       javax.management.openmbean.OpenType OpenType}. The mapping is specified in the {@linkplain
 *       javax.management.MXBean MXBean} specification for details.
 * </ul>
 *
 * </blockquote>
 *
 * <p>The {@link #getPlatformManagementInterfaces getPlatformManagementInterfaces} method returns
 * all management interfaces supported in the Java virtual machine including the standard management
 * interfaces listed in the tables below as well as the management interfaces extended by the JDK
 * implementation.
 *
 * <p>A Java virtual machine has a single instance of the following management interfaces:
 *
 ...
 * </table>
 *
 * <p>A Java virtual machine has zero or a single instance of the following management interfaces.
 *
 * <table class="striped" style="margin-left:2em">
 * <caption style="display:none">The list of Management Interfaces and their single instances</caption>
 * <thead>
 ...
 * </table>
 *
 * <p>A Java virtual machine may have one or more instances of the following management interfaces.
 *
 * <table class="striped" style="margin-left:2em">
 * <caption style="display:none">The list of Management Interfaces and their single instances</caption>
 * <thead>
 ...
 * @see <a href="package-summary.html#examples">Ways to Access Management Metrics</a>
 * @see javax.management.MXBean
 * @author Mandy Chung
 * @since 1.5
 */
@SuppressWarnings({"removal", "doclint:reference"}) // cross-module links
public class ManagementFactory {
  ...
  /** String representation of the {@code ObjectName} for the {@link ClassLoadingMXBean}. */
  public static final String CLASS_LOADING_MXBEAN_NAME = "java.lang:type=ClassLoading";

  /** String representation of the {@code ObjectName} for the {@link CompilationMXBean}. */
  public static final String COMPILATION_MXBEAN_NAME = "java.lang:type=Compilation";

  /** String representation of the {@code ObjectName} for the {@link MemoryMXBean}. */
  public static final String MEMORY_MXBEAN_NAME = "java.lang:type=Memory";

  /** String representation of the {@code ObjectName} for the {@link OperatingSystemMXBean}. */
  public static final String OPERATING_SYSTEM_MXBEAN_NAME = "java.lang:type=OperatingSystem";

  /** String representation of the {@code ObjectName} for the {@link RuntimeMXBean}. */
  public static final String RUNTIME_MXBEAN_NAME = "java.lang:type=Runtime";

  /** String representation of the {@code ObjectName} for the {@link ThreadMXBean}. */
  public static final String THREAD_MXBEAN_NAME = "java.lang:type=Threading";

  /**
   * The domain name and the type key property in the {@code ObjectName} for a {@link
   * GarbageCollectorMXBean}. The unique {@code ObjectName} for a {@code GarbageCollectorMXBean} can
   * be formed by appending this string with "{@code ,name=}<i>collector's name</i>".
   */
  ...
  /**
   * The domain name and the type key property in the {@code ObjectName} for a {@link
   * MemoryManagerMXBean}. The unique {@code ObjectName} for a {@code MemoryManagerMXBean} can be
   * formed by appending this string with "{@code ,name=}<i>manager's name</i>".
   */
  public static final String MEMORY_MANAGER_MXBEAN_DOMAIN_TYPE = "java.lang:type=MemoryManager";

  /**
   * The domain name and the type key property in the {@code ObjectName} for a {@link
   * MemoryPoolMXBean}. The unique {@code ObjectName} for a {@code MemoryPoolMXBean} can be formed
   * by appending this string with {@code ,name=}<i>pool's name</i>.
   */
   ...
   * returns {@code null} if the Java virtual machine has no compilation system.
   *
   * @return a {@link CompilationMXBean} object for the Java virtual machine or {@code null} if the
  ...
  /**
   * Returns a list of {@link MemoryPoolMXBean} objects in the Java virtual machine. The Java
   * virtual machine can have one or more memory pools. It may add or remove memory pools during
   * execution.
   *
   ...
   * virtual machine may have one or more {@code GarbageCollectorMXBean} objects. It may add or
   * remove {@code GarbageCollectorMXBean} during execution.
   *
   ...
   * this method, it first creates the platform {@code MBeanServer} by calling the {@link
   * javax.management.MBeanServerFactory#createMBeanServer MBeanServerFactory.createMBeanServer}
   * method and registers each platform MXBean in this platform {@code MBeanServer} with its {@link
   * PlatformManagedObject#getObjectName ObjectName}. This method, in subsequent calls, will simply
   * return the initially created platform {@code MBeanServer}.
   *
   ...
   * registered and deregistered into the platform {@code MBeanServer}.
   *
   * <p>If the system property {@code javax.management.builder.initial} is set, the platform {@code
   * MBeanServer} creation will be done by the specified {@link
   ...
   * the same {@code MBeanServer} and hence allow for easier network publishing and discovery. Name
   * conflicts with the platform MXBeans should be avoided.
   *
   * @return the platform {@code MBeanServer}; the platform MXBeans are registered into the platform
   *     {@code MBeanServer} at the first time this method is called.
   * @throws SecurityException if there is a security manager and the caller does not have the
   ...
   * name</a> that forwards its method calls through the given {@code MBeanServerConnection}.
   *
   * <p>This method is equivalent to:
   *
   * <blockquote>
   *
   * {@link java.lang.reflect.Proxy#newProxyInstance Proxy.newProxyInstance}{@code
   * (mxbeanInterface.getClassLoader(), new Class[] { mxbeanInterface }, handler)}
   *
   * </blockquote>
   *
   * where {@code handler} is an {@link java.lang.reflect.InvocationHandler InvocationHandler} to
   * which method invocations to the MXBean interface are dispatched. This {@code handler} converts
   * an input parameter from an MXBean data type to its mapped open type before forwarding to the
   * {@code MBeanServer} and converts a return value from an MXBean method call through the {@code
   * MBeanServer} from an open type to the corresponding return type declared in the MXBean
   * interface.
   *
   * <p>If the MXBean is a notification emitter (i.e., it implements {@link
   * javax.management.NotificationEmitter NotificationEmitter}), both the {@code mxbeanInterface}
   * and {@code NotificationEmitter} will be implemented by this proxy.
   *
   * <p><b>Notes:</b>
   *
   * <ol>
   *   <li>Using an MXBean proxy is a convenience remote access to a platform MXBean of a running
   *       virtual machine. All method calls to the MXBean proxy are forwarded to an {@code
   *       MBeanServerConnection} where {@link java.io.IOException IOException} may be thrown when
   *       the communication problem occurs with the connector server. If thrown, {@link
   *       java.io.IOException IOException} will be wrappped in {@link
   *       java.lang.reflect.UndeclaredThrowableException UndeclaredThrowableException}. An
   *       application remotely accessing the platform MXBeans using proxy should prepare to catch
   *       {@code UndeclaredThrowableException} and handle its {@linkplain
   *       java.lang.reflect.UndeclaredThrowableException#getCause() cause} as if that cause had
   *       been thrown by the {@code MBeanServerConnection} interface.
   ...
   *       in the enum class loaded in the client application. If thrown, {@link
   *       java.io.InvalidObjectException InvalidObjectException} will be wrappped in {@link
   ...
   *       by {@code MBeanServerInvocationHandler} does not handle the properties of the platform
   *       MXBeans described in the <a href="#MXBean">class specification</a>.
   * </ol>
   *
   * @param connection the {@code MBeanServerConnection} to forward to.
   * @param mxbeanName the name of a platform MXBean within {@code connection} to forward to. {@code
   *     mxbeanName} must be in the format of {@link ObjectName ObjectName}.
   * @param mxbeanInterface the MXBean interface to be implemented by the proxy.
   * @param <T> an {@code mxbeanInterface} type parameter
   * @return a proxy for a platform MXBean interface of a given <a href="#MXBeanNames">MXBean
   *     name</a> that forwards its method calls through the given {@code MBeanServerConnection}, or
   *     {@code null} if not exist.
   * @throws IllegalArgumentException if
   *     <ul>
   *       <li>{@code mxbeanName} is not with a valid {@link ObjectName ObjectName} format, or
   *       <li>the named MXBean in the {@code connection} is not a MXBean provided by the platform,
   *           or
   *       <li>the named MXBean is not registered in the {@code MBeanServerConnection}, or
   *       <li>the named MXBean is not an instance of the given {@code mxbeanInterface}
   *     </ul>
   *
  ...
  public static <T> T newPlatformMXBeanProxy(
    ...
    @SuppressWarnings("removal")
    ClassLoader loader =
        AccessController.doPrivileged((PrivilegedAction<ClassLoader>) () -> cls.getClassLoader());
  ...
  private static boolean isInstanceOf(
      MBeanServerConnection connection, ObjectName objName, String intfName)
    ...
    if ("java.util.logging.LoggingMXBean".equals(intfName)) {
      if (connection.isInstanceOf(objName, PlatformLoggingMXBean.class.getName())) {
        return true;
      }
    }
  ...
  /**
   * Returns the platform MXBean implementing the given {@code mxbeanInterface} which is specified
   * to have one single instance in the Java virtual machine. This method may return {@code null} if
   * the management interface is not implemented in the Java virtual machine (for example, a Java
   ...
   *     the Java virtual machine if implemented.
   * @param <T> an {@code mxbeanInterface} type parameter
   * @return the platform MXBean that implements {@code mxbeanInterface}, or {@code null} if not
   *     exist.
  ...
  private static Stream<String> getProxyNames(
      PlatformComponent<?> pc, MBeanServerConnection conn, Class<?> intf) throws IOException {
    if (pc.mbeanInterfaceNames().contains(intf.getName())) {
      if (pc.isSingleton()) {
        return Stream.of(pc.getObjectNamePattern());
      } else {
        return conn.queryNames(Util.newObjectName(pc.getObjectNamePattern()), null).stream()
  ...
  private static class PlatformMBeanFinder {
    ...
    static PlatformComponent<?> findSingleton(Class<?> mbeanIntf) {
          ...
          getMap().values().stream()
              ...
              .reduce(
                  (p1, p2) -> {
                    if (p2 != null) {
                      throw new IllegalArgumentException(
                          mbeanIntf.getName() + " can have more than one instance");
                    } else {
                      return p1;
                    }
                  });
      ...
      if (singleton == null) {
        throw new IllegalArgumentException(
            mbeanIntf.getName() + " is not a platform management interface");
      }
