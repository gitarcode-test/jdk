...
class DefaultPlatformMBeanProvider extends PlatformMBeanProvider {
  ...
  private List<PlatformComponent<?>> init() {
    ...
    initMBeanList.add(
        new PlatformComponent<ClassLoadingMXBean>() {
    ...
    initMBeanList.add(
        new PlatformComponent<CompilationMXBean>() {
    ...
    initMBeanList.add(
        new PlatformComponent<MemoryMXBean>() {
          ...
          public Map<String, MemoryMXBean> nameToMBeanMap() {
            return Collections.singletonMap(
                ManagementFactory.MEMORY_MXBEAN_NAME, ManagementFactoryHelper.getMemoryMXBean());
          }
        });

    /** Garbage Collector in the Java virtual machine. */
    initMBeanList.add(
        new PlatformComponent<MemoryManagerMXBean>() {
          ...
          public Set<Class<? extends MemoryManagerMXBean>> mbeanInterfaces() {
            return Stream.of(MemoryManagerMXBean.class, GarbageCollectorMXBean.class)
                .collect(Collectors.toSet());
          }
          ...
          public Map<String, MemoryManagerMXBean> nameToMBeanMap() {
            ...
            if (list.isEmpty()) {
              ...
              for (MemoryManagerMXBean gcm : list) {
                map.put(gcm.getObjectName().getCanonicalName(), gcm);
              }
            }
            return map;
          }
    ...
    initMBeanList.add(
        new PlatformComponent<MemoryPoolMXBean>() {
          ...
          public Map<String, MemoryPoolMXBean> nameToMBeanMap() {
            List<MemoryPoolMXBean> list = ManagementFactoryHelper.getMemoryPoolMXBeans();
            ...
            if (list.isEmpty()) {
              ...
              for (MemoryPoolMXBean mpm : list) {
                map.put(mpm.getObjectName().getCanonicalName(), mpm);
              }
            }
            return map;
          }
    ...
    initMBeanList.add(
        new PlatformComponent<RuntimeMXBean>() {
          ...
          public Map<String, RuntimeMXBean> nameToMBeanMap() {
            return Collections.singletonMap(
                ManagementFactory.RUNTIME_MXBEAN_NAME, ManagementFactoryHelper.getRuntimeMXBean());
          }
        });

    /** Threading system of the Java virtual machine. */
    initMBeanList.add(
        new PlatformComponent<ThreadMXBean>() {
          ...
          public Map<String, ThreadMXBean> nameToMBeanMap() {
            return Collections.singletonMap(
                ManagementFactory.THREAD_MXBEAN_NAME, ManagementFactoryHelper.getThreadMXBean());
          }
        });
    ...
    if (ManagementFactoryHelper.isPlatformLoggingMXBeanAvailable()) {
      /** Logging facility. */
      initMBeanList.add(
          new PlatformComponent<PlatformLoggingMXBean>() {
    ...
    /** Buffer pools. */
    initMBeanList.add(
        new PlatformComponent<BufferPoolMXBean>() {
          ...
          public Map<String, BufferPoolMXBean> nameToMBeanMap() {
            List<BufferPoolMXBean> list = ManagementFactoryHelper.getBufferPoolMXBeans();
