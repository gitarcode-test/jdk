 ...
 *
 * <p>The exit codes for the main method are:
 *
 * <p>0: success 1: any error that the jar cannot be signed or verified, including: keystore loading
 * error TSP communication error jarsigner command line error... otherwise: error codes from -strict
 *
...
public class Main {
  ...
  private static final java.util.ResourceBundle rb =
      java.util.ResourceBundle.getBundle("sun.security.tools.jarsigner.Resources");
  ...
  private static final DisabledAlgorithmConstraints JAR_DISABLED_CHECK =
      new DisabledAlgorithmConstraints(DisabledAlgorithmConstraints.PROPERTY_JAR_DISABLED_ALGS);
    ...
    try {
      ...
      if (providers != null) {
        for (String provName : providers) {
          try {
            KeyStoreUtil.loadProviderByName(provName, providerArgs.get(provName));
            if (debug) {
              System.out.println("loadProviderByName: " + provName);
            }
          } catch (IllegalArgumentException e) {
            throw new Exception(String.format(rb.getString("provider.name.not.found"), provName));
          }
        }
      }
      ...
      if (providerClasses != null) {
        ...
        for (String provClass : providerClasses) {
          try {
            KeyStoreUtil.loadProviderByClass(provClass, providerArgs.get(provClass), cl);
            if (debug) {
              System.out.println("loadProviderByClass: " + provClass);
            }
          } catch (ClassCastException cce) {
            throw new Exception(String.format(rb.getString("provclass.not.a.provider"), provClass));
          } catch (IllegalArgumentException e) {
            throw new Exception(
                String.format(rb.getString("provider.class.not.found"), provClass), e.getCause());
          }
        }
      }
  ...
  String[] parseArgs(String args[]) throws Exception {
    ...
    if (confFile != null) {
      args = KeyStoreUtil.expandArgs("jarsigner", confFile, command, null, args);
    }
    ...
    if (token && keypass != null) {
      System.err.println(
          MessageFormat.format(
              rb.getString(".keypass.can.not.be.specified.if.storetype.is.{0}"), storetype));
      usage();
    }
  ...
  static void fullusage() {
    ...
    System.out.println(rb.getString(".jarsigner.version"));
    System.out.println();
    ...
    System.out.println();
    System.out.println(rb.getString(".version.print.the.program.version"));
    System.out.println();
    ...
    System.out.println();
    System.out.println(rb.getString(".certs.revocation.check"));
    System.out.println();
    System.out.println(rb.getString(".tsa.url.location.of.the.Timestamping.Authority"));
    System.out.println();
    System.out.println(
        rb.getString(".tsacert.alias.public.key.certificate.for.Timestamping.Authority"));
    System.out.println();
    System.out.println(rb.getString(".tsapolicyid.tsapolicyid.for.Timestamping.Authority"));
    System.out.println();
    System.out.println(
        rb.getString(".tsadigestalg.algorithm.of.digest.data.in.timestamping.request"));
    System.out.println();
    ...
    System.out.println();
    System.out.println(rb.getString(".add.provider.option"));
    System.out.println(rb.getString(".providerArg.option.1"));
    System.out.println();
    System.out.println(rb.getString(".providerClass.option"));
    System.out.println(rb.getString(".providerArg.option.2"));
    System.out.println();
    System.out.println(rb.getString(".providerPath.option"));
    System.out.println();
    System.out.println(rb.getString(".strict.treat.warnings.as.errors"));
    System.out.println();
    System.out.println(rb.getString(".conf.url.specify.a.pre.configured.options.file"));
    System.out.println();
    System.out.println(rb.getString(".print.this.help.message"));
    System.out.println();
    ...
    try {
        ...
        try (InputStream is = jf.getInputStream(je)) {
          ...
          if (signatureRelated(name) && SignatureFileVerifier.isBlockOrSF(name)) {
            ...
            if (uncompressedSize > SignatureFileVerifier.MAX_SIG_FILE_SIZE) {
              unparsableSignatures.putIfAbsent(
                  alias, String.format(rb.getString("history.unparsable"), name));
              continue;
            }
      ...
      if (man != null) {
        ...
        while (e.hasMoreElements()) {
          ...
          allAliasesFound = (inStoreWithAlias & SOME_ALIASES_NOT_FOUND) == 0;
          ...
          if (verbose != null) {
            ...
            sb.append(isSigned ? rb.getString("s") : rb.getString("SPACE"))
                ...
                .append(
                    (inStoreWithAlias & NOT_ALIAS) != 0 ? rb.getString("X") : rb.getString("SPACE"))
          ...
          if (isSigned) {
            ...
            if (signatureRelated(name)) {
              sb.append('\n')
                  .append(tab)
                  .append(rb.getString(".Signature.related.entries."))
                  .append("\n\n");
      ...
      if (verbose != null) {
            ...
            if (verbose.equals("grouped")) {
              ...
              if (files.size() > 1) {
                System.out.println(
                    files.get(0)
                        + " "
                        + String.format(rb.getString(".and.d.more."), files.size() - 1));
              } else {
        ...
        if (hasUnsignedEntry) {
          System.out.println(rb.getString(".q.unsigned.entry"));
        }
      }
      if (man == null) {
        System.out.println();
        ...
        for (String s : digestMap.keySet()) {
          ...
          if (p7 != null) {
            ...
            try {
              ...
              String sigAlg =
                  SignerInfo.makeSigAlg(
                      si.getDigestAlgorithmId(), si.getDigestEncryptionAlgorithmId());
              ...
              if (tsToken != null) {
                ...
                String tsSigAlg =
                    SignerInfo.makeSigAlg(
                        tsSi.getDigestAlgorithmId(), tsSi.getDigestEncryptionAlgorithmId());
                ...
                Calendar c =
                    Calendar.getInstance(
                        TimeZone.getTimeZone("UTC"), Locale.getDefault(Locale.Category.FORMAT));
                Date tsDate = tsTokenInfo.getDate();
                c.setTime(tsDate);
                JarConstraintsParameters jcp = new JarConstraintsParameters(chain, tsDate);
                JarConstraintsParameters jcpts =
                    new JarConstraintsParameters(tsSi.getCertificateChain(tsToken), tsDate);
                history =
                    String.format(
                ...
                JarConstraintsParameters jcp = new JarConstraintsParameters(chain, null);
                history =
                    String.format(
  ...
  private void displayMessagesAndResult(boolean isSigning) {
    ...
    boolean signerNotExpired = expireDate == null || expireDate.after(new Date());
      ...
      if ((legacyAlg & 8) == 8) {
        warnings.add(
            String.format(
                ...
                KeyUtil.fullDisplayAlgName(privateKey),
      ...
      if ((legacyAlg & 8) == 8) {
        warnings.add(
            String.format(
                rb.getString(
                    "The.1.signing.key.has.a.keysize.of.2.which.is.considered.a.security.risk..This.key.size.will.be.disabled.in.a.future.update."),
                KeyUtil.fullDisplayAlgName(weakPublicKey),
                KeyUtil.getKeySize(weakPublicKey)));
      }
    }
    ...
    if (hasExpiredTsaCert) {
      ...
      if (!signerNotExpired || !errors.isEmpty()) {
        ...
        if (expireDate != null) {
          warnings.add(
              String.format(
                  rb.getString("The.timestamp.expired.1.but.usable.2"), tsaExpireDate, expireDate));
        }
    ...
    if (hasExpiringTsaCert && expireDate != null) {
      if (expireDate.after(tsaExpireDate)) {
        warnings.add(
            String.format(
                rb.getString("The.timestamp.will.expire.within.one.year.on.1.but.2"),
                tsaExpireDate,
                expireDate));
      } else {
        warnings.add(
            String.format(
                rb.getString("The.timestamp.will.expire.within.one.year.on.1"), tsaExpireDate));
      }
    }
        ...
        warnings.add(
            String.format(
                isSigning
                    ...
                    : rb.getString("bad.timestamp.verifying"),
                expireDate));
      } else {
        warnings.add(
            String.format(
                isSigning
                    ...
                    : rb.getString("no.timestamp.verifying"),
                expireDate));
      }
    }
    ...
    if (isSigning || verbose != null) {
      ...
      if (!hasExpiringCert && !hasExpiredCert && expireDate != null && signerNotExpired) {
        info.add(
            String.format(rb.getString("The.signer.certificate.will.expire.on.1."), expireDate));
      }
      if (!noTimestamp) {
        if (!hasExpiringTsaCert && !hasExpiredTsaCert && tsaExpireDate != null) {
          if (signerNotExpired) {
            info.add(String.format(rb.getString("The.timestamp.will.expire.on.1."), tsaExpireDate));
          } else {
            info.add(
                String.format(
                    rb.getString("signer.cert.expired.1.but.timestamp.good.2."),
                    expireDate,
                    tsaExpireDate));
          }
        }
  ...
  private String verifyWithWeak(
      String alg,
      Set<CryptoPrimitive> primitiveSet,
      boolean tsa,
    ...
    if (algParams != null) {
      try {
        ...
        disabledAlgFound = true;
        return String.format(rb.getString("with.algparams.disabled"), alg, algParams);
      }
    }
    ...
    if (algParams != null) {
      try {
        ...
        legacyAlg |= 2;
        legacySigAlg = alg;
        return String.format(rb.getString("with.algparams.weak"), alg, algParams);
      }
    }
    return alg;
  }
  ...
  private String verifyWithWeak(PublicKey key, JarConstraintsParameters jcp) {
    ...
    try {
      ...
      if (key instanceof ECKey) {
        return String.format(
            rb.getString("key.bit.eccurve.disabled"), kLen, KeyUtil.fullDisplayAlgName(key));
      } else {
    ...
    try {
      ...
      if (key instanceof ECKey) {
        return String.format(
            rb.getString("key.bit.eccurve.weak"), kLen, KeyUtil.fullDisplayAlgName(key));
      } else {
  ...
  private void checkWeakSign(
      String alg, Set<CryptoPrimitive> primitiveSet, boolean tsa, JarConstraintsParameters jcp) {
   ...
   *
   * <p>[<tab>] <cert-type> [", " <subject-DN>] [" (" <keystore-entry-alias> ")"] [<validity-period>
   * | <expiry-warning>] [<key-usage-warning>]
   *
   * <p>Note: no newline character at the end.
   *
   * <p>This method sets global flags like hasExpiringCert, hasExpiredCert, notYetValidCert,
   * badKeyUsage, badExtendedKeyUsage, badNetscapeCertType, hasExpiringTsaCert, hasExpiredTsaCert.
   *
      ...
      if (checkUsage) {
        ...
        if (bad[0] || bad[1] || bad[2]) {
          ...
          certStr
              ...
              .append(
                  MessageFormat.format(
                      rb.getString(".{0}.extension.does.not.support.code.signing."), x));
        }
      }
    }
  ...
  private String printTimestamp(String tab, Timestamp timestamp) {
    ...
    return new StringBuilder()
        .append(tab)
        .append("[")
        .append(signTimeForm.format(source))
        .append("]")
        .toString();
  }
  ...
  /**
   * Maps certificates (as keys) to alias names associated in the keystore {@link #keystore} (as
   * values).
   */
  ...
  void signJar(String jarName, String alias) throws Exception {
    ...
    JarConstraintsParameters jcp = new JarConstraintsParameters(Arrays.asList(certChain), null);
    ...
    CertPath cp =
        CertificateFactory.getInstance("X.509").generateCertPath(Arrays.asList(certChain));
    ...
    if (tsaURI != null) {
      if (verbose != null) {
        ...
        if (tsaUrl != null) {
          ...
          System.out.println(
              rb.getString("TSA.certificate.") + printCert(true, "", tsaCert, null, false, cpcp));
        }
      }
      builder.tsa(tsaURI);
      if (tSADigestAlg != null) {
    ...
    try {
      Event.setReportListener(
          Event.ReporterCategory.ZIPFILEATTRS, (t, o) -> externalFileAttributesDetected = true);
    ...
    if (signedjar == null) {
      ...
      if (!signedJarFile.renameTo(jarFile)) {
        ...
        if (jarFile.renameTo(origJar)) {
          ...
          MessageFormat form =
              new MessageFormat(rb.getString("attempt.to.rename.jarFile.to.origJar.failed"));
  ...
  /**
   * signature-related files include: . META-INF/MANIFEST.MF . META-INF/SIG-* . META-INF/*.SF .
   * META-INF/*.DSA . META-INF/*.RSA . META-INF/*.EC
   */
  ...
  /** Returns a string of signer info, with a newline at the end. Called by verifyJar(). */
  ...
  /**
   * Fills info on certs and timestamp into a StringBuilder, sets warning flags (through printCert)
   * and validates cert chains.
   *
  ...
  private String certsAndTSInfo(
      String tab1, String tab2, List<? extends Certificate> certs, Timestamp ts) throws Exception {
    ...
    for (Certificate c : certs) {
      CertPathConstraintsParameters cpcp =
          new CertPathConstraintsParameters(
              (X509Certificate) c, Validator.VAR_CODE_SIGNING, anchor, timestamp);
    ...
    try {
      ...
      sb.append(tab2)
          .append(rb.getString(".Invalid.certificate.chain."))
    ...
    if (ts != null) {
      ...
      for (Certificate c : tschain) {
        CertPathConstraintsParameters cpcp =
            new CertPathConstraintsParameters(
                (X509Certificate) c, Validator.VAR_TSA_SERVER, anchor, timestamp);
        sb.append(printCert(true, tab2, c, null, false, cpcp));
        sb.append('\n');
      }
      try {
        validateCertChain(Validator.VAR_TSA_SERVER, ts.getSignerCertPath().getCertificates(), null);
      } catch (Exception e) {
        tsaChainNotValidated = true;
        tsaChainNotValidatedReason = e;
        sb.append(tab2)
            .append(rb.getString(".Invalid.TSA.certificate.chain."))
            .append(e.getLocalizedMessage())
            .append("]\n");
      }
    }
    if (certs.size() == 1 && KeyStoreUtil.isSelfSigned((X509Certificate) certs.get(0))) {
  ...
  void loadKeyStore(String keyStoreName, boolean prompt) {
    ...
    try {
      ...
      throw new RuntimeException(
          rb.getString("unable.to.instantiate.keystore.class.") + kse.getMessage());
    }
  }
   ...
   * @param bad 3 booleans to show if the KeyUsage, ExtendedKeyUsage, NetscapeCertType has
   *     codeSigning flag turned on. If null, the class field badKeyUsage, badExtendedKeyUsage,
  ...
  void getAliasInfo(String alias) throws Exception {
    ...
    try {
      ...
      try {
        if (!token && keypass == null) key = store.getKey(alias, storepass);
        else key = store.getKey(alias, keypass);
    ...
    if (!(key instanceof PrivateKey)) {
      MessageFormat form =
          new MessageFormat(rb.getString("key.associated.with.alias.not.a.private.key"));
      Object[] source = {alias};
      error(form.format(source));
    } else {
   ...
   */
  void validateCertChain(String variant, List<? extends Certificate> certs, Timestamp parameter)
      throws Exception {
    try {
      Validator.getInstance(Validator.TYPE_PKIX, variant, pkixParameters)
          .validate(certs.toArray(new X509Certificate[certs.size()]), null, parameter);
    } catch (Exception e) {
      if (debug) {
        e.printStackTrace();
      }
      ...
      if (variant.equals(Validator.VAR_TSA_SERVER) && e instanceof ValidatorException) {
        ...
        if (e.getCause() != null && e.getCause() instanceof CertPathValidatorException) {
          ...
          if ((t instanceof CertificateExpiredException && hasExpiredTsaCert)) {
      ...
      if (variant.equals(Validator.VAR_CODE_SIGNING) && e instanceof ValidatorException) {
        // Throw cause if it's CertPathValidatorException,
        if (e.getCause() != null && e.getCause() instanceof CertPathValidatorException) {
          e = (Exception) e.getCause();
          Throwable t = e.getCause();
          if ((t instanceof CertificateExpiredException && hasExpiredCert)
              || (t instanceof CertificateNotYetValidException && notYetValidCert)) {
