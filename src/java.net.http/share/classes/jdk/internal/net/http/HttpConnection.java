 ...
 *
 * <p>Subtypes are: PlainHttpConnection: regular direct TCP connection to server
 * PlainProxyConnection: plain text proxy connection PlainTunnelingConnection: opens plain text
 * (CONNECT) tunnel to server AsyncSSLConnection: TLS channel direct to server
...
abstract class HttpConnection implements Closeable {
  ...
  static final Logger DEBUG_LOGGER =
      Utils.getDebugLogger(() -> "HttpConnection(SocketTube(?))", Utils.DEBUG);
   ...
   *
   * <p>Returns a CompletableFuture that completes when the underlying TCP connection has been
   ...
   *
   * <p>Returns a CompletableFuture that completes when any additional, type specific, setup has
   * been done. Must be called after connectAsync.
   */
  ...
  /**
   * Tells whether, or not, this connection is proxied. Returns true for tunnel connections, or
   * clear connection to any host through proxy.
   */
  abstract boolean isProxied();

  /**
   * Returns the address of the proxy used by this connection. Returns the proxy address for tunnel
   * connections, or clear connection to any host through proxy. Returns {@code null} otherwise.
   */
  abstract InetSocketAddress proxy();

  /** Tells whether, or not, this connection is open. */
  final boolean isOpen() {
    return channel().isOpen() && (connected() ? !getConnectionFlow().isFinished() : true);
  }

  /**
   * Forces a call to the native implementation of the connection's channel to verify that this
   * channel is still open.
   *
   * <p>This method should only be called just after an HTTP/1.1 connection is retrieved from the
   * HTTP/1.1 connection pool. It is used to trigger an early detection of the channel state, before
   * handling the connection over to the HTTP stack. It helps minimizing race conditions where the
   * selector manager thread hasn't woken up - or hasn't raised the event, before the connection was
   * retrieved from the pool. It helps reduce the occurrence of "HTTP/1.1 parser received no bytes"
   * exception, when the server closes the connection while it's being taken out of the pool.
   *
   * <p>This method attempts to read one byte from the underlying channel. Because the connection
   * was in the pool - there should be nothing to read.
   *
   * <p>If {@code read} manages to read a byte off the connection, this is a protocol error: the
   * method closes the connection and returns false. If {@code read} returns EOF, the method closes
   * the connection and returns false. If {@code read} throws an exception, the method returns
   * false. Otherwise, {@code read} returns 0, the channel appears to be still open, and the method
   * returns true.
   *
  ...
  final boolean checkOpen() {
    if (isOpen()) {
      try {
        ...
        debug.log("Pooled connection is no longer operational: %s", x.toString());
        return false;
      }
    }
    return false;
  ...
  /**
   * Returns the HTTP publisher associated with this connection. May be null if invoked before
   * connecting.
   */
  ...
  private static final Predicate<String> testRequiredHTTP2TLSVersion =
      proto -> proto.equals("TLSv1.2") || proto.equals("TLSv1.3");

  /**
   * Returns true if the given client's SSL parameter protocols contains at least one TLS version
   * that HTTP/2 requires.
   */
  ...
  /**
   * Factory for retrieving HttpConnections. A connection can be retrieved from the connection pool,
   * or a new one created if none available.
   *
   * <p>The given {@code addr} is the ultimate destination. Any proxies, etc, are determined from
   * the request. Returns a concrete instance which is one of the following: {@link
   * PlainHttpConnection} {@link PlainTunnelingConnection}
   *
   * <p>The returned connection, if not from the connection pool, must have its, connect() or
   * connectAsync() method invoked, which ( when it completes successfully ) renders the connection
   * usable for requests.
   */
  public static HttpConnection getConnection(
      InetSocketAddress addr, HttpClientImpl client, HttpRequestImpl request, Version version) {
    ...
    if (!secure) {
      ...
      if (c != null && c.checkOpen() /* may have been eof/closed when in the pool */) {
        ...
        if (DEBUG_LOGGER.on())
          DEBUG_LOGGER.log(
              conn.getConnectionFlow() + ": plain connection retrieved from HTTP/1.1 pool");
        return c;
      } else {
      ...
      if (c != null && c.isOpen()) {
        ...
        if (DEBUG_LOGGER.on())
          DEBUG_LOGGER.log(
              conn.getConnectionFlow() + ": SSL connection retrieved from HTTP/1.1 pool");
        return c;
      } else {
        String[] alpn = null;
  ...
  private static HttpConnection getSSLConnection(
    ...
    else return new AsyncSSLConnection(addr, client, alpn);
  }

  /**
   * This method is used to build a filter that will accept or veto (header-name, value) tuple for
   * transmission on the wire. The filter is applied to the headers when sending the headers to the
   * remote party. Which tuple is accepted/vetoed depends on:
   *
   * <pre>
  ...
  private static ProxyHeaders proxyTunnelHeaders(HttpRequestImpl request) {
    ...
    HttpHeaders systemHeaders =
        HttpHeaders.of(request.getSystemHeadersBuilder().map(), Utils.PROXY_TUNNEL_FILTER);
    return new ProxyHeaders(userHeaders, systemHeaders);
  }

  /* Returns either a plain HTTP connection or a plain tunnelling connection
   * for proxied WebSocket */
  private static HttpConnection getPlainConnection(
      InetSocketAddress addr,
      InetSocketAddress proxy,
      HttpRequestImpl request,
      HttpClientImpl client) {
    ...
    if (proxy == null) return new PlainHttpConnection(addr, client);
    else return new PlainProxyConnection(proxy, client);
  }
  ...
  void closeOrReturnToCache(HttpHeaders hdrs) {
    ...
    ConnectionPool pool = client.connectionPool();
    boolean keepAlive =
        hdrs.firstValue("Connection").map((s) -> !s.equalsIgnoreCase("close")).orElse(true);
    ...
    if (keepAlive && isOpen()) {
      ...
      Log.logTrace(
          "Closing connection (keepAlive={0}, isOpen={1}): {2}", keepAlive, isOpen(), this);
      close();
    }
  }
  ...
  /**
   * Returns an unmodifiable list of {@link SNIServerName}s that were used during TLS handshake of
   * this connection. If this connection doesn't represent a TLS based connection or if no SNI
  ...
  /**
   * A publisher that makes it possible to publish (write) ordered (normal priority) and unordered
   * (high priority) buffers downstream.
   */
  final class PlainHttpPublisher implements HttpPublisher {
    ...
    final SequentialScheduler writeScheduler = new SequentialScheduler(this::flushTask);
    ...
    final class HttpWriteSubscription implements Flow.Subscription {
      ...
      public void request(long n) {
        ...
        demand.increase(n);
        if (debug.on())
          debug.log("HttpPublisher: got request of " + n + " from " + getConnectionFlow());
        writeScheduler.runOrSchedule();
      }

      @Override
      public void cancel() {
        if (debug.on()) debug.log("HttpPublisher: cancelled by " + getConnectionFlow());
      }
      ...
      void flush() {
        while (!isEmpty() && demand.tryDecrement()) {
          ...
          if (debug.on())
            debug.log(
                "HttpPublisher: sending "
                    + Utils.remaining(elem)
                    + " bytes ("
                    + elem.size()
                    + " buffers) to "
                    + getConnectionFlow());
