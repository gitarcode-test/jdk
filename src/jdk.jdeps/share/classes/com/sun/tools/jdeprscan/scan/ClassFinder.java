...
public class ClassFinder {
  ...
  /**
   * Searches the class path for a class with the given name, returning a ClassFile for it. Returns
   * null if not found.
   *
  ...
  interface PathEntry {
    /**
     * Returns a ClassFile instance corresponding to this name, or null if it's not present in this
     * entry.
     *
     * @param className the class to search for
     * @return a ClassFile instance, or null if not found
   ...
   *
   * <p>JRT filesystem structure is: /packages/<dotted-pkgname>/<modlink> where modlink is a
   * symbolic link to /modules/<modname> which is the top of the usual package-class hierarchy
   */
  class JrtPathEntry implements PathEntry {
    ...
    public ClassModel find(String className) {
      ...
      String pkg = "/packages/" + className.substring(0, end).replace('/', '.');
