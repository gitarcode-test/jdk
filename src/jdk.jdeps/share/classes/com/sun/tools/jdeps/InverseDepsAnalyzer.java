...
public class InverseDepsAnalyzer extends DepsAnalyzer {
   ...
   *
   * <p>Inverse transitive dependency will find all nodes that depend upon the returned set of
   * archives directly and indirectly.
   */
  ...
  /**
   * Finds all inverse transitive dependencies using the given requires set as the targets, if
   * non-empty. If the given requires set is empty, use the archives depending the packages
   * specified in -regex or -p options.
   */
  public Set<Deque<Archive>> inverseDependences() throws IOException {
    ...
    try {
      ...
      Stream<Archive> archives =
          Stream.concat(
              configuration.initialArchives().stream(), configuration.classPathArchives().stream());
      if (apiOnly) {
  ...
  private Set<Deque<Archive>> findPaths(Graph<Archive> graph, Archive target) {
    ...
    while (!deque.isEmpty()) {
      ...
      Set<Edge<Archive>> unvisitedDeps =
          graph.edgesFrom(node).stream()
      ...
      while (!path.isEmpty()) {
        if (visited.containsAll(graph.edgesFrom(path.peekLast()))) path.removeLast();
        else break;
      }
    }
