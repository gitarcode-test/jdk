...
public class JdepsFilter implements Dependency.Filter, Analyzer.Filter {
  ...
  public boolean matches(String cn) {
    ...
    if (includePattern != null) return includePattern.matcher(cn).matches();
   ...
   *
   * <p>This method can be used to determine if the given source should eagerly be processed.
   */
  ...
  public boolean accepts(Dependency d) {
    if (d.getOrigin().equals(d.getTarget())) return false;
  ...
  public boolean accepts(
      Location origin, Archive originArchive, Location target, Archive targetArchive) {
    if (findJDKInternals) {
