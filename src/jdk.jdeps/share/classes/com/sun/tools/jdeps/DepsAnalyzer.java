 ...
 * Dependency Analyzer.
 *
 * <p>Type of filters: source filter: -include <pattern> target filter: -package, -regex, --require
 *
 * <p>The initial archive set for analysis includes 1. archives specified in the command line
 * arguments 2. observable modules matching the source filter 3. classpath archives matching the
 * source filter or target filter 4. --add-modules and -m root modules
 */
public class DepsAnalyzer {
  ...
  public boolean run(boolean compileTimeView, int maxDepth) throws IOException {
    try {
      ...
      // transitive analysis
      if (depth > 1) {
        if (compileTimeView) transitiveArchiveDeps(depth - 1);
        else transitiveDeps(depth - 1);
      }
   ...
   *
   * <p>If --require is set, they should be excluded.
   */
  ...
  /**
   * Returns the dependences, either class name or package name as specified in the given verbose
   * level.
   */
  ...
  private void transitiveArchiveDeps(int depth) throws IOException {
    Stream<Location> deps = archives.stream().flatMap(Archive::getDependencies);
  ...
  private void transitiveDeps(int depth) throws IOException {
    ...
    do {
      ...
      while ((target = unresolved.poll()) != null) {
        if (finder.isParsed(target)) continue;
        ...
        if (archive != null) {
          ...
          Set<Location> targets =
              apiOnly ? finder.parseExportedAPIs(archive, name) : finder.parse(archive, name);

          // build unresolved dependencies
          targets.stream().filter(t -> !finder.isParsed(t)).forEach(deque::add);
        }
      }
      unresolved = deque;
  ...
  public boolean include(Archive source) {
    ...
    return !module.isSystem() || configuration.rootModules().contains(source);
  }
  ...
  public static class Node {
    ...
    public String toString() {
      ...
      if (info != Info.REQUIRES && info != Info.REQUIRES_TRANSITIVE) sb.append(source).append("/");

      sb.append(name);
      if (info == Info.QUALIFIED_EXPORTED_API) sb.append(" (qualified)");
      else if (info == Info.JDK_INTERNAL_API) sb.append(" (JDK internal)");
      else if (info == Info.INTERNAL_API) sb.append(" (internal)");
      return sb.toString();
    }
   ...
   *
   * <p>Each Node represents a module and each edge is a dependence. No analysis on "requires
   * transitive".
   */
   ...
   *
   * <p>Each Node represents a class or package per the specified verbose level. Each edge indicates
   */
  ...
  private void addEdge(Graph.Builder<Node> builder, Analyzer.Dep dep) {
    ...
    if (source == target) {
      ...
      } else if (!source.getModule().isJDK() && module.isJDK()) info = Info.JDK_INTERNAL_API;
      // qualified exports or inaccessible
      else if (module.isExported(pn, source.getModule().name())) info = Info.QUALIFIED_EXPORTED_API;
      else info = Info.INTERNAL_API;
    }
