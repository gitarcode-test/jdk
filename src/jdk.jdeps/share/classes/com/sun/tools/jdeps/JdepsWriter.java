...
public abstract class JdepsWriter {
  ...
  static class DotFileWriter extends JdepsWriter {
    ...
    @Override
    void generateOutput(Collection<Archive> archives, Analyzer analyzer) throws IOException {
    ...
    class DotFileFormatter implements Analyzer.Visitor, AutoCloseable {
      ...
      public void visitDependence(
        ...
        writer.format(
            ...
            tag.isEmpty() ? target : String.format("%s (%s)", target, tag));
      }
    }
    ...
    class SummaryDotFile implements Analyzer.Visitor, AutoCloseable {
      ...
      public void visitDependence(
        ...
        writer.format("  %-50s -> \"%s\"%s;%n", String.format("\"%s\"", origin), targetName, label);
      }
  ...
  /**
   * If the given target is JDK module and the source accesses a private JDK API, the returned value
   * will have "JDK internal API" prefix
   *
   * <p>For non-JDK archives, this method returns the file name of the archive.
   */
