...
class NativeMethodFinder {
  ...
  public static NativeMethodFinder create(
      ClassResolver classesToScan, ClassResolver systemClassResolver)
      throws JNativeScanFatalError, IOException {
    return new NativeMethodFinder(classesToScan, systemClassResolver);
  }

  public SortedMap<ClassFileSource, SortedMap<ClassDesc, List<RestrictedUse>>> findAll()
      throws JNativeScanFatalError {
              ...
              .forEach(
                  methodModel -> {
                    if (methodModel.flags().has(AccessFlag.NATIVE)) {
                      ...
                      SortedSet<MethodRef> perMethod =
                          new TreeSet<>(Comparator.comparing(MethodRef::toString));
                      methodModel
                          .code()
                          .ifPresent(
                              code -> {
                                try {
                                  ...
                                  throw new JNativeScanFatalError(
                                      "Error while processing method: "
                                          + MethodRef.ofModel(methodModel),
                                      e);
                                }
                              });
                      if (!perMethod.isEmpty()) {
                        perClass.add(
                            new RestrictedMethodRefs(MethodRef.ofModel(methodModel), perMethod));
                      }
                    }
                  });
          if (!perClass.isEmpty()) {
    ...
    return cache.computeIfAbsent(
        ...
        methodRef -> {
          if (methodRef.owner().isArray()) {
            // no restricted methods in arrays atm, and we can't look them up since they have no
            // class file
            return false;
          }
          ...
          // If we are here, the method was referenced through a subclass of the class containing
          // the actual
          // method declaration. We could implement a method resolver (that needs to be version
          // aware
          // as well) to find the method model of the declaration, but it's not really worth it.
          // None of the restricted methods (atm) are exposed through more than 1 public type, so
          // it's not
