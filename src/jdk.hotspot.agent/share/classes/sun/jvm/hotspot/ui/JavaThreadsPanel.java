...
/**
 * This panel contains a JTable which displays the list of Java threads as their native thread
 * identifiers combined with their Java names. It allows selection and examination of any of the
 * threads.
 */
public class JavaThreadsPanel extends SAPanel implements ActionListener {
  ...
  static {
    VM.registerVMInitializedObserver((o, a) -> initialize(VM.getVM().getTypeDataBase()));
  }
  ...
  /**
   * Constructor assumes the threads panel is created while the VM is suspended. Subsequent resume
   * and suspend operations of the VM will cause the threads panel to clear and fill itself back in,
   * respectively.
   */
  ...
  /**
   * A splitpane panel which contains the thread table and the Thread Info. the thread info is
   * toggleable
   */
