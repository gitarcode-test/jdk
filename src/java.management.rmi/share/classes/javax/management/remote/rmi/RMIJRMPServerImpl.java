...
/**
 * An {@link RMIServer} object that is exported through JRMP and that creates client connections as
 * RMI objects exported through JRMP. User code does not usually reference this class directly.
 *
 * @see RMIServerImpl
 * @since 1.5
 */
public class RMIJRMPServerImpl extends RMIServerImpl {
  ...
  /**
   * Creates a new {@link RMIServer} object that will be exported on the given port using the given
   * socket factories.
   *
   * @param port the port on which this object and the {@link RMIConnectionImpl} objects it creates
   *     will be exported. Can be zero, to indicate any available port.
   * @param csf the client socket factory for the created RMI objects. Can be null.
   * @param ssf the server socket factory for the created RMI objects. Can be null.
   * @param env the environment map. Can be null.
   * @exception IOException if the {@link RMIServer} object cannot be created.
   * @exception IllegalArgumentException if <code>port</code> is negative.
   */
  public RMIJRMPServerImpl(
      int port, RMIClientSocketFactory csf, RMIServerSocketFactory ssf, Map<String, ?> env)
      throws IOException {

    super(env);

    if (port < 0) throw new IllegalArgumentException("Negative port: " + port);
    ...
    String credentialsFilter = (String) this.env.get(RMIConnectorServer.CREDENTIALS_FILTER_PATTERN);
   ...
   * @exception IOException if the stub cannot be obtained - e.g the RMIJRMPServerImpl has not been
   *     exported yet.
   */
  ...
  /**
   * Creates a new client connection as an RMI object exported through JRMP. The port and socket
   * factories for the new {@link RMIConnection} object are the ones supplied to the <code>
   * RMIJRMPServerImpl</code> constructor.
   *
   ...
   * @exception IOException if the new {@link RMIConnection} object cannot be created or exported.
   */
  protected RMIConnection makeClient(String connectionId, Subject subject) throws IOException {
    ...
    RMIConnection client =
        new RMIConnectionImpl(this, connectionId, getDefaultClassLoader(), subject, env);
  ...
  /**
   * Called by {@link #close()} to close the connector server by unexporting this object. After
   * returning from this method, the connector server must not accept any new connections.
   *
   * @exception IOException if the attempt to close the connector server failed.
   */
  ...
  /**
   * Check that a type in the remote invocation of {@link RMIServerImpl#newClient} is one of the
   * {@code allowedTypes}.
   *
   ...
   * @return {@code ObjectInputFilter.Status.ALLOWED} if the class is allowed, otherwise {@code
  ...
  ObjectInputFilter.Status newClientCheckInput(ObjectInputFilter.FilterInfo filterInfo) {
    ...
    if (allowedTypes != null && filterInfo.serialClass() != null) {
      ...
      if (allowedTypes.contains(type)) status = ObjectInputFilter.Status.ALLOWED;
      else status = ObjectInputFilter.Status.REJECTED;
    }
    return status;
  }
