...
public final class OrderResourcesPlugin extends AbstractPlugin {
  ...
  public ResourcePool transform(ResourcePool in, ResourcePoolBuilder out) {
    in.entries()
        .filter(resource -> resource.type().equals(ResourcePoolEntry.Type.CLASS_OR_RESOURCE))
    ...
    for (String pattern : patterns) {
      if (pattern.startsWith("@")) {
        ...
        ToIntFunction<String> function =
            (path) -> matcher.matches(JRT_FILE_SYSTEM.getPath(path)) ? result : Integer.MAX_VALUE;
