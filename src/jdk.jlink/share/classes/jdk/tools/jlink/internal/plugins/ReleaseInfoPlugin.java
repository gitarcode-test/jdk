...
public final class ReleaseInfoPlugin extends AbstractPlugin {
      ...
      case "del":
        {
          ...
          Utils.parseList(keys).forEach(release::remove);
        }
        break;
  ...
  public ResourcePool transform(ResourcePool in, ResourcePoolBuilder out) {
    ...
    ResourcePoolModule javaBase = in.moduleView().findModule("java.base").orElse(null);
    ...
    desc.version().ifPresent(v -> release.put("JAVA_VERSION", quote(parseVersion(v))));
    ...
    release.put(
        ...
        new ModuleSorter(in.moduleView())
            .sorted()
            .map(ResourcePoolModule::name)
            .collect(Collectors.joining(" ", "\"", "\"")));

    // create a TOP level ResourcePoolEntry for "release" file.
    out.add(
        ResourcePoolEntry.create(
            "/java.base/release", ResourcePoolEntry.Type.TOP, releaseFileContent()));
    return out.build();
  }
  ...
  private static String parseVersion(ModuleDescriptor.Version v) {
    return Runtime.Version.parse(v.toString()).version().stream()
        .map(Object::toString)
        .collect(Collectors.joining("."));
  }
