...
public final class LegalNoticeFilePlugin extends AbstractPlugin {
  ...
  private void dedupLegalNoticeEntry(ResourcePoolEntry entry) {
    ...
    Optional<ResourcePoolEntry> otarget =
        entries.stream()
    ...
    if (otarget.isEmpty()) {
      if (errorIfNotSameContent) {
        ...
        Optional<ResourcePoolEntry> ores =
            entries.stream().filter(e -> e.linkedTarget() == null).findAny();

        if (ores.isPresent()) {
          throw new PluginException(
              ores.get().path() + " " + entry.path() + " contain different content");
        }
      }
      entries.add(entry);
    } else {
      entries.add(ResourcePoolEntry.createSymLink(entry.path(), entry.type(), otarget.get()));
    }
  }
