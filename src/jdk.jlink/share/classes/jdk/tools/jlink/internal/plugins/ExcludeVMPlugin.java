...
public final class ExcludeVMPlugin extends AbstractPlugin {
  ...
  /**
   * VM paths: /java.base/lib/{architecture}/{server|client|minimal}/{shared lib} e.g.:
   * /java.base/lib/server/libjvm.so /java.base/lib/server/libjvm.dylib
   */
  ...
  private ResourcePoolEntry handleJvmCfgFile(
      ResourcePoolEntry orig, TreeSet<Jvm> existing, TreeSet<Jvm> removed) throws IOException {
    if (keepAll) {
      return orig;
    }
    ...
    try (BufferedReader reader =
        new BufferedReader(new InputStreamReader(orig.content(), StandardCharsets.UTF_8))) {
      reader
          .lines()
          .forEach(
              (s) -> {
