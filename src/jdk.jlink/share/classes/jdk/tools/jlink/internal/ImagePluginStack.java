...
public final class ImagePluginStack {
  ...
  private static final class CheckOrderResourcePoolManager extends ResourcePoolManager {
    ...
    public CheckOrderResourcePoolManager(
        ByteOrder order, List<ResourcePoolEntry> orderedList, StringTable table) {
      super(order, table);
  ...
  public ImagePluginStack(
      ImageBuilder imageBuilder, List<Plugin> plugins, Plugin lastSorter, boolean validate) {
  ...
  /**
   * Resource Plugins stack entry point. All resources are going through all the plugins.
   *
  ...
  public ResourcePool visitResources(ResourcePoolManager resources) throws Exception {
    ...
    if (resources.isEmpty()) {
      return new ResourcePoolManager(resources.byteOrder(), resources.getStringTable())
          .resourcePool();
    }
    PreVisitStrings previsit = new PreVisitStrings();
    resourcePrevisitors.forEach(
        (p) -> {
    ...
    for (Plugin p : plugins) {
      ...
      if (p == lastSorter) {
        if (frozenOrder != null) {
          throw new Exception(
              "Order of resources is already frozen. Plugin " + p.getName() + " is badly located");
        }
        // Create a special Resource pool to compute the indexes.
        resMgr = new OrderedResourcePoolManager(resPool.byteOrder(), resources.getStringTable());
      } else { // If we have an order, inject it
        if (frozenOrder != null) {
          resMgr =
              new CheckOrderResourcePoolManager(
                  resPool.byteOrder(), frozenOrder, resources.getStringTable());
        } else {
          resMgr = new ResourcePoolManager(resPool.byteOrder(), resources.getStringTable());
        }
      }
      try {
  ...
  /** This pool wrap the original pool and automatically uncompress ResourcePoolEntry if needed. */
    ...
    private ResourcePoolEntry getUncompressed(ResourcePoolEntry res) {
      if (res != null) {
        if (res instanceof ResourcePoolManager.CompressedModuleData) {
          try {
            byte[] bytes =
                decompressor.decompressResource(
                    byteOrder(),
   ...
   */
  public void storeFiles(ResourcePool original, ResourcePool transformed, BasicImageWriter writer)
      throws Exception {
