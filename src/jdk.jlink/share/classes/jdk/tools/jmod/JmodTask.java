...
public class JmodTask {
  ...
  public int run(String[] args) {
    ...
    try {
      ...
      if (e.showUsage) showUsageSummary();
      return EXIT_CMDERR;
    } catch (Exception x) {
  ...
  private boolean describe() throws IOException {
    try (JmodFile jf = new JmodFile(options.jmodFile)) {
      try (InputStream in = jf.getInputStream(Section.CLASSES, MODULE_INFO)) {
        ModuleInfo.Attributes attrs = ModuleInfo.read(in, null);
        describeModule(attrs.descriptor(), attrs.target(), attrs.recordedHashes());
        return true;
      } catch (IOException e) {
        throw new CommandException("err.module.descriptor.not.found");
      }
  ...
  static <T> String toLowerCaseString(Collection<T> c) {
    ...
    return " "
        + c.stream().map(e -> e.toString().toLowerCase(Locale.ROOT)).sorted().collect(joining(" "));
  }
  ...
  private void describeModule(ModuleDescriptor md, ModuleTarget target, ModuleHashes hashes)
    ...
    if (md.isOpen()) sb.append(" open");
    if (md.isAutomatic()) sb.append(" automatic");
    sb.append("\n");
    ...
    md.uses().stream().sorted().forEach(s -> sb.append("uses ").append(s).append("\n"));
    ...
    md.exports().stream()
        ...
        .forEach(
            e ->
                sb.append("qualified exports ")
                    .append(e.source())
                    .append(" to")
                    .append(toLowerCaseString(e.targets()))
                    .append("\n"));
    ...
    md.opens().stream()
        ...
        .forEach(
            o ->
                sb.append("qualified opens ")
                    .append(o.source())
                    .append(toLowerCaseString(o.modifiers()))
                    .append(" to")
                    .append(toLowerCaseString(o.targets()))
                    .append("\n"));
    ...
    if (target != null) {
      ...
      if (!targetPlatform.isEmpty()) sb.append("platform ").append(targetPlatform).append("\n");
    }
  ...
  private boolean create() throws IOException {
    ...
    try {
      try (JmodOutputStream jos =
          JmodOutputStream.newOutputStream(tempTarget, options.date, options.compressLevel)) {
        jmod.write(jos);
      }
  ...
  private class JmodFileWriter {
    ...
    /**
     * Returns a supplier of an input stream to the module-info.class on the class path of
     ...
     *
     * <p>The updated module-info.class will have a Packages attribute with the set of
     * module-private/non-exported packages.
     *
     * <p>If --module-version, --main-class, or other options were provided then the corresponding
     * class file attributes are added to the module-info here.
     */
    void writeModuleInfo(JmodOutputStream out, Set<String> packages) throws IOException {
      ...
      try (InputStream in = miSupplier.get()) {
        ...
        // --main-class
        if (mainClass != null) extender.mainClass(mainClass);
        ...
        // --module-version
        if (moduleVersion != null) extender.version(moduleVersion);
    ...
    private void validatePackages(ModuleDescriptor descriptor, Set<String> packages) {
      ...
      if (!nonExistPackages.isEmpty()) {
        throw new CommandException(
            "err.missing.export.or.open.packages", descriptor.name(), nonExistPackages);
      }
    }
    ...
    private ModuleHashes computeHashes(ModuleDescriptor descriptor) {
      ...
      ModuleReference mref =
          new ModuleReference(descriptor, uri) {
            @Override
      ...
      ModuleFinder finder =
          ModuleFinder.compose(
              options.moduleFinder,
              new ModuleFinder() {
                @Override
                public Optional<ModuleReference> find(String name) {
                  if (descriptor.name().equals(name)) return Optional.of(mref);
    ...
    /** Returns true if it's a .class or a resource with an effective package name. */
    ...
    String toPackageName(Path path) {
      ...
      if (index != -1) return name.substring(0, index).replace(File.separatorChar, '.');
    ...
    String toPackageName(ZipEntry entry) {
      String name = entry.getName();
      int index = name.lastIndexOf("/");
      if (index != -1) return name.substring(0, index).replace('/', '.');
    ...
    void processClasses(JmodOutputStream out, List<Path> classpaths) throws IOException {
    ...
    void processSection(JmodOutputStream out, Section section, Path path) throws IOException {
          ...
          new SimpleFileVisitor<Path>() {
            ...
            public FileVisitResult visitFile(Path file, BasicFileAttributes attrs)
              ...
              if (name.equals(MODULE_INFO)) {
                if (!Section.CLASSES.equals(section)) warning("warn.ignore.entry", name, section);
  ...
  private class Hasher {
     ...
     *
     * <p>If a module name `M` is specified, it will compute the hashes of modules that depend upon
     * M directly or indirectly matching the specified --hash-modules pattern and record in the
     * ModuleHashes attribute in M's module-info.class.
     *
     * <p>If name is null, this Hasher will determine which modules to record hashes, i.e. the
     * module in a subgraph of modules to be hashed and that has no outgoing edges. It will record
     * in each of these modules, say `M`, with the hashes of modules that depend upon M directly or
     * indirectly matching the specified --hash-modules pattern.
     *
    ...
    /**
     * Returns a map of a module M to record hashes of the modules that depend upon M directly or
     * indirectly.
     *
     * <p>For jmod hash command, the returned map contains one entry for each module M that has no
     * outgoing edges to any of the modules matching the specified --hash-modules pattern.
     *
     * <p>Each entry represents a leaf node in a connected subgraph containing M and other candidate
     * modules from the module graph where M's outgoing edges to any module other than the ones
     * matching the specified --hash-modules pattern are excluded.
     */
    Map<String, ModuleHashes> computeHashes() {
      if (hashesBuilder == null) return null;
    ...
    /**
     * Reads the given input stream of module-info.class and write the extended module-info.class
     * with the given ModuleHashes
     *
    ...
    void updateModuleInfo(String name, ModuleHashes moduleHashes) throws IOException {
    ...
    private void updateModularJar(Path target, Path tempTarget, ModuleHashes moduleHashes)
        throws IOException {
    ...
    private void updateJmodFile(Path target, Path tempTarget, ModuleHashes moduleHashes)
        throws IOException {

      try (JmodFile jf = new JmodFile(target);
          JmodOutputStream jos =
              JmodOutputStream.newOutputStream(tempTarget, options.date, options.compressLevel)) {
        jf.stream()
            .forEach(
                e -> {
  ...
  /**
   * An abstract converter that given a string representing a list of paths, separated by the
   * File.pathSeparator, returns a List of java.nio.Path's. Specific subclasses should do whatever
   * validation is required on the individual path elements, if any.
   */
  ...
  static class ClassPathConverter extends AbstractPathConverter {
    ...
    public Path toPath(String value) {
      try {
        ...
        if (Files.notExists(path)) throw new CommandException("err.path.not.found", path);
        if (!(Files.isDirectory(path)
  ...
  static class DirPathConverter extends AbstractPathConverter {
    ...
    public Path toPath(String value) {
      try {
        Path path = CWD.resolve(value);
        if (Files.notExists(path)) throw new CommandException("err.path.not.found", path);
        if (!Files.isDirectory(path)) throw new CommandException("err.path.not.a.dir", path);
        return path;
      } catch (InvalidPathException x) {
        throw new CommandException("err.path.not.valid", value);
      }
    ...
    public Path convert(String value) {
      try {
        ...
        if (Files.exists(path)) {
          if (!Files.isDirectory(path)) throw new CommandException("err.cannot.create.dir", path);
        }
        return path;
      } catch (InvalidPathException x) {
        throw new CommandException("err.path.not.valid", value);
  ...
  static class WarnIfResolvedReasonConverter implements ValueConverter<ModuleResolution> {
    @Override
    public ModuleResolution convert(String value) {
      if (value.equals("deprecated")) return ModuleResolution.empty().withDeprecated();
      else if (value.equals("deprecated-for-removal"))
        return ModuleResolution.empty().withDeprecatedForRemoval();
      else if (value.equals("incubating")) return ModuleResolution.empty().withIncubating();
      else throw new CommandException("err.bad.WarnIfResolvedReason", value);
    }

    @Override
    public Class<ModuleResolution> valueType() {
  ...
  static class PathMatcherConverter implements ValueConverter<PathMatcher> {
    ...
    @Override
    public Class<PathMatcher> valueType() {
  ...
  /** This formatter is adding the @filename option and does the required formatting. */
  ...
  private void handleOptions(String[] args) {
    ...
    OptionSpec<List<Path>> classPath =
        parser
            .accepts("class-path", getMessage("main.opt.class-path"))
            .withRequiredArg()
            .withValuesConvertedBy(ClassPathConverter.INSTANCE);

    OptionSpec<List<Path>> cmds =
        parser
            .accepts("cmds", getMessage("main.opt.cmds"))
            .withRequiredArg()
            .withValuesConvertedBy(DirPathConverter.INSTANCE);

    OptionSpec<List<Path>> config =
        parser
            .accepts("config", getMessage("main.opt.config"))
            .withRequiredArg()
            .withValuesConvertedBy(DirPathConverter.INSTANCE);

    OptionSpec<Path> dir =
        parser
            .accepts("dir", getMessage("main.opt.extractDir"))
            .withRequiredArg()
            .withValuesConvertedBy(new ExtractDirPathConverter());

    OptionSpec<Void> dryrun = parser.accepts("dry-run", getMessage("main.opt.dry-run"));

    OptionSpec<PathMatcher> excludes =
        parser
            .accepts("exclude", getMessage("main.opt.exclude"))
            .withRequiredArg()
            .withValuesConvertedBy(new PathMatcherConverter());

    OptionSpec<Pattern> hashModules =
        parser
            .accepts("hash-modules", getMessage("main.opt.hash-modules"))
            .withRequiredArg()
            .withValuesConvertedBy(new PatternConverter());

    OptionSpec<Void> help =
        parser.acceptsAll(List.of("h", "help", "?"), getMessage("main.opt.help")).forHelp();

    OptionSpec<Void> helpExtra = parser.accepts("help-extra", getMessage("main.opt.help-extra"));

    OptionSpec<List<Path>> headerFiles =
        parser
            .accepts("header-files", getMessage("main.opt.header-files"))
            .withRequiredArg()
            .withValuesConvertedBy(DirPathConverter.INSTANCE);

    OptionSpec<List<Path>> libs =
        parser
            .accepts("libs", getMessage("main.opt.libs"))
            .withRequiredArg()
            .withValuesConvertedBy(DirPathConverter.INSTANCE);

    OptionSpec<List<Path>> legalNotices =
        parser
            .accepts("legal-notices", getMessage("main.opt.legal-notices"))
            .withRequiredArg()
            .withValuesConvertedBy(DirPathConverter.INSTANCE);

    OptionSpec<String> mainClass =
        parser
            .accepts("main-class", getMessage("main.opt.main-class"))
            .withRequiredArg()
            .describedAs(getMessage("main.opt.main-class.arg"));

    OptionSpec<List<Path>> manPages =
        parser
            .accepts("man-pages", getMessage("main.opt.man-pages"))
            .withRequiredArg()
            .withValuesConvertedBy(DirPathConverter.INSTANCE);

    OptionSpec<List<Path>> modulePath =
        parser
            .acceptsAll(List.of("p", "module-path"), getMessage("main.opt.module-path"))
            .withRequiredArg()
            .withValuesConvertedBy(DirPathConverter.INSTANCE);

    OptionSpec<Version> moduleVersion =
        parser
            .accepts("module-version", getMessage("main.opt.module-version"))
            .withRequiredArg()
            .withValuesConvertedBy(new ModuleVersionConverter());

    OptionSpec<String> targetPlatform =
        parser
            .accepts("target-platform", getMessage("main.opt.target-platform"))
            .withRequiredArg()
            .describedAs(getMessage("main.opt.target-platform.arg"));

    OptionSpec<Void> doNotResolveByDefault =
        parser.accepts(
            "do-not-resolve-by-default", getMessage("main.opt.do-not-resolve-by-default"));

    OptionSpec<ModuleResolution> warnIfResolved =
        parser
            .accepts("warn-if-resolved", getMessage("main.opt.warn-if-resolved"))
            .withRequiredArg()
            .withValuesConvertedBy(new WarnIfResolvedReasonConverter());

    OptionSpec<Void> version = parser.accepts("version", getMessage("main.opt.version"));

    OptionSpec<LocalDateTime> date =
        parser
            .accepts("date", getMessage("main.opt.date"))
            .withRequiredArg()
            .withValuesConvertedBy(new DateConverter());

    OptionSpec<Integer> compress =
        parser
            .accepts("compress", getMessage("main.opt.compress"))
            .withRequiredArg()
    ...
    try {
      ...
      if (words.isEmpty()) throw new CommandException("err.missing.mode").showUsage(true);
      ...
      if (opts.has(classPath)) options.classpath = getLastElement(opts.valuesOf(classPath));
      if (opts.has(cmds)) options.cmds = getLastElement(opts.valuesOf(cmds));
      if (opts.has(config)) options.configs = getLastElement(opts.valuesOf(config));
      if (opts.has(dir)) options.extractDir = getLastElement(opts.valuesOf(dir));
      if (opts.has(dryrun)) options.dryrun = true;
      if (opts.has(excludes)) options.excludes = opts.valuesOf(excludes); // excludes is repeatable
      if (opts.has(libs)) options.libs = getLastElement(opts.valuesOf(libs));
      if (opts.has(headerFiles)) options.headerFiles = getLastElement(opts.valuesOf(headerFiles));
      if (opts.has(manPages)) options.manPages = getLastElement(opts.valuesOf(manPages));
      if (opts.has(legalNotices))
        options.legalNotices = getLastElement(opts.valuesOf(legalNotices));
      if (opts.has(date)) options.date = opts.valueOf(date);
      ...
      if (opts.has(mainClass)) options.mainClass = getLastElement(opts.valuesOf(mainClass));
      ...
      if (opts.has(doNotResolveByDefault)) {
        if (options.moduleResolution == null) options.moduleResolution = ModuleResolution.empty();
      ...
      if (opts.has(compress)) {
        if (!options.mode.equals(Mode.CREATE)) {
          throw new CommandException("err.compress.wrong.mode").showUsage(true);
        }
      ...
      if (options.mode.equals(Mode.HASH)) {
        ...
        if (words.size() >= 2) {
          ...
          if (Files.notExists(path)) throw new CommandException("err.jmod.not.found", path);

          options.jmodFile = path;
        }
        if (words.size() > 2)
          throw new CommandException("err.unknown.option", words.subList(2, words.size()))
              .showUsage(true);
      } else {
        if (words.size() <= 1)
        ...
        if (options.mode.equals(Mode.CREATE) && Files.exists(path))
          throw new CommandException("err.file.already.exists", path);
            ...
            && Files.notExists(path)) throw new CommandException("err.jmod.not.found", path);
  ...
  static boolean isValidJavaIdentifier(String mainClass) {
    if (mainClass.length() == 0) return false;

    if (!Character.isJavaIdentifierStart(mainClass.charAt(0))) return false;
    ...
    if (mainClass.charAt(n - 1) == '.') return false;

    return true;
  }

  static <E> E getLastElement(List<E> list) {
    if (list.size() == 0) throw new InternalError("Unexpected 0 list size");
