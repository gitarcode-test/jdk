...
class JImageTask {
  ...
  private static final TaskHelper TASK_HELPER = new TaskHelper(JIMAGE_BUNDLE);
  ...
  int run(String[] args) {
    ...
    try {
      ...
      try {
        ...
        remaining = args.length > 1 ? Arrays.copyOfRange(args, 1, args.length) : new String[0];
      ...
      if (options.task == null && !options.help && !options.version && !options.fullVersion) {
        throw TASK_HELPER.newBadArgs("err.not.a.task", command != null ? command : "<unspecified>");
      }
      ...
      if (options.help) {
        if (options.task == null) {
          ...
          try {
            log.println(
                TASK_HELPER.getMessage(
                    "main.usage." + options.task.toString().toLowerCase(Locale.ROOT)));
      ...
      if (options.version || options.fullVersion) {
        if (options.task == null && !unhandled.isEmpty()) {
          throw TASK_HELPER.newBadArgs(
              "err.not.a.task", Stream.of(args).collect(Collectors.joining(" ")));
        }
  ...
  private interface ModuleAction {
    public void apply(BasicImageReader reader, String oldModule, String newModule)
        throws IOException, BadArgs;
  }

  private interface ResourceAction {
    public void apply(BasicImageReader reader, String name, ImageLocation location)
        throws IOException, BadArgs;
  }

  private void extract(BasicImageReader reader, String name, ImageLocation location)
      throws IOException, BadArgs {
    ...
    if (parent.exists()) {
      if (!parent.isDirectory()) {
        throw TASK_HELPER.newBadArgs("err.cannot.create.dir", parent.getAbsolutePath());
      }
  ...
  private void iterate(
      JImageAction jimageAction, ModuleAction moduleAction, ResourceAction resourceAction)
      throws IOException, BadArgs {
          ...
          for (String name : entryNames) {
            ...
            if (!ImageResourcesTree.isTreeInfoResource(name)) {
              if (moduleAction != null) {
                ...
                String newModule = offset != -1 ? name.substring(1, offset) : "<unknown>";
  ...
  private boolean run() throws Exception, BadArgs {
    switch (options.task) {
      ...
      case VERIFY:
        ...
        break;
      default:
        throw TASK_HELPER.newBadArgs("err.not.a.task", options.task.name()).showUsage(true);
    }
    return true;
  }
}
