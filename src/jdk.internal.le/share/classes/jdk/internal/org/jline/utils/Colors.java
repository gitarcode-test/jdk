...
public class Colors {
  ...
  public static final double[] sRGB_typical_environment =
      vc(D50, 200.0, 200.0 / 5, averageSurrounding);
  ...
  private static double cie00(double[] lab1, double[] lab2) {
    ...
    double c_star_average_ab_pot_7 =
        c_star_average_ab_pot_3 * c_star_average_ab_pot_3 * c_star_average_ab;
    double G =
        0.5
            * (1.0
                - Math.sqrt(
                    c_star_average_ab_pot_7 / (c_star_average_ab_pot_7 + 6103515625.0))); // 25^7
    ...
    double delta_H_prime =
        2.0 * Math.sqrt(C_prime_1 * C_prime_2) * Math.sin(Math.toRadians(delta_h_prime / 2.0));
    ...
    double S_L =
        1.0
            + ((0.015 * L_prime_average_minus_50_square)
                / Math.sqrt(20.0 + L_prime_average_minus_50_square));
    ...
    double R_C =
        2.0 * Math.sqrt(C_prime_average_pot_7 / (C_prime_average_pot_7 + 6103515625.0)); // 25^7
  ...
  static double[] lch2lab(double[] lch) {
    ...
    return new double[] {
      lch[0], lch[1] * Math.cos(lch[2] * toRad), lch[1] * Math.sin(lch[2] * toRad)
    };
  }
  ...
  private static double[] forwardTransform(double[] XYZ, double[] vc) {
    ...
    // calculate corresponding (sharpened) cone response considering various luminance level and
    // surround conditions
    // in D
    ...
    double e =
        ((12500.0 / 13.0) * vc[VC_N_C] * vc[VC_N_CB]) * (Math.cos(Math.toRadians(h) + 2.0) + 3.8);
    // get t
    double t =
        e
    ...
    double Q =
        (4.0 / vc[VC_C]) * Math.sqrt(J / 100.0) * (vc[VC_A_W] + 4.0) * Math.pow(vc[VC_F_L], 0.25);
  ...
  static double[] vc(double[] xyz_w, double L_A, double Y_b, double[] surrounding) {
    ...
    double D =
        Math.max(
            0.0,
            Math.min(1.0, vc[VC_F] * (1.0 - (1.0 / 3.6) * Math.pow(Math.E, (-L_A - 42.0) / 92.0))));
    double Yw = xyz_w[1];
    double[] RGB_c =
        new double[] {
          (D * Yw / RGB_w[0]) + (1.0 - D),
    ...
    vc[VC_F_L] =
        0.2 * kpow4 * (L_Ax5) + 0.1 * Math.pow(1.0 - kpow4, 2.0) * Math.pow(L_Ax5, 1.0 / 3.0);
    ...
    vc[VC_N_CB] =
        vc[
            VC_N_BB]; // chromatic contrast factors (calculate increase in J, Q, and C caused by
                      // dark backgrounds)
    ...
    vc[VC_A_W] =
        (2.0 * RGBPrime_aw[0] + RGBPrime_aw[1] + RGBPrime_aw[2] / 20.0 - 0.305) * vc[VC_N_BB];
