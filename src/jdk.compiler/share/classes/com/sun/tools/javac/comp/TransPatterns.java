...
public class TransPatterns extends TreeTranslator {
  ...
  public static TransPatterns instance(Context context) {
    ...
    if (instance == null) instance = new TransPatterns(context);
    return instance;
  }
  ...
  public void visitTypeTest(JCInstanceOf tree) {
    if (tree.pattern instanceof JCPattern pattern) {
      ...
      //                              $extraConditions)
      Type tempType = tree.expr.type.hasTag(BOT) ? syms.objectType : tree.expr.type;
      ...
      try {
        ...
        if (currentValue != exprSym) {
          resultExpression =
              make.at(tree.pos)
                  .LetExpr(make.VarDef(currentValue, translatedExpr), resultExpression)
  ...
  public void visitBindingPattern(JCBindingPattern tree) {
    ...
    if (bindingVar != null && !bindingVar.isUnnamedVariable()) {
      JCAssign fakeInit =
          (JCAssign)
              make.at(TreeInfo.getStartPos(tree))
                  .Assign(
                      make.Ident(bindingVar),
                      convert(
                          make.Ident(currentValue).setType(currentValue.erasure(types)),
                          castTargetType))
                  .setType(bindingVar.erasure(types));
      LetExpr nestedLE = make.LetExpr(List.of(make.Exec(fakeInit)), make.Literal(true));
  ...
  private UnrolledRecordPattern unrollRecordPattern(JCRecordPattern recordPattern) {
    ...
    BindingSymbol tempBind =
        new BindingSymbol(
            Flags.SYNTHETIC,
            names.fromString(
                target.syntheticNameChar() + "b" + target.syntheticNameChar() + variableIndex++),
            recordType,
            currentMethodSym);
    ...
    while (components.nonEmpty()) {
      ...
      if (nestedPattern instanceof JCRecordPattern nestedRecordPattern) {
        ...
        allowNull =
            types.isSubtype(
                componentType, types.boxedTypeOrType(types.erasure(nestedBinding.type)));
      }
      JCMethodInvocation componentAccessor =
          make.at(recordPattern.pos())
              .App(
                  make.Select(
                      convert(make.Ident(recordBinding), recordBinding.type), component.accessor))
      ...
      JCExpression accessedComponentValue = convert(componentAccessor, componentType);
      JCInstanceOf firstLevelCheck =
          (JCInstanceOf)
              make.TypeTest(accessedComponentValue, nestedBinding).setType(syms.booleanType);
    ...
    return new UnrolledRecordPattern(
        (JCBindingPattern) make.BindingPattern(recordBindingVar).setType(recordBinding.type),
        guard);
  }
  ...
  public void visitSwitch(JCSwitch tree) {
    handleSwitch(tree, tree.selector, tree.cases, tree.hasUnconditionalPattern, tree.patternSwitch);
  }
  ...
  private void handleSwitch(
    ...
    if (patternSwitch) {
      Type seltype =
          selector.type.hasTag(BOT) || target.usesReferenceOnlySelectorTypes()
              ? syms.objectType
              : selector.type;
      ...
      // =>
      // int $idx = 0;
      // $RESTART: switch (invokeDynamic typeSwitch($constant, typeof($pattern1), typeof($pattern2),
      // typeof($pattern3))($obj, $idx)) {
      //     case 0:
      //         $stats1$
      //     case 1:
                ...
                l -> {
                  if (l instanceof JCPatternCaseLabel patternLabel) {
                    ...
                    if (pattern instanceof JCRecordPattern recordPattern) {
                      ...
                      JCPatternCaseLabel newPatternCaseLabel =
                          make.PatternCaseLabel(deconstructed.primaryPattern());
      ...
      VarSymbol temp =
          new VarSymbol(
              ...
              currentMethodSym);
      boolean hasNullCase =
          cases.stream().flatMap(c -> c.labels.stream()).anyMatch(p -> TreeInfo.isNullCaseLabel(p));
      ...
      statements.append(
          make.at(tree.pos).VarDef(temp, needsNullCheck ? attr.makeNullCheck(selector) : selector));
      VarSymbol index =
          new VarSymbol(
              Flags.SYNTHETIC,
      ...
      List<Type> staticArgTypes =
          List.of(
              ...
              types.makeArrayType(
                  new ClassType(
                      syms.classType.getEnclosingType(),
                      List.of(
      ...
      MethodType indyType =
          new MethodType(
              List.of(seltype, syms.intType), syms.intType, List.nil(), syms.methodClass);
      DynamicMethodSymbol dynSym =
          new DynamicMethodSymbol(
              bootstrapName, syms.noSymbol, bsm.asHandle(), indyType, staticArgValues);

      JCFieldAccess qualifier = make.Select(make.QualIdent(bsm.owner), dynSym.name);
      qualifier.sym = dynSym;
      qualifier.type = syms.intType;
      selector =
          make.Apply(List.nil(), qualifier, List.of(make.Ident(temp), make.Ident(index)))
      ...
      for (var c : cases) {
        ...
        if (hasJoinedNull) {
          clearedPatterns =
              c.labels.stream().filter(l -> !TreeInfo.isNullCaseLabel(l)).collect(List.collector());
        }

        boolean validCaseLabelList;
        if (clearedPatterns.size() > 1) {
          validCaseLabelList =
              clearedPatterns.stream().allMatch(cP -> cP.hasTag(Tag.PATTERNCASELABEL));
        } else {
        ...
        if ((validCaseLabelList && !previousCompletesNormally) || c.guard != null) {
          List<JCPatternCaseLabel> labels =
              clearedPatterns.stream().map(cp -> (JCPatternCaseLabel) cp).collect(List.collector());
          bindingContext = new BasicBindingContext();
          VarSymbol prevCurrentValue = currentValue;
          try {
            currentValue = temp;
            ...
            // if all patterns are the same we do not need an extra test for the first (it is
            // covered by the BSM)
            boolean multiplePatternsAndDifferent =
                labels.size() > 1
                    && !labels.stream()
                        .map(l -> l.pat.type.tsym)
                        .allMatch(labels.get(0).pat.type.tsym::equals);
            ...
            for (JCPatternCaseLabel label : labels) {
              ...
              if (multiplePatternsAndDifferent) {
                test =
                    makeBinary(
                        Tag.AND, makeTypeTest(make.Ident(temp), make.Type(label.pat.type)), test);
              }
            ...
            c.stats =
                c.stats.prepend(
                    make.If(
                        makeUnary(Tag.NOT, test).setType(syms.booleanType),
                        make.Block(
                            0,
                            List.of(
                                make.Exec(
        ...
        previousCompletesNormally =
            c.caseKind == CaseTree.CaseKind.STATEMENT && c.completesNormally;
      }
      ...
      if (tree.hasTag(Tag.SWITCH)) {
        ...
        LetExpr r =
            (LetExpr)
                make.LetExpr(statements.toList(), (JCSwitchExpression) tree).setType(tree.type);
    ...
    while (cases.nonEmpty()) {
      ...
      if (currentCase.caseKind == CaseKind.STATEMENT
          ...
          && (cases.tail.head.guard != null
              || cases.tail.head.labels.stream()
                  .anyMatch(
                      cl -> cl instanceof JCPatternCaseLabel p && p.syntheticGuard != null))) {
  ...
  // where:
  private void fixupContinue(
      JCTree switchTree, JCCase c, VarSymbol indexVariable, int currentCaseIndex) {
    // inject 'index = currentCaseIndex + 1;` before continue which has the current switch as the
    // target
    new TreeScanner() {
      @Override
      public void visitCase(JCCase c) {
        if (c.stats.size() == 1
            && c.stats.head instanceof JCContinue cont
            && cont.target == switchTree) {
          JCExpressionStatement setIndex =
              make.Exec(
                  make.Assign(
                          make.Ident(indexVariable), makeLit(syms.intType, currentCaseIndex + 1))
                      .setType(syms.intType));
  ...
  void appendBreakIfNeeded(JCTree switchTree, List<JCCase> cases, JCCase c) {
    if (c.caseKind == CaseTree.CaseKind.RULE || (cases.last() == c && c.completesNormally)) {
      JCTree pos = c.stats.nonEmpty() ? c.stats.last() : c;
  ...
  private Symbol.DynamicVarSymbol makePrimitive(DiagnosticPosition pos, Type primitiveType) {
    ...
    MethodSymbol bsm =
        rs.resolveInternalMethod(
            pos, env, syms.constantBootstrapsType, bootstrapName, bsm_staticArgs, List.nil());
    ...
    return new Symbol.DynamicVarSymbol(
        names.fromString(primitiveGenerator.sb.toString()),
        syms.noSymbol,
  ...
  private Symbol.DynamicVarSymbol makeBooleanConstant(DiagnosticPosition pos, int constant) {
    ...
    return new Symbol.DynamicVarSymbol(
        constant == 0 ? names.fromString("FALSE") : names.fromString("TRUE"),
        syms.noSymbol,
        new Symbol.MethodHandleSymbol(bsm),
  ...
  JCMethodInvocation makeApply(JCExpression selector, Name name, List<JCExpression> args) {
    MethodSymbol method =
        rs.resolveInternalMethod(
            currentClassTree.pos(), env, selector.type, name, TreeInfo.types(args), List.nil());
    JCMethodInvocation tree =
        make.App(make.Select(selector, method), args)
  ...
  /**
   * Considering a list of cases, find consecutive cases with the same binding pattern as their
   * label, and type tests with binding patterns as the first element in the guard. These cases are
   * then merged into a single case, and a nested switch is generated from the first element of the
   * guard.
   *
   * <p>For example:
   *
   * <p>OUTER: switch (selector) { case Box b when b.o() instanceof String s -> {} case Box b when
   * b.o() instanceof Integer i-> {} case Box b when b.o() instanceof Number n -> {} ... } => OUTER:
   * switch (selector) { case Box b -> switch (b.o()) { case String s -> {} case Integer i -> {}
   * case Number n -> {} default -> continue OUTER; //continue matching on next case of the outer
   * switch } ... }
   */
  private List<JCCase> processCases(JCTree currentSwitch, List<JCCase> inputCases) {
    interface AccummulatorResolver {
      public void resolve(
          VarSymbol commonBinding,
    ...
    AccummulatorResolver resolveAccummulator =
        (commonBinding, commonNestedExpression, commonNestedBinding) -> {
          ...
          if (accummulator.size() > 1) {
            ...
            for (List<JCCase> accList = accummulator.toList();
                accList.nonEmpty();
              ...
              hasUnconditional =
                  (!types.erasure(binding.type).isPrimitive()
                          ? instanceofCheck.allowNull
                          : types.isUnconditionallyExact(
                              commonNestedExpression.type, types.erasure(binding.type)))
              ...
              if (hasUnconditional) {
                newLabel = List.of(make.ConstantCaseLabel(makeNull()), jcPatternCaseLabelWithGuard);
              } else {
                newLabel = List.of(jcPatternCaseLabelWithGuard);
              }
              nestedCases.add(
                  make.Case(
                      CaseKind.STATEMENT, newLabel, accummulated.guard, accummulated.stats, null));
              ...
              nestedCases.add(
                  make.Case(
                      ...
                      hasUnconditional
                          ...
                          : List.of(make.ConstantCaseLabel(makeNull()), make.DefaultCaseLabel()),
                      null,
            ...
            result.add(
                make.Case(
                    CaseKind.STATEMENT, List.of(leadingTest), null, List.of(newSwitch), null));
          } else {
  ...
  private LoadableConstant createEnumDesc(
      DiagnosticPosition pos, ClassSymbol enumClass, Name constant) {
    MethodSymbol classDesc =
        rs.resolveInternalMethod(
            pos, env, syms.classDescType, names.of, List.of(syms.stringType), List.nil());
    MethodSymbol enumDesc =
        rs.resolveInternalMethod(
            pos,
            env,
            syms.enumDescType,
            names.of,
            List.of(syms.classDescType, syms.stringType),
            List.nil());
    return invokeMethodWrapper(
        pos,
        enumDesc.asHandle(),
        invokeMethodWrapper(
            pos, classDesc.asHandle(), LoadableConstant.String(enumClass.flatname.toString())),
        LoadableConstant.String(constant.toString()));
  }

  private LoadableConstant invokeMethodWrapper(
      DiagnosticPosition pos, MethodHandleSymbol toCall, LoadableConstant... params) {
    List<Type> bsm_staticArgs =
        List.of(
            syms.methodHandleLookupType,
            syms.stringType,
    ...
    MethodSymbol bsm =
        rs.resolveInternalMethod(
            pos, env, syms.constantBootstrapsType, names.invoke, bsm_staticArgs, List.nil());
    ...
    return new DynamicVarSymbol(
        bsm.name, bsm.owner, bsm.asHandle(), toCall.getReturnType(), actualParams);
  }
  ...
  public void visitBlock(JCBlock tree) {
    ListBuffer<JCStatement> statements = new ListBuffer<>();
    bindingContext =
        new BindingDeclarationFenceBindingContext() {
          boolean tryPrepend(BindingSymbol binding, JCVariableDecl var) {
            // {
            //    if (E instanceof T N) {
            //        return ;
    ...
    try {
      ...
      try {
        if (isInit) {
          ...
          currentMethodSym =
              new MethodSymbol(tree.flags | Flags.BLOCK, names.empty, null, currentClass);
          variableIndex = 0;
          deconstructorCalls = null;
        }
  ...
  public void visitVarDef(JCVariableDecl tree) {
    ...
    try {
      ...
      if (currentMethodSym == null) {
        ...
        currentMethodSym =
            new MethodSymbol(
                (tree.mods.flags & Flags.STATIC) | Flags.BLOCK, names.empty, null, currentClass);
        variableIndex = 0;
      }
  ...
  private void preparePatternMatchingCatchIfNeeded(JCBlock tree) {
    if (deconstructorCalls != null) {
      ...
      JCCatch patternMatchingCatch =
          make.Catch(
              make.VarDef(ctch, null),
              make.Block(
                  0,
                  List.of(
                      make.Throw(
                          makeNewClass(
                              syms.matchExceptionType,
                              List.of(
                                  makeApply(make.Ident(ctch), names.toString, List.nil()),
                                  make.Ident(ctch)))))));
  ...
  class BasicBindingContext extends BindingContext {
    ...
    VarSymbol bindingDeclared(BindingSymbol varSymbol) {
      ...
      if (res == null) {
        res =
            new VarSymbol(
                varSymbol.flags() & ~Flags.MATCH_BINDING,
                varSymbol.name,
                varSymbol.type,
                currentMethodSym);
    ...
    JCExpression decorateExpression(JCExpression expr) {
      ...
      // =>
      // (let T N; (let T' N$temp = E; N$temp instanceof T && (N = (T) N$temp == (T) N$temp)) &&
      // /*use of N*/)
      for (VarSymbol vsym : hoistedVarMap.values()) {
        expr =
            make.at(expr.pos).LetExpr(makeHoistedVarDecl(expr.pos, vsym), expr).setType(expr.type);
      }
      return expr;
    }
