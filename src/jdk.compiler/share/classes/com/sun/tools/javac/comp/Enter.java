...
/**
 * This class enters symbols for all encountered definitions into the symbol table. The pass
 * consists of high-level two phases, organized as follows:
 *
 * <p>In the first phase, all class symbols are entered into their enclosing scope, descending
 * recursively down the tree for classes which are members of other classes. The class symbols are
 * given a TypeEnter object as completer.
 *
 * <p>In the second phase classes are completed using TypeEnter.complete(). Completion might occur
 * on demand, but any classes that are not completed that way will be eventually completed by
 * processing the `uncompleted' queue. Completion entails determination of a class's parameters,
 * supertype and interfaces, as well as entering all symbols defined in the class into its scope,
 * with the exception of class symbols which have been entered in phase 1.
 *
 * <p>Whereas the first phase is organized as a sweep through all compiled syntax trees, the second
 * phase is on-demand. Members of a class are entered when the contents of a class are first
 * accessed. This is accomplished by installing completer objects in class symbols for compiled
 * classes which invoke the type-enter phase for the corresponding class tree.
 *
...
public class Enter extends JCTree.Visitor {
  ...
  /**
   * The queue of all classes that might still need to be completed; saved and initialized by
   * main().
   */
  ListBuffer<ClassSymbol> uncompleted;

  /**
   * The queue of classes that should have typeEnter completer installed after all the classes are
   * discovered.
   */
  ...
  public Env<AttrContext> classEnv(JCClassDecl tree, Env<AttrContext> env) {
    Env<AttrContext> localEnv = env.dup(tree, env.info.dup(WriteableScope.create(tree.sym)));
  ...
  /**
   * The scope in which a member definition in environment env is to be entered This is usually the
   * environment's scope, except for class environments, where the local scope is for type
   * variables, and the this and super symbol only, and members go into the class member scope.
   */
  ...
  public void visitTopLevel(JCCompilationUnit tree) {
    ...
    if (TreeInfo.isModuleInfo(tree)) {
      ...
      if (pd != null) {
        ...
        if (pd.annotations.nonEmpty() || pkginfoOpt == PkgInfo.ALWAYS || tree.docComments != null) {
          if (isPkgInfo) {
            addEnv = true;
          } else if (pd.annotations.nonEmpty()) {
            log.error(pd.annotations.head.pos(), Errors.PkgAnnotationsSbInPackageInfoJava);
          }
        }
      } else {
      ...
      if (isPkgInfo) {
        ...
        if (env0 != null) {
          ...
          if (!fileManager.isSameFile(tree.sourcefile, tree0.sourcefile)) {
            log.warning(pd != null ? pd.pid.pos() : null, Warnings.PkgInfoAlreadySeen(tree.packge));
          }
        }
  ...
  private final TreeScanner setPackageSymbols =
      new TreeScanner() {
  ...
  public void visitClassDef(JCClassDecl tree) {
    ...
    if (owner.kind == PCK) {
      ...
      if ((tree.mods.flags & PUBLIC) != 0 && !classNameMatchesFileName(c, env)) {
        ...
        log.error(tree.pos(), Errors.ClassPublicShouldBeInFile(topElement, tree.name));
      }
    ...
    if (owner.kind != PCK && (c.flags_field & STATIC) == 0) {
      ...
      Symbol owner1 = owner;
      while (owner1.kind.matches(KindSelector.VAL_MTH) && (owner1.flags_field & STATIC) == 0) {
        owner1 = owner1.owner;
      }
      if (owner1.kind == TYP) {
  ...
  /**
   * Main method: enter classes from the list of toplevel trees, possibly skipping TypeEnter for all
   * but 'c' by placing them on the uncompleted list.
   *
   * @param trees The list of trees to be processed.
