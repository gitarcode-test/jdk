...
public class Resolve {
  ...
  protected Resolve(Context context) {
    ...
    compactMethodDiags =
        options.isSet(Option.XDIAGS, "compact")
  ...
  void reportVerboseResolutionDiagnostic(
      DiagnosticPosition dpos,
      Name name,
      Type site,
      List<Type> argtypes,
    ...
    for (Candidate c : currentResolutionContext.candidates) {
      if (currentResolutionContext.step != c.step
          ...
          || (!c.isApplicable()
              && !verboseResolutionMode.contains(VerboseResolutionMode.INAPPLICABLE))) {
        continue;
      } else {
        subDiags.append(
            c.isApplicable()
        ...
        if (c.sym == bestSoFar) mostSpecificPos = pos;
        pos++;
      }
    }
    ...
    List<Type> argtypes2 =
        argtypes.map(
            deferredAttr
            .new RecoveryDeferredTypeMap(
                AttrMode.SPECULATIVE, bestSoFar, currentResolutionContext.step));
    JCDiagnostic main =
        diags.note(
            log.currentSource(),
            dpos,
  ...
  public boolean isAccessible(Env<AttrContext> env, TypeSymbol c, boolean checkInner) {
    ...
    */
    if (env.enclMethod != null && (env.enclMethod.mods.flags & ANONCONSTR) != 0) return true;

    if (env.info.visitingServiceImplementation && env.toplevel.modle == c.packge().modle) {
      return true;
    }
  ...
  // where
  /**
   * Is given class a subclass of given base class, or an inner class of a subclass? Return null if
   ...
   *
   * @param env The current environment.
   * @param site The type of which the tested symbol is regarded as a member.
  ...
  public boolean isAccessible(Env<AttrContext> env, Type site, Symbol sym, boolean checkInner) {
    ...
    */
    ...
    if (env.info.visitingServiceImplementation && env.toplevel.modle == sym.packge().modle) {
      return true;
    }
    ...
    switch ((short) (sym.flags() & AccessFlags)) {
      case PRIVATE:
        return (env.enclClass.sym == sym.owner // fast special case
                || env.enclClass.sym.outermostClass() == sym.owner.outermostClass())
            && sym.isInheritedIn(site.tsym, types);
      case 0:
        return (env.toplevel.packge == sym.owner.owner // fast special case
  ...
  private boolean notOverriddenIn(Type site, Symbol sym) {
    if (sym.kind != MTH || sym.isConstructor() || sym.isStatic()) return true;
    else {
      Symbol s2 = ((MethodSymbol) sym).implementation(site.tsym, types, true);
      return (s2 == null
          || s2 == sym
          || sym.owner == s2.owner
          || (sym.owner.isInterface() && s2.owner == syms.objectType.tsym)
  ...
  // where
  /**
   * Is given protected symbol accessible if it is selected from given site and the selection takes
   * place in given class?
   *
  ...
  /**
   * Performs a recursive scan of a type looking for accessibility problems from current attribution
   * environment
   */
      ...
      new Types.SimpleVisitor<Void, Env<AttrContext>>() {
        ...
        public Void visitClassType(ClassType t, Env<AttrContext> env) {
          ...
          if (!isAccessible(env, t, true)) {
            accessBase(
                new AccessError(env, null, t.tsym),
                env.tree.pos(),
                env.enclClass.sym,
                t,
                t.tsym.name,
  ...
  /**
   * Try to instantiate the type of a method so that it fits given type arguments and argument
   * types. If successful, return the method's instantiated type, else return null. The
   * instantiation will take into account an additional leading formal parameter if the method is an
   * instance method seen as a member of an under determined site. In this case, we treat site as an
   * additional parameter and the parameters of the class containing the method as additional type
   * variables that get instantiated.
   *
   * @param env The current environment
  ...
  Type rawInstantiate(
    ...
    if (!mt.hasTag(FORALL) && typeargtypes.nonEmpty()) {
      ...
      if (typeargtypes.length() != pmt.tvars.length())
        // not enough args
        throw new InapplicableMethodException(
            diags.fragment(Fragments.WrongNumberTypeArgs(Integer.toString(pmt.tvars.length()))));
      // Check type arguments are within bounds
      List<Type> formals = pmt.tvars;
      List<Type> actuals = typeargtypes;
      while (formals.nonEmpty() && actuals.nonEmpty()) {
        List<Type> bounds =
            types.subst(types.getBounds((TypeVar) formals.head), pmt.tvars, typeargtypes);
        for (; bounds.nonEmpty(); bounds = bounds.tail) {
          if (!types.isSubtypeUnchecked(actuals.head, bounds.head, warn)) {
            throw new InapplicableMethodException(
                diags.fragment(Fragments.ExplicitParamDoNotConformToBounds(actuals.head, bounds)));
          }
        }
    ...
    if (instNeeded) {
      return infer.instantiateMethod(
          env,
          tvars,
          (MethodType) mt,
          resultInfo,
    ...
    DeferredAttr.DeferredAttrContext dc =
        currentResolutionContext.deferredAttrContext(m, infer.emptyContext, resultInfo, warn);
    currentResolutionContext.methodCheck.argumentsAcceptable(
        env, dc, argtypes, mt.getParameterTypes(), warn);
    dc.complete();
    return mt;
  }
  ...
  Type checkMethod(
    ...
    try {
      ...
      currentResolutionContext.attrMode =
          (resultInfo.pt == Infer.anyPoly) ? AttrMode.SPECULATIVE : DeferredAttr.AttrMode.CHECK;
      ...
      return rawInstantiate(
          ...
          argtypes,
          typeargtypes,
          step.isBoxingRequired(),
          step.isVarargsRequired(),
          warn);
    } finally {
  ...
  Type instantiate(
    ...
    try {
      return rawInstantiate(
          env, site, m, resultInfo, argtypes, typeargtypes, allowBoxing, useVarargs, warn);
  ...
  /**
   * This interface defines an entry point that should be used to perform a method check. A method
   * check usually consist in determining as to whether a set of types (actuals) is compatible with
   * another set of types (formals). Since the notion of compatibility can vary depending on the
   * circumstances, this interfaces allows to easily add new pluggable method check routines.
   */
  interface MethodCheck {
    /**
     * Main method check routine. A method check usually consist in determining as to whether a set
     * of types (actuals) is compatible with another set of types (formals). If an incompatibility
     * is found, an unchecked exception is assumed to be thrown.
     */
    void argumentsAcceptable(
        Env<AttrContext> env,
  ...
  /**
   * Dummy method check object. All methods are deemed applicable, regardless of their formal
   * parameter types.
   */
  MethodCheck nilMethodCheck =
      new MethodCheck() {
        public void argumentsAcceptable(
            Env<AttrContext> env,
            DeferredAttrContext deferredAttrContext,
            List<Type> argtypes,
            List<Type> formals,
  ...
  /**
   * Base class for 'real' method checks. The class defines the logic for iterating through formals
   * and actuals and provides and entry point that can be used by subclasses in order to define the
   * actual check logic.
   */
  abstract class AbstractMethodCheck implements MethodCheck {
    ...
    protected void reportMC(
        DiagnosticPosition pos,
        MethodCheckDiag diag,
      ...
      throw inferDiag
          ...
          : methodCheckFailure.setMessage(
              diags.create(DiagnosticType.FRAGMENT, log.currentSource(), pos, key, args));
    }

    /**
     * To eliminate the overhead associated with allocating an exception object in such an hot
     * execution path, we use flyweight pattern - and share the same exception instance across
     * multiple method check failures.
     */
  ...
  /**
   * Arity-based method check. A method is applicable if the number of actuals supplied conforms to
   * the method signature.
   */
  MethodCheck arityMethodCheck =
      new AbstractMethodCheck() {
        @Override
        void checkArg(
            DiagnosticPosition pos,
            boolean varargs,
  ...
  /**
   * Main method applicability routine. Given a list of actual types A, a list of formal types F,
   * determines whether the types in A are compatible (by method invocation conversion) with the
   * types in F.
   *
   * <p>Since this routine is shared between overload resolution and method type-inference, a
   * (possibly empty) inference context is used to convert formal types to the corresponding 'undet'
   * form ahead of a compatibility check so that constraints can be propagated and collected.
   *
   * <p>Moreover, if one or more types in A is a deferred type, this routine uses DeferredAttr in
   * order to perform deferred attribution. If one or more actual deferred types are stuck, they are
   * placed in a queue and revisited later after the remainder of the arguments have been seen. If
   * this is not sufficient to 'unstuck' the argument, a cyclic inference error is called out.
   *
   * <p>A method check handler (see above) is used in order to report errors.
   */
  MethodCheck resolveMethodCheck =
      new AbstractMethodCheck() {
        ...
        public void argumentsAcceptable(
          ...
          if (deferredAttrContext.phase.isVarargsRequired()) {
            if (deferredAttrContext.mode == AttrMode.CHECK) {
              varargsAccessible(
                  env, types.elemtype(formals.last()), deferredAttrContext.inferenceContext);
            }
          }
        }

        /**
         * Test that the runtime array element type corresponding to 't' is accessible. 't' should
         * be the varargs element type of either the method invocation type signature (after
         * inference completes) or the method declaration signature (before inference completes).
         */
        private void varargsAccessible(
            final Env<AttrContext> env, final Type t, final InferenceContext inferenceContext) {
          if (inferenceContext.free(t)) {
            inferenceContext.addFreeTypeListener(
                List.of(t),
                solvedContext ->
                    varargsAccessible(env, solvedContext.asInstType(t), solvedContext));
          } else {
            if (!isAccessible(env, types.erasure(t))) {
              Symbol location = env.enclClass.sym;
              reportMC(
                  env.tree,
                  MethodCheckDiag.INACCESSIBLE_VARARGS,
                  inferenceContext,
                  t,
        ...
        private ResultInfo methodCheckResult(
            final boolean varargsCheck,
            Type to,
            final DeferredAttr.DeferredAttrContext deferredAttrContext,
            Warner rsWarner) {
          CheckContext checkContext =
              new MethodCheckContext(
                  !deferredAttrContext.phase.isBoxingRequired(), deferredAttrContext, rsWarner) {
                MethodCheckDiag methodDiag =
                    varargsCheck ? MethodCheckDiag.VARARG_MISMATCH : MethodCheckDiag.ARG_MISMATCH;
  ...
  /**
   * This class handles method reference applicability checks; since during these checks it's
   * sometime possible to have inference variables on the actual argument types list, the method
   * applicability check must be extended so that inference variables are 'opened' as needed.
   */
  ...
  /**
   * Check context to be used during method applicability checks. A method check context might
  ...
  abstract class MethodCheckContext implements CheckContext {
    ...
    public MethodCheckContext(
        boolean strict, DeferredAttrContext deferredAttrContext, Warner rsWarner) {
      this.strict = strict;
    ...
    public boolean compatible(Type found, Type req, Warner warn) {
      ...
      return strict
          ? types.isSubtypeUnchecked(
              inferenceContext.asUndetVar(found), inferenceContext.asUndetVar(req), warn)
          : types.isConvertible(
              inferenceContext.asUndetVar(found), inferenceContext.asUndetVar(req), warn);
    }
  ...
  /**
   * ResultInfo class to be used during method applicability checks. Check for deferred types goes
   * through special path.
   */
  class MethodResultInfo extends ResultInfo {
    ...
    protected Type check(DiagnosticPosition pos, Type found) {
      if (found.hasTag(DEFERRED)) {
        ...
        Type capturedType =
            pos == null || pos.getTree() == null
                ? types.capture(uResult)
                : checkContext.inferenceContext().cachedCapture(pos.getTree(), uResult, true);
        return super.check(pos, chk.checkNonVoid(pos, capturedType));
      }
    }

    /**
     * javac has a long-standing 'simplification' (see 6391995): given an actual argument type, the
     * method check is performed on its upper bound. This leads to inconsistencies when an argument
     * type is checked against itself. For example, given a type-variable T, it is not true that
     * {@code U(T) <: T}, so we need to guard against that.
     */
    private Type U(Type found) {
      return found == pt ? found : types.cvarUpperBound(found);
    }
  ...
  /**
   * Most specific method applicability routine. Given a list of actual types A, a list of formal
   * types F1, and a list of formal types F2, the routine determines as to whether the types in F1
   * can be considered more specific than those in F2 w.r.t. argument types A.
   */
  class MostSpecificCheck implements MethodCheck {
    ...
    public void argumentsAcceptable(
      ...
      formals2 =
          adjustArgs(
              formals2,
              deferredAttrContext.msym,
              formals1.length(),
              deferredAttrContext.phase.isVarargsRequired());
      while (formals2.nonEmpty()) {
        ResultInfo mresult =
            methodCheckResult(formals2.head, deferredAttrContext, warn, actuals.head);
    ...
    /**
     * Subclass of method check context class that implements most specific method conversion. If
     * the actual type under analysis is a deferred type a full blown structural analysis is carried
     * out.
     */
    class MostSpecificCheckContext extends MethodCheckContext {
      ...
      public MostSpecificCheckContext(
          DeferredAttrContext deferredAttrContext, Warner rsWarner, Type actual) {
      ...
      private boolean compatibleBySubtyping(Type found, Type req) {
        ...
        return types.isSubtypeNoCapture(
            found, deferredAttrContext.inferenceContext.asUndetVar(req));
      }
      ...
      private boolean functionalInterfaceMostSpecific(Type t, Type s, JCTree tree) {
        ...
        while (tParams.nonEmpty() && tParamsNoCapture.nonEmpty() && sParams.nonEmpty()) {
          ...
          Type tParamNoCapture =
              types.subst(tParamsNoCapture.head, tTypeParamsNoCapture, tTypeParams);
      ...
      /**
       * Tests whether one functional interface type can be considered more specific than another
       * unrelated functional interface type for the scanned expression.
       */
      class MostSpecificFunctionReturnChecker extends DeferredAttr.PolyScanner {
        ...
        public void visitReference(JCMemberReference tree) {
          if (sRet.hasTag(VOID)) {
            ...
            result &=
                (retValIsPrimitive == tRet.isPrimitive())
        ...
        private JCExpression asExpr(JCExpression expr) {
          if (expr.type.hasTag(DEFERRED)) {
            JCTree speculativeTree =
                ((DeferredType) expr.type).speculativeTree(deferredAttrContext);
            if (speculativeTree != deferredAttr.stuckTree) {
   ...
   * @param c The class to search for the field. This is always a superclass or implemented
   *     interface of site's class.
   */
  Symbol findField(Env<AttrContext> env, Type site, Name name, TypeSymbol c) {
    ...
    for (Symbol s : c.members().getSymbolsByName(name)) {
      if (s.kind == VAR && (s.flags_field & SYNTHETIC) == 0) {
        return isAccessible(env, site, s) ? s : new AccessError(env, site, s);
      }
    }
   ...
   * @param site The type of the qualifying expression, in which identifier is searched.
   * @param name The identifier's name.
   */
  public VarSymbol resolveInternalField(
      DiagnosticPosition pos, Env<AttrContext> env, Type site, Name name) {
    Symbol sym = findField(env, site, name, site.tsym);
    if (sym.kind == VAR) return (VarSymbol) sym;
    else throw new FatalError(diags.fragment(Fragments.FatalErrCantLocateField(name)));
  }
  ...
  Symbol findVar(DiagnosticPosition pos, Env<AttrContext> env, Name name) {
    ...
    while (env1.outer != null) {
      ...
      if (sym.exists()) {
        if (sym.kind == VAR && sym.owner.kind == TYP && (sym.flags() & STATIC) == 0) {
          if (staticOnly) return new StaticError(sym);
    ...
    if (bestSoFar.exists()) return bestSoFar;
    ...
    for (Scope sc : new Scope[] {env.toplevel.namedImportScope, env.toplevel.starImportScope}) {
      for (Symbol currentSymbol : sc.getSymbolsByName(name)) {
        if (currentSymbol.kind != VAR) continue;
  ...
  Symbol selectBest(
    ...
    try {
      ...
      // If bestSoFar.kind was HIDDEN(AccessError)/WRONG_MTH/WRONG_MTHS, return an
      // InapplicableSymbolsError(kind is WRONG_MTHS).
      ...
      switch (bestSoFar.kind) {
        ...
        case HIDDEN:
          if (bestSoFar instanceof AccessError accessError) {
            ...
            currentResolutionContext.addInapplicableCandidate(
                accessError.sym,
                accessError.getDiagnostic(
    ...
    if (!isAccessible(env, site, sym)) {
      ...
      JCDiagnostic curDiagnostic =
          curAccessError.getDiagnostic(
              JCDiagnostic.DiagnosticType.FRAGMENT, null, null, site, null, argtypes, typeargtypes);
  ...
  Symbol mostSpecific(
    ...
    switch (m2.kind) {
        ...
        if (m1SignatureMoreSpecific && m2SignatureMoreSpecific) {
          ...
          if (m1Owner != m2Owner) {
            if (types.asSuper(m1Owner.type, m2Owner) != null
                ...
                && m1.overrides(m2, m1Owner, types, false)) return m1;
            if (types.asSuper(m2Owner.type, m1Owner) != null
                && ((m2.owner.flags_field & INTERFACE) == 0
                    || (m1.owner.flags_field & INTERFACE) != 0)
                && m2.overrides(m1, m2Owner, types, false)) return m2;
          }
      ...
      case AMBIGUOUS:
        ...
        if (m1MoreSpecificThanAnyAmbiguous) return m1;
        ...
        if (!allAmbiguousMoreSpecificThanM1) e.addAmbiguousSymbol(m1);
        return e;
      default:
  ...
  // where
  private boolean signatureMoreSpecific(
      List<Type> actuals,
      Env<AttrContext> env,
      Type site,
      Symbol m1,
    ...
    try {
      ...
      Type mst =
          instantiate(
              ...
              null,
              adjustArgs(
                  types.cvarLowerBounds(types.memberType(site, m1).getParameterTypes()),
                  m1,
                  maxLength,
                  useVarargs),
              null,
              ...
              useVarargs,
              noteWarner);
      return mst != null && !noteWarner.hasLint(Lint.LintCategory.UNCHECKED);
    } finally {
  ...
  Symbol findMethodInScope(
    ...
    for (Symbol s : sc.getSymbolsByName(name, new LookupFilter(abstractok))) {
      bestSoFar =
          selectBest(env, site, argtypes, typeargtypes, s, bestSoFar, allowBoxing, useVarargs);
    }
    return bestSoFar;
  }
  ...
  class LookupFilter implements Predicate<Symbol> {
    ...
    public boolean test(Symbol s) {
      ...
      return s.kind == MTH
          && (flags & SYNTHETIC) == 0
          && (abstractOk || (flags & DEFAULT) != 0 || (flags & ABSTRACT) == 0);
    }
  }
  ...
  private Symbol findMethod(
    ...
    Symbol concrete =
        bestSoFar.kind.isValid() && (bestSoFar.flags() & ABSTRACT) == 0
            ? bestSoFar
            : methodNotFound;
    ...
    if (isInterface && bestSoFar.kind.isResolutionError()) {
      ...
      if (bestSoFar.kind.isValid()) {
        ...
        bestSoFar =
            new MethodSymbol(bestSoFar.flags_field, bestSoFar.name, bestSoFar.type, intype.tsym) {
              @Override
  ...
  /**
   * Return an Iterable object to scan the superclasses of a given type. It's crucial that the scan
   * is done lazily, as we don't want to accidentally access more supertypes than strictly needed
   * (as this could trigger completion errors if some of the not-needed supertypes are
   * missing/ill-formed).
   */
  Iterable<TypeSymbol> superclasses(final Type intype) {
    return () ->
        new Iterator<TypeSymbol>() {
  ...
  Symbol findFun(
      ...
      try {
        Symbol sym =
            findMethod(
                ...
                useVarargs);
        if (sym.exists()) {
          if (sym.kind == MTH && sym.owner.kind == TYP && (sym.flags() & STATIC) == 0) {
            if (staticOnly) return new StaticError(sym);
            if (env1.info.ctorPrologue && env1 == env) return new RefBeforeCtorCalledError(sym);
          }
          return sym;
        } else {
          bestSoFar = bestOf(bestSoFar, sym);
    ...
    Symbol sym =
        findMethod(
            env, syms.predefClass.type, name, argtypes, typeargtypes, allowBoxing, useVarargs);
    if (sym.exists()) return sym;
    ...
    for (Symbol currentSym : env.toplevel.namedImportScope.getSymbolsByName(name)) {
      ...
      if (currentSym.kind == MTH) {
        if (currentSym.owner.type != origin.type) currentSym = currentSym.clone(origin);
  ...
  private final RecoveryLoadClass doRecoveryLoadClass =
      new RecoveryLoadClass() {
        @Override
        public Symbol loadClass(Env<AttrContext> env, Name name) {
          List<Name> candidates = Convert.classCandidates(name);
          return lookupInvisibleSymbol(
              env,
              name,
              n ->
                  () ->
                      createCompoundIterator(candidates, c -> syms.getClassesForName(c).iterator()),
              (ms, n) -> {
              ...
              sym -> sym.kind == Kind.TYP,
              typeNotFound);
        }
      };

  private final RecoveryLoadClass namedImportScopeRecovery =
      (env, name) -> {
        Scope importScope = env.toplevel.namedImportScope;
        Symbol existing =
            importScope.findFirst(
                Convert.shortName(name), sym -> sym.kind == TYP && sym.flatName() == name);
  ...
  private boolean isImportOnDemand(Env<AttrContext> env, Name name) {
    if (!env.tree.hasTag(IMPORT)) return false;

    JCTree qualid = ((JCImport) env.tree).qualid;

    if (!qualid.hasTag(SELECT)) return false;

    if (TreeInfo.name(qualid) != names.asterisk) return false;

    return TreeInfo.fullName(((JCFieldAccess) qualid).selected) == name;
  }

  private <S extends Symbol> Symbol lookupInvisibleSymbol(
      Env<AttrContext> env,
      Name name,
    ...
    // it depends on that are exported for any or this module, the class/package may exist
    // internally
    ...
    for (S sym : candidates) {
      if (validate.test(sym)) return createInvisibleSymbolError(env, sym);
    }
   ...
   * @param c The class to search for the member type. This is always a superclass or implemented
   *     interface of site's class.
   */
  Symbol findImmediateMemberType(Env<AttrContext> env, Type site, Name name, TypeSymbol c) {
    for (Symbol sym : c.members().getSymbolsByName(name)) {
      if (sym.kind == TYP) {
        return isAccessible(env, site, sym) ? sym : new AccessError(env, site, sym);
      }
    }
    return typeNotFound;
  }
  ...
  Symbol findInheritedMemberType(Env<AttrContext> env, Type site, Name name, TypeSymbol c) {
    ...
    for (List<Type> l = types.interfaces(c.type);
      ...
      if (!bestSoFar.kind.isResolutionError()
          ...
          && sym.owner != bestSoFar.owner) bestSoFar = new AmbiguityError(bestSoFar, sym);
      else bestSoFar = bestOf(bestSoFar, sym);
    }
    return bestSoFar;
  }
  ...
  Symbol findMemberType(Env<AttrContext> env, Type site, Name name, TypeSymbol c) {
    ...
    if (sym != typeNotFound) return sym;
  ...
  Symbol findGlobalType(
      Env<AttrContext> env, Scope scope, Name name, RecoveryLoadClass recoveryLoadClass) {
    Symbol bestSoFar = typeNotFound;
  ...
  Symbol findTypeVar(Env<AttrContext> env, Name name, boolean staticOnly) {
    for (Symbol sym : env.info.scope.getSymbolsByName(name)) {
      if (sym.kind == TYP) {
        if (sym.type.hasTag(TYPEVAR) && (staticOnly || (isStatic(env) && sym.owner.kind == TYP)))
  ...
  Symbol findType(Env<AttrContext> env, Name name) {
    ...
    for (Env<AttrContext> env1 = env; env1.outer != null; env1 = env1.outer) {
      ...
      if (tyvar != typeNotFound) {
        if (env.baseClause
            || sym == typeNotFound
            || (tyvar.kind == TYP && tyvar.exists() && tyvar.owner.kind == MTH)) {
          return tyvar;
        }
      }
      ...
      if (staticOnly
          ...
          && sym.type.getEnclosingType().isParameterized()) return new StaticError(sym);
      else if (sym.exists()) return sym;
      else bestSoFar = bestOf(bestSoFar, sym);

      JCClassDecl encl = env1.baseClause ? (JCClassDecl) env1.tree : env1.enclClass;
      if ((encl.sym.flags() & STATIC) != 0) staticOnly = true;
    }
   ...
   *
   * @param pos position on which report warnings, if any; null warnings should not be reported
   * @param env The current environment.
   * @param name The identifier's name.
   * @param kind Indicates the possible symbol kinds (a subset of VAL, TYP, PCK).
   */
  Symbol findIdent(DiagnosticPosition pos, Env<AttrContext> env, Name name, KindSelector kind) {
    try {
      return checkNonExistentType(
          checkRestrictedType(pos, findIdentInternal(pos, env, name, kind), name));
    } catch (ClassFinder.BadClassFile err) {
      return new BadClassFileError(err);
    } catch (CompletionFailure cf) {
  ...
  Symbol findIdentInternal(
      DiagnosticPosition pos, Env<AttrContext> env, Name name, KindSelector kind) {
    Symbol bestSoFar = typeNotFound;
    Symbol sym;
    ...
    if (kind.contains(KindSelector.PCK)) return lookupPackage(env, name);
    else return bestSoFar;
  }
   ...
   * @param env The current environment.
   * @param name The identifier's name.
   * @param kind Indicates the possible symbol kinds (a nonempty subset of TYP, PCK).
   */
  Symbol findIdentInPackage(
      DiagnosticPosition pos, Env<AttrContext> env, TypeSymbol pck, Name name, KindSelector kind) {
    return checkNonExistentType(
        checkRestrictedType(pos, findIdentInPackageInternal(env, pck, name, kind), name));
  }

  Symbol findIdentInPackageInternal(
      Env<AttrContext> env, TypeSymbol pck, Name name, KindSelector kind) {
    Name fullname = TypeSymbol.formFullName(name, pck);
    Symbol bestSoFar = typeNotFound;
    if (kind.contains(KindSelector.TYP)) {
      RecoveryLoadClass recoveryLoadClass =
          allowModules
                  && !kind.contains(KindSelector.PCK)
                  && !pck.exists()
   ...
   * @param name The identifier's name.
   * @param kind Indicates the possible symbol kinds (a subset of VAL, TYP).
   */
  Symbol findIdentInType(
      DiagnosticPosition pos, Env<AttrContext> env, Type site, Name name, KindSelector kind) {
    try {
      return checkNonExistentType(
          checkRestrictedType(pos, findIdentInTypeInternal(env, site, name, kind), name));
    } catch (ClassFinder.BadClassFile err) {
      return new BadClassFileError(err);
    } catch (CompletionFailure cf) {
      chk.completionError(pos, cf);
  ...
  private Symbol checkNonExistentType(Symbol symbol) {
     ...
     */
    return symbol instanceof ClassSymbol c && c.type.isErroneous() && c.classfile == null
        ? typeNotFound
        : symbol;
  }
  ...
  /**
   * If `sym' is a bad symbol: report error and return errSymbol else pass through unchanged,
   * additional arguments duplicate what has been used in trying to find the symbol {@literal (-->
   * flyweight pattern)}. This improves performance since we expect misses to happen frequently.
   *
   ...
   * @param argtypes The invocation's value arguments, if we looked for a method.
   * @param typeargtypes The invocation's type arguments, if we looked for a method.
  ...
  /**
   * Variant of the generalized access routine, to be used for generating method resolution
   * diagnostics
   */
  Symbol accessMethod(
      Symbol sym,
    ...
    return accessInternal(
        sym, pos, location, site, name, qualified, argtypes, typeargtypes, methodLogResolveHelper);
  }
  ...
  /**
   * Variant of the generalized access routine, to be used for generating variable, type resolution
   * diagnostics
   */
  Symbol accessBase(
      Symbol sym,
      ...
      boolean qualified) {
    return accessInternal(
        sym, pos, location, site, name, qualified, List.nil(), null, basicLogResolveHelper);
  }
  ...
  interface LogResolveHelper {
    ...
    List<Type> getArgumentTypes(
        ResolveError errSym, Symbol accessedSym, Name name, List<Type> argtypes);
  }

  LogResolveHelper basicLogResolveHelper =
      new LogResolveHelper() {
        public boolean resolveDiagnosticNeeded(
  ...
  LogResolveHelper silentLogResolveHelper =
      new LogResolveHelper() {
        public boolean resolveDiagnosticNeeded(
            Type site, List<Type> argtypes, List<Type> typeargtypes) {
          return false;
  ...
  LogResolveHelper methodLogResolveHelper =
      new LogResolveHelper() {
        ...
        public List<Type> getArgumentTypes(
            ResolveError errSym, Symbol accessedSym, Name name, List<Type> argtypes) {
          return argtypes.map(
              new ResolveDeferredRecoveryMap(
                  AttrMode.SPECULATIVE, accessedSym, currentResolutionContext.step));
        }
      };
  ...
  void checkNonAbstract(DiagnosticPosition pos, Symbol sym) {
    if ((sym.flags() & ABSTRACT) != 0 && (sym.flags() & DEFAULT) == 0)
      log.error(pos, Errors.AbstractCantBeAccessedDirectly(kindName(sym), sym, sym.location()));
  }
  ...
  Symbol resolveIdent(DiagnosticPosition pos, Env<AttrContext> env, Name name, KindSelector kind) {
    return accessBase(findIdent(pos, env, name, kind), pos, env.enclClass.sym.type, name, false);
  }
  ...
  Symbol resolveQualifiedMethod(
    ...
    return resolveQualifiedMethod(
        new MethodResolutionContext(), pos, env, location, site, name, argtypes, typeargtypes);
  }

  private Symbol resolveQualifiedMethod(
      MethodResolutionContext resolveContext,
      DiagnosticPosition pos,
      Env<AttrContext> env,
      Symbol location,
      Type site,
    ...
    return lookupMethod(
        ...
        location,
        resolveContext,
        new BasicLookupHelper(name, site, argtypes, typeargtypes) {
          @Override
          Symbol doLookup(Env<AttrContext> env, MethodResolutionPhase phase) {
            return findMethod(
                env,
  ...
  /**
   * Find or create an implicit method of exactly the given type (after erasure). Searches in a side
   * table, not the main scope of the site. This emulates the lookup process required by JSR 292 in
   * JVM.
   *
   ...
   */
  Symbol findPolymorphicSignatureInstance(
      Env<AttrContext> env, final Symbol spMethod, List<Type> argtypes) {
    Type mtype =
        infer.instantiatePolymorphicSignatureInstance(
            env, (MethodSymbol) spMethod, currentResolutionContext, argtypes);
  ...
  Symbol findPolymorphicSignatureInstance(final Symbol spMethod, Type mtype) {
    for (Symbol sym : polymorphicSignatureScope.getSymbolsByName(spMethod.name)) {
      ...
      // type and owner
      if (types.isSameType(mtype, sym.type) && spMethod.owner == sym.owner) {
        return sym;
      }
    }
    ...
    if (types.isSameType(spReturnType, syms.objectType)) {
      ...
      mtype =
          new MethodType(
              mtype.getParameterTypes(), spReturnType, mtype.getThrownTypes(), syms.methodClass);
    }

    // Create the desired method
    // Retain static modifier is to support invocations to
    // MethodHandle.linkTo* methods
    long flags = ABSTRACT | HYPOTHETICAL | spMethod.flags() & (Flags.AccessFlags | Flags.STATIC);
    Symbol msym =
        new MethodSymbol(flags, spMethod.name, mtype, spMethod.owner) {
          @Override
          public Symbol baseSymbol() {
            return spMethod;
          }
  ...
  public MethodSymbol resolveInternalMethod(
    ...
    Symbol sym =
        resolveQualifiedMethod(
            resolveContext, pos, env, site.tsym, site, name, argtypes, typeargtypes);
    if (sym.kind == MTH) return (MethodSymbol) sym;
    else throw new FatalError(diags.fragment(Fragments.FatalErrCantLocateMeth(name)));
  }
  ...
  Symbol resolveConstructor(
    ...
    return resolveConstructor(
        new MethodResolutionContext(), pos, env, site, argtypes, typeargtypes);
  }
   ...
   * @param typeargtypes The types of the invocation's type arguments.
   */
  public MethodSymbol resolveInternalConstructor(
      DiagnosticPosition pos,
      Env<AttrContext> env,
      Type site,
      List<Type> argtypes,
    ...
    if (sym.kind == MTH) return (MethodSymbol) sym;
    else throw new FatalError(diags.fragment(Fragments.FatalErrCantLocateCtor(site)));
  }

  Symbol findConstructor(
      DiagnosticPosition pos,
      Env<AttrContext> env,
      Type site,
      List<Type> argtypes,
      ...
      boolean allowBoxing,
      boolean useVarargs) {
    Symbol sym = findMethod(env, site, names.init, argtypes, typeargtypes, allowBoxing, useVarargs);
   ...
   * @param pos The position to use for error reporting.
   * @param env The environment current at the constructor invocation.
   * @param site The type of class for which a constructor is searched. The scope of this class has
   *     been touched in attribution.
   * @param argtypes The types of the constructor invocation's value arguments.
   * @param typeargtypes The types of the constructor invocation's type arguments.
   */
  Symbol resolveDiamond(
      DiagnosticPosition pos,
      Env<AttrContext> env,
      Type site,
      List<Type> argtypes,
  ...
  /**
   * This method scans all the constructor symbol in a given class scope - assuming that the
   * original scope contains a constructor of the kind: {@code Foo(X x, Y y)}, where X,Y are class
   * type-variables declared in Foo, a method check is executed against the modified constructor
   * type: {@code <X,Y>Foo<X,Y>(X x, Y y)}. This is crucial in order to enable diamond inference.
   * The inferred return type of the synthetic constructor IS the inferred type for the diamond
   * operator.
   */
  private Symbol findDiamond(
      Env<AttrContext> env,
    ...
    for (final Symbol sym : tsym.members().getSymbolsByName(names.init)) {
      ...
      if (sym.kind == MTH && (sym.flags_field & SYNTHETIC) == 0) {
        List<Type> oldParams = sym.type.hasTag(FORALL) ? ((ForAll) sym.type).tvars : List.nil();
        Type constrType =
            new ForAll(
                site.tsym.type.getTypeArguments().appendList(oldParams),
                types.createMethodTypeWithReturn(sym.type.asMethodType(), site));
        MethodSymbol newConstr =
            new MethodSymbol(sym.flags(), names.init, constrType, site.tsym) {
              @Override
              public Symbol baseSymbol() {
                return sym;
              }
        ...
        bestSoFar =
            selectBest(
                env, site, argtypes, typeargtypes, newConstr, bestSoFar, allowBoxing, useVarargs);
      }
    }
    return bestSoFar;
  }

  Symbol getMemberReference(
      DiagnosticPosition pos,
      Env<AttrContext> env,
    ...
    ReferenceLookupHelper lookupHelper =
        makeReferenceLookupHelper(referenceTree, site, name, List.nil(), null, VARARITY);
  ...
  ReferenceLookupHelper makeReferenceLookupHelper(
      JCMemberReference referenceTree,
      Type site,
      Name name,
      List<Type> argtypes,
    ...
    if (!name.equals(names.init)) {
      ...
      return new MethodReferenceLookupHelper(
          referenceTree, name, site, argtypes, typeargtypes, maxPhase);
    } else if (site.hasTag(ARRAY)) {
      // array constructor reference
      return new ArrayConstructorReferenceLookupHelper(
          referenceTree, site, argtypes, typeargtypes, maxPhase);
    } else {
      // class constructor reference
      return new ConstructorReferenceLookupHelper(
          referenceTree, site, argtypes, typeargtypes, maxPhase);
    }
  }

  /**
   * Resolution of member references is typically done as a single overload resolution step, where
   * the argument types A are inferred from the target functional descriptor.
   *
   * <p>If the member reference is a method reference with a type qualifier, a two-step lookup
   * process is performed. The first step uses the expected argument list A, while the second step
   * discards the first type from A (which is treated as a receiver type).
   *
   * <p>There are two cases in which inference is performed: (i) if the member reference is a
   * constructor reference and the qualifier type is raw - in which case diamond inference is used
   * to infer a parameterization for the type qualifier; (ii) if the member reference is an unbound
   * reference where the type qualifier is raw - in that case, during the unbound lookup the
   * receiver argument type is used to infer an instantiation for the raw qualifier type.
   *
   * <p>When a multi-step resolution process is exploited, the process of picking the resulting
   * symbol is delegated to an helper class {@link
   * com.sun.tools.javac.comp.Resolve.ReferenceChooser}.
   *
   * <p>This routine returns a pair (T,S), where S is the member reference symbol, and T is the type
   * of the class in which S is defined. This is necessary as the type T might be dynamically
   * inferred (i.e. if constructor reference has a raw qualifier).
   */
  Pair<Symbol, ReferenceLookupHelper> resolveMemberReference(
      Env<AttrContext> env,
      JCMemberReference referenceTree,
      Type site,
      Name name,
    ...
    ReferenceLookupHelper boundLookupHelper =
        makeReferenceLookupHelper(referenceTree, site, name, argtypes, typeargtypes, VARARITY);
    ...
    Symbol boundSym =
        lookupMethod(
            boundEnv, env.tree.pos(), site.tsym, boundSearchResolveContext, boundLookupHelper);
    boolean isStaticSelector = TreeInfo.isStaticSelector(referenceTree.expr, names);
    ReferenceLookupResult boundRes =
        new ReferenceLookupResult(boundSym, boundSearchResolveContext, isStaticSelector);
    ...
    if (unboundLookupHelper != null) {
      ...
      unboundSym =
          lookupMethod(
              ...
              unboundLookupHelper);
      unboundRes =
          new ReferenceLookupResult(unboundSym, unboundSearchResolveContext, isStaticSelector);
      if (dumpMethodReferenceSearchResults) {
        dumpMethodReferenceSearchResults(
            referenceTree, unboundSearchResolveContext, unboundSym, false);
      }
    }
    ...
    res = new Pair<>(bestRes.sym, bestRes == unboundRes ? unboundLookupHelper : boundLookupHelper);
    env.info.pendingResolutionPhase =
        bestRes == unboundRes
  ...
  /**
   * This class is used to represent a method reference lookup result. It keeps track of two things:
   * (i) the symbol found during a method reference lookup and (ii) the static kind of the lookup
   * (see {@link com.sun.tools.javac.comp.Resolve.ReferenceLookupResult.StaticKind}).
   */
  static class ReferenceLookupResult {

    /**
     * Static kind associated with a method reference lookup. Erroneous lookups end up with the
     * UNDEFINED kind; successful lookups will end up with either STATIC, NON_STATIC, depending on
     * whether all applicable candidates are static or non-static methods, respectively. If a
     * successful lookup has both static and non-static applicable methods, its kind is set to BOTH.
     */
    enum StaticKind {
      STATIC,
      NON_STATIC,
    ...
    ReferenceLookupResult(
        Symbol sym, MethodResolutionContext resolutionContext, boolean isStaticSelector) {
    ...
    private static StaticKind staticKind(
        Symbol sym, MethodResolutionContext resolutionContext, boolean isStaticSelector) {
    ...
    /**
     * Error recovery helper: can this lookup result be ignored (for the purpose of returning some
     * 'better' result) ?
     */
    boolean canIgnore() {
      switch (sym.kind) {
        case ABSENT_MTH:
          return true;
        case WRONG_MTH:
          InapplicableSymbolError errSym = (InapplicableSymbolError) sym.baseSymbol();
          return new Template(MethodCheckDiag.ARITY_MISMATCH.regex())
              .matches(errSym.errCandidate().snd);
        case WRONG_MTHS:
          InapplicableSymbolsError errSyms = (InapplicableSymbolsError) sym.baseSymbol();
  ...
  abstract class ReferenceChooser {
    ...
    abstract ReferenceLookupResult unboundResult(
        ReferenceLookupResult boundRes, ReferenceLookupResult unboundRes);
  }

  /**
   * This chooser implements the selection strategy used during a full lookup; this logic is
   * described in JLS SE 8 (15.3.2).
   */
  ReferenceChooser basicReferenceChooser =
      new ReferenceChooser() {
        ...
        ReferenceLookupResult boundResult(ReferenceLookupResult boundRes) {
          return !boundRes.isSuccess() || boundRes.hasKind(StaticKind.NON_STATIC)
              ? boundRes
              : // the search produces a non-static method
              ReferenceLookupResult.error(new BadMethodReferenceError(boundRes.sym, false));
        }

        @Override
        ReferenceLookupResult unboundResult(
            ReferenceLookupResult boundRes, ReferenceLookupResult unboundRes) {
          if (boundRes.isSuccess()
              && boundRes.sym.isStatic()
            ...
            return boundRes;
          } else if (unboundRes.isSuccess()
              && !unboundRes.sym.isStatic()
            ...
            return ReferenceLookupResult.error(
                new BadMethodReferenceError(
                    boundRes.isSuccess() ? boundRes.sym : unboundRes.sym, true));
          } else {
            // both searches fail to produce a result - pick 'better' error using heuristics (error
            // recovery)
  ...
  /**
   * This chooser implements the selection strategy used during an arity-based lookup; this logic is
   * described in JLS SE 8 (15.12.2.1).
   */
      ...
      new ReferenceChooser() {
        ...
        ReferenceLookupResult boundResult(ReferenceLookupResult boundRes) {
          return (!boundRes.isSuccess() || !boundRes.hasKind(StaticKind.STATIC))
              ? boundRes
              : // the search has at least one applicable non-static method
              ReferenceLookupResult.error(new BadMethodReferenceError(boundRes.sym, false));
        }
  ...
  /**
   * Helper for defining custom method-like lookup logic; a lookup helper provides hooks for (i) the
   * actual lookup logic and (ii) accessing the lookup result (this step might result in compiler
  ...
  abstract class LookupHelper {
    ...
    /**
     * Search for a symbol under a given overload resolution phase - this method is usually called
     * several times, once per each overload resolution phase
     */
    ...
    abstract Symbol access(
        Env<AttrContext> env, DiagnosticPosition pos, Symbol location, Symbol sym);
  }
  ...
  /**
   * Helper class for member reference lookup. A reference lookup helper defines the basic logic for
   * member reference lookup; a method gives access to an 'unbound' helper used to perform an
   * unbound member reference lookup.
   */
  abstract class ReferenceLookupHelper extends LookupHelper {
    ...
    /**
     * Returns an unbound version of this lookup helper. By default, this method returns an dummy
     * lookup helper.
     */
  ...
  /**
   * Helper class for method reference lookup. The lookup logic is based upon Resolve.findMethod; in
   * certain cases, this helper class has a corresponding unbound helper class (see
   * UnboundMethodReferenceLookupHelper). In such cases, non-static lookup results are thrown away.
   */
  class MethodReferenceLookupHelper extends ReferenceLookupHelper {
      ...
      if (TreeInfo.isStaticSelector(referenceTree.expr, names)) {
        if (argtypes.nonEmpty()
            && (argtypes.head.hasTag(NONE)
                || types.isSubtypeUnchecked(
                    inferenceContext.asUndetVar(argtypes.head), originalSite))) {
          return new UnboundMethodReferenceLookupHelper(
              referenceTree, name, originalSite, argtypes, typeargtypes, maxPhase);
        } else {
          return new ReferenceLookupHelper(
              referenceTree, name, site, argtypes, typeargtypes, maxPhase) {
            @Override
            ReferenceLookupHelper unboundLookup(InferenceContext inferenceContext) {
              return this;
    ...
    Symbol access(Env<AttrContext> env, DiagnosticPosition pos, Symbol location, Symbol sym) {
      if (originalSite.hasTag(TYPEVAR) && sym.kind == MTH) {
        sym = (sym.flags() & Flags.PRIVATE) != 0 ? new AccessError(env, site, sym) : sym;
  ...
  /**
   * Helper class for unbound method reference lookup. Essentially the same as the basic method
   * reference lookup helper; main difference is that static lookup results are thrown away. If
   * qualifier type is raw, an attempt to infer a parameterized type is made using the first actual
   * argument (that would otherwise be ignored during the lookup).
   */
  ...
  /**
   * Helper class for array constructor lookup; an array constructor lookup is simulated by looking
   * up a method that returns the array type specified as qualifier, and that accepts a single int
  ...
  /**
   * Helper class for constructor reference lookup. The lookup logic is based upon either
   * Resolve.findMethod or Resolve.findDiamond - depending on whether the constructor reference
   * needs diamond inference (this is the case if the qualifier type is raw). A special erroneous
   * symbol is returned if the lookup returns the constructor of an inner class and there's no
        ...
        this.site =
            new ClassType(
                ...
                !(site.tsym.isInner() && site.getEnclosingType().isRaw())
                    ? site.tsym.type.getTypeArguments()
                    : List.nil(),
                site.tsym,
                site.getMetadata());
        needsInference = true;
  ...
  /**
   * Main overload resolution routine. On each overload resolution step, a lookup helper class is
   * used to perform the method/constructor lookup; at the end of the lookup, the helper is used to
   * validate the results (this last step might trigger overload resolution diagnostics).
   */
  Symbol lookupMethod(
      Env<AttrContext> env,
      DiagnosticPosition pos,
  ...
  Symbol resolveSelf(DiagnosticPosition pos, Env<AttrContext> env, TypeSymbol c, Name name) {
    ...
    if (c.isInterface()
      ...
      for (Type t : pruneInterfaces(env.enclClass.type)) {
        if (t.tsym == c) {
          if (env.info.ctorPrologue) log.error(pos, Errors.CantRefBeforeCtorCalled(name));
          env.info.defaultSuperCallSite = t;
          return new VarSymbol(
              0, names._super, types.asSuper(env.enclClass.type, c), env.enclClass.sym);
        }
      }
      ...
      for (Type i : types.directSupertypes(env.enclClass.type)) {
        if (i.tsym.isSubClass(c, types) && i.tsym != c) {
          log.error(pos, Errors.IllegalDefaultSuperCall(c, Fragments.RedundantSupertype(c, i)));
   ...
   *
   * <p>This is only allowed when: - The field is being assigned a value (i.e., written but not
   * read) - The field is not inherited from a superclass - The assignment is not within a lambda,
   * because that would require capturing 'this' which is not allowed prior to super().
   *
   * <p>Note, this method doesn't catch all such scenarios, because this method is invoked for
   * symbol "x" only for "x = 42" but not for "this.x = 42". We also don't verify that the field has
   * no initializer, which is required. To catch those cases, we rely on similar logic in
   * Attr.checkAssignable().
   */
  private boolean isAllowedEarlyReference(
      DiagnosticPosition pos, Env<AttrContext> env, VarSymbol v) {
    ...
    if (!(env.tree instanceof JCAssign assign)) return false;

    // The assignment statement must not be within a lambda
    if (env.info.isLambda) return false;
    ...
    if (isEarlyReference(env, base, v) && v.owner != env.enclClass.sym) return false;
   ...
   *
   * <p>This means the variable is an instance field of the current class and it appears in an early
   * initialization context of it (i.e., one of its constructor prologues).
   *
   * <p>Such a reference is only allowed for assignments to non-initialized fields that are not
   * inherited from a superclass, though that is not enforced by this method.
   *
   * @param env The current environment
  ...
  Symbol resolveSelfContaining(
      DiagnosticPosition pos, Env<AttrContext> env, Symbol member, boolean isSuperCall) {
  ...
  private Symbol resolveSelfContainingInternal(
      Env<AttrContext> env, Symbol member, boolean isSuperCall) {
  ...
  Type resolveImplicitThis(
      DiagnosticPosition pos, Env<AttrContext> env, Type t, boolean isSuperCall) {
    Type thisType =
        (t.tsym.owner.kind.matches(KindSelector.VAL_MTH)
                ? resolveSelf(pos, env, t.getEnclosingType().tsym, names._this)
                : resolveSelfContaining(pos, env, t.tsym, isSuperCall))
            .type;
  ...
  private void logResolveError(
    ...
    JCDiagnostic d =
        error.getDiagnostic(
            JCDiagnostic.DiagnosticType.ERROR, pos, location, site, name, argtypes, typeargtypes);
    if (d != null) {
  ...
  /**
   * Root class for resolution errors. Subclass of ResolveError represent a different kinds of
   * resolution error - as such they must specify how they map into concrete compiler diagnostics.
   */
  abstract class ResolveError extends Symbol {
    ...
    /**
     * Create an external representation for this erroneous symbol to be used during attribution -
     * by default this returns the symbol of a brand new error type which stores the original type
     ...
     * @param typeargtypes The invocation's type arguments, if we looked for a method.
     */
    abstract JCDiagnostic getDiagnostic(
  ...
  /**
   * This class is the root class of all resolution errors caused by an invalid symbol being found
   * during resolution.
   */
  ...
  class BadRestrictedTypeError extends ResolveError {
    ...
    JCDiagnostic getDiagnostic(
        ...
        List<Type> typeargtypes) {
      return diags.create(
          dkind, log.currentSource(), pos, "illegal.ref.to.restricted.type", typeName);
    }
  }

  /**
   * InvalidSymbolError error class indicating that a symbol matching a given name does not exists
   * in a given site.
   */
  class SymbolNotFoundError extends ResolveError {
    ...
    JCDiagnostic getDiagnostic(
      ...
      if (name == names.error) return null;
      ...
      if (!location.name.isEmpty()) {
        if (location.kind == PCK && !site.tsym.exists() && location.name != names.java) {
          return diags.create(dkind, log.currentSource(), pos, "doesnt.exist", location);
        }
        hasLocation = !location.name.equals(names._this) && !location.name.equals(names._super);
      }
    ...
    private JCDiagnostic getLocationDiag(Symbol location, Type site) {
      if (location.kind == VAR) {
        return diags.fragment(Fragments.Location1(kindName(location), location, location.type));
      } else {
        return diags.fragment(Fragments.Location(typeKindName(site), site, null));
      }
    }
  }

  /**
   * InvalidSymbolError error class indicating that a given symbol (either a method, a constructor
   * or an operand) is not applicable given an actual arguments/type argument list.
   */
  class InapplicableSymbolError extends ResolveError {
    ...
    protected InapplicableSymbolError(
        Kind kind, String debugName, MethodResolutionContext context) {
      super(kind, debugName);
    ...
    public Symbol access(Name name, TypeSymbol location) {
      ...
      TypeSymbol errSymbol =
          types.createErrorType(name, location, cand != null ? cand.fst.type : syms.errSymbol.type)
              .tsym;
      if (cand != null) {
  ...
  /**
   * ResolveError error class indicating that a symbol (either methods, constructors or operand) is
   * not applicable given an actual arguments/type argument list.
   */
  class InapplicableSymbolsError extends InapplicableSymbolError {
    ...
    JCDiagnostic getDiagnostic(
      ...
      Map<Symbol, JCDiagnostic> filteredCandidates =
          compactMethodDiags ? filterCandidates(candidatesMap) : mapCandidates();
      ...
      if (filteredCandidates.size() > 1) {
        ...
        return new JCDiagnostic.MultilineDiagnostic(
            err, candidateDetails(filteredCandidates, site));
        ...
        JCDiagnostic d =
            new InapplicableSymbolError(resolveContext) {
              @Override
              protected Pair<Symbol, JCDiagnostic> errCandidate() {
                return p;
              }
        ...
        return new SymbolNotFoundError(ABSENT_MTH)
            .getDiagnostic(dkind, pos, location, site, name, argtypes, typeargtypes);
      }
    }
    ...
    private List<JCDiagnostic> candidateDetails(
        Map<Symbol, JCDiagnostic> candidatesMap, Type site) {
      List<JCDiagnostic> details = List.nil();
      for (Map.Entry<Symbol, JCDiagnostic> _entry : candidatesMap.entrySet()) {
        Symbol sym = _entry.getKey();
        JCDiagnostic detailDiag =
            diags.fragment(
                Fragments.InapplicableMethod(
                    Kinds.kindName(sym),
    ...
    protected Pair<Symbol, JCDiagnostic> errCandidate() {
      ...
      if (filteredCandidates.size() == 1) {
        return Pair.of(
            filteredCandidates.keySet().iterator().next(),
  ...
  /**
   * DiamondError error class indicating that a constructor symbol is not applicable given an actual
   * arguments/type argument list using diamond inference.
   */
  class DiamondError extends InapplicableSymbolError {
    ...
    JCDiagnostic getDiagnostic(
      ...
      return diags.create(
          dkind, log.currentSource(), pos, key, Fragments.Diamond(site.tsym), details);
    }
  }
  ...
  class AccessError extends InvalidSymbolError {
    ...
    JCDiagnostic getDiagnostic(
      ...
      if (sym.name == names.init && sym.owner != site.tsym) {
        return new SymbolNotFoundError(ABSENT_MTH)
          ...
          || (env != null && this.site != null && !isAccessible(env, this.site))) {
        if (sym.owner.kind == PCK) {
          return diags.create(
              dkind,
              log.currentSource(),
              pos,
              "not.def.access.package.cant.access",
              sym,
              sym.location(),
              inaccessiblePackageReason(env, sym.packge()));
        } else if (sym.packge() != syms.rootPackage && !symbolPackageVisible(env, sym)) {
          return diags.create(
              dkind,
              log.currentSource(),
              pos,
              "not.def.access.class.intf.cant.access.reason",
              sym,
              sym.location(),
              sym.location().packge(),
              inaccessiblePackageReason(env, sym.packge()));
        ...
        return diags.create(
            dkind, log.currentSource(), pos, "not.def.public.cant.access", sym, sym.location());
      }
    }
    ...
    private String toString(Type type) {
      ...
      if (type != null) {
        ...
        if (type.tsym != null) sb.append("packge:").append(type.tsym.packge());
        sb.append("]");
      }
      return sb.toString();
    }
  ...
  class InvisibleSymbolError extends InvalidSymbolError {
    ...
    JCDiagnostic getDiagnostic(
      ...
      if (sym.kind == PCK) {
        ...
        return diags.create(dkind, log.currentSource(), pos, "package.not.visible", sym, details);
      }
      ...
      if (pos.getTree() != null) {
        ...
        if (o.kind == PCK) {
          ...
          return diags.create(dkind, log.currentSource(), pos, "package.not.visible", o, details);
        }
      }
    ...
    if (!env.toplevel.modle.readModules.contains(sym.modle)) {
      ...
      if (sym.modle != syms.unnamedModule) {
        if (env.toplevel.modle != syms.unnamedModule) {
          return diags.fragment(
              Fragments.NotDefAccessDoesNotRead(env.toplevel.modle, sym, sym.modle));
        } else {
          return diags.fragment(Fragments.NotDefAccessDoesNotReadFromUnnamed(sym, sym.modle));
        }
      } else {
        return diags.fragment(Fragments.NotDefAccessDoesNotReadUnnamed(sym, env.toplevel.modle));
      }
    } else {
      if (sym.packge().modle.exports.stream().anyMatch(e -> e.packge == sym)) {
        ...
        if (env.toplevel.modle != syms.unnamedModule) {
          return diags.fragment(
              Fragments.NotDefAccessNotExportedToModule(sym, sym.modle, env.toplevel.modle));
        } else {
          return diags.fragment(
              Fragments.NotDefAccessNotExportedToModuleFromUnnamed(sym, sym.modle));
        }
      } else {
        // not exported:
        if (env.toplevel.modle != syms.unnamedModule) {
          return diags.fragment(Fragments.NotDefAccessNotExported(sym, sym.modle));
        } else {
          return diags.fragment(Fragments.NotDefAccessNotExportedFromUnnamed(sym, sym.modle));
        }
      }
    }
  }
  ...
  /**
   * InvalidSymbolError error class indicating that an instance member has erroneously been accessed
  ...
  class StaticError extends InvalidSymbolError {
    ...
    JCDiagnostic getDiagnostic(
        ...
        List<Type> typeargtypes) {
      Symbol errSym =
          ((sym.kind == TYP && sym.type.hasTag(CLASS)) ? types.erasure(sym.type).tsym : sym);
      return diags.create(
          dkind, log.currentSource(), pos, "non-static.cant.be.ref", kindName(sym), errSym);
    }
  }

  /**
   * Specialization of {@link InvalidSymbolError} for illegal early accesses within a constructor
   * prologue.
   */
  class RefBeforeCtorCalledError extends StaticError {
    ...
    JCDiagnostic getDiagnostic(
      ...
      Symbol errSym =
          ((sym.kind == TYP && sym.type.hasTag(CLASS)) ? types.erasure(sym.type).tsym : sym);
      return diags.create(dkind, log.currentSource(), pos, "cant.ref.before.ctor.called", errSym);
    }
  }

  /**
   * InvalidSymbolError error class indicating that a pair of symbols (either methods, constructors
   * or operands) are ambiguous given an actual arguments/type argument list.
   */
  class AmbiguityError extends ResolveError {
    ...
    /**
     * If multiple applicable methods are found during overload and none of them is more specific
     * than the others, attempt to merge their signatures.
     */
   ...
   * been found, but pointing to a class for which an enclosing instance is not available.
   */
  ...
  class BadClassFileError extends InvalidSymbolError {
    ...
    JCDiagnostic getDiagnostic(
        ...
        List<Type> typeargtypes) {
      JCDiagnostic d =
          diags.create(dkind, log.currentSource(), pos, "cant.access", ex.sym, ex.getDetailValue());
  ...
  /**
   * Helper class for method resolution diagnostic simplification. Certain resolution diagnostic are
   * rewritten as simpler diagnostic where the enclosing resolution diagnostic (i.e. 'inapplicable
   * method') is stripped away, as it doesn't carry additional info. The logic for matching a given
   * diagnostic is given in terms of a template hierarchy: a diagnostic template can be specified
   * programmatically, so that only certain diagnostics are matched. Each templete is then
   * associated with a rewriter object that carries out the task of rewtiting the diagnostic to a
   * simpler one.
   */
  static class MethodResolutionDiagHelper {
    ...
    interface DiagnosticRewriter {
      JCDiagnostic rewriteDiagnostic(
          JCDiagnostic.Factory diags,
          DiagnosticPosition preferredPos,
          DiagnosticSource preferredSource,
          DiagnosticType preferredKind,
          JCDiagnostic d);
    }

    /**
     * A diagnostic template is made up of two ingredients: (i) a regular expression for matching a
     * diagnostic key and (ii) a list of sub-templates for matching diagnostic arguments.
     */
    static class Template {
      ...
      /**
       * Returns true if the regex matches the diagnostic key and if all diagnostic arguments are
       * matches by corresponding sub-templates.
       */
      boolean matches(Object o) {
        JCDiagnostic d = (JCDiagnostic) o;
        Object[] args = d.getArgs();
        if (!d.getCode().matches(regex) || subTemplates.length != d.getArgs().length) {
          return false;
        }
    ...
    static class ArgMismatchRewriter implements DiagnosticRewriter {
      ...
      @Override
      public JCDiagnostic rewriteDiagnostic(
          JCDiagnostic.Factory diags,
          DiagnosticPosition preferredPos,
          DiagnosticSource preferredSource,
          DiagnosticType preferredKind,
    ...
    static final Template inferArgMismatchTemplate =
        new Template(MethodCheckDiag.ARG_MISMATCH.regex(), skip, skip) {
          ...
          boolean containsAny(JCDiagnostic d, List<Type> ts) {
            return Stream.of(d.getArgs()).anyMatch(o -> containsPredicate.test(o, ts));
          }
        };
    ...
    /**
     * Main entry point for diagnostic rewriting - given a diagnostic, see if any templates matches
     * it, and rewrite it accordingly.
     */
    static JCDiagnostic rewrite(
        JCDiagnostic.Factory diags,
        DiagnosticPosition pos,
            ...
            case WRONG_MTHS:
              ...
              switch (sym.kind) {
                case WRONG_MTH:
                  ...
                  // result instead
                  return bestSoFar.kind == WRONG_MTHS ? bestSoFar : sym;
                case ABSENT_MTH:
  ...
  /**
   * A resolution context is used to keep track of intermediate results of overload resolution, such
   * as list of method that are not applicable (used to generate more precise diagnostics) and so
   * on. Resolution contexts can be nested - this means that when each overload resolution routine
   * should work within the resolution context it created.
   */
  class MethodResolutionContext {
    ...
    DeferredAttrContext deferredAttrContext(
        Symbol sym, InferenceContext inferenceContext, ResultInfo pendingResult, Warner warn) {
      DeferredAttrContext parent =
          (pendingResult == null)
              ? deferredAttr.emptyDeferredAttrContext
              : pendingResult.checkContext.deferredAttrContext();
      return deferredAttr
      .new DeferredAttrContext(attrMode, sym, step, inferenceContext, parent, warn);
    }

    /**
     * This class represents an overload resolution candidate. There are two kinds of candidates:
     * applicable methods and inapplicable methods; applicable methods have a pointer to the
     * instantiated method type, while inapplicable candidates contain further details about the
