...
/**
 * Root class for abstract syntax tree nodes. It provides definitions for specific tree nodes as
 * subclasses nested inside.
 *
 * <p>Each subclass is highly standardized. It generally contains only tree fields for the syntactic
 * subcomponents of the node. Some classes that represent identifier uses or definitions also define
 * a Symbol field that denotes the represented identifier. Classes for non-local jumps also carry
 * the jump target as a field. The root class Tree itself defines fields for the tree's type and
 * position. No other fields are kept in a tree node; instead parameters are passed to methods
 * accessing the node.
 *
 * <p>Except for the methods defined by com.sun.source, the only method defined in subclasses is
 * `visit' which applies a given visitor to the tree. The actual tree processing is done by visitor
 * classes in other packages. The abstract class Visitor, as well as an Factory interface for trees,
 * are defined as inner classes in Tree.
 *
 * <p>To avoid ambiguities with the Tree API in com.sun.source all sub classes should, by
 * convention, start with JC (javac).
 *
...
public abstract class JCTree implements Tree, Cloneable, DiagnosticPosition {
  ...
  public enum Tag {
    ...
    public Tag noAssignOp() {
      if (noAssignTag != null) return noAssignTag;
  ...
  public static class JCCompilationUnit extends JCTree implements CompilationUnitTree {
    ...
    /**
     * A table that stores all documentation comments indexed by the tree nodes they refer to.
    ...
    public JCPackageDecl getPackage() {
      ...
      if (!defs.isEmpty() && defs.head.hasTag(PACKAGEDEF)) return (JCPackageDecl) defs.head;
      return null;
    }
    ...
    public List<JCImportBase> getImports() {
      ...
      for (JCTree tree : defs) {
        if (tree instanceof JCImportBase imp) imports.append(imp);
        else if (!tree.hasTag(PACKAGEDEF) && !tree.hasTag(SKIP)) break;
      }
  ...
  public abstract static class JCImportBase extends JCTree implements ImportTree {
  ...
  /**
   * Common supertype for all poly expression trees (lambda, method references, conditionals, method
  ...
  public static class JCClassDecl extends JCStatement implements ClassTree {
    ...
    @DefinedBy(Api.COMPILER_TREE)
    public Kind getKind() {
      if ((mods.flags & Flags.ANNOTATION) != 0) return Kind.ANNOTATION_TYPE;
      else if ((mods.flags & Flags.INTERFACE) != 0) return Kind.INTERFACE;
      else if ((mods.flags & Flags.ENUM) != 0) return Kind.ENUM;
      else if ((mods.flags & Flags.RECORD) != 0) return Kind.RECORD;
      else return Kind.CLASS;
    }
  ...
  public static class JCVariableDecl extends JCStatement implements VariableTree {
    ...
    protected JCVariableDecl(
        JCModifiers mods, Name name, JCExpression vartype, JCExpression init, VarSymbol sym) {
  ...
  public static class JCBlock extends JCStatement implements BlockTree {
    ...
    /**
     * If this block contains record pattern, it is necessary to catch exceptions from the
     * deconstructors and wrap them. The {@code patternMatchingCatch} keeps the list of the
     * deconstructor invocations, and the additional catch block that wraps the exceptions.
     */
  ...
  public static class JCTry extends JCStatement implements TryTree {
    ...
    protected JCTry(
        List<JCTree> resources, JCBlock body, List<JCCatch> catchers, JCBlock finalizer) {
      this.body = body;
  ...
  public static class JCMethodInvocation extends JCPolyExpression implements MethodInvocationTree {
    ...
    protected JCMethodInvocation(
        List<JCExpression> typeargs, JCExpression meth, List<JCExpression> args) {
  ...
  public static class JCBinary extends JCOperatorExpression implements BinaryTree {
    ...
    protected JCBinary(Tag opcode, JCExpression lhs, JCExpression rhs, OperatorSymbol operator) {
      this.opcode = opcode;
      this.lhs = lhs;
      this.rhs = rhs;
      this.operator = operator;
  ...
  public static class JCInstanceOf extends JCExpression implements InstanceOfTree {
    ...
    /**
     * {@code true} if this instanceof test should have value {@code true} when the {@code expr} is
     * {@code null}.
     */
    ...
    @DefinedBy(Api.COMPILER_TREE)
    public JCTree getType() {
      return pattern instanceof JCPattern
          ? pattern.hasTag(BINDINGPATTERN) ? ((JCBindingPattern) pattern).var.vartype : null
          : pattern;
    }
  ...
  public static class JCMemberReference extends JCFunctionalExpression
    ...
    /**
     * Javac-dependent classification for member references, based on relevant properties w.r.t.
     * code-generation
     */
    ...
    public JCMemberReference(
        ReferenceMode mode, Name name, JCExpression expr, List<JCExpression> typeargs) {
      this.mode = mode;
      this.name = name;
      this.expr = expr;
    ...
    public Object getValue() {
      switch (typetag) {
        ...
        case CHAR:
          ...
          char c = (char) ci;
          if (c != ci) throw new AssertionError("bad value for char literal");
          return c;
        default:
          return value;
      }
  ...
  public static class JCTypeParameter extends JCTree implements TypeParameterTree {
    ...
    protected JCTypeParameter(
        Name name, List<JCExpression> bounds, List<JCAnnotation> annotations) {
      this.name = name;
      this.bounds = bounds;
      this.annotations = annotations;
    }
  ...
  public static class JCModuleDecl extends JCTree implements ModuleTree {
    ...
    protected JCModuleDecl(
        JCModifiers mods, ModuleKind kind, JCExpression qualId, List<JCDirective> directives) {
      this.mods = mods;
      this.kind = kind;
      this.qualId = qualId;
  ...
  public abstract static class Visitor {
    ...
    public void visitModuleImport(JCModuleImport that) {
      visitTree(that);
    }
    ...
    public void visitConstantCaseLabel(JCConstantCaseLabel that) {
      visitTree(that);
    }

    public void visitPatternCaseLabel(JCPatternCaseLabel that) {
      visitTree(that);
    }
