...
public class TreeInfo {
  ...
  public static boolean isCompactConstructor(JCTree tree) {
    // the record flag is only set to the canonical constructor
    return isCanonicalConstructor(tree)
        && (((JCMethodDecl) tree).sym.flags_field & COMPACT_RECORD_CONSTRUCTOR) != 0;
  }
  ...
  public static boolean hasConstructors(List<JCTree> trees) {
    for (List<JCTree> l = trees; l.nonEmpty(); l = l.tail) if (isConstructor(l.head)) return true;
    return false;
  }
  ...
  /**
   * Check if the given tree is an explicit reference to the 'this' instance of the class currently
   * being compiled. This is true if tree is: - An unqualified 'this' identifier - A 'super'
   * identifier qualified by a class name whose type is 'currentClass' or a supertype - A 'this'
   * identifier qualified by a class name whose type is 'currentClass' or a supertype but also NOT
   * an enclosing outer class of 'currentClass'.
   */
  public static boolean isExplicitThisReference(
      Types types, Type.ClassType currentClass, JCTree tree) {
    switch (tree.getTag()) {
      case PARENS:
      ...
      case SELECT:
        {
          ...
          if (!selectedType.hasTag(TypeTag.CLASS)) return false;
          Symbol.ClassSymbol currentClassSym =
              (Symbol.ClassSymbol) ((Type.ClassType) types.erasure(currentClass)).tsym;
          Symbol.ClassSymbol selectedClassSym =
              (Symbol.ClassSymbol) ((Type.ClassType) selectedType).tsym;
  ...
  public static List<Type> recordFieldTypes(JCClassDecl tree) {
    return recordFields(tree).stream().map(vd -> vd.type).collect(List.collector());
  }
  ...
  /**
   * Is the given method a constructor containing a super() and/or this() call? The "target" is
   * either names._this, names._super, or null for either/both.
   */
  ...
  public static JCMethodInvocation findConstructorCall(JCMethodDecl md) {
    if (!TreeInfo.isConstructor(md) || md.body == null) return null;
    return new ConstructorCallFinder(md.name.table.names).find(md).head;
  }

  /**
   * Finds all calls to this() and/or super() in a given constructor. We can't assume they will be
   * "top level" statements, because some synthetic calls to super() are added inside { } blocks. So
   * we must recurse through the method's entire syntax tree.
   */
  private static class ConstructorCallFinder extends TreeScanner {
    ...
    public void visitApply(JCMethodInvocation invoke) {
      ...
      if ((name == names._this || name == names._super)) calls.append(invoke);
  ...
  public static void mapSuperCalls(
      JCBlock block, Function<? super JCExpressionStatement, ? extends JCStatement> mapper) {
  ...
  private static class SuperCallTranslator extends TreeTranslator {
     ...
     * Constructor.
     *
     * @param translator translates super() invocations, returning replacement statement or null for
     *     no change
     */
    ...
    // Because it returns void, anywhere super() can legally appear must be a location where a
    // JCStatement
    // could also appear, so it's OK that we are replacing a JCExpressionStatement with a
    // JCStatement here.
    @Override
  ...
  public static boolean isStaticSelector(JCTree base, Names names) {
    ...
    switch (base.getTag()) {
      ...
      case SELECT:
        return isStaticSym(base) && isStaticSelector(((JCFieldAccess) base).selected, names);
      case TYPEAPPLY:
      case TYPEARRAY:
        return true;
      case ANNOTATED_TYPE:
  ...
  public static boolean isNull(JCTree tree) {
    if (!tree.hasTag(LITERAL)) return false;
  ...
  public static boolean isInAnnotation(Env<?> env, JCTree tree) {
    ...
    if (tp != null) {
      for (Tree t : tp) {
        if (t.getKind() == Tree.Kind.ANNOTATION) return true;
      }
    }
    return false;
  }
  ...
  public static String getCommentText(Env<?> env, JCTree tree) {
    DocCommentTable docComments =
        (tree.hasTag(JCTree.Tag.TOPLEVEL))
  ...
  /**
   * The position of the first statement in a block, or the position of the block itself if it is
   * empty.
   */
  ...
  public static int endPos(JCTree tree) {
    ...
    else if (tree.hasTag(SYNCHRONIZED)) return endPos(((JCSynchronized) tree).body);
    ...
    } else if (tree.hasTag(SWITCH) && ((JCSwitch) tree).endpos != Position.NOPOS) {
  ...
  /**
   * Get the start position for a tree node. The start position is defined to be the position of the
   * first character of the first token of the node's source text.
   *
      ...
      case MODULEDEF:
        {
          ...
          return md.mods.annotations.isEmpty() ? md.pos : md.mods.annotations.head.pos;
        }
      case PACKAGEDEF:
        {
          JCPackageDecl pd = (JCPackageDecl) tree;
          return pd.annotations.isEmpty() ? pd.pos : pd.annotations.head.pos;
        }
      case APPLY:
      ...
      case CLASSDEF:
        {
          ...
          if (node.mods.pos != Position.NOPOS) return node.mods.pos;
          break;
        }
      case CONDEXPR:
      ...
      case NEWCLASS:
        {
          ...
          if (node.encl != null) return getStartPos(node.encl);
          break;
        }
      case VARDEF:
        {
  ...
  public static int getEndPos(JCTree tree, EndPosTable endPosTable) {
    ...
    if (mapPos != Position.NOPOS) return mapPos;
  ...
  /**
   * A DiagnosticPosition with the preferred position set to the end position of given tree, if it
   * is a block with defined endpos.
   */
  ...
  /**
   * Find the position for reporting an error about a symbol, where that symbol is defined somewhere
   * in the given tree.
   */
  ...
  public static DiagnosticPosition diagnosticPositionFor(
      final Symbol sym, final JCTree tree, boolean returnNullIfNotFound) {
  ...
  public static DiagnosticPosition diagnosticPositionFor(
    ...
    class DiagScanner extends DeclScanner {
      ...
      public void visitIdent(JCIdent that) {
        if (!checkMatch(that, that.sym)) super.visitIdent(that);
      }

      public void visitSelect(JCFieldAccess that) {
        if (!checkMatch(that, that.sym)) super.visitSelect(that);
      }
    }
  ...
  public static DiagnosticPosition diagnosticPositionFor(
      final Symbol sym, final List<? extends JCTree> trees) {
    return trees.stream()
        .map(t -> TreeInfo.diagnosticPositionFor(sym, t))
        .filter(t -> t != null)
        .findFirst()
        .get();
  }
  ...
  private static class DeclScanner extends TreeScanner {
    ...
    public void scan(JCTree tree) {
      if (tree != null && result == null) tree.accept(this);
    }

    public void visitTopLevel(JCCompilationUnit that) {
      if (!checkMatch(that, that.packge)) super.visitTopLevel(that);
    }
    ...
    public void visitPackageDef(JCPackageDecl that) {
      if (!checkMatch(that, that.packge)) super.visitPackageDef(that);
    }

    public void visitClassDef(JCClassDecl that) {
      if (!checkMatch(that, that.sym)) super.visitClassDef(that);
    }

    public void visitMethodDef(JCMethodDecl that) {
      if (!checkMatch(that, that.sym)) super.visitMethodDef(that);
    }

    public void visitVarDef(JCVariableDecl that) {
      if (!checkMatch(that, that.sym)) super.visitVarDef(that);
    }

    public void visitTypeParameter(JCTypeParameter that) {
      if (that.type == null || !checkMatch(that, that.type.tsym)) super.visitTypeParameter(that);
    }
    ...
    protected boolean checkMatch(JCTree that, Symbol thatSym) {
      ...
      if (this.sym.getKind() == ElementKind.RECORD_COMPONENT) {
        if (thatSym != null
            && thatSym.getKind() == ElementKind.FIELD
            && (thatSym.flags_field & RECORD) != 0) {
  ...
  /**
   * Return the statement referenced by a label. If the label refers to a loop or switch, return
   * that switch otherwise return the labelled statement itself
   */
  ...
  public static JCTree skipParens(JCTree tree) {
    if (tree.hasTag(PARENS)) return skipParens((JCParens) tree);
    else return tree;
  }
  ...
  /**
   * If this tree is an identifier or a field or a parameterized type, return its name, otherwise
   * return null.
   */
  ...
  private static Symbol symbolForImpl(JCTree node) {
    ...
    switch (node.getTag()) {
      case TOPLEVEL:
        ...
        if (isModuleInfo(cut) && moduleDecl != null) return symbolFor(moduleDecl);
        return cut.packge;
      case MODULEDEF:
      ...
      case TYPEPARAMETER:
        if (node.type != null) return node.type.tsym;
        return null;
      default:
        return null;
    }
  ...
  public static long firstFlag(long flags) {
    long flag = 1;
    while ((flag & flags) == 0) flag = flag << 1;
    return flag;
  }
  ...
  public static final int notExpression = -1, // not an expression
  ...
  /**
   * Returns the underlying type of the tree if it is an annotated type, or the tree itself
   * otherwise.
   */
  ...
  private static class TypeAnnotationFinder extends TreeScanner {
    ...
    @Override
    public void scan(JCTree tree) {
      if (foundTypeAnno || tree == null) return;
      super.scan(tree);
    }
  ...
  public static boolean isBooleanWithValue(JCExpression guard, int value) {
    ...
    return constValue != null && guard.type.hasTag(BOOLEAN) && ((int) constValue) == value;
  }

  public static boolean isNullCaseLabel(JCCaseLabel label) {
    return label.hasTag(CONSTANTCASELABEL) && TreeInfo.isNull(((JCConstantCaseLabel) label).expr);
  }
}
