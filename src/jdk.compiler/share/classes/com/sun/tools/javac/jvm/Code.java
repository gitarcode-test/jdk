...
/**
 * An internal structure that corresponds to the code attribute of methods in a classfile. The class
 * also provides some utility operations to generate bytecode instructions.
 *
...
public class Code {
  ...
  /**
   * A chain for jumps to be resolved before the next opcode is emitted. We do this lazily to avoid
   * jumps to jumps.
   */
  Chain pendingJumps = null;

  /**
   * The position of the currently statement, if we are at the start of this statement, NOPOS
   * otherwise. We need this to emit line numbers lazily, which we need to do because of
  ...
  /**
   * Construct a code object, given the settings of the fatcode, debugging info switches and the
   * CharacterRangeTable.
   */
  public Code(
      MethodSymbol meth,
  ...
  public static int width(List<Type> types) {
    int w = 0;
    for (List<Type> l = types; l.nonEmpty(); l = l.tail) w = w + width(l.head);
    return w;
  }
  ...
  public void emitInvokespecial(Symbol member, Type mtype) {
    ...
    emit2(poolWriter.putMember(member));
    state.pop(argsize);
    if (member.isConstructor()) state.markInitialized((UninitializedType) state.peek());
    state.pop(1);
    state.push(mtype.getReturnType());
  }
  ...
  public <P extends PoolConstant> void emitop2(
      int op, P constant, ToIntBiFunction<PoolWriter, P> poolFunc) {
  ...
  public void align(int incr) {
    if (alive) while (cp % incr != 0) emitop0(nop);
  }

  /** Place a byte into code at address pc. Pre: {@literal pc + 1 <= cp }. */
  private void put1(int pc, int op) {
    code[pc] = (byte) op;
  }

  /** Place two bytes into code at address pc. Pre: {@literal pc + 2 <= cp }. */
  ...
  /** Place four bytes into code at address pc. Pre: {@literal pc + 4 <= cp }. */
  ...
  void emitCLDCStackMap(int pc, int localsSize) {
    ...
    for (int i = 0; i < localsSize; i++) {
      if (state.defined.isMember(i) && lvar[i] != null) {
        ...
        if (!(vtype instanceof UninitializedType)) vtype = types.erasure(vtype);
  ...
  public static class Chain {
    ...
    /**
     * The machine state after the jump instruction. Invariant: all elements of a chain list have
     * the same stacksize and compatible stack and register contents.
     */
    Code.State state;
  ...
  /**
   * Emit a branch with given opcode; return its chain. branch differs from jump in that jsr is
   * treated as no-op.
   */
  ...
  public void resolve(Chain chain, int target) {
    ...
    for (; chain != null; chain = chain.next) {
      Assert.check(state != chain.state && (target > chain.pc || isStatementStart()));
      if (target >= cp) {
        target = cp;
      ...
      if (get1(chain.pc) == goto_ && chain.pc + 3 == target && target == cp && !fixedPc) {
        ...
        if (fatcode) put4(chain.pc + 1, target - chain.pc);
        else if (target - chain.pc < Short.MIN_VALUE || target - chain.pc > Short.MAX_VALUE)
          fatcode = true;
        else put2(chain.pc + 1, target - chain.pc);
        Assert.check(
            !alive
      ...
      if (cp == target) {
        changed = true;
        if (debugCode) System.err.println("resolving chain state=" + chain.state);
        if (alive) {
  ...
  public void resolve(Chain chain) {
    Assert.check(
        !alive
            || chain == null
            || state.stacksize == chain.state.stacksize && state.nlocks == chain.state.nlocks);
  ...
  public void compressCatchTable() {
    ...
    for (char[] catchEntry : catchInfo) {
      ...
      if (startpc == endpc || (startpc == (endpc - 1) && handlerPcs.contains(startpc))) {
        continue;
      } else {
  ...
  public void addLineNumber(char startPc, char lineNumber) {
    if (lineDebugInfo) {
      if (lineInfo.nonEmpty() && lineInfo.head[0] == startPc) lineInfo = lineInfo.tail;
  ...
  public void markStatBegin() {
    if (alive && lineDebugInfo) {
      ...
      if (cp1 == cp && line1 == line) addLineNumber(cp1, line1);
    }
  ...
  class State implements Cloneable {
    ...
    void push(Type t) {
      ...
      if (stacksize > max_stack) max_stack = stacksize;
    }
    ...
    Type pop2() {
      ...
      Type result = stack[stacksize];
      stack[stacksize] = null;
      Assert.check(stack[stacksize + 1] == null && result != null && width(result) == 2);
      return result;
    }
    ...
    void dump(int pc) {
      ...
      if (pc == -1) System.out.println();
      else System.out.println(" at " + pc);
      System.err.println(" stack (from bottom):");
      for (int i = 0; i < stacksize; i++) System.err.println("  " + i + ": " + stack[i]);
      ...
      if (lastLocal >= 0) System.err.println(" locals:");
      for (int i = 0; i <= lastLocal; i++) {
        ...
        if (defined.isMember(i)) {
          ...
          } else if (var.sym == null) System.err.println("UNKNOWN!");
          else System.err.println("" + var.sym + " of type " + var.sym.erasure(types));
        } else {
  ...
  static class LocalVar {
    ...
    public String toString() {
      ...
      StringBuilder sb =
          new StringBuilder().append(sym).append(" in register ").append((int) reg).append(" \n");
      for (Range r : aliveRanges) {
        sb.append(" starts at pc=")
            .append(Integer.toString(((int) r.start_pc)))
            .append(" length=")
            .append(Integer.toString(((int) r.length)))
            .append("\n");
      }
      return sb.toString();
    }
  ...
  public void setDefined(Bits newDefined) {
    if (alive && newDefined != state.defined) {
      ...
      for (int adr = diff.nextBit(0); adr >= 0; adr = diff.nextBit(adr + 1)) {
        if (adr >= nextreg) state.defined.excl(adr);
        else if (state.defined.isMember(adr)) setUndefined(adr);
        else setDefined(adr);
      }
    }
  }
  ...
  private void endScope(int adr) {
    ...
    if (v != null) {
      ...
      /**
       * the call to curCP() can implicitly adjust the current cp, if so the alive range of local
       * variables may be modified. Thus we need all of them. For this reason assigning null to the
       * given address should be the last action to do.
       */
      lvar[adr] = null;
    }
    state.defined.excl(adr);
  ...
  private void fillLocalVarPosition(LocalVar lv) {
    if (lv == null
        ...
        || !lv.sym.hasTypeAnnotations()) return;
  ...
  public void fillExceptionParameterPositions() {
    for (int i = 0; i < varBufferSize; ++i) {
      ...
      if (lv == null
          ...
          || !lv.sym.isExceptionParameter()) continue;
      ...
      for (Attribute.TypeCompound ta : lv.sym.getRawTypeAttributes()) {
        ...
        if (p.hasCatchType()) {
          ...
          if (idx == -1)
            Assert.error(
                "Could not find exception index for type annotation "
                    + ta
                    + " on exception parameter");
  ...
  void putVar(LocalVar var) {
    ...
    boolean keepLocalVariables =
        varDebugInfo || (var.sym.isExceptionParameter() && var.sym.hasTypeAnnotations());
    ...
    boolean ignoredSyntheticVar =
        (var.sym.flags() & Flags.SYNTHETIC) != 0
    ...
    if (varBuffer == null) varBuffer = new LocalVar[20];
    else varBuffer = ArrayUtils.ensureCapacity(varBuffer, varBufferSize);
