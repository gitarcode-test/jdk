...
/**
 * The FilerImplementation class must maintain a number of constraints. First, multiple attempts to
 * open the same path within the same invocation of the tool results in an IOException being thrown.
 * For example, trying to open the same source file twice:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Additionally, creating a source file that corresponds to an already created class file (or
 * vice versa) also results in an IOException since each type can only be created once. However, if
 * the Filer is used to create a text file named *.java that happens to correspond to an existing
 * class file, a warning is *not* generated. Similarly, a warning is not generated for a binary file
 * named *.class and an existing source file.
 *
 * <p>The reason for this difference is that source files and class files are registered with the
 * tool and can get passed on as declarations to the next round of processing. Files that are just
 * named *.java and *.class are not processed in that manner; although having extra source files and
 * class files on the source path and class path can alter the behavior of the tool and any final
 * compile.
 *
...
public class JavacFiler implements Filer, Closeable {
  ...
  private class FilerOutputFileObject extends ForwardingFileObject<FileObject> {
    ...
    public synchronized OutputStream openOutputStream() throws IOException {
      if (opened) throw new IOException(ALREADY_OPENED);
      opened = true;
  ...
  /**
   * Wrap a {@code OutputStream} returned from the {@code JavaFileManager} to properly register
   * source or class files when they are closed.
   */
  ...
  /**
   * Wrap a {@code Writer} returned from the {@code JavaFileManager} to properly register source or
   * class files when they are closed.
   */
  private class FilerWriter extends FilterWriter {
     ...
     * @param typeName name of source file or {@code null} if just a text file
     */
  ...
  /**
   * Names of source files closed in this round. This set must be synchronized. Its iterators should
   ...
   * Its iterators should preserve insertion order.
   */
  private final Map<ModuleSymbol, Map<String, JavaFileObject>> generatedClasses;

  /**
   * JavaFileObjects for source files closed in this round. This set must be synchronized. Its
   * iterators should preserve insertion order.
   */
  ...
  public JavaFileObject createSourceFile(CharSequence nameAndModule, Element... originatingElements)
      throws IOException {
    Pair<ModuleSymbol, String> moduleAndClass = checkOrInferModule(nameAndModule);
    return createSourceOrClassFile(
        moduleAndClass.fst, true, moduleAndClass.snd, originatingElements);
  }

  @Override
  @DefinedBy(Api.ANNOTATION_PROCESSING)
  public JavaFileObject createClassFile(CharSequence nameAndModule, Element... originatingElements)
      throws IOException {
    Pair<ModuleSymbol, String> moduleAndClass = checkOrInferModule(nameAndModule);
    return createSourceOrClassFile(
        moduleAndClass.fst, false, moduleAndClass.snd, originatingElements);
  }

  private Pair<ModuleSymbol, String> checkOrInferModule(CharSequence moduleAndPkg)
  ...
  private JavaFileObject createSourceOrClassFile(
      ModuleSymbol mod, boolean isSourceFile, String name, Element... originatingElements)
      throws IOException {
    ...
    if (lint) {
      ...
      if (periodIndex != -1) {
        ...
        if (base.equals(extn)) log.warning(Warnings.ProcSuspiciousClassName(name, extn));
      }
    }
    ...
    JavaFileObject.Kind kind =
        (isSourceFile ? JavaFileObject.Kind.SOURCE : JavaFileObject.Kind.CLASS);

    JavaFileObject fileObject =
        fileManager.getJavaFileForOutputForOriginatingFiles(
            loc, name, kind, originatingFiles(originatingElements));
    checkFileReopening(fileObject, true);

    if (lastRound) log.warning(Warnings.ProcFileCreateLastRound(name));

    if (isSourceFile) aggregateGeneratedSourceNames.add(Pair.of(mod, name));
  ...
  public FileObject createResource(
      ...
      throws IOException {
    Tuple3<Location, ModuleSymbol, String> locationModuleAndPackage =
        checkOrInferModule(location, moduleAndPkg, true);
    ...
    if (pkg.length() > 0) checkName(pkg);

    FileObject fileObject =
        fileManager.getFileForOutputForOriginatingFiles(
            location, pkg, relativeName.toString(), originatingFiles(originatingElements));
    checkFileReopening(fileObject, true);

    if (fileObject instanceof JavaFileObject javaFileObject)
      return new FilerOutputJavaFileObject(msym, null, javaFileObject);
    else return new FilerOutputFileObject(msym, null, fileObject);
  }
    ...
    if (location instanceof StandardLocation standardLocation) {
      if (!standardLocation.isOutputLocation())
        throw new IllegalArgumentException(
            "Resource creation not supported in location " + standardLocation);
    }
  }
  ...
  public FileObject getResource(
      ...
      throws IOException {
    Tuple3<Location, ModuleSymbol, String> locationModuleAndPackage =
        checkOrInferModule(location, moduleAndPkg, false);
    location = locationModuleAndPackage.a;
    String pkg = locationModuleAndPackage.c;
    ...
    if (location.isOutputLocation()) {
      fileObject =
          fileManager.getFileForOutputForOriginatingFiles(location, pkg, relativeName.toString());
    } else {
      fileObject = fileManager.getFileForInput(location, pkg, relativeName.toString());
    }
    if (fileObject == null) {
  ...
  private Tuple3<JavaFileManager.Location, ModuleSymbol, String> checkOrInferModule(
      JavaFileManager.Location location, CharSequence moduleAndPkg, boolean write)
      throws IOException {
    String moduleAndPkgString = moduleAndPkg.toString();
    int slash = moduleAndPkgString.indexOf('/');
    ...
    if (slash == (-1)) {
      ...
      if (location.isOutputLocation()) {
        ...
        if (msym != null) {
          Location moduleLoc = fileManager.getLocationForModule(location, msym.name.toString());
      ...
      if (defaultTargetModule == null) {
        throw new FilerException(
            "No module specified and the location is either "
    ...
    if (multiModuleLocation) {
      ...
      throw new FilerException(
          "Module specified but the location is neither "
  ...
  private ModuleSymbol inferModule(String pkg) {
    if (modules.getDefaultModule() == syms.noModule) return modules.getDefaultModule();
  ...
  private void checkName(String name, boolean allowUnnamedPackageInfo) throws FilerException {
    if (!SourceVersion.isName(name) && !isPackageInfo(name, allowUnnamedPackageInfo)) {
      if (lint) log.warning(Warnings.ProcIllegalFileName(name));
  ...
  private void checkNameAndExistence(
      ModuleSymbol mod, String typename, boolean allowUnnamedPackageInfo) throws FilerException {
    ...
    if (alreadySeen) {
      if (lint) log.warning(Warnings.ProcTypeRecreate(typename));
    ...
    if (!mod.isUnnamed() && !typename.contains(".")) {
      throw new FilerException(
          "Attempt to create a type in unnamed package of a named module: " + typename);
    }
  }
  ...
  private boolean containedInInitialInputs(String typename) {
    ...
    if (existingClass != null) {
      ...
      if (existingPackage != null) sourceFile = existingPackage.sourcefile;
    }
    return (sourceFile == null) ? false : initialInputs.contains(sourceFile);
  }

  /**
   * Check to see if the file has already been opened; if so, throw an exception, otherwise add it
   * to the set of files.
   */
  private void checkFileReopening(FileObject fileObject, boolean forWriting) throws FilerException {
    if (isInFileObjectHistory(fileObject, forWriting)) {
      if (lint) log.warning(Warnings.ProcFileReopening(fileObject.getName()));
      throw new FilerException("Attempt to reopen a file for path " + fileObject.getName());
    }
    if (forWriting) fileObjectHistory.add(fileObject);
  }
        ...
        try {
          ...
          if (existing != null
              && ((existing.sourcefile != null
                      && fileManager.isSameFile(existing.sourcefile, fileObject))
                  || (existing.classfile != null
                      && fileManager.isSameFile(existing.classfile, fileObject)))) {
            return true;
          }
        } catch (IllegalArgumentException e) {
          // ignore...
  ...
  public boolean newFiles() {
    return (!generatedSourceNames.isEmpty()) || (!generatedClasses.isEmpty());
  }
  ...
  public void warnIfUnclosedFiles() {
    if (!openTypeNames.isEmpty()) log.warning(Warnings.ProcUnclosedTypeFiles(openTypeNames));
  }
  ...
  public void setInitialState(
      Collection<? extends JavaFileObject> initialInputs, Collection<String> initialClassNames) {
  ...
  private void closeFileObject(ModuleSymbol mod, String typeName, FileObject fileObject) {
    ...
    if ((typeName != null)) {
      ...
      switch (javaFileObject.getKind()) {
        ...
        case CLASS:
          generatedClasses
              .computeIfAbsent(mod, m -> Collections.synchronizedMap(new LinkedHashMap<>()))
              .put(typeName, javaFileObject);
          openTypeNames.remove(typeName);
          break;
